...
void egl_set_swap_interval(egl_ctx_data_t *egl, int interval)
{
   ...
   KINGSN_LOG("[EGL]: eglSwapInterval(%u)\n", interval);
   if (!_egl_swap_interval(egl->dpy, interval))
   {
      KINGSN_ERR("[EGL]: eglSwapInterval() failed.\n");
      egl_report_error();
   }
}
...
static EGLDisplay get_egl_display(EGLenum platform, void *native)
{
   if (platform != EGL_NONE)
   {
      ...
      if (check_egl_version(1, 5))
      {
         ...
         KINGSN_LOG("[EGL] Found EGL client version >= 1.5, trying eglGetPlatformDisplay\n");
      ...
      if (check_egl_client_extension("EGL_EXT_platform_base"))
      {
         ...
         KINGSN_LOG("[EGL] Found EGL_EXT_platform_base, trying eglGetPlatformDisplayEXT\n");
   ...
   KINGSN_LOG("[EGL] Falling back to eglGetDisplay\n");
...
bool egl_get_native_visual_id(egl_ctx_data_t *egl, EGLint *value)
{
   ...
   {
      KINGSN_ERR("[EGL]: egl_get_native_visual_id failed.\n");
      return false;
   }
      ...
      void *display_data)
{
   ...
   {
      KINGSN_ERR("[EGL]: No configs to choose from.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[EGL]: No EGL configs with appropriate attributes.\n");
      return false;
   }
   ...
   if (i == *count)
   {
      KINGSN_ERR("[EGL]: No EGL config found which satifies requirements.\n");
      return false;
   }
      ...
      egl_accept_config_cb_t cb)
{
   ...
   if (dpy == EGL_NO_DISPLAY)
   {
      KINGSN_ERR("[EGL]: Couldn't get EGL display.\n");
      return false;
   }
   ...
   KINGSN_LOG("[EGL]: EGL version: %d.%d\n", *major, *minor);
...
bool egl_create_context(egl_ctx_data_t *egl, const EGLint *egl_attribs)
{
   ...
   if (egl->use_hw_ctx)
   {
      egl->hw_ctx = _egl_create_context(egl->dpy, egl->config, egl->ctx,
            egl_attribs);
      KINGSN_LOG("[EGL]: Created shared context: %p.\n", (void*)egl->hw_ctx);
...
bool egl_create_surface(egl_ctx_data_t *egl, void *native_window)
{
   ...
   KINGSN_LOG("[EGL]: Current context: %p.\n", (void*)_egl_get_current_context());
...
bool egl_has_config(egl_ctx_data_t *egl)
{
   if (!egl->config)
   {
      KINGSN_ERR("[EGL]: No EGL configurations available.\n");
      return false;
   }
   return true;
}
