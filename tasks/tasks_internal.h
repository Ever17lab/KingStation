...
void *task_push_http_transfer(const char *url, bool mute, const char *type,
      ks_task_callback_t cb, void *userdata);

void *task_push_http_transfer_with_user_agent(const char *url, bool mute, const char *type,
      const char* user_agent, ks_task_callback_t cb, void *userdata);
...
void *task_push_http_post_transfer_with_user_agent(const char* url, const char* post_data, bool mute,
   const char* type, const char* user_agent, ks_task_callback_t cb, void* user_data);

task_retriever_info_t *http_task_get_transfer_list(void);

bool task_push_bluetooth_scan(ks_task_callback_t cb);

bool task_push_wifi_scan(ks_task_callback_t cb);
bool task_push_wifi_enable(ks_task_callback_t cb);
bool task_push_wifi_disable(ks_task_callback_t cb);
bool task_push_wifi_disconnect(ks_task_callback_t cb);
bool task_push_wifi_connect(ks_task_callback_t cb, void*);

bool task_push_netplay_lan_scan(ks_task_callback_t cb);

bool task_push_netplay_crc_scan(uint32_t crc, char* name,
      const char *hostname, const char *corename, const char* subsystem);

bool task_push_netplay_lan_scan_rooms(ks_task_callback_t cb);
...
void task_file_load_handler(ks_task_t *task);
