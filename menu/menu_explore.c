...
static explore_state_t *explore_build_list(void)
{
   ...
   *rdbs                                          = NULL;
   ...
   libks_vfs_implementation_dir *dir           = NULL;
   ...
   /* Index all playlists */
   for (dir = ks_vfs_opendir_impl(directory_playlist, false); dir;)
   {
      ...
      if (!ks_vfs_readdir_impl(dir))
      {
         ks_vfs_closedir_impl(dir);
         break;
      }

      fname                             = ks_vfs_dirent_get_name_impl(dir);
      if (fname)
         fext                           = strrchr(fname, '.');
      ...
      for (j = 0; j < playlist_size(playlist); j++)
      {
         ...
         if (!rdb_num)
         {
            ...
            if (libksdb_open(tmp, newrdb.handle) != 0)
            {
               /* Invalid RDB file */
               libksdb_free(newrdb.handle);
   ...
   for (i = 0; i != RBUF_LEN(rdbs); i++)
   {
      ...
      bool more                = 
         (
          libksdb_cursor_open(rdb->handle, cur, NULL) == 0
          && libksdb_cursor_read_item(cur, &item) == 0);
      ...
      libksdb_close(rdb->handle);
      libksdb_free(rdb->handle);
