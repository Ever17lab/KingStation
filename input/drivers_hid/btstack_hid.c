      ...
      case HCI_EVENT_PACKET:
         switch (packet[0])
         {
            case BTSTACK_EVENT_STATE:
               KINGSN_LOG("[BTstack]: HCI State %d.\n", packet[2]);
            ...
            case HCI_EVENT_COMMAND_COMPLETE:
               ...
               if (COMMAND_COMPLETE_EVENT(packet, (*hci_read_bd_addr_ptr)))
               {
                  ...
                  if (!packet[5])
                     KINGSN_LOG("[BTpad]: Local address is %s.\n",
                           bd_addr_to_str_ptr(event_addr));
                  else
                     KINGSN_LOG("[BTpad]: Failed to get local address (Status: %02X).\n",
                           packet[5]);
               }
               break;
            ...
            case HCI_EVENT_INQUIRY_RESULT:
               if (packet[2])
               {
                  ...
                  KINGSN_LOG("[BTpad]: Inquiry found device\n");
                  memset(connection, 0, sizeof(struct btstack_hid_adapter));
            ...
            case L2CAP_EVENT_CHANNEL_OPENED:
               {
                  ...
                  if (!packet[2])
                  {
                     if (!connection)
                     {
                        KINGSN_LOG("[BTpad]: Got L2CAP 'Channel Opened' event for unrecognized device.\n");
                        break;
                     }

                     KINGSN_LOG("[BTpad]: L2CAP channel opened: (PSM: %02X)\n", psm);
                     ...
                     switch (psm)
                     {
                        ...
                        case PSM_HID_INTERRUPT:
                           ...
                           break;
                        default:
                           KINGSN_LOG("[BTpad]: Got unknown L2CAP PSM, ignoring (PSM: %02X).\n", psm);
                           break;
                     }

                     if (connection->channels[0] && connection->channels[1])
                     {
                        KINGSN_LOG("[BTpad]: Got both L2CAP channels, requesting name.\n");
                        btpad_queue_hci_remote_name_request(cmd, connection->address, 0, 0, 0);
                     }
                  }
                  else
                     KINGSN_LOG("[BTpad]: Got failed L2CAP 'Channel Opened' event (PSM: %02X, Status: %02X).\n", psm, packet[2]);
               }
               break;
            ...
            case L2CAP_EVENT_INCOMING_CONNECTION:
               {
                  ...
                  if (!connection)
                  {
                     ...
                     KINGSN_LOG("[BTpad]: Got new incoming connection\n");
                  ...
                  KINGSN_LOG("[BTpad]: Incoming L2CAP connection (PSM: %02X).\n",
                        psm);
            ...
            case HCI_EVENT_REMOTE_NAME_REQUEST_COMPLETE:
               {
                  ...
                  if (!connection)
                  {
                     KINGSN_LOG("[BTpad]: Got unexpected remote name, ignoring.\n");
                     break;
                  }

                  KINGSN_LOG("[BTpad]: Got %.200s.\n", (char*)&packet[9]);
            ...
            case HCI_EVENT_PIN_CODE_REQUEST:
               KINGSN_LOG("[BTpad]: Sending Wiimote PIN.\n");
            ...
            case HCI_EVENT_DISCONNECTION_COMPLETE:
               {
                  ...
                  if (!packet[2])
                  {
                     ...
                     KINGSN_LOG("[BTpad]: Got failed 'Disconnection Complete' event (Status: %02X).\n", packet[2]);
               }
               break;

            case L2CAP_EVENT_SERVICE_REGISTERED:
               if (packet[2])
                  KINGSN_LOG("[BTpad]: Got failed 'Service Registered' event (PSM: %02X, Status: %02X).\n",
...
{
   KINGSN_LOG("[BTstack]: Thread started");
   ...
   KINGSN_LOG("[BTstack]: Turning on...\n");
   bt_send_cmd_ptr(btstack_set_power_mode_ptr, HCI_POWER_ON);

   KINGSN_LOG("BTstack: Thread running...\n");
#ifdef __APPLE__
   CFRunLoopRun();
#endif

   KINGSN_LOG("[BTstack]: Thread done.\n");
      ...
      unsigned port)
{
   unsigned i;
   int16_t ret                          = 0;
   const struct ks_keybind *binds    = (const struct ks_keybind*)binds_data;
   btstack_hid_t         *hid            = (btstack_hid_t*)data;
