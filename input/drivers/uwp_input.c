...
{
   input_keymaps_init_keyboard_lut(kingsn_key_map_uwp);
      ...
      unsigned id)
{
   switch (device)
   {
      ...
      case KS_DEVICE_KEYBOARD:
         return (id < KSK_LAST) && uwp_keyboard_pressed(id);

      case KS_DEVICE_MOUSE:
      case KINGSN_DEVICE_MOUSE_SCREEN:
         return uwp_mouse_state(port, id, device == KINGSN_DEVICE_MOUSE_SCREEN);

      case KS_DEVICE_POINTER:
      case KINGSN_DEVICE_POINTER_SCREEN:
         return uwp_pointer_state(index, id, device == KINGSN_DEVICE_POINTER_SCREEN);
   }
