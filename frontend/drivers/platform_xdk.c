      ...
      void *args, void *params_data)
{
   ...
   ks_main_log_file_init("/KingStation-log.txt");
#endif
#endif
...
static void frontend_xdk_exec(const char *path, bool should_load_content)
{
   ...
   if (should_load_content && !path_is_empty(KINGSN_PATH_CONTENT))
      snprintf((char*)ptr.Data, sizeof(ptr.Data), "%s", path_get(KINGSN_PATH_CONTENT));
...
static bool frontend_xdk_set_fork(enum frontend_fork fork_mode)
{
   switch (fork_mode)
   {
      ...
      case FRONTEND_FORK_RESTART:
         KINGSN_LOG("FRONTEND_FORK_SALAMANDER_RESTART\n");
