 ...
 **/
static int task_http_iterate_transfer(ks_task_t *task)
{
...
static void task_http_transfer_handler(ks_task_t *task)
{
...
static bool task_http_finder(ks_task_t *task, void *user_data)
{
...
static bool task_http_retriever(ks_task_t *task, void *data)
{
...
static void http_transfer_progress_cb(ks_task_t *task)
{
#ifdef KINGSN_INTERNAL
   if (task)
...
static void* task_push_http_transfer_generic(
      ...
      ks_task_callback_t cb, void *user_data)
{
...
void* task_push_http_transfer_file(const char* url, bool mute,
      const char* type,
      ks_task_callback_t cb, file_transfer_t* transfer_data)
{
   const char *s   = NULL;
   char tmp[255]   = "";
   ks_task_t *t = NULL;
   ...
   t = (ks_task_t*)task_push_http_transfer_generic(
         net_http_connection_new(url, "GET", NULL),
...
void* task_push_http_post_transfer(const char *url,
      ...
      const char *type, ks_task_callback_t cb, void *user_data)
{
   if (string_is_empty(url))
      return NULL;
   return task_push_http_transfer_generic(
