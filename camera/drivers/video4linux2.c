...
static bool init_mmap(void *data)
{
   ...
   {
      if (errno == EINVAL)
         KINGSN_ERR("[V4L2]: %s does not support memory mapping.\n", v4l->dev_name);
      else
         KINGSN_ERR("[V4L2]: xioctl of VIDIOC_REQBUFS failed.\n");
      return false;
   }

   if (req.count < 2)
   {
      KINGSN_ERR("[V4L2]: Insufficient buffer memory on %s.\n", v4l->dev_name);
      return false;
   }
   ...
   if (!v4l->buffers)
   {
      KINGSN_ERR("[V4L2]: Out of memory allocating V4L2 buffers.\n");
      return false;
   }
   ...
   for (v4l->n_buffers = 0; v4l->n_buffers < req.count; v4l->n_buffers++)
   {
      ...
      {
         KINGSN_ERR("[V4L2]: Error - xioctl VIDIOC_QUERYBUF.\n");
         return false;
      }
...
static bool init_device(void *data)
{
   ...
   {
      if (errno == EINVAL)
         KINGSN_ERR("[V4L2]: %s is no V4L2 device.\n", v4l->dev_name);
      else
         KINGSN_ERR("[V4L2]: Error - VIDIOC_QUERYCAP.\n");
      return false;
   }

   if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE))
   {
      KINGSN_ERR("[V4L2]: %s is no video capture device.\n", v4l->dev_name);
      return false;
   }

   if (!(cap.capabilities & V4L2_CAP_STREAMING))
   {
      KINGSN_ERR("[V4L2]: %s does not support streaming I/O (V4L2_CAP_STREAMING).\n",
            v4l->dev_name);
      return false;
   }
   ...
   {
      KINGSN_ERR("[V4L2]: Error - VIDIOC_S_FMT\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[V4L2]: The V4L2 device doesn't support YUYV.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[V4L2]: The V4L2 device doesn't support progressive nor interlaced video.\n");
      return false;
   }

   KINGSN_LOG("[V4L2]: device: %u x %u.\n", v4l->width, v4l->height);
...
static void v4l_stop(void *data)
{
   ...
   if (xioctl(v4l->fd, VIDIOC_STREAMOFF, &type) == -1)
      KINGSN_ERR("[V4L2]: Error - VIDIOC_STREAMOFF.\n");
...
static bool v4l_start(void *data)
{
   ...
   for (i = 0; i < v4l->n_buffers; i++)
   {
      ...
      {
         KINGSN_ERR("[V4L2]: Error - VIDIOC_QBUF.\n");
         return false;
      }
   }
   ...
   {
      KINGSN_ERR("[V4L2]: Error - VIDIOC_STREAMON.\n");
      return false;
   }
      ...
      unsigned width, unsigned height)
{
   ...
   if ((caps & (UINT64_C(1) << KS_CAMERA_BUFFER_RAW_FRAMEBUFFER)) == 0)
   {
      KINGSN_ERR("[V4L2]: Returns raw framebuffers.\n");
      return NULL;
   }
   ...
   {
      KINGSN_ERR("[V4L2]: %s is no device.\n", v4l->dev_name);
      goto error;
   }
   ...
   if (v4l->fd == -1)
   {
      KINGSN_ERR("[V4L2]: Cannot open '%s': %d, %s\n", v4l->dev_name,
   ...
   {
      KINGSN_ERR("[V4L2]: Failed to allocate output buffer.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[V4L2]: Failed to create scaler.\n");
      goto error;
   }
...
error:
   KINGSN_ERR("[V4L2]: Failed to initialize camera.\n");
   v4l_free(v4l);
   return NULL;
}
...
static bool preprocess_image(void *data)
{
   ...
   ks_assert(buf.index < v4l->n_buffers);
