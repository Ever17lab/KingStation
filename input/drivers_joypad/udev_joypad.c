...
static int udev_add_pad(struct udev_device *dev, unsigned p, int fd, const char *path)
{
   ...
   if (ioctl(fd, EVIOCGBIT(EV_FF, sizeof(ffbit)), ffbit) >= 0)
   {
      if (test_bit(FF_RUMBLE, ffbit))
         KINGSN_LOG("[udev]: Pad #%u (%s) supports force feedback.\n",
               p, path);
...
static void udev_check_device(struct udev_device *dev, const char *path)
{
   ...
   {
      case -1:
         KINGSN_ERR("[udev]: Failed to add pad: %s.\n", path);
         close(fd);
         break;
      case 1:
      ...
      enum ks_rumble_effect effect, uint16_t strength)
{
   ...
   if (strength && strength != pad->configured_strength[effect])
   {
      ...
      {
         KINGSN_ERR("Failed to set rumble effect on pad #%u.\n", i);
         return false;
      }
   ...
   if ((!!strength) != (!!old_strength))
   {
      ...
      {
         KINGSN_ERR("[udev]: Failed to play rumble effect #%u on pad #%u.\n",
               effect, i);
         return false;
      }
   }
