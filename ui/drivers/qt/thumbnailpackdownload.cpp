...
#define THUMBNAILPACK_URL_HEADER "http://thumbnailpacks.libks.com/"
#define THUMBNAILPACK_EXTENSION ".zip"

static void extractThumbnailPackCB(ks_task_t *task,
...
void MainWindow::onThumbnailPackDownloadNetworkError(QNetworkReply::NetworkError code)
{
   ...
   KINGSN_ERR("[Qt]: Network error code %d received: %s\n", code, errorStringData);
...
void MainWindow::onThumbnailPackDownloadFinished()
{
   ...
   if (code != 200)
   {
      ...
      if (!redirectUrl.isEmpty())
      {
         ...
         KINGSN_LOG("[Qt]: Thumbnail pack download got redirect with HTTP code %d: %s\n", code, redirectUrlData);
      ...
      KINGSN_ERR("[Qt]: Thumbnail pack download failed with HTTP status code: %d\n", code);
   ...
   if (error == QNetworkReply::NoError)
   {
      ...
      if (newFile.exists() && !newFile.remove())
         KINGSN_ERR("[Qt]: Thumbnail pack download finished, but old file could not be deleted.\n");
      else
      {
         if (m_thumbnailPackDownloadFile.rename(newFileName))
         {
            ...
            if (settings)
            {
               KINGSN_LOG("[Qt]: Thumbnail pack download finished successfully.\n");
               emit extractArchiveDeferred(newFileName, settings->paths.directory_thumbnails, TEMP_EXTENSION, extractThumbnailPackCB);
            }
         }
         else
         {
            KINGSN_ERR("[Qt]: Thumbnail pack download finished, but temp file could not be renamed.\n");
   ...
   else
   {
      ...
      KINGSN_ERR("[Qt]: Thumbnail pack download ended prematurely: %s\n", errorData);
...
void MainWindow::downloadAllThumbnails(QString system, QUrl url)
{
   ...
   if (m_thumbnailPackDownloadFile.isOpen())
   {
      KINGSN_ERR("[Qt]: File is already open.\n");
      return;
   }
   else
   {
      ...
      if (!m_thumbnailPackDownloadFile.open(QIODevice::WriteOnly))
      {
         ...
         KINGSN_ERR("[Qt]: Could not open file for writing: %s\n", fileNameData);
         return;
      }
   }

   KINGSN_LOG("[Qt]: Starting thumbnail pack download...\n");
   KINGSN_LOG("[Qt]: Downloading URL %s\n", urlData);
...
void MainWindow::onThumbnailPackExtractFinished(bool success)
{
   ...
   if (!success)
   {
      KINGSN_ERR("[Qt]: Thumbnail pack extraction failed.\n");
      emit showErrorMessageDeferred(msg_hash_to_str(MSG_DECOMPRESSION_FAILED));
      return;
   }

   KINGSN_LOG("[Qt]: Thumbnail pack extracted successfully.\n");
