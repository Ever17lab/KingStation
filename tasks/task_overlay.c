      ...
      bool normalized, float alpha_mod, float range_mod)
{
   ...
   {
      KINGSN_ERR("[Overlay]: Base overlay is not set and not using normalized coordinates.\n");
      ret = false;
      goto end;
   }
   ...
   {
      KINGSN_ERR("[Overlay]: Didn't find key: %s.\n", overlay_desc_key);
      ret = false;
      goto end;
   }
   ...
   {
      KINGSN_ERR("[Overlay]: Failed to split overlay desc.\n");
      ret = false;
      goto end;
   }
   ...
   if (list.size < 6)
   {
      KINGSN_ERR("[Overlay]: Overlay desc is invalid. Requires at least 6 tokens.\n");
      ret = false;
      goto end;
   }
   ...
   else if (strstr(key, "ksk_") == key)
   {
      desc->type          = OVERLAY_TYPE_KEYBOARD;
      desc->ks_key_idx = input_config_translate_str_to_rk(key + 7);
   }
   else
   {
      char      *save = NULL;
      ...
      if (BIT256_GET(desc->button_mask, KINGSN_OVERLAY_NEXT))
      {
   ...
   {
      KINGSN_ERR("[Overlay]: Hitbox type (%s) is invalid. Use \"radial\" or \"rect\".\n", box);
      ret = false;
      goto end;
   }
      ...
      case OVERLAY_TYPE_ANALOG_RIGHT:
         {
            ...
            {
               KINGSN_ERR("[Overlay]: Analog hitbox type must be \"radial\".\n");
               ret = false;
               goto end;
            }
      ...
      size_t idx, size_t size)
{
   ...
   for (i = 0; i < current->size; i++)
   {
      ...
      if (!string_is_empty(next))
      {
         ...
         if (next_idx < 0)
         {
            KINGSN_ERR("[Overlay]: Couldn't find overlay called: \"%s\".\n",
                  next);
            return false;
         }
      }
...
static void task_overlay_resolve_iterate(ks_task_t *task)
{
   ...
   {
      KINGSN_ERR("[Overlay]: Failed to resolve next targets.\n");
...
static void task_overlay_deferred_loading(ks_task_t *task)
{
   size_t i                  = 0;
   overlay_loader_t *loader  = (overlay_loader_t*)task->state;
      ...
      case OVERLAY_IMAGE_TRANSFER_DESC_ITERATE:
         for (i = 0; i < overlay->pos_increment; i++)
         {
            if (overlay->pos < overlay->size)
            {
               ...
               {
                  KINGSN_ERR("[Overlay]: Failed to load overlay descs for overlay #%u.\n",
...
static void task_overlay_deferred_load(ks_task_t *task)
{
   unsigned i;
   overlay_loader_t *loader  = (overlay_loader_t*)task->state;
   config_file_t       *conf = loader->conf;
   ...
   for (i = 0; i < loader->pos_increment; i++, loader->pos++)
   {
      ...
      {
         KINGSN_ERR("[Overlay]: Failed to read number of descs from config key: %s.\n",
      ...
      if (!overlay_desc)
      {
         KINGSN_ERR("[Overlay]: Failed to allocate descs.\n");
         goto error;
      }
      ...
      if (!texture_img)
      {
         KINGSN_ERR("[Overlay]: Failed to allocate load_images.\n");
         goto error;
      }
      ...
      if (!string_is_empty(overlay->config.paths.path))
      {
         ...
         {
            KINGSN_ERR("[Overlay]: Failed to load image: %s.\n",
               ...
               overlay->config.rect.array, sizeof(overlay->config.rect.array)))
      {
         ...
         if (     !string_split_noalloc(
               ...
               || list.size < 4)
         {
            KINGSN_ERR("[Overlay]: Failed to split rect \"%s\" into at least four tokens.\n",
...
static void task_overlay_free(ks_task_t *task)
{
   unsigned i;
   overlay_loader_t *loader  = (overlay_loader_t*)task->state;
   struct overlay *overlay   = &loader->overlays[loader->pos];
...
static void task_overlay_handler(ks_task_t *task)
{
   overlay_loader_t *loader  = (overlay_loader_t*)task->state;
...
static bool task_overlay_finder(ks_task_t *task, void *user_data)
{
