...
static bool core_backup_get_backup_dir(
      const char *dir_libks, const char *dir_core_assets,
      ...
      char *backup_dir, size_t len)
{
   ...
   if (!string_is_empty(last_underscore))
      if (!string_is_equal(last_underscore, "_libks"))
         *last_underscore = '\0';
   ...
   if (!path_is_directory(backup_dir))
   {
      ...
      {
         KINGSN_ERR("[core backup] Failed to create backup directory: %s.\n", backup_dir);
         return false;
      }
   }
      ...
      case CORE_BACKUP_TYPE_LIB:
         {
            ...
            backup_file = intfstream_open_file(
                  backup_path, KS_VFS_FILE_ACCESS_READ,
...
enum core_backup_type core_backup_get_core_path(
      const char *backup_path, const char *dir_libks,
      char *core_path, size_t len)
{
   ...
   if (string_is_empty(backup_path) || string_is_empty(dir_libks))
      return backup_type;
      ...
      case CORE_BACKUP_TYPE_ARCHIVE:
         {
            ...
            *period = '\0';
            ...
            fill_pathname_join(core_path, dir_libks,
                  core_filename, len);
