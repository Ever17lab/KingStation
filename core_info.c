...
#if defined(KINGSN_MOBILE) || (defined(KINGSN_CONSOLE) && !defined(PSP) && !defined(_3DS) && !defined(VITA) && !defined(HW_WUP))
   {
      ...
      bool dir_show_hidden_files)
{
   ...
   const char       *path_basedir   = libks_info_dir;
   bool                          ok = false;
...
/*
 * Matches core path A and B "base" filename (ignoring everything after _libks)
 *
 * Ex:
 *   snes9x_libks.dll and snes9x_libks_android.so are matched
 *   snes9x__2005_libks.dll and snes9x_libks_android.so are NOT matched
 */
