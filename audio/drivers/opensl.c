      ...
      unsigned *new_rate)
{
   ...
   KINGSN_LOG("[OpenSL]: Requested audio latency: %u ms.", latency);
   ...
   KINGSN_LOG("[OpenSL]: Setting audio latency: Block size = %u, Blocks = %u, Total = %u ...\n",
...
error:
   KINGSN_ERR("[OpenSL]: Couldn't initialize OpenSL ES driver, error code: [%d].\n", (int)res);
   sl_free(sl);
   return NULL;
}
...
static ssize_t sl_write(void *data, const void *buf_, size_t size)
{
   ...
   while (size)
   {
      ...
      if (sl->buffer_ptr >= sl->buf_size)
      {
         ...
         {
            KINGSN_ERR("[OpenSL]: Failed to write! (Error: 0x%x)\n", (unsigned)res);
            return -1;
         }
      }
   }
