...
static void d3d10_set_filtering(void* data, unsigned index, bool smooth, bool ctx_scaling)
{
   ...
   {
      if (smooth)
         d3d10->samplers[KINGSN_FILTER_UNSPEC][i] = d3d10->samplers[KINGSN_FILTER_LINEAR][i];
      else
         d3d10->samplers[KINGSN_FILTER_UNSPEC][i] = d3d10->samplers[KINGSN_FILTER_NEAREST][i];
   }
}
...
static bool d3d10_gfx_set_shader(void* data, enum kingsn_shader_type type, const char* path)
{
   ...
   {
      KINGSN_WARN("[D3D10] Only Slang shaders are supported. Falling back to stock.\n");
      return false;
   }
   ...
   for (i = 0; i < d3d10->shader_preset->passes; source = &d3d10->pass[i++].rt)
   {
      ...
      if (!slang_process(
               d3d10->shader_preset, i, KINGSN_SHADER_HLSL, 40, &semantics_map,
...
static void d3d10_gfx_free(void* data)
{
   ...
   for (i = 0; i < KINGSN_WRAP_MAX; i++)
   {
      Release(d3d10->samplers[KINGSN_FILTER_LINEAR][i]);
      Release(d3d10->samplers[KINGSN_FILTER_NEAREST][i]);
   }
      ...
      input_driver_t** input, void** input_data)
{
   ...
   {
      KINGSN_ERR("[D3D10]: win32_set_video_mode failed.\n");
      goto error;
   }
...

   {
      ...
      for (i = 0; i < KINGSN_WRAP_MAX; i++)
      {
         ...
         D3D10CreateSamplerState(d3d10->device, &desc,
               &d3d10->samplers[KINGSN_FILTER_LINEAR][i]);

         desc.Filter = D3D10_FILTER_MIN_MAG_MIP_POINT;
         D3D10CreateSamplerState(d3d10->device, &desc,
               &d3d10->samplers[KINGSN_FILTER_NEAREST][i]);
      }
   }
...
#include "d3d_shaders/snowflake_sm4.hlsl.h"
         ...
         video_driver_get_hw_context()->version_major == 11)
   {
      ...
      d3d10->hw.iface.interface_type    = KS_HW_RENDER_INTERFACE_D3D10;
      d3d10->hw.iface.interface_version = KS_HW_RENDER_INTERFACE_D3D10_VERSION;
...

   {
      ...
      for (;;)
      {
         ...
         KINGSN_LOG("[D3D10]: Found GPU at index %d: %s\n", i, str);
      ...
      if (0 <= gpu_index && gpu_index <= i && (gpu_index < D3D10_MAX_GPU_COUNT))
      {
         ...
         KINGSN_LOG("[D3D10]: Using GPU index %d.\n", gpu_index);
         video_driver_set_gpu_device_string(d3d10->gpu_list->elems[gpu_index].data);
      }
      else
      {
         KINGSN_WARN("[D3D10]: Invalid GPU index %d, using first device found.\n", gpu_index);
      ...
      unsigned width, unsigned height)
{
   ...
   for (i = 0; i < d3d10->shader_preset->passes; i++)
   {
      ...
      KINGSN_LOG("[D3D10]: Updating framebuffer size %u x %u.\n", width, height);
      ...
      video_frame_info_t* video_info)
{
   ...
   if (texture)
   {
      ...
      D3D10SetPShaderSamplers(
            context, 0, 1, &d3d10->samplers[KINGSN_FILTER_UNSPEC][KINGSN_WRAP_DEFAULT]);
      ...
      void* video_data, void* data, bool threaded, enum texture_filter_type filter_type)
{
   ...
   switch (filter_type)
   {
      ...
      case TEXTURE_FILTER_LINEAR:
         texture->sampler = d3d10->samplers[KINGSN_FILTER_LINEAR][KINGSN_WRAP_EDGE];
         break;
      case TEXTURE_FILTER_MIPMAP_NEAREST:
         texture->desc.MiscFlags = D3D10_RESOURCE_MISC_GENERATE_MIPS;
         /* fallthrough */
      case TEXTURE_FILTER_NEAREST:
         texture->sampler = d3d10->samplers[KINGSN_FILTER_NEAREST][KINGSN_WRAP_EDGE];
         break;
   }
...
#if 0
   static bool
d3d10_get_hw_render_interface(void* data, const struct ks_hw_render_interface** iface)
{
   d3d10_video_t* d3d10 = (d3d10_video_t*)data;
   *iface               = (const struct ks_hw_render_interface*)&d3d10->hw.iface;
