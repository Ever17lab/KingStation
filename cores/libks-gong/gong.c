...
#define GONG_CORE_PREFIX(s) libks_gong_##s
#else
...
static ks_log_printf_t GONG_CORE_PREFIX(log_cb);
static ks_video_refresh_t GONG_CORE_PREFIX(video_cb);
static ks_input_poll_t GONG_CORE_PREFIX(input_poll_cb);
static ks_input_state_t GONG_CORE_PREFIX(input_state_cb);
static ks_audio_sample_t GONG_CORE_PREFIX(audio_cb);
static ks_audio_sample_batch_t GONG_CORE_PREFIX(audio_batch_cb);
static ks_environment_t GONG_CORE_PREFIX(environ_cb);
...
static const struct ks_controller_description pads1[] = {
   { "Joypad", KS_DEVICE_JOYPAD },
   { NULL, 0 },
};

static const struct ks_controller_description pads2[] = {
   { "Joypad", KS_DEVICE_JOYPAD },
   { NULL, 0 },
};

static const struct ks_controller_info ports[] = {
   { pads1, 1 },
   { pads2, 1 },
   { 0 },
};
...
struct ks_input_descriptor desc[] = {
   { 0, KS_DEVICE_JOYPAD, 0, KS_DEVICE_ID_JOYPAD_UP,    "D-Pad Up" },
   { 0, KS_DEVICE_JOYPAD, 0, KS_DEVICE_ID_JOYPAD_DOWN,  "D-Pad Down" },
   { 0, KS_DEVICE_ANALOG, KS_DEVICE_INDEX_ANALOG_LEFT, KS_DEVICE_ID_ANALOG_Y,    "Left Analog Y" },
   { 0, KS_DEVICE_ANALOG, KS_DEVICE_INDEX_ANALOG_RIGHT, KS_DEVICE_ID_ANALOG_Y,    "Right Analog Y" },
...
static void check_variables(void)
{
   ...
   if (GONG_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      int i;
...
void GONG_CORE_PREFIX(ks_get_system_info)(struct ks_system_info *info)
{
...
void GONG_CORE_PREFIX(ks_get_system_av_info)(struct ks_system_av_info *info)
{
...
void GONG_CORE_PREFIX(ks_init)(void)
{
   struct ks_log_callback log;

   g_state = (State*)calloc(1, sizeof(*g_state));

   if (GONG_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_LOG_INTERFACE, &log))
...
void GONG_CORE_PREFIX(ks_deinit)(void)
{
   if (video_buf)
      free(video_buf);
...
void GONG_CORE_PREFIX(ks_set_environment)(ks_environment_t cb)
{
   ...
   cb(KS_ENVIRONMENT_SET_CONTROLLER_INFO, (void*)ports);
}

void GONG_CORE_PREFIX(ks_set_video_refresh)(ks_video_refresh_t cb)
{
   GONG_CORE_PREFIX(video_cb) = cb;
}

void GONG_CORE_PREFIX(ks_set_audio_sample)(ks_audio_sample_t cb)
{
   GONG_CORE_PREFIX(audio_cb) = cb;
}

void GONG_CORE_PREFIX(ks_set_audio_sample_batch)(ks_audio_sample_batch_t cb)
{
   GONG_CORE_PREFIX(audio_batch_cb) = cb;
}

void GONG_CORE_PREFIX(ks_set_input_poll)(ks_input_poll_t cb)
{
   GONG_CORE_PREFIX(input_poll_cb) = cb;
}

void GONG_CORE_PREFIX(ks_set_input_state)(ks_input_state_t cb)
{
   GONG_CORE_PREFIX(input_state_cb) = cb;
}

void GONG_CORE_PREFIX(ks_set_controller_port_device)(unsigned a, unsigned b)
{
}

void GONG_CORE_PREFIX(ks_reset)(void)
{
   memset(g_state, 0, sizeof(*g_state));
   check_variables();
}

size_t GONG_CORE_PREFIX(ks_serialize_size)(void)
{
   return STATE_SIZE;
}

bool GONG_CORE_PREFIX(ks_serialize)(void *data, size_t size)
{
   if (size != STATE_SIZE)
      return false;
...
bool GONG_CORE_PREFIX(ks_unserialize)(const void *data, size_t size)
{
   if (size != STATE_SIZE)
      return false;
...
void GONG_CORE_PREFIX(ks_cheat_reset)(void)
{
}

void GONG_CORE_PREFIX(ks_cheat_set)(unsigned a, bool b, const char * c)
{
}

bool GONG_CORE_PREFIX(ks_load_game)(const struct ks_game_info *info)
{
   enum ks_pixel_format fmt = KS_PIXEL_FORMAT_XRGB8888;

   check_variables();

   if (!GONG_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt))
   {
      if (GONG_CORE_PREFIX(log_cb))
         GONG_CORE_PREFIX(log_cb)(KS_LOG_INFO, "XRGB8888 is not supported.\n");
      return false;
   }
...
bool GONG_CORE_PREFIX(ks_load_game_special)(unsigned a, const struct ks_game_info *b, size_t c)
{
   return false;
}

void GONG_CORE_PREFIX(ks_unload_game)(void)
{
}

unsigned GONG_CORE_PREFIX(ks_get_region)(void)
{
   return KS_REGION_NTSC;
}

void* GONG_CORE_PREFIX(ks_get_memory_data)(unsigned id)
{
   return NULL;
}

size_t GONG_CORE_PREFIX(ks_get_memory_size)(unsigned id)
{
   return 0;
}
...
void GONG_CORE_PREFIX(ks_run)(void)
{
   int i = 0;
   int port = 0;
   bool updated = false;
   ks_inputs inputs[MAX_PLAYERS] = {{0}};

   if (GONG_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE_UPDATE, &updated) && updated)
      check_variables();
   ...
   for (port = 0; port < MAX_PLAYERS; port++)
   {
      for (i = 0; i < 16; i++)
      {
         if (GONG_CORE_PREFIX(input_state_cb)(port, KS_DEVICE_JOYPAD, 0, i))
         {
            inputs[port].realinput |= 1 << i;
         }
      }

      inputs[port].analogYLeft = GONG_CORE_PREFIX(input_state_cb)(port, KS_DEVICE_ANALOG, KS_DEVICE_INDEX_ANALOG_LEFT, KS_DEVICE_ID_ANALOG_Y) / 5000.0f;
      inputs[port].analogYRight = GONG_CORE_PREFIX(input_state_cb)(port, KS_DEVICE_ANALOG, KS_DEVICE_INDEX_ANALOG_RIGHT, KS_DEVICE_ID_ANALOG_Y) / 5000.0f;

      if (inputs[port].analogYLeft > 0)
         inputs[port].realinput |= (1 << KS_DEVICE_ID_JOYPAD_DOWN);
      else if (inputs[port].analogYRight > 0)
         inputs[port].realinput |= (1 << KS_DEVICE_ID_JOYPAD_DOWN);

      if (inputs[port].analogYLeft < 0)
         inputs[port].realinput |= (1 << KS_DEVICE_ID_JOYPAD_UP);
      else if (inputs[port].analogYRight < 0)
         inputs[port].realinput |= (1 << KS_DEVICE_ID_JOYPAD_UP);
      ...
      if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_UP))
         process_joypad(&g_state->g_input[port].buttons[B_MOVE_UP], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_UP));
      else if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_DOWN))
         process_joypad(&g_state->g_input[port].buttons[B_MOVE_DOWN], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_DOWN));
      else if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_DOWN))
         process_joypad(&g_state->g_input[port].buttons[B_MOVE_DOWN], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_DOWN));

      if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_A))
         process_joypad(&g_state->g_input[port].buttons[B_SPEED_UP], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_A));
      else if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_B))
         process_joypad(&g_state->g_input[port].buttons[B_SPEED_UP], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_B));
      else if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_X))
         process_joypad(&g_state->g_input[port].buttons[B_SPEED_UP], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_X));
      else if (is_key_up_or_down(inputs[port].input, inputs[port].not_input, KS_DEVICE_ID_JOYPAD_Y))
         process_joypad(&g_state->g_input[port].buttons[B_SPEED_UP], inputs[port].realinput & (1 << KS_DEVICE_ID_JOYPAD_Y));
...
unsigned GONG_CORE_PREFIX(ks_api_version)(void)
{
