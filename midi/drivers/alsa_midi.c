...
static bool alsa_midi_get_avail_ports(struct string_list *ports, unsigned caps)
{
   ...
   if (r < 0)
   {
      KINGSN_ERR("[MIDI]: snd_seq_open failed with error %d.\n", r);
      return false;
   }
...
static bool alsa_midi_set_input(void *p, const char *input)
{
   ...
   if (r < 0)
   {
      KINGSN_ERR("[MIDI]: snd_seq_create_simple_port failed with error %d.\n", r);
      return false;
   }
   ...
   if (r < 0)
      KINGSN_ERR("[MIDI]: snd_seq_subscribe_port failed with error %d.\n", r);
...
static bool alsa_midi_set_output(void *p, const char *output)
{
   ...
   if (r < 0)
   {
      KINGSN_ERR("[MIDI]: snd_seq_connect_to failed with error %d.\n", r);
      return false;
   }
   ...
   if (d->out_queue < 0)
   {
      KINGSN_ERR("[MIDI]: snd_seq_alloc_queue failed with error %d.\n", d->out_queue);
      return false;
   }
   ...
   if (r < 0)
   {
       KINGSN_ERR("[MIDI]: snd_seq_start_queue failed with error %d.\n", r);
       return false;
   }
...
static bool alsa_midi_read(void *p, midi_event_t *event)
{
...
#ifdef DEBUG
      if (r != -EAGAIN)
         KINGSN_ERR("[MIDI]: snd_seq_event_input failed with error %d.\n", r);
#endif
      return false;
   }
   ...
   else if (ev->type == SND_SEQ_EVENT_SYSEX)
   {
      ...
      {
         KINGSN_ERR("[MIDI]: SysEx event too big.\n");
         r = -1;
      }
#endif
   }
...
#ifdef DEBUG
   if (r < 0)
      KINGSN_ERR("[MIDI]: snd_seq_event_output failed with error %d.\n", r);
#endif
...
#ifdef DEBUG
   if (r < 0)
      KINGSN_ERR("[MIDI]: snd_seq_drain_output failed with error %d.\n", r);
#endif
