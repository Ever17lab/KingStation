...
if [ "$OS" = 'BSD' ]; then
   ...
   die 1 'Error: Cygwin is not a supported platform. See https://bot.libks.com/docs/compilation/windows/'
elif [ "$OS" = 'SunOS' ]; then
   SORT='gsort'
fi
...
if [ "$LIBKS" ]; then
   die : 'Notice: Explicit libks used, disabling dynamic libks loading ...'
   add_opt DYNAMIC no
else
   LIBKS="-lks"
fi
...
[ "$HAVE_DYNAMIC" = 'yes' ] || {
   check_lib '' KS "$LIBKS" ks_init "$DYLIB" '' '' 'Cannot find libks, did you forget --with-libks="-lks"?'
   add_define MAKEFILE libks "$LIBKS"
}

add_define MAKEFILE ASSETS_DIR "${ASSETS_DIR:-$SHARE_DIR}/KingStation"
add_define MAKEFILE BIN_DIR "${BIN_DIR:-${PREFIX}/bin}"
add_define MAKEFILE DOC_DIR "${DOC_DIR:-${SHARE_DIR}/doc/KingStation}"
...
if [ "$HAVE_NETWORKING" != 'no' ]; then
   ...
   if [ "$OS" = 'Win32' ]; then
      ...
      if [ "$HAVE_GETADDRINFO" != 'yes' ]; then
         ...
         die : 'Notice: KingStation will use legacy socket support'
      fi
   fi
...
if [ "$HAVE_DYLIB" = 'no' ] && [ "$HAVE_DYNAMIC" = 'yes' ]; then
   die 1 'Error: Dynamic loading of libks is enabled, but your platform does not appear to have dlopen(), use --disable-dynamic or --with-libks="-lks".'
fi
