...
static void cg_error_handler(CGcontext ctx, CGerror error, void *data)
{
   ...
   switch (error)
   {
      case CG_INVALID_PARAM_HANDLE_ERROR:
         KINGSN_ERR("CG: Invalid param handle.\n");
         break;

      case CG_INVALID_PARAMETER_ERROR:
         KINGSN_ERR("CG: Invalid parameter.\n");
         break;
...
static void gl_cg_reset_attrib(void *data)
{
   ...
   ks_assert(cg->attribs_index <= ARRAY_SIZE(cg->attribs_elems));
...
static void gl_cg_deinit_progs(void *data)
{
   ...
   KINGSN_LOG("[CG]: Destroying programs.\n");
...
static void gl_cg_deinit_context_state(void *data)
{
   ...
   if (cg->cgCtx)
   {
      KINGSN_LOG("[CG]: Destroying context.\n");
...
static void gl_cg_set_program_base_attrib(void *data, unsigned i)
{
   ...
   for (; param; param = cgGetNextParameter(param))
   {
      ...
      KINGSN_LOG("[CG]: Found semantic \"%s\" in prog #%u.\n", semantic, i);
...
error:
   KINGSN_ERR("Failed to compile passthrough shader, is something wrong with your environment?\n");
   return false;
}
...
static bool gl_cg_load_plain(void *data, const char *path)
{
   ...
   {
      KINGSN_LOG("[CG]: Loading stock Cg file.\n");
      cg->prg[1] = cg->prg[0];
   }
   else
   {
      ...
      KINGSN_LOG("[CG]: Loading Cg file: %s\n", path);
...
static bool gl_cg_load_shader(void *data, unsigned i)
{
   ...
   KINGSN_LOG("[CG]: Loading Cg shader: \"%s\".\n",
...
static bool gl_cg_load_preset(void *data, const char *path)
{
   ...
   KINGSN_LOG("[CG]: Loading Cg meta-shader: %s\n", path);
   ...
   {
      KINGSN_ERR("Failed to parse CGP file.\n");
      return false;
   }

   if (cg->shader->passes > GFX_MAX_SHADERS - 3)
   {
      KINGSN_WARN("Too many shaders ... Capping shader amount to %d.\n",
            GFX_MAX_SHADERS - 3);
   ...
   for (i = 0; i < cg->shader->passes; i++)
   {
      ...
      {
         KINGSN_ERR("Failed to load shaders ...\n");
         return false;
      }
   }
   ...
   {
      KINGSN_ERR("Failed to load lookup textures ...\n");
      return false;
   }
...
static void *gl_cg_init(void *data, const char *path)
{
   ...
   if (!cg->cgCtx)
   {
      KINGSN_ERR("Failed to create Cg context.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("Invalid profile type\n");
      goto error;
   }

   KINGSN_LOG("[CG]: Vertex profile: %s\n",   cgGetProfileString(cg->cgVProf));
   KINGSN_LOG("[CG]: Fragment profile: %s\n", cgGetProfileString(cg->cgFProf));
...

   {
      ...
      if (!string_is_empty(path) && type != KINGSN_SHADER_CG)
      {
         KINGSN_ERR("[CG]: Invalid shader type, falling back to stock.\n");
         path = NULL;
      }
...
static bool gl_cg_filter_type(void *data, unsigned idx, bool *smooth)
{
   cg_shader_data_t *cg = (cg_shader_data_t*)data;
   if (cg && idx &&
         (cg->shader->pass[idx - 1].filter != KINGSN_FILTER_UNSPEC)
      )
   {
      *smooth = (cg->shader->pass[idx - 1].filter == KINGSN_FILTER_LINEAR);
      return true;
   }
