...
{
   state[KS_DEVICE_INDEX_ANALOG_LEFT] [KS_DEVICE_ID_ANALOG_X] = WIIU_READ_STICK(vpad->leftStick.x);
   state[KS_DEVICE_INDEX_ANALOG_LEFT] [KS_DEVICE_ID_ANALOG_Y] = WIIU_READ_STICK(vpad->leftStick.y);
   state[KS_DEVICE_INDEX_ANALOG_RIGHT][KS_DEVICE_ID_ANALOG_X] = WIIU_READ_STICK(vpad->rightStick.x);
   state[KS_DEVICE_INDEX_ANALOG_RIGHT][KS_DEVICE_ID_ANALOG_Y] = WIIU_READ_STICK(vpad->rightStick.y);
}
...
static void log_coords(int16_t x, int16_t y)
{
   ...
   KINGSN_LOG("[wpad]: calibrated point: %s%04x, %s%04x\n",
         x_negative ? "-" : "",
         x_digit,
         y_negative ? "-" : "",
         y_digit);
      ...
      uint64_t *buttons, VPADStatus *vpad, VPADChan channel)
{
   ...
   state[WIIU_DEVICE_INDEX_TOUCHPAD][KS_DEVICE_ID_ANALOG_X] = scale_touchpad(
         viewport.x, viewport.x + viewport.width, -0x7fff, 0x7fff, point.x);
   state[WIIU_DEVICE_INDEX_TOUCHPAD][KS_DEVICE_ID_ANALOG_Y] = scale_touchpad(
         viewport.y, viewport.y + viewport.height, -0x7fff, 0x7fff, point.y);

#if 0
   log_coords(state[WIIU_DEVICE_INDEX_TOUCHPAD][KS_DEVICE_ID_ANALOG_X],
         state[WIIU_DEVICE_INDEX_TOUCHPAD][KS_DEVICE_ID_ANALOG_Y]);
#endif
