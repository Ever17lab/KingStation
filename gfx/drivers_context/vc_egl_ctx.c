...
static INLINE bool gfx_ctx_vc_egl_query_extension(vc_ctx_data_t *vc, const char *ext)
{
   ...
   KINGSN_LOG("Querying EGL extension: %s => %s\n",
...
static void *gfx_ctx_vc_init(void *video_driver)
{
   ...
   if (g_egl_inited)
   {
      KINGSN_ERR("[VC/EGL]: Attempted to re-initialize driver.\n");
      return NULL;
   }
      ...
      const video_info_t *video)
{
   ...
   vc->res        = video->input_scale * KINGSN_SCALE_BASE;
   ...
   {
      KINGSN_ERR("[VideoCore:EGLImage] failed to create PbufferSurface\n");
      goto fail;
   }
   ...
   {
      KINGSN_ERR("[VideoCore:EGLImage] failed to create context\n");
      goto fail;
   }
   ...
   if (result == EGL_FALSE)
   {
      KINGSN_ERR("[VideoCore:EGLImage] failed to make context current\n");
      goto fail;
   }
