      ...
      struct audio_lock *region, HRESULT res)
{
   ...
   {
#ifdef DEBUG
      KINGSN_WARN("[DirectSound error]: %s\n", "DSERR_BUFFERLOST");
#endif
      ...
      unsigned *new_rate)
{
      ...
      if (list && list->elems)
      {
         if (list->elems)
         {
            ...
            if (idx_found == -1 && isdigit(dev[0]))
            {
               ...
               KINGSN_LOG("[DirectSound]: Fallback, device index is a single number index instead: %d.\n", idx_found);

               if (idx_found != -1)
               {
                  if (idx_found < (int32_t)list->size)
                  {
                     KINGSN_LOG("[DirectSound]: Corresponding name: %s\n", list->elems[idx_found].data);
                     selected_device = (LPGUID)list->elems[idx_found].userdata;
                  }
               }
            }
   ...
   KINGSN_LOG("[DirectSound]: Setting buffer size of %u bytes\n", ds->buffer_size);
   KINGSN_LOG("[DirectSound]: Latency = %u ms\n", (unsigned)((1000 * ds->buffer_size) / wfx.nAvgBytesPerSec));
...
error:
   KINGSN_ERR("[DirectSound] Error occurred in init.\n");
   if (list)
      string_list_free(list);
   dsound_free(ds);
   return NULL;
