...
static void apple_gamecontroller_joypad_poll_internal(GCController *controller)
{
    ...
    else
    {
        ...
        pause              = *buttons & (1 << KS_DEVICE_ID_JOYPAD_START);
        select             = *buttons & (1 << KS_DEVICE_ID_JOYPAD_SELECT);
        l3                 = *buttons & (1 << KS_DEVICE_ID_JOYPAD_L3);
        r3                 = *buttons & (1 << KS_DEVICE_ID_JOYPAD_R3);
    ...
    if (controller.extendedGamepad)
    {
        ...
        *buttons             |= gp.dpad.up.pressed         ? (1 << KS_DEVICE_ID_JOYPAD_UP)    : 0;
        *buttons             |= gp.dpad.down.pressed       ? (1 << KS_DEVICE_ID_JOYPAD_DOWN)  : 0;
        *buttons             |= gp.dpad.left.pressed       ? (1 << KS_DEVICE_ID_JOYPAD_LEFT)  : 0;
        *buttons             |= gp.dpad.right.pressed      ? (1 << KS_DEVICE_ID_JOYPAD_RIGHT) : 0;
        *buttons             |= gp.buttonA.pressed         ? (1 << KS_DEVICE_ID_JOYPAD_B)     : 0;
        *buttons             |= gp.buttonB.pressed         ? (1 << KS_DEVICE_ID_JOYPAD_A)     : 0;
        *buttons             |= gp.buttonX.pressed         ? (1 << KS_DEVICE_ID_JOYPAD_Y)     : 0;
        *buttons             |= gp.buttonY.pressed         ? (1 << KS_DEVICE_ID_JOYPAD_X)     : 0;
        *buttons             |= gp.leftShoulder.pressed    ? (1 << KS_DEVICE_ID_JOYPAD_L)     : 0;
        *buttons             |= gp.rightShoulder.pressed   ? (1 << KS_DEVICE_ID_JOYPAD_R)     : 0;
        *buttons             |= gp.leftTrigger.pressed     ? (1 << KS_DEVICE_ID_JOYPAD_L2)    : 0;
        *buttons             |= gp.rightTrigger.pressed    ? (1 << KS_DEVICE_ID_JOYPAD_R2)    : 0;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 120100 || __TV_OS_VERSION_MAX_ALLOWED >= 120100
        if (@available(iOS 12.1, *))
        {
            *buttons         |= gp.leftThumbstickButton.pressed ? (1 << KS_DEVICE_ID_JOYPAD_L3) : 0;
            *buttons         |= gp.rightThumbstickButton.pressed ? (1 << KS_DEVICE_ID_JOYPAD_R3) : 0;
        }
#endif
        ...
        if (@available(iOS 13, *))
        {
            ...
            *buttons         |= gp.buttonOptions.pressed ? (1 << KS_DEVICE_ID_JOYPAD_SELECT) : 0;
            ...
            if (gp.buttonMenu.pressed )
            {
                if (gp.leftShoulder.pressed)
                    *buttons     |= 1 << KS_DEVICE_ID_JOYPAD_SELECT;
                else if (gp.leftTrigger.pressed)
                    *buttons     |= 1 << KS_DEVICE_ID_JOYPAD_L3;
                else if (gp.rightTrigger.pressed)
                    *buttons     |= 1 << KS_DEVICE_ID_JOYPAD_R3;
                else
                    *buttons     |= 1 << KS_DEVICE_ID_JOYPAD_START;
            }
        }
#endif
...
static void apple_gamecontroller_joypad_register(GCGamepad *gamepad)
{
    ...
    else
    {
...

        {
           ...
           {
              mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_START);
              mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_L);
              mfi_buttons[slot] |= (1 << KS_DEVICE_ID_JOYPAD_SELECT);
           ...
           {
              mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_L2);
              mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_START);
              mfi_buttons[slot] |= (1 << KS_DEVICE_ID_JOYPAD_L3);
              dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_L3);
                    });
              return;
           ...
           {
              mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_R2);
              mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_START);
              mfi_buttons[slot] |= (1 << KS_DEVICE_ID_JOYPAD_R3);
              dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    mfi_buttons[slot] &= ~(1 << KS_DEVICE_ID_JOYPAD_R3);
                    });
              return;
