 ...
 *  Copyright (C) 2018 The KingStation team
 *
...
static bool winmm_midi_queue_read(winmm_midi_queue_t *q, midi_event_t *ev)
{
   ...
   {
#ifdef DEBUG
      KINGSN_ERR("[MIDI]: Input queue read failed (event data too small).\n");
#endif
      return false;
   }
...
static bool winmm_midi_queue_write(winmm_midi_queue_t *q, const midi_event_t *ev)
{
   ...
   {
#ifdef DEBUG
      KINGSN_ERR("[MIDI]: Input queue overflow.\n");
#endif
      return false;
   }
   ...
   {
#ifdef DEBUG
      KINGSN_ERR("[MIDI]: Input queue write failed (event too big).\n");
#endif
      return false;
   }
      ...
      DWORD_PTR q, DWORD_PTR par1, DWORD_PTR par2)
{
   ...
   else if (msg == MIM_DATA)
   {
      ...
      {
#ifdef DEBUG
         KINGSN_ERR("[MIDI]: Input event dropped.\n");
#endif
      }
   }
   ...
   {
#ifdef DEBUG
      KINGSN_WARN("[MIDI]: SysEx input not implemented, event dropped.\n");
#endif
   }
}
   ...
   for (i = 0; i < dev_count; ++i)
   {
      ...
      if (mmr == MMSYSERR_NOERROR)
      {
         if (string_is_equal(caps.szPname, dev_name))
         {
            ...
            if (mmr != MMSYSERR_NOERROR)
               KINGSN_ERR("[MIDI]: midiInOpen failed with error %d.\n", mmr);
            break;
         }
      }
      else
         KINGSN_WARN("[MIDI]: midiInGetDevCapsA failed with error %d.\n", mmr);
   }
   ...
   for (i = 0; i < dev_count; ++i)
   {
      ...
      if (mmr == MMSYSERR_NOERROR)
      {
         if (string_is_equal(caps.szPname, dev_name))
         {
            ...
            if (mmr != MMSYSERR_NOERROR)
               KINGSN_ERR("[MIDI]: midiStreamOpen failed with error %d.\n", mmr);
            break;
         }
      }
      else
         KINGSN_WARN("[MIDI]: midiOutGetDevCapsA failed with error %d.\n", mmr);
   }
...
static bool winmm_midi_init_clock(HMIDISTRM out_dev, double *tick_dur)
{
   ...
   if (mmr != MMSYSERR_NOERROR)
   {
      KINGSN_ERR("[MIDI]: Current tempo unavailable (error %d).\n", mmr);
      return false;
   }
   ...
   if (mmr != MMSYSERR_NOERROR)
   {
      KINGSN_ERR("[MIDI]: Time division change failed (error %d).\n", mmr);
      return false;
   }

   *tick_dur = (double)tempo.dwTempo / (double)division.dwTimeDiv;
#ifdef DEBUG
   KINGSN_LOG("[MIDI]: Tick duration %f us.\n", *tick_dur);
#endif
      ...
      winmm_midi_buffer_t *bufs)
{
   ...
   for (i = 0; i < WINMM_MIDI_BUF_CNT; ++i)
   {
      ...
      if (mmr != MMSYSERR_NOERROR)
      {
         KINGSN_ERR("[MIDI]: midiOutPrepareHeader failed with error %d.\n", mmr);
...
static void winmm_midi_free_output_buffers(HMIDISTRM dev, winmm_midi_buffer_t *bufs)
{
   ...
   for (i = 0; i < WINMM_MIDI_BUF_CNT; ++i)
   {
      ...
      if (mmr != MMSYSERR_NOERROR)
         KINGSN_ERR("[MIDI]: midiOutUnprepareHeader failed with error %d.\n", mmr);
   }
}
...
static bool winmm_midi_get_avail_inputs(struct string_list *inputs)
{
   ...
   for (i = 0; i < dev_count; ++i)
   {
      ...
      if (mmr != MMSYSERR_NOERROR)
      {
         KINGSN_ERR("[MIDI]: midiInGetDevCapsA failed with error %d.\n", mmr);
         return false;
      }

      if (!string_list_append(inputs, caps.szPname, attr))
      {
         KINGSN_ERR("[MIDI]: string_list_append failed.\n");
         return false;
      }
   }
...
static bool winmm_midi_get_avail_outputs(struct string_list *outputs)
{
   ...
   for (i = 0; i < dev_count; ++i)
   {
      ...
      if (mmr != MMSYSERR_NOERROR)
      {
         KINGSN_ERR("[MIDI]: midiOutGetDevCapsA failed with error %d.\n", mmr);
         return false;
      }
...
static void *winmm_midi_init(const char *input, const char *output)
{
   ...
   if (input)
   {
      ...
      else
      {
         ...
         if (mmr != MMSYSERR_NOERROR)
         {
            KINGSN_ERR("[MIDI]: midiInStart failed with error %d.\n", mmr);
            err = true;
         }
      }
   }
   ...
   if (output)
   {
      ...
      else
      {
         ...
         if (mmr != MMSYSERR_NOERROR)
         {
            KINGSN_ERR("[MIDI]: midiStreamRestart failed with error %d.\n", mmr);
            err = true;
         }
      }
   }
...
static bool winmm_midi_flush(void *p)
{
   ...
   if (buf->header.dwBytesRecorded)
   {
      ...
      {
#ifdef DEBUG
         KINGSN_ERR("[MIDI]: midiStreamOut failed with error %d.\n", mmr);
#endif
