      ...
      char *s2, size_t len2)
{
   *w = 19;
   ...
   if (!path_is_empty(KINGSN_PATH_CONFIG))
      fill_pathname_base(s, path_get(KINGSN_PATH_CONFIG),
            len);
   else
      ...
      char *s2, size_t len2)
{
   ...
   unsigned user_idx                     = (type - MENU_SETTINGS_INPUT_DESC_BEGIN) / (KINGSN_FIRST_CUSTOM_BIND + 8);
   unsigned btn_idx                      = (type - MENU_SETTINGS_INPUT_DESC_BEGIN) - (KINGSN_FIRST_CUSTOM_BIND + 8) * user_idx;
   ...
   {
      if (remap_idx < KINGSN_FIRST_CUSTOM_BIND)
         strlcpy(s, descriptor, len);
      else if (!string_is_empty(descriptor) && remap_idx >= KINGSN_FIRST_CUSTOM_BIND && remap_idx % 2 == 0)
         snprintf(s, len, "%s %c", descriptor, '+');
      else if (remap_idx >= KINGSN_FIRST_CUSTOM_BIND && remap_idx % 2 != 0)
         snprintf(s, len, "%s %c", descriptor, '-');
   }
   ...
   char *s2, size_t len2)
{
   ...
   for (key_id = 0; key_id < KINGSN_MAX_KEYS - 1; key_id++)
   {
      if (remap_id == key_descriptors[key_id].key)
         break;
   }

   if (key_descriptors[key_id].key != KSK_FIRST)
   {
...
static void menu_action_setting_disp_set_label_perf_counters(
      ...
      char *s2, size_t len2)
{
   struct ks_perf_counter **counters = ks_get_perf_counter_kingsn();
   unsigned offset = type - MENU_SETTINGS_PERF_COUNTERS_BEGIN;
   general_disp_set_label_perf_counters(counters, offset, s, len,
         s2, len, path, w);
}

static void menu_action_setting_disp_set_label_libks_perf_counters(
      file_list_t* list,
      unsigned *w, unsigned type, unsigned i,
      const char *label,
      char *s, size_t len,
      ...
      char *s2, size_t len2)
{
   struct ks_perf_counter **counters = ks_get_perf_counter_libks();
   unsigned offset = type - MENU_SETTINGS_LIBKS_PERF_COUNTERS_BEGIN;
   general_disp_set_label_perf_counters(counters, offset, s, len,
         s2, len, path, w);
}
      ...
      char *s2, size_t len2)
{
   ...
   *w = 19;
   ...
   if (!string_is_empty(path_get(KINGSN_PATH_BASENAME)))
      strlcpy(s, path_basename(path_get(KINGSN_PATH_BASENAME)), len);
      ...
      char *s2, size_t len2)
{
   menu_file_list_cbs_t *cbs = (menu_file_list_cbs_t*)
      list->list[i].actiondata;
   kingsn_setting_t *setting  = cbs->setting;
      ...
      menu_file_list_cbs_t *cbs, unsigned type)
{
   ...
   info_range_list_t info_list[] = {
      ...
      },
      {
         ...
         MENU_SETTINGS_LIBKS_PERF_COUNTERS_END,
         menu_action_setting_disp_set_label_libks_perf_counters
      },
      {
