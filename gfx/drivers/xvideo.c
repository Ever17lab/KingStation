...
static void xv_set_nonblock_state(void *data, bool state, bool c, unsigned d)
{
   ...
   if (atom != None && xv->port)
      ...
      KINGSN_WARN("Failed to set SYNC_TO_VBLANK attribute.\n");
}
...
static void xv_init_font(xv_t *xv, const char *font_path, unsigned font_size)
{
            ...
            video_font_size))
   {
      ...
      KINGSN_LOG("[XVideo]: Could not initialize fonts.\n");
}
      ...
      input_driver_t **input, void **input_data)
{
   ...
   if (!g_x11_dpy)
   {
      KINGSN_ERR("[XVideo]: Cannot connect to the X server.\n");
      KINGSN_ERR("[XVideo]: Check DISPLAY variable and if X is running.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[XVideo]: XShm extension not found.\n");
      goto error;
   }
   ...
   if (ret != Success)
   {
      if (ret == XvBadExtension)
         KINGSN_ERR("[XVideo]: Xv extension not found.\n");
      else if (ret == XvBadAlloc)
         KINGSN_ERR("[XVideo]: XvQueryAdaptors() failed to allocate memory.\n");
      else
         KINGSN_ERR("[XVideo]: Unkown error in XvQueryAdaptors().\n");
   ...
   if (adaptor_count == 0)
   {
      KINGSN_ERR("[XVideo]: XvQueryAdaptors() found 0 adaptors.\n");
      goto error;
   }
   ...
   for (i = 0; i < adaptor_count; i++)
   {
      ...
      KINGSN_LOG("[XVideo]: Found suitable XvPort #%u\n", (unsigned)xv->port);
      break;
   }
   ...
   if (xv->port == 0)
   {
      KINGSN_ERR("[XVideo]: Failed to find valid XvPort or format.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[XVideo]: Unable to find Xv-compatible visual.\n");
      goto error;
   }
   ...
   if (video->fullscreen && video_disable_composition)
   {
      ...
      KINGSN_LOG("[XVideo]: Requesting compositor bypass.\n");
   ...
   if (!xv->image)
   {
      KINGSN_ERR("[XVideo]: XShmCreateImage failed.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[XVideo]: XShmAttach failed.\n");
      goto error;
   }
...
static bool xv_check_resize(xv_t *xv, unsigned width, unsigned height)
{
   ...
   if (xv->width != (width << 1) || xv->height != (height << 1))
   {
      ...
      if (xv->image == None)
      {
         KINGSN_ERR("[XVideo]: Failed to create image.\n");
         return false;
      }
      ...
      {
         KINGSN_ERR("[XVideo]: Failed to init SHM.\n");
         return false;
      }
      ...
      if (!XShmAttach(g_x11_dpy, &xv->shminfo))
      {
         KINGSN_ERR("[XVideo]: Failed to reattch XvShm image.\n");
         return false;
      }
      XSync(g_x11_dpy, False);
      memset(xv->image->data, 128, xv->image->data_size);
