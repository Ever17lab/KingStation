      ...
      input_driver_t **input, void **input_data)
{
   ...
   (void)input;
   (void)input_data;
   KINGSN_ERR("Sanity check fail! Threaded mustn't be reinit.\n");
   abort();
   return NULL;
}
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (!thr->nonblock)
   {
      ...
      ks_time_t target_frame_time = (ks_time_t)
         roundf(1000000 / video_info->refresh_rate);
      ks_time_t target = thr->last_time + target_frame_time;

      /* Ideally, use absolute time, but that is only a good idea on POSIX. */
      while (thr->frame.updated)
      {
         ks_time_t current = cpu_features_get_time_usec();
         ks_time_t delta   = target - current;
      ...
      input_driver_t **input, void **input_data)
{
   ...
   max_size                  = info.input_scale * KINGSN_SCALE_BASE;
   max_size                 *= max_size;
...
static void video_thread_free(void *data)
{
   ...
   KINGSN_LOG("Threaded video stats: Frames pushed: %u, Frames dropped: %u.\n",
