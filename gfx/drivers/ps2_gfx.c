...
#include "../../libks-common/include/libks_gskit_ps2.h"
...
typedef struct ps2_video
{
   ...
   struct ks_hw_render_interface_gskit_ps2 iface;
...
static void init_ps2_video(ps2_video_t *ps2)
{
   ...
   ps2->iface.interface_type    = KS_HW_RENDER_INTERFACE_GSKIT_PS2;
   ps2->iface.interface_version = KS_HW_RENDER_INTERFACE_GSKIT_PS2_VERSION;
...
static void prim_texture(GSGLOBAL *gsGlobal, GSTEXTURE *texture, int zPosition, bool force_aspect, struct ks_hw_ps2_insets padding)
{
   float x1, y1, x2, y2;
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (frame)
   {
      struct ks_hw_ps2_insets padding = empty_ps2_insets;
      ...
      const struct ks_hw_render_interface** iface)
{
   ...
   *iface                    = 
      (const struct ks_hw_render_interface*)&ps2->iface;
   return true;
}
