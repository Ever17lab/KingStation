...
{
#ifdef HAVE_SDL2
   /* TODO: see if a kingsn <-> sdl translation is needed. */
   if (pad->controller)
...
static void sdl_pad_connect(unsigned id)
{
   ...
   if (!success)
   {
      KINGSN_ERR("[SDL]: Couldn't open joystick #%u: %s.\n", id, SDL_GetError());
   ...
   if (g_has_haptic)
   {
      ...
      if (!pad->haptic)
         KINGSN_WARN("[SDL]: Couldn't open haptic device of the joypad #%u: %s\n",
               id, SDL_GetError());
   }
   ...
   if (pad->haptic)
   {
      ...
      {
         pad->rumble_effect = -2;
         KINGSN_WARN("[SDL]: Device #%u does not support rumble.\n", id);
      }
   }
#else
   pad->num_axes    = SDL_JoystickNumAxes(pad->joypad);
...
static void *sdl_joypad_init(void *data)
{
   ...
   if (SDL_InitSubSystem(SDL_INIT_HAPTIC) < 0)
      KINGSN_WARN("[SDL]: Failed to initialize haptic device support: %s\n",
            SDL_GetError());
   else
      g_has_haptic = true;
#endif
...
#ifdef HAVE_SDL2
static bool sdl_joypad_set_rumble(unsigned pad, enum ks_rumble_effect effect, uint16_t strength)
{
   SDL_HapticEffect efx;
   ...
   if (joypad->rumble_effect == -1)
   {
      ...
      {
         KINGSN_WARN("[SDL]: Failed to create rumble effect for joypad %u: %s\n",
                    pad, SDL_GetError());
   ...
   {
      KINGSN_WARN("[SDL]: Failed to set rumble effect on joypad %u: %s\n",
                          pad, SDL_GetError());
      return false;
   }
