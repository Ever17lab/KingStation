...
#define DOCS_URL                    "http://docs.libks.com/"
...
#ifdef HAVE_LIBKSDB
static void scan_finished_handler(ks_task_t *task,
   ...
   ,m_itemsCountLabel(new QLabel(this))
{
   ...
   QString                      configDir = QFileInfo(path_get(KINGSN_PATH_CONFIG)).dir().absolutePath();
   ...
   qRegisterMetaType<ks_task_callback_t>("ks_task_callback_t");
   ...
   connect(this, SIGNAL(extractArchiveDeferred(QString,QString,QString,ks_task_callback_t)), this, SLOT(onExtractArchive(QString,QString,QString,ks_task_callback_t)), Qt::QueuedConnection);
...
#ifdef HAVE_OPENSSL
   {
      ...
      KINGSN_LOG("[Qt]: TLS supports %d ciphers.\n", method->num_ciphers());
#endif
      KINGSN_LOG("[Qt]: Using %s\n", OPENSSL_VERSION_TEXT);
   }
#endif
}
...
void MainWindow::showWelcomeScreen()
{
   ...
   const QString welcomeText = QStringLiteral(""
      ...
      "<br>\n"
      "Documentation for KingStation, libks and cores:<br>\n"
      "<a href=\"https://docs.libks.com/\">https://docs.libks.com/</a>");
...
QString MainWindow::changeThumbnail(const QImage &image, QString type)
{
   ...
   {
      if (dir.mkpath("."))
         KINGSN_LOG("[Qt]: Created directory: %s\n", dirData);
      else
      {
         KINGSN_ERR("[Qt]: Could not create directory: %s\n", dirData);
         return QString();
      }
   }
   ...
   KINGSN_ERR("[Qt]: Could not save image: %s\n", thumbData);
   return QString();
}
...
QHash<QString, QString> MainWindow::getSelectedCore()
{
   ...
   switch(coreSelection)
   {
      case CORE_SELECTION_CURRENT:
         coreHash["core_path"] = path_get(KINGSN_PATH_CORE);
         break;
...
void MainWindow::loadContent(const QHash<QString, QString> &contentHash)
{
   ...
   {
      case CORE_SELECTION_CURRENT:
         corePathArray     = path_get(KINGSN_PATH_CORE);
...
void MainWindow::setCoreActions()
{
         ...
         && m_settings->value("suggest_loaded_core_first", false).toBool())
   {
      ...
      comboBoxMap["core_path"]      = path_get(KINGSN_PATH_CORE);
...
void MainWindow::renamePlaylistItem(QListWidgetItem *item, QString newName)
{
   ...
   {
      KINGSN_ERR("[Qt]: Could not rename playlist.\n");
      item->setText(oldName);
   }
...
int MainWindow::onExtractArchive(QString path, QString extractionDir, QString tempExtension, ks_task_callback_t cb)
{
   unsigned i;
   ...
   if (!file_list || file_list->size == 0)
   {
      ...
      KINGSN_ERR("[Qt]: Downloaded archive is empty?\n");
      return -1;
   }
         ...
         if (!fileObj.remove())
         {
            ...
            if (fileTemp.exists())
            {
               if (!fileTemp.remove())
               {
                  ...
                  KINGSN_ERR("[Qt]: Could not delete file: %s\n", file_list->elems[i].data);
                  return -1;
               }
            }
            ...
            if (!fileObj.rename(fileTemp.fileName()))
            {
               ...
               KINGSN_ERR("[Qt]: Could not rename file: %s\n", file_list->elems[i].data);
               return -1;
            }
         }
      }
