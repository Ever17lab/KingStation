...
- (bool)_initMetal
{
...

   {
      ...
      if (err != nil)
      {
         KINGSN_ERR("[Metal]: error creating pipeline state %s\n", err.localizedDescription.UTF8String);
         return NO;
      }
      ...
      if (err != nil)
      {
         KINGSN_ERR("[Metal]: error creating pipeline state (no alpha) %s\n", err.localizedDescription.UTF8String);
         return NO;
      }
   }
...
@implementation FrameView
{
   ...
   id<MTLSamplerState> _samplers[KINGSN_FILTER_MAX][KINGSN_WRAP_MAX];
...
- (void)_initSamplers
{
   ...
   /* Initialize samplers */
   for (unsigned i = 0; i < KINGSN_WRAP_MAX; i++)
   {
      switch (i)
      {
      ...
      _samplers[KINGSN_FILTER_LINEAR][i] = ss;

      sd.minFilter = MTLSamplerMinMagFilterNearest;
      sd.magFilter = MTLSamplerMinMagFilterNearest;

      ss = [_context.device newSamplerStateWithDescriptor:sd];
      _samplers[KINGSN_FILTER_NEAREST][i] = ss;
   }
}

- (void)setFilteringIndex:(int)index smooth:(bool)smooth
{
   for (int i = 0; i < KINGSN_WRAP_MAX; i++)
   {
      if (smooth)
         _samplers[KINGSN_FILTER_UNSPEC][i] = _samplers[KINGSN_FILTER_LINEAR][i];
      else
         _samplers[KINGSN_FILTER_UNSPEC][i] = _samplers[KINGSN_FILTER_NEAREST][i];
   }
}
...
- (BOOL)setShaderFromPath:(NSString *)path
{
   ...
   @try
   {
      ...
      for (i = 0; i < shader->passes; source = &_engine.pass[i++].rt)
      {
         ...
         if (!slang_process(shader, i, KINGSN_SHADER_METAL, 20000, &semantics_map, &_engine.pass[i].semantics))
            return NO;
         ...
         @try
         {
            ...
            if (err != nil)
            {
               ...
               {
                  save_msl = true;
                  KINGSN_ERR("[Metal]: unable to compile vertex shader: %s\n", err.localizedDescription.UTF8String);
                  return NO;
               }
#if DEBUG
               KINGSN_WARN("[Metal]: warnings compiling vertex shader: %s\n", err.localizedDescription.UTF8String);
#endif
            }
            ...
            if (err != nil)
            {
               ...
               {
                  save_msl = true;
                  KINGSN_ERR("[Metal]: unable to compile fragment shader: %s\n", err.localizedDescription.UTF8String);
                  return NO;
               }
#if DEBUG
               KINGSN_WARN("[Metal]: warnings compiling fragment shader: %s\n", err.localizedDescription.UTF8String);
#endif
            }
            ...
            {
               save_msl = true;
               KINGSN_ERR("[Metal]: error creating pipeline state for pass %d: %s\n", i,
                         err.localizedDescription.UTF8String);
               return NO;
            }
         ...
         } @finally
         {
            if (save_msl)
            {
               ...
               if (err != nil)
               {
                  KINGSN_ERR("[Metal]: unable to save vertex shader source: %s\n", err.localizedDescription.UTF8String);
               }
               ...
               if (err != nil)
               {
                  KINGSN_ERR("[Metal]: unable to save fragment shader source: %s\n",
                            err.localizedDescription.UTF8String);
               }
            }
