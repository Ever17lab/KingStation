...
static void on_applet_hook(AppletHookType hook, void *param)
{
   ...
   switch (hook)
   {
      case AppletHookType_OnExitRequest:
         KINGSN_LOG("Got AppletHook OnExitRequest, exiting.\n");
      ...
      case AppletHookType_OnFocusState:
         ...
         KINGSN_LOG("Got AppletHook OnFocusState - new focus state is %d\n", focus_state);
      ...
      int *argc, char *argv[], void *args, void *params_data)
{
   ...
   ks_main_log_file_init(SD_PREFIX "/KingStation-log.txt");
#endif
#endif
...
extern void ks_get_system_info(struct ks_system_info *info);
...
static void frontend_switch_init(void *data)
{
   ...
   {
       KINGSN_WARN("Error initializing psm\n");
   }
...
void frontend_switch_process_args(int *argc, char *argv[])
{
   ...
   if (*argc >= 1)
   {
      ...
      path_set(KINGSN_PATH_CORE, argv[0]);
   }
#endif
}
