...
static bool video_shader_parse_pass(config_file_t *conf, struct video_shader_pass *pass, unsigned i)
{
   ...
   {
      KINGSN_ERR("[Shaders]:  Couldn't parse shader source (%s).\n", shader_name);
      return false;
   }
   ...
   if (config_get_bool(conf, filter_name_buf, &tmp_bool))
   {
      ...
      pass->filter = smooth ? KINGSN_FILTER_LINEAR : KINGSN_FILTER_NEAREST;
   }
   else
      pass->filter = KINGSN_FILTER_UNSPEC;
   ...
   scale->valid   = true;
   scale->type_x  = KINGSN_SCALE_INPUT;
   scale->type_y  = KINGSN_SCALE_INPUT;
   scale->scale_x = 1.0;
   scale->scale_y = 1.0;
   ...
   if (*scale_type_x)
   {
      if (string_is_equal(scale_type_x, "source"))
         ...
         scale->type_x = KINGSN_SCALE_VIEWPORT;
      else if (string_is_equal(scale_type_x, "absolute"))
         scale->type_x = KINGSN_SCALE_ABSOLUTE;
      else
      {
         KINGSN_ERR("[Shaders]:  Invalid attribute.\n");
         return false;
      }
   }
   ...
   if (*scale_type_y)
   {
      if (string_is_equal(scale_type_y, "source"))
         ...
         scale->type_y = KINGSN_SCALE_VIEWPORT;
      else if (string_is_equal(scale_type_y, "absolute"))
         scale->type_y = KINGSN_SCALE_ABSOLUTE;
      else
      {
         KINGSN_ERR("[Shaders]:  Invalid attribute.\n");
         return false;
   ...
   if (scale->type_x == KINGSN_SCALE_ABSOLUTE)
   {
   ...
   if (scale->type_y == KINGSN_SCALE_ABSOLUTE)
   {
      if (config_get_int(conf, attr_name_buf, &iattr))
         scale->abs_y = iattr;
      else
...
static bool video_shader_parse_textures(config_file_t *conf, struct video_shader *shader)
{
         ...
         shader->luts++, id = strtok_r(NULL, ";", &save))
   {
      ...
      {
         KINGSN_ERR("[Shaders]:  Cannot find path to texture \"%s\" ...\n", id);
         free(textures);
         return false;
      }
      ...
      if (config_get_bool(conf, id_filter, &smooth))
         shader->lut[shader->luts].filter = smooth ? KINGSN_FILTER_LINEAR : KINGSN_FILTER_NEAREST;
      else
         shader->lut[shader->luts].filter = KINGSN_FILTER_UNSPEC;
...
bool video_shader_resolve_parameters(struct video_shader *shader)
{
   ...
   KINGSN_DBG("[Shaders]:  Finding Parameters in Shader Passes (#pragma parameter)\n");
...
/* First try to use the more robust slang implementation to support #includes. */
      ...
      while ((shader->num_parameters < ARRAY_SIZE(shader->parameters)) && (line_index < lines.size))
      {
         ...
         KINGSN_DBG("[Shaders]:     Found #pragma parameter %s (%s) %f %f %f %f in pass %d\n",
...
bool video_shader_load_current_parameter_values(config_file_t *conf, struct video_shader *shader)
{
   ...
   if (!conf)
   {
      KINGSN_ERR("[Shaders]: Load Parameter Values - Config is Null.\n");
      return false;
   }
   ...
   for (i = 0; i < shader->num_parameters; i++)
   {
      ...
      if (entry)
      {
         ...
         {
            KINGSN_DBG("[Shaders]:    Loading base parameter values\n");
         ...
         if (config_get_float(conf, shader->parameters[i].id, &parameter->current))
            KINGSN_DBG("[Shaders]:      Load parameter value:   %s = %f.\n", shader->parameters[i].id, parameter->current);
         else
            KINGSN_WARN("[Shaders]:      Load parameter value: name %s is set in preset but couldn't load its value.\n", 
                        shader->parameters[i].id);
      }
   }
...
bool video_shader_write_root_preset(const struct video_shader *shader, const char *path)
{
   ...
   KINGSN_DBG("[Shaders]:  Saving FULL PRESET to: %s\n", path);
   ...
   for (i = 0; i < shader->passes; i++)
   {
      ...
      if (pass->filter != KINGSN_FILTER_UNSPEC)
      {
         snprintf(key, sizeof(key), "filter_linear%u", i);
         config_set_bool(conf, key, pass->filter == KINGSN_FILTER_LINEAR);
      }
   ...
   if (shader->luts)
   {
      ...
      for (i = 0; i < shader->luts; i++)
      {
         ...
         if (shader->lut[i].filter != KINGSN_FILTER_UNSPEC)
         {
            char key[128];
            key[0]  = '\0';
            strlcpy(key, shader->lut[i].id, sizeof(key));
            strlcat(key, "_linear", sizeof(key));
            config_set_bool(conf, key, shader->lut[i].filter == KINGSN_FILTER_LINEAR);
         }
...
config_file_t *video_shader_get_root_preset_config(const char *path)
{
   ...
   while (conf->reference)
   {
      ...
      {
         KINGSN_ERR("[Shaders] - Get Root Preset - Exceeded maximum reference depth(%u) without finding a full preset. "
      ...
      if (!conf)
      {
         KINGSN_WARN("[Shaders]:  Could not read shader preset in #reference line: %s\n", nested_reference_path);
         goto end;
      }
...
bool video_shader_check_reference_chain_for_save(const char *path_to_save, const char *reference_path)
{
   ...
   if (!conf)
   {
      KINGSN_ERR("[Shaders]:  Could not read the #reference preset: %s\n", reference_path);
      return_val = false;
   }
   else
   {
      ...
      while (conf->reference)
      {
         ...
         if (reference_depth > SHADER_MAX_REFERENCE_DEPTH)
         {
            KINGSN_ERR("[Shaders] - Check Reference Chain for Save - Exceeded maximum reference depth(%u) without "
         ...
         {
            KINGSN_WARN("[Shaders]:  Saving preset:\n"
                       "                                              %s\n"
                       "                                          With a #reference of:\n"
                       "                                              %s\n"
                                          ...
                                          const struct video_shader *shader)
{
   ...
   {
      KINGSN_WARN("[Shaders]: Saving Full Preset because the loaded Shader does not have "
   ...
   {
      KINGSN_WARN("[Shaders]: Saving Full Preset because we can't save a reference to the "
   ...
   if (!reference_conf)
   {
      KINGSN_WARN("[Shaders]: Saving Full Preset because the initially loaded preset can't be loaded. "
      ...
      if (reference_conf->reference)
      {
         ...
         */
         if (string_is_equal(path_to_reference, path_to_save_conformed))
         {
            ...
            {
               KINGSN_WARN("[Shaders]: Saving Full Preset because we can't save a preset which "
   ...
   {
      KINGSN_WARN("[Shaders]: Saving Full Preset because saving a Simple Preset would result "
                  "in a cyclical reference, or a preset in the reference chain could not be read.\n");
      goto end;
   }

   KINGSN_DBG("[Shaders]:  Reading Preset to Compare with Current Values: %s\n", path_to_save_conformed);
   ...
   {
      KINGSN_WARN("[Shaders]:  Saving Full Preset because we could not load the preset from the #reference line: %s.\n", 
                  path_to_reference);
      goto end;
   }
   ...
   */
   ...
   {
      KINGSN_WARN("[Shaders]: passes (Number of Passes) "
   ...
   */
   ...
   if (shader->luts)
   {
      for (i = 0; i < shader->luts; i++)
      {
         ...
         if (!string_is_equal(referenced_shader->lut[i].path, shader->lut[i].path))
         {
            ...
            KINGSN_DBG("[Shaders]:  Texture override %s = %s.\n", shader->lut[i].id, path_for_save);
   ...
   KINGSN_DBG("[Shaders]:  Saving simple preset to: %s\n", path_to_save_conformed);
                              ...
                              bool reference)
{
   ...
   if (reference)
   {
      ...
      {
         KINGSN_WARN("[Shaders]:  Failed writing Simple Preset to %s - "
...
bool video_shader_load_root_config_into_shader(config_file_t *conf, struct video_shader *shader)
{
   ...
   KINGSN_DBG("\n");
   KINGSN_DBG("[Shaders]:  Reading ROOT PRESET:  %s\n", conf->path);

   if (!config_get_uint(conf, "shaders", &num_passes))
   {
      KINGSN_ERR("[Shaders]:  Cannot find \"shaders\" param. Preset is invalid.\n");
      goto fail;
   }

   if (!num_passes)
   {
      KINGSN_ERR("[Shaders]:  Need to define at least 1 shader pass. Preset is invalid.\n");
      goto fail;
   }
   ...
   KINGSN_DBG("[Shaders]:      Number of Passes:  %u\n", shader->passes);
   KINGSN_DBG("[Shaders]:      Number of Textures:  %u\n", shader->luts);

   /* Log Texture Names & Paths */
   for (i = 0; i < shader->luts; i++)
      KINGSN_DBG("[Shaders]:        %s = %s.\n", shader->lut[i].id, shader->lut[i].path);
   ...
   if (shader->num_parameters)
   {
      ...
      for (i = 0; i < shader->num_parameters; i++)
      {
         ...
         if (entry)
         {
            ...
            KINGSN_DBG("[Shaders]:      Parameter:  %s = %f.\n",
                        shader->parameters[i].id, 
   ...
   if (shader->luts)
   {
      ...
      for (i = 0; i < shader->luts; i++)
      {
         ...
         if (entry)
         {
            ...
            KINGSN_DBG("[Shaders]:      Texture:    %s = %s.\n", 
                        shader->lut[i].id, 
                        shader->lut[i].path);
...
bool video_shader_load_preset_into_shader(const char *path, struct video_shader *shader)
{
   ...
   {
      KINGSN_LOG("\n");
      KINGSN_WARN("[Shaders]:  Could not read root preset: %s \n", path);
      ret = false;
      goto end;
   }
   ...
   {
      KINGSN_DBG("[Shaders]:  Start reading FULL Preset: %s\n", path);
   ...
   {
      KINGSN_DBG("[Shaders]:  Start reading SIMPLE Preset: %s\n", path);
   ...
   KINGSN_DBG("\n");
   KINGSN_DBG("[Shaders]:  Crawl Preset Reference Chain\n");
   ...
   while (conf->reference)
   {
      ...
      KINGSN_DBG("[Shaders]:    Preset (Depth %u):  %s \n", i, conf->path);
      ...
      KINGSN_DBG("[Shaders]:      #reference     = %s \n", reference_preset_path);
   ...
   KINGSN_DBG("\n");
   KINGSN_DBG("[Shaders]:  Start Applying Simple Preset Overrides\n");

   while (i)
   {
      config_file_t *override_conf = config_file_new_from_path_to_string(override_conf_paths[i]);
      
      KINGSN_DBG("[Shaders]:    Depth %u Apply Overrides\n", i);
      KINGSN_DBG("[Shaders]:      Apply values from:   %s\n", override_conf->path);
   ...
   KINGSN_DBG("[Shaders]:  End Apply Overrides\n");
   KINGSN_DBG("\n");
...
const char *video_shader_type_to_str(enum kingsn_shader_type type)
{
   switch (type)
   {
      case KINGSN_SHADER_CG:
         return "Cg";
      case KINGSN_SHADER_HLSL:
         return "HLSL";
      case KINGSN_SHADER_GLSL:
         return "GLSL";
      case KINGSN_SHADER_SLANG:
         return "Slang";
      case KINGSN_SHADER_METAL:
         return "Metal";
      case KINGSN_SHADER_NONE:
         return "none";
      default:
         break;
   }
 ...
 **/
bool video_shader_is_supported(enum kingsn_shader_type type)
{
...
const char *video_shader_get_preset_extension(enum kingsn_shader_type type)
{
   switch (type)
   {
      case KINGSN_SHADER_GLSL:
...
enum kingsn_shader_type video_shader_get_type_from_ext(const char *ext, bool *is_preset)
{
