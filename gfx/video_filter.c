...
struct kingsn_softfilter
{
   ...
   struct kingsn_soft_plug *plugs;
   unsigned num_plugs;

   unsigned max_width, max_height;
   enum ks_pixel_format pix_fmt, out_pix_fmt;
...
softfilter_find_implementation(kingsn_softfilter_t *filt, const char *ident)
{
   unsigned i;
...
static bool create_softfilter_graph(kingsn_softfilter_t *filt,
      ...
      unsigned threads)
{
   ...
   {
      KINGSN_ERR("Could not find 'filter' array in config.\n");
      return false;
   }

   if (filt->num_plugs == 0)
   {
      KINGSN_ERR("No filter plugs found. Exiting...\n");
      return false;
   }
   ...
   if (!filt->impl)
   {
      KINGSN_ERR("Could not find implementation.\n");
      return false;
   }
   ...
   {
      case KS_PIXEL_FORMAT_XRGB8888:
         input_fmt = SOFTFILTER_FMT_XRGB8888;
         break;
      case KS_PIXEL_FORMAT_RGB565:
   ...
   {
      KINGSN_ERR("Softfilter does not support input format.\n");
      return false;
   }
   ...
   if (output_fmts & input_fmt)
      ...
      filt->out_pix_fmt = KS_PIXEL_FORMAT_XRGB8888;
   else if (output_fmts & SOFTFILTER_FMT_RGB565)
      filt->out_pix_fmt = KS_PIXEL_FORMAT_RGB565;
   else
   {
      KINGSN_ERR("Did not find suitable output format for softfilter.\n");
      return false;
   }
   ...
   filt->impl_data = filt->impl->create(
         ...
         threads != KINGSN_SOFTFILTER_THREADS_AUTO ? threads :
         cpu_features_get_core_amount(), cpu_features,
         &userdata);
   if (!filt->impl_data)
   {
      KINGSN_ERR("Failed to create softfilter state.\n");
      return false;
   }
   ...
   if (!threads)
   {
      KINGSN_ERR("Invalid number of threads.\n");
      return false;
   }

   filt->threads = threads;
   KINGSN_LOG("Using %u threads for softfilter.\n", threads);

   filt->packets = (struct softfilter_work_packet*)
      calloc(threads, sizeof(*filt->packets));
   if (!filt->packets)
   {
      KINGSN_ERR("Failed to allocate softfilter packets.\n");
      return false;
   }
...
static bool append_softfilter_plugs(kingsn_softfilter_t *filt,
      struct string_list *list)
{
   ...
   filt->plugs = (struct kingsn_soft_plug*)
      ...
      struct string_list *list)
{
   ...
   for (i = 0; i < list->size; i++)
   {
      ...
      struct kingsn_soft_plug *new_plugs            = NULL;
      dylib_t lib                                  = 
      ...
      new_plugs = (struct kingsn_soft_plug*)
      ...
      KINGSN_LOG("[SoftFilter]: Found plug: %s (%s).\n",
...
kingsn_softfilter_t *kingsn_softfilter_new(const char *filter_config,
      unsigned threads,
      enum ks_pixel_format in_pixel_format,
      unsigned max_width, unsigned max_height)
{
   ...
   kingsn_softfilter_t *filt      = (kingsn_softfilter_t*)
   ...
   {
      KINGSN_ERR("[SoftFilter]: Did not find config: %s\n", filter_config);
      goto error;
   }
   ...
   if (!plugs)
   {
      KINGSN_ERR("[SoftFilter]: Could not build up string list...\n");
      goto error;
   }
#endif
   if (!append_softfilter_plugs(filt, plugs))
   {
      KINGSN_ERR("[SoftFitler]: Failed to append softfilter plugins...\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[SoftFitler]: Failed to create softfilter graph...\n");
      goto error;
   }
...
{
   kingsn_softfilter_get_output_size(filt, width, height,
      ...
      size_t input_stride)
{
   ...
   if (filt->threads > 1)
   {
      ...
      {
#if 0
         KINGSN_LOG("Firing off filter thread %u ...\n", i);
#endif
      ...
      {
#if 0
         KINGSN_LOG("Waiting for filter thread %u ...\n", i);
#endif
         slock_lock(filt->thread_data[i].lock);
