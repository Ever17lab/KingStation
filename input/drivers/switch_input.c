      ...
      unsigned id)
{
   ...
   switch (device)
   {
      ...
      case KS_DEVICE_KEYBOARD:
         return ((id < KSK_LAST) && 
               sw->keyboard_state[kingsn_keysym_lut[(enum ks_key)id]]);
...
static void* switch_input_init(const char *joypad_driver)
{
   ...
   */
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_switch);
...
static uint64_t switch_input_get_capabilities(void *data)
{
   ...
   uint64_t caps =  (1 << KS_DEVICE_JOYPAD) | (1 << KS_DEVICE_ANALOG);

#ifdef HAVE_LIBNX
   caps |= (1 << KS_DEVICE_POINTER) | (1 << KS_DEVICE_KEYBOARD) | (1 << KS_DEVICE_MOUSE);
#endif
      ...
      enum ks_sensor_action action, unsigned event_rate)
{
   ...
   switch(action)
   {
      case KS_SENSOR_ILLUMINANCE_ENABLE:
         available = false;
         appletIsIlluminanceAvailable(&available);
         return available;

      case KS_SENSOR_ILLUMINANCE_DISABLE:
      ...
      unsigned port, unsigned id)
{
   ...
   if (id >= KS_SENSOR_ACCELEROMETER_X && id <= KS_SENSOR_GYROSCOPE_Z)
   {
