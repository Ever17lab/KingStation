...
static unsigned input_unify_ev_key_code(unsigned code)
{
    ...
    * where multiple keysyms translate to the same KSK_* code,
    * so unify remote control keysyms to keyboard keysyms here.
    *
    * Addendum: The kingsn_keysym_lut lookup table also becomes
    * unusable if more than one keysym translates to the same
    * KSK_* code, so certain keys must be left unmapped in
    * kingsn_key_map_linux and instead be handled here */
   switch (code)
   {
      case KEY_OK:
      case KEY_SELECT:
      ...
      enum udev_input_dev_type type, const char *devnode, device_handle_cb cb)
{
   ...
   {
      KINGSN_ERR("Failed to add FD (%d) to epoll list (%s).\n",
            fd, strerror(errno));
   }
#elif defined(HAVE_KQUEUE)
   ...
   {
      KINGSN_ERR("Failed to add FD (%d) to kqueue list (%s).\n",
            fd, strerror(errno));
   }
#endif
      ...
      int *x, int *y, udev_input_mouse_t *mouse)
{
   ...
   if (r && (dx || dy) && 
         video_driver_display_type_get() != KINGSN_DISPLAY_X11)
   {
      int minX = view.x;
      ...
      unsigned port, unsigned id, bool screen)
{
   ...
   if (id != KS_DEVICE_ID_MOUSE_X && id != KS_DEVICE_ID_MOUSE_Y &&
      ...
      unsigned id)
{
   ...
   switch (device)
   {
      ...
      case KS_DEVICE_KEYBOARD:
         return (id < KSK_LAST) && udev_keyboard_pressed(udev, id);
         ...
         switch ( id )
         {
            ...
            case KS_DEVICE_ID_LIGHTGUN_TRIGGER:
               return udev_input_lightgun_state(udev, joypad,
                     ...
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_TRIGGER);
            case KS_DEVICE_ID_LIGHTGUN_RELOAD:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_RELOAD);
            case KS_DEVICE_ID_LIGHTGUN_AUX_A:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_AUX_A);
            case KS_DEVICE_ID_LIGHTGUN_AUX_B:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_AUX_B);
            case KS_DEVICE_ID_LIGHTGUN_AUX_C:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_AUX_C);
            case KS_DEVICE_ID_LIGHTGUN_START:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_START);
            case KS_DEVICE_ID_LIGHTGUN_SELECT:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_SELECT);
            case KS_DEVICE_ID_LIGHTGUN_DPAD_UP:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_DPAD_UP);
            case KS_DEVICE_ID_LIGHTGUN_DPAD_DOWN:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_DPAD_DOWN);
            case KS_DEVICE_ID_LIGHTGUN_DPAD_LEFT:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_DPAD_LEFT);
            case KS_DEVICE_ID_LIGHTGUN_DPAD_RIGHT:
               return udev_input_lightgun_state(udev, joypad,
                     joypad_info,
                     binds,
                     keyboard_mapping_blocked,
                     port, device, idx, KINGSN_LIGHTGUN_DPAD_RIGHT);
               /*deprecated*/
            ...
            case KS_DEVICE_ID_LIGHTGUN_PAUSE:
               if (binds[port][KINGSN_LIGHTGUN_START].valid)
               {
...
#ifdef DEBUG
            if (!check)
               KINGSN_ERR("[udev] Failed to open device: %s (%s).\n",
                     devnode, strerror(errno));
            else
               KINGSN_LOG("[udev]: %s #%d (%s).\n",
...
static void *udev_input_init(const char *joypad_driver)
{
   ...
   if (!udev->num_devices)
      KINGSN_WARN("[udev]: Couldn't open any keyboard, mouse or touchpad. Are permissions set correctly for /dev/input/event*?\n");
   ...
   KINGSN_WARN("[udev]: Full-screen pointer won't be available.\n");
#endif
...
static void udev_input_grab_mouse(void *data, bool state)
{
   ...
   if (video_driver_display_type_get() != KINGSN_DISPLAY_X11)
   {
      KINGSN_WARN("[udev]: Mouse grab/ungrab feature unavailable.\n");
      return;
   }
