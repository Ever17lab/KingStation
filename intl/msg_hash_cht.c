      ...
      switch (idx)
      {
         ...
         case KINGSN_GRAB_MOUSE_TOGGLE:
            snprintf(s, len,
                  "鼠標捕獲開關 \n"
                  " \n"
                  "當鼠標捕獲開啟時，KingStation將會隱藏鼠標指針，並 \n"
         ...
         case KINGSN_QUIT_KEY:
            snprintf(s, len,
                  "用於正常退出 KingStation 的按鍵 \n"
                  " \n"
                  "使用任何強制手段來退出KingStation（如發送SIGKILL） \n"
                  "可能導致系統無法正確存儲數據。"
#ifdef __unix__
   ...
   switch (msg)
   {
      ...
      case MENU_ENUM_LABEL_CACHE_DIRECTORY:
         snprintf(s, len,
               "緩存目錄 \n"
               " \n"
               "被KingStation解壓的遊戲內容會臨時存放到這個文 \n"
               "件夾。");
         break;
      case MENU_ENUM_LABEL_HISTORY_LIST_ENABLE:
         snprintf(s, len,
               "若開啟，所有在KingStation中加載過的文件 \n"
               "都會自動的放入最近使用歷史列表中。");
         break;
      ...
      case MENU_ENUM_LABEL_VALUE_WHAT_IS_A_CORE_DESC:
         snprintf(s, len,
               "KingStation本身並不能做什麼事情。 \n"
               " \n"
               "如果想在上面幹點什麼，你需要向它加載一個程 \n"
               "序。 \n"
               "\n"
               "我們把這樣的程序叫做「Libks核心」，簡單 \n"
               "的稱呼其為「核心」。 \n"
               " \n"
               "你可以從「加載核心」菜單中選擇一個核心。 \n"
               " \n"
