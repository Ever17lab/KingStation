...
static size_t find_buffersize(jack_t *jd, int latency, unsigned out_rate)
{
   ...
   KINGSN_LOG("[JACK]: Jack latency is %d frames.\n", jack_latency);

   buffer_frames     = frames - jack_latency;
   min_buffer_frames = jack_get_buffer_size(jd->client) * 2;

   KINGSN_LOG("[JACK]: Minimum buffer size is %d frames.\n", min_buffer_frames);
      ...
      unsigned *new_rate)
{
   ...
   jd->client = jack_client_open("KingStation", JackNullOption, NULL);
   if (!jd->client)
      goto error;
   ...
   *new_rate = jack_get_sample_rate(jd->client);
   ...
   {
      KINGSN_ERR("[JACK]: Failed to register ports.\n");
      goto error;
   }
   ...
   if (!jports)
   {
      KINGSN_ERR("[JACK]: Failed to get ports.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[JACK]: Failed to activate Jack...\n");
      goto error;
   }
   ...
   for (i = 0; i < 2; i++)
   {
      ...
      {
         KINGSN_ERR("[JACK]: Failed to connect to Jack port.\n");
         goto error;
      }
   }
