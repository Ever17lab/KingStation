...
static void gdi_gfx_create(gdi_t *gdi)
{
   ...
   if (!ctx || !ctx->get_os)
   {
      KINGSN_ERR("[GDI] No frontend driver found.\n");
      return;
   }
   ...
   {
      KINGSN_LOG("[GDI] Win98 or lower detected, using slow frame conversion method for RGB444.\n");
      gdi->lte_win98 = true;
   }
}
      ...
      input_driver_t **input, void **input_data)
{
   ...
   *input_data                          = NULL;
   ...
   KINGSN_LOG("[GDI]: Detecting screen resolution %ux%u.\n", full_x, full_y);
   ...
   KINGSN_LOG("[GDI]: Using resolution %ux%u\n", temp_width, temp_height);
