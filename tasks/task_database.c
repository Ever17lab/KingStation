...
static int task_database_iterate_start(ks_task_t *task,
      ...
      uint64_t offset, uint64_t size, char *serial)
{
   ...
   if (offset != 0 || size < (uint64_t) file_size)
   {
      ...
      intfstream_close(fd);
      free(fd);
      fd = intfstream_open_memory(data, KS_VFS_FILE_ACCESS_READ,
      ...
      uint64_t offset, size_t size, uint32_t *crc)
{
   ...
   if (offset != 0 || size < (uint64_t) file_size)
   {
      ...
      free(fd);
      fd = intfstream_open_memory(data, KS_VFS_FILE_ACCESS_READ,
            KS_VFS_FILE_ACCESS_HINT_NONE, size);
...
static int task_database_gdi_get_crc(const char *name, uint32_t *crc)
{
   ...
   KINGSN_LOG("GDI '%s' primary track: %s\n", name, track_path);
      ...
      const char *name)
{
   ...
   while (cue_next_file(fd, name, path, sizeof(path)))
   {
      for (i = db->list_ptr; i < db->list->size; ++i)
      {
         ...
         {
            KINGSN_LOG("Pruning file referenced by cue: %s\n", path);
   ...
   while (gdi_next_file(fd, name, path, sizeof(path)))
   {
      for (i = db->list_ptr; i < db->list->size; ++i)
      {
         if (db->list->elems[i].data
               && string_is_equal(path, db->list->elems[i].data))
         {
            KINGSN_LOG("Pruning file referenced by gdi: %s\n", path);
            free(db->list->elems[i].data);
            db->list->elems[i].data = NULL;
         }
      }
...
#if defined(KINGSN_INTERNAL)
#if 0
   KINGSN_LOG("Found match in database !\n");

   KINGSN_LOG("Path: %s\n", db_path);
   KINGSN_LOG("CRC : %s\n", db_crc);
   KINGSN_LOG("Playlist Path: %s\n", db_playlist_path);
   KINGSN_LOG("Entry Path: %s\n", entry_path);
   KINGSN_LOG("Playlist not NULL: %d\n", playlist != NULL);
   KINGSN_LOG("ZIP entry: %s\n", archive_name);
   KINGSN_LOG("entry path str: %s\n", entry_path_str);
#endif
#else
      ...
      const char *archive_entry)
{
   ...
   if (db_state->info)
   {
      ...
      {
#if 0
         KINGSN_LOG("CRC32: 0x%08X , entry CRC32: 0x%08X (%s).\n",
      ...
      database_info_handle_t *db, const char *name)
{
   ...
   if (db_state->info)
   {
      ...
      {
#if 0
         KINGSN_LOG("serial: %s , entry serial: %s (%s).\n",
...
static void task_database_handler(ks_task_t *task)
{
...
#ifdef KINGSN_INTERNAL
static void task_database_progress_cb(ks_task_t *task)
{
   if (task)
