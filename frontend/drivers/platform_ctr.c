...
static void frontend_ctr_exec(const char* path, bool should_load_game)
{
   ...
   KINGSN_LOG("Attempt to load core: [%s].\n", path);
#ifndef IS_SALAMANDER
   if (should_load_game && !path_is_empty(KINGSN_PATH_CONTENT))
   {
      ...
      arg_data[args + 1] = NULL;
      args++;
      KINGSN_LOG("content path: [%s].\n", path_get(KINGSN_PATH_CONTENT));
   }
#endif
   ...
   if (path && path[0])
   {
      ...
      else
      {
         ...
         KINGSN_WARN("\n");
         KINGSN_WARN("Warning:\n");
         KINGSN_WARN("First core launch may take 20\n");
         KINGSN_WARN("seconds! Do not force quit\n");
         KINGSN_WARN("before then or your memory\n");
         KINGSN_WARN("card may be corrupted!\n");
         KINGSN_WARN("\n");
         KINGSN_WARN("\n");
...
static void ctr_check_dspfirm(void)
{
   ...
   else
   {
      ...
      else
      {
         ...
         KINGSN_WARN("\n");
         KINGSN_WARN("Warning:\n");
         KINGSN_WARN("3DS DSP dump is missing.\n");
         KINGSN_WARN("A working DSP dump is required\n");
         KINGSN_WARN("for correct operation.\n");
         KINGSN_WARN("\n");
         KINGSN_WARN("\n");
      }
   }
