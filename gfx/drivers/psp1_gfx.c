      ...
      input_driver_t **input, void **input_data)
{
   ...
   else
   {
      ...
      pixel_format          =
         (video_driver_get_pixel_format() == KS_PIXEL_FORMAT_0RGB1555)
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (((uint32_t)frame&0x04000000) || (frame == KS_HW_FRAME_BUFFER_VALID))
      psp->hw_render = true;
   else if (frame)
      psp->hw_render = false;
                               ...
                               unsigned width, unsigned height, float alpha)
{
   ...
   ks_assert((width*height) < (480 * 272));
#endif
...
static bool psp_set_shader(void *data, enum kingsn_shader_type type, const char *path) { return false; }
