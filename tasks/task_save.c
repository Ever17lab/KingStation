...
static void autosave_thread(void *data)
{
   ...
   while (!save->quit)
   {
      ...
      differ = string_is_not_equal_fast(save->buffer, save->ks_buffer,
            save->bufsize);
      if (differ)
         memcpy(save->buffer, save->ks_buffer, save->bufsize);
      ...
      if (differ)
      {
         ...
         if (save->compress_files)
            ...
            file = intfstream_open_file(save->path,
                  KS_VFS_FILE_ACCESS_WRITE, KS_VFS_FILE_ACCESS_HINT_NONE);
      ...
      unsigned interval, bool compress)
{
   ...
   memcpy(handle->buffer, handle->ks_buffer, handle->bufsize);
...
{
   unsigned i;
   ks_ctx_serialize_info_t serial_info;
...
static void undo_save_state_cb(ks_task_t *task,
      void *task_data,
 ...
 **/
static void task_save_handler_finished(ks_task_t *task,
 ...
 **/
static void task_save_handler(ks_task_t *task)
{
   int written;
   ssize_t remaining;
   ...
   if (!state->file)
   {
      if (state->compress_files)
         state->file   = intfstream_open_rzip_file(
               state->path, KS_VFS_FILE_ACCESS_WRITE);
      else
 ...
 **/
static void task_load_handler_finished(ks_task_t *task,
      save_task_state_t *state)
{
 ...
 **/
static void task_load_handler(ks_task_t *task)
{
 ...
 *
 **/
static void content_load_state_cb(ks_task_t *task,
      void *task_data,
      void *user_data, const char *error)
{
   ks_ctx_serialize_info_t serial_info;
 ...
 **/
static void save_state_cb(ks_task_t *task,
      void *task_data,
      void *user_data, const char *error)
{
   save_task_state_t *state   = (save_task_state_t*)task_data;
 ...
 *
 **/
static void content_load_and_save_state_cb(ks_task_t *task,
      void *task_data,
      void *user_data, const char *error)
{
   load_task_data_t *load_data = (load_task_data_t*)task_data;
...
static bool task_save_state_finder(ks_task_t *task, void *user_data)
{
   if (!task)
      return false;
...
bool content_rename_state(const char *origin, const char *dest)
{
   ...
   KINGSN_ERR("[State]: Error %d renaming file \"%s\".\n", ret, origin);
   return false;
}
...
static bool content_get_memory(ks_ctx_memory_info_t *mem_info,
...
bool content_load_ram_file(unsigned slot)
{
   ...
   if (rc > 0)
   {
      ...
      {
         KINGSN_WARN("[SRAM]: SRAM is larger than implementation expects, "
      ...
      size_t size, unsigned type)
{
   ...
   KINGSN_WARN("[SRAM]: Succeeded in saving RAM data to \"%s\".\n", path);
   return true;
}
...
bool content_save_ram_file(unsigned slot, bool compress)
{
   ...
   if (!write_success)
   {
      ...
      KINGSN_WARN("[SRAM]: Attempting to recover ...\n");
      ...
      {
         KINGSN_WARN("[SRAM]: Failed ... Cannot recover save file.\n");
      }
      return false;
   }
