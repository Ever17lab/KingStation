...
static void init_filters( init_t* impl, snes_ntsc_setup_t const* setup )
{
	...
	/* generate luma (y) filter using sinc kernel */
	{
		...
		for ( i = 0; i < kernel_half * 2 + 1; i++ )
		{
			...
			kernels [x] *= sum;
			ks_assert( kernels [x] == kernels [x] ); /* catch numerical instability */
		}
	}
...
extern pixel_info_t const KingStation_snes_ntsc_pixels [alignment_count];
...
static void gen_kernel( init_t* impl, float y, float i, float q, snes_ntsc_rgb_t* out )
{
	...
	do
	{
		...
		pixel_info_t const* pixel = KingStation_snes_ntsc_pixels;
