      ...
      const char *path, struct parport_joypad *pad)
{
   ...
   *pad->ident = '\0';
   ...
   if (pad->fd >= 0)
   {
       ...
       * KINGSN_ERR because they could be
      ...
      else
      {
         ...
         if (ioctl(pad->fd, PPWDATA, &data) < 0)
            KINGSN_WARN("[Joypad]: Failed to restore original data register on %s\n", path);
         KINGSN_WARN("[Joypad]: Failed to save original control register on %s\n", path);
#endif
         goto error;
      }
      ...
      if (!set_control)
         KINGSN_WARN("[Joypad]: Failed to clear nStrobe and nIRQ bits on %s\n", path);
...
static void parport_free_pad(struct parport_joypad *pad)
{
   ...
   if (ioctl(pad->fd, PPWDATA, &data) < 0)
      KINGSN_ERR("[Joypad]: Failed to restore original data register on %s\n", pad->ident);

   data = pad->saved_control;
   if (ioctl(pad->fd, PPWDATA, &data) < 0)
      KINGSN_ERR("[Joypad]: Failed to restore original control register on %s\n", pad->ident);

   if (ioctl(pad->fd, PPRELEASE) < 0)
      KINGSN_ERR("[Joypad]: Failed to release parallel port %s\n", pad->ident);
      ...
      if (parport_joypad_init_pad(path, pad))
      {
         ...
         if (found_enabled_button)
         {
            if (found_disabled_button)
            {
               ...
               KINGSN_WARN("[Joypad]: Pin(s) %son %s were low"
         ...
         {
            KINGSN_WARN("[Joypad]: All pins low on %s, assuming"
                  " nothing connected\n", path);
