      ...
      gl_core_buffer_locations *loc, bool flatten)
{
   ...
   try
   {
      ...
      for (auto &res : vertex_resources.stage_inputs)
      {
         ...
         vertex_compiler.set_name(res.id, string("KINGSN_ATTRIBUTE_") + to_string(location));
      ...
      {
         uint32_t location = vertex_compiler.get_decoration(res.id, spv::DecorationLocation);
         vertex_compiler.set_name(res.id, string("KINGSN_VARYING_") + to_string(location));
         vertex_compiler.unset_decoration(res.id, spv::DecorationLocation);
      }
      ...
      for (auto &res : fragment_resources.stage_inputs)
      {
         ...
         fragment_compiler.set_name(res.id, string("KINGSN_VARYING_") + to_string(location));
         fragment_compiler.unset_decoration(res.id, spv::DecorationLocation);
      }

      if (vertex_resources.push_constant_buffers.size() > 1)
      {
         KINGSN_ERR("[GLCore]: Cannot have more than one push constant buffer.\n");
         return 0;
      }

      for (auto &res : vertex_resources.push_constant_buffers)
      {
         vertex_compiler.set_name(res.id, "KINGSN_PUSH_VERTEX_INSTANCE");
         vertex_compiler.set_name(res.base_type_id, "KINGSN_PUSH_VERTEX");
      }

      if (vertex_resources.uniform_buffers.size() > 1)
      {
         KINGSN_ERR("[GLCore]: Cannot have more than one uniform buffer.\n");
         return 0;
      }
      ...
      for (auto &res : vertex_resources.uniform_buffers)
      {
         ...
         vertex_compiler.set_name(res.id, "KINGSN_UBO_VERTEX_INSTANCE");
         vertex_compiler.set_name(res.base_type_id, "KINGSN_UBO_VERTEX");
      ...
      {
         KINGSN_ERR("[GLCore]: Cannot have more than one push constant block.\n");
         return 0;
      }

      for (auto &res : fragment_resources.push_constant_buffers)
      {
         fragment_compiler.set_name(res.id, "KINGSN_PUSH_FRAGMENT_INSTANCE");
         fragment_compiler.set_name(res.base_type_id, "KINGSN_PUSH_FRAGMENT");
      }
      ...
      for (auto &res : fragment_resources.uniform_buffers)
      {
         ...
         fragment_compiler.set_name(res.id, "KINGSN_UBO_FRAGMENT_INSTANCE");
         fragment_compiler.set_name(res.base_type_id, "KINGSN_UBO_FRAGMENT");
      ...
      for (auto &res : fragment_resources.sampled_images)
      {
         ...
         fragment_compiler.set_name(res.id, string("KINGSN_TEXTURE_") + to_string(binding));
         fragment_compiler.unset_decoration(res.id, spv::DecorationDescriptorSet);
         fragment_compiler.unset_decoration(res.id, spv::DecorationBinding);
...
#if 0
      KINGSN_LOG("[GLCore]: Vertex shader:\n========\n%s\n=======\n", vertex_source.c_str());
      KINGSN_LOG("[GLCore]: Fragment shader:\n========\n%s\n=======\n", fragment_source.c_str());
#endif

      if (!vertex_shader || !fragment_shader)
      {
         KINGSN_ERR("[GLCore]: One or more shaders failed to compile.\n");
         if (vertex_shader)
      ...
      {
         uint32_t location = vertex_compiler.get_decoration(res.id, spv::DecorationLocation);
         glBindAttribLocation(program, location, (string("KINGSN_ATTRIBUTE_") + to_string(location)).c_str());
      }
      ...
      if (!status)
      {
         ...
         if (length > 0)
         {
            ...
            if (info_log)
            {
               ...
               KINGSN_ERR("[GLCore]: Failed to link program: %s\n", info_log);
               free(info_log);
      ...
      if (loc)
      {
         ...
         if (flatten)
         {
            loc->flat_ubo_vertex           = glGetUniformLocation(program, "KINGSN_UBO_VERTEX");
            loc->flat_ubo_fragment         = glGetUniformLocation(program, "KINGSN_UBO_FRAGMENT");
            loc->flat_push_vertex          = glGetUniformLocation(program, "KINGSN_PUSH_VERTEX");
            loc->flat_push_fragment        = glGetUniformLocation(program, "KINGSN_PUSH_FRAGMENT");
         }
         else
         {
            loc->buffer_index_ubo_vertex   = glGetUniformBlockIndex(program, "KINGSN_UBO_VERTEX");
            loc->buffer_index_ubo_fragment = glGetUniformBlockIndex(program, "KINGSN_UBO_FRAGMENT");
         }
      }
      ...
      {
         GLint location = glGetUniformLocation(program, (string("KINGSN_TEXTURE_") + to_string(binding)).c_str());
         if (location >= 0)
   ...
   {
      KINGSN_ERR("[GLCore]: Failed to cross compile program: %s\n", e.what());
      if (program != 0)
         glDeleteProgram(program);
      return 0;
   }
...
#if 0
         KINGSN_WARN("[GLCore]: No CLAMP_TO_BORDER in GLES3. Falling back to edge clamp.\n");
#endif
   ...
   if (status != GL_FRAMEBUFFER_COMPLETE)
   {
      ...
      switch (status)
      {
         case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            KINGSN_ERR("[GLCore]: Incomplete attachment.\n");
            break;

         case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            KINGSN_ERR("[GLCore]: Incomplete, missing attachment.\n");
            break;
         ...
         case GL_FRAMEBUFFER_UNSUPPORTED:
            {
               ...
               KINGSN_ERR("[GLCore]: Unsupported FBO, falling back to RGBA8.\n");
...
void Pass::reflect_parameter(const std::string &name, slang_semantic_meta &meta)
{
   if (meta.uniform)
   {
      int vert = glGetUniformLocation(pipeline, (std::string("KINGSN_UBO_VERTEX_INSTANCE.") + name).c_str());
      int frag = glGetUniformLocation(pipeline, (std::string("KINGSN_UBO_FRAGMENT_INSTANCE.") + name).c_str());
   ...
   {
      int vert = glGetUniformLocation(pipeline, (std::string("KINGSN_PUSH_VERTEX_INSTANCE.") + name).c_str());
      int frag = glGetUniformLocation(pipeline, (std::string("KINGSN_PUSH_FRAGMENT_INSTANCE.") + name).c_str());
   ...
   for (i = 0; i < meta.size(); i++)
   {
      ...
      if (m->uniform)
      {
         int vert = glGetUniformLocation(pipeline,
               (std::string("KINGSN_UBO_VERTEX_INSTANCE.") + n).c_str());
         int frag = glGetUniformLocation(pipeline,
               (std::string("KINGSN_UBO_FRAGMENT_INSTANCE.") + n).c_str());
      ...
      {
         int vert = glGetUniformLocation(pipeline,
               (std::string("KINGSN_PUSH_VERTEX_INSTANCE.") + n).c_str());
         int frag = glGetUniformLocation(pipeline,
               (std::string("KINGSN_PUSH_FRAGMENT_INSTANCE.") + n).c_str());
      ...
      const Size2D &source) const
{
   ...
   switch (pass_info.scale_type_x)
   {
      ...
      case GLSLANG_FILTER_CHAIN_SCALE_VIEWPORT:
         width = (KingStation_get_rotation() % 2 ? current_viewport.height : current_viewport.width) * pass_info.scale_x;
         break;
   ...
   switch (pass_info.scale_type_y)
   {
      ...
      case GLSLANG_FILTER_CHAIN_SCALE_VIEWPORT:
         height = (KingStation_get_rotation() % 2 ? current_viewport.width : current_viewport.height) * pass_info.scale_y;
         break;
...
bool gl_core_filter_chain::init_history()
{
   ...
   {
      KINGSN_LOG("[GLCore]: Not using frame history.\n");
      return true;
   }
   ...
   KINGSN_LOG("[GLCore]: Using history of %u frames.\n", unsigned(required_images));
...
bool gl_core_filter_chain::init_feedback()
{
   ...
   for (i = 0; i < passes.size() - 1; i++)
   {
      ...
      if (use_feedback)
         KINGSN_LOG("[GLCore]: Using framebuffer feedback for pass #%u.\n", i);
   }

   if (!use_feedbacks)
   {
      KINGSN_LOG("[GLCore]: Not using framebuffer feedback.\n");
      return true;
   }
...
bool gl_core_filter_chain::init()
{
   ...
   for (i = 0; i < passes.size(); i++)
   {
      KINGSN_LOG("[slang]: Building pass #%u (%s)\n", i,
      ...
      const gl_core_filter_chain_texture &texture)
{
    ...
    * GL HW render interface in libks is kinda garbage now ... */
      ...
      const video_shader_lut *shader)
{
   ...
   return unique_ptr<gl_core_shader::StaticTexture>(new gl_core_shader::StaticTexture(shader->id,
            ...
            shader->filter != KINGSN_FILTER_NEAREST,
            levels > 1,
            kingsn_wrap_to_address(shader->wrap)));
}
      ...
      video_shader *shader)
{
   ...
   for (i = 0; i < shader->luts; i++)
   {
      ...
      if (!image)
      {
         KINGSN_ERR("[GLCore]: Failed to load LUT \"%s\".\n", shader->lut[i].path);
         return false;
      }
      ...
      const char *path, glslang_filter_chain_filter filter)
{
   ...
   for (i = 0; i < shader->passes; i++)
   {
      ...
      {
         KINGSN_ERR("[GLCore]: Failed to compile shader: \"%s\".\n",
               pass->source.path);
         goto error;
      }
      ...
      for (auto &meta_param : output.meta.parameters)
      {
         ...
         {
            KINGSN_ERR("[GLCore]: Exceeded maximum number of parameters (%u).\n", GFX_MAX_PARAMETERS);
            goto error;
         }
         ...
         if (itr != shader->parameters + shader->num_parameters)
         {
            ...
            {
               KINGSN_ERR("[GLCore]: Duplicate parameters found for \"%s\", but arguments do not match.\n",
                     itr->id);
               goto error;
            }
      ...
      else
      {
         pass_info.source_filter =
            pass->filter == KINGSN_FILTER_LINEAR 
            ? GLSLANG_FILTER_CHAIN_LINEAR 
            : GLSLANG_FILTER_CHAIN_NEAREST;
      }
      pass_info.address       = kingsn_wrap_to_address(pass->wrap);
      ...
      pass_info.mip_filter    = pass->filter != KINGSN_FILTER_NEAREST && pass_info.max_levels > 1
         ? GLSLANG_FILTER_CHAIN_LINEAR 
         : GLSLANG_FILTER_CHAIN_NEAREST;
      ...
      if (!pass->fbo.valid)
      {
         ...
         if (scale_viewport)
         {
            ...
            if (explicit_format)
               KINGSN_WARN("[slang]: Using explicit format for last pass in chain,"
         ...
         else
         {
            ...
            KINGSN_LOG("[slang]: Using render target format %s for pass output #%u.\n",
