      ...
      unsigned id)
{
   ...
   switch (device)
   {
      ...
      case KS_DEVICE_ANALOG:
         break;
#if 0
      case KS_DEVICE_SENSOR_ACCELEROMETER:
         switch (id)
         {
            /* Fixed range of 0x000 - 0x3ff */
            case KS_DEVICE_ID_SENSOR_ACCELEROMETER_X:
               return ps3->accelerometer_state[port].x;
            case KS_DEVICE_ID_SENSOR_ACCELEROMETER_Y:
               return ps3->accelerometer_state[port].y;
            case KS_DEVICE_ID_SENSOR_ACCELEROMETER_Z:
