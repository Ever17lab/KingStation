      ...
      bool fullscreen)
{
   ...
   if (fullscreen && !windowed_fullscreen)
   {
      if (x11_enter_fullscreen(g_x11_dpy, width, height))
      {
         ...
         KINGSN_ERR("[X/EGL]: Entering true fullscreen failed. Will attempt windowed mode.\n");
   }

   wm_name = x11_get_wm_name(g_x11_dpy);
   if (wm_name)
   {
      KINGSN_LOG("[X/EGL]: Window manager is %s.\n", wm_name);

      if (true_full && strcasestr(wm_name, "xfwm"))
      {
         KINGSN_LOG("[X/EGL]: Using override-redirect workaround.\n");
   ...
   if (fullscreen || g_x11_screen != 0)
   {
      ...
      if (xinerama_get_coord(g_x11_dpy, g_x11_screen,
         ...
         KINGSN_LOG("[X/EGL]: Using Xinerama on screen #%u.\n", g_x11_screen);
      else
         KINGSN_LOG("[X/EGL]: Xinerama is not active on screen.\n");
   ...
   KINGSN_LOG("[X/EGL]: X = %d, Y = %d, W = %u, H = %u.\n",
   ...
   if (fullscreen && video_disable_composition)
   {
      ...
      KINGSN_LOG("[X/EGL]: Requesting compositor bypass.\n");
   ...
   if (true_full)
   {
      KINGSN_LOG("[X/EGL]: Using true fullscreen.\n");
   ...
   else if (fullscreen)
   {
      ...
      XMapRaised(g_x11_dpy, g_x11_win);
      KINGSN_LOG("[X/EGL]: Using windowed fullscreen.\n");
