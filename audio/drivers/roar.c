      ...
      unsigned block_frames, unsigned *new_rate)
{
   ...
   if (!(vss = roar_vs_new_simple(device, "KingStation", rate, 2, ROAR_CODEC_PCM_S, 16, ROAR_DIR_PLAY, &err)))
   {
      KINGSN_ERR("RoarAudio: \"%s\"\n", roar_vs_strerr(err));
      free(roar);
      return NULL;
   }
...
static void ra_set_nonblock_state(void *data, bool state)
{
   ...
   {
      KINGSN_ERR("Can't set nonblocking. Will not be able to fast-forward.\n");
   }
