...
#elif defined(HAVE_MATERIALUI) && defined(KINGSN_MOBILE)
...
struct config_bool_setting
{
   const char *ident;
   bool *ptr;
   enum kingsn_override_setting override;
   bool def_enable;
   bool def;
   bool handle;
};
...
static unsigned old_libks_device[MAX_USERS];
      ...
      settings_t *settings, int *size)
{
...
#endif
   SETTING_PATH("libks_directory",
         settings->paths.directory_libks, false, NULL, false);
   SETTING_PATH("core_options_path",
         settings->paths.path_core_options, false, NULL, true);
   SETTING_PATH("libks_info_path",
         settings->paths.path_libks_info, false, NULL, true);
   ...
   SETTING_PATH("savefile_directory",
         dir_get_ptr(KINGSN_DIR_SAVEFILE), true, NULL, false);
   SETTING_PATH("savestate_directory",
         dir_get_ptr(KINGSN_DIR_SAVESTATE), true, NULL, false);
#ifdef HAVE_MENU
      ...
      settings_t *settings, int *size)
{
   ...
   SETTING_OVERRIDE(KINGSN_OVERRIDE_SETTING_NETPLAY_STATELESS_MODE);
   ...
   SETTING_BOOL("menu_pause_libks",           &settings->bools.menu_pause_libks, false, false, false);
   ...
   SETTING_BOOL("menu_timedate_enable",          &settings->bools.menu_timedate_enable, true, false, false);
   SETTING_BOOL("menu_battery_level_enable",     &settings->bools.menu_battery_level_enable, true, false, false);
   ...
   SETTING_OVERRIDE(KINGSN_OVERRIDE_SETTING_LOG_TO_FILE);
      ...
      settings_t *settings, int *size)
{
   ...
   SETTING_UINT("libks_log_level",           &settings->uints.libks_log_level, true, DEFAULT_LIBKS_LOG_LEVEL, false);
...
#endif
#ifdef HAVE_NETWORKING
   SETTING_UINT("netplay_ip_port",              &settings->uints.netplay_port,         true, KINGSN_DEFAULT_PORT, false);
   SETTING_OVERRIDE(KINGSN_OVERRIDE_SETTING_NETPLAY_IP_PORT);
   ...
   SETTING_UINT("video_stream_port",            &settings->uints.video_stream_port,    true, KINGSN_STREAM_DEFAULT_PORT, false);
      ...
      settings_t *settings, int *size)
{
   ...
   SETTING_OVERRIDE(KINGSN_OVERRIDE_SETTING_NETPLAY_CHECK_FRAMES);
#endif
#ifdef HAVE_WASAPI
...
void config_set_defaults(void *data)
{
   ...
   *settings->arrays.cheevos_token                    = '\0';
   ...
   {
      for (j = 0; j < KINGSN_BIND_LIST_END; j++)
      {
         const struct ks_keybind *keyval = &input_config_binds[i][j];
         if (keyval->valid)
            ks_assert(j == keyval->id);
      }
   }
   ...
   *settings->arrays.input_keyboard_layout                = '\0';
...
#endif
      input_config_set_device(i, KS_DEVICE_JOYPAD);
    ...
    * for another config. */
   if (!KingStation_override_setting_is_set(KINGSN_OVERRIDE_SETTING_SAVE_PATH, NULL))
      dir_clear(KINGSN_DIR_SAVEFILE);
   if (!KingStation_override_setting_is_set(KINGSN_OVERRIDE_SETTING_STATE_PATH, NULL))
      dir_clear(KINGSN_DIR_SAVESTATE);

   *settings->paths.path_libks_info = '\0';
   *settings->paths.directory_libks = '\0';
   ...
   *settings->paths.directory_audio_filter = '\0';
   ...
   kingsn_ctl(KINGSN_CTL_UNSET_UPS_PREF, NULL);
   kingsn_ctl(KINGSN_CTL_UNSET_BPS_PREF, NULL);
   kingsn_ctl(KINGSN_CTL_UNSET_IPS_PREF, NULL);
   ...
   *settings->paths.log_dir = '\0';
   ...
   if (!string_is_empty(g_defaults.dirs[DEFAULT_DIR_CORE]))
      fill_pathname_expand_special(settings->paths.directory_libks,
   ...
   if (!string_is_empty(g_defaults.dirs[DEFAULT_DIR_CORE_INFO]))
      fill_pathname_expand_special(settings->paths.path_libks_info,
   ...
   if (!string_is_empty(g_defaults.dirs[DEFAULT_DIR_SAVESTATE]))
      dir_set(KINGSN_DIR_SAVESTATE, g_defaults.dirs[DEFAULT_DIR_SAVESTATE]);

   if (!string_is_empty(g_defaults.dirs[DEFAULT_DIR_SRAM]))
      dir_set(KINGSN_DIR_SAVEFILE, g_defaults.dirs[DEFAULT_DIR_SRAM]);
   ...
   if (!string_is_empty(g_defaults.path_config))
   {
      ...
      fill_pathname_expand_special(temp_str,
            ...
            sizeof(temp_str));
      path_set(KINGSN_PATH_CONFIG, temp_str);
   }
   ...
   if (DEFAULT_BLOCK_CONFIG_READ)
      kingsn_ctl(KINGSN_CTL_SET_BLOCK_CONFIG_READ, NULL);
   else
      kingsn_ctl(KINGSN_CTL_UNSET_BLOCK_CONFIG_READ, NULL);
#endif
...
static config_file_t *open_default_config_file(void)
{
   ...
   if (!conf)
   {
      ...
      if (!saved)
      {
         ...
         KINGSN_ERR("Failed to create new config file in: \"%s\".\n",
               conf_path);
         goto error;
      }

      KINGSN_WARN("Created new config file in: \"%s\".\n", conf_path);
   }
#elif defined(OSX)
   if (!fill_pathname_application_data(application_data,
            sizeof(application_data)))
   ...
   if (has_application_data)
   {
      fill_pathname_join(conf_path, application_data,
            FILE_PATH_MAIN_CONFIG, sizeof(conf_path));
      KINGSN_LOG("Looking for config in: \"%s\".\n", conf_path);
      conf = config_file_new_from_path_to_string(conf_path);
   }
   ...
   if (!conf && has_application_data)
   {
      ...
      if (dir_created)
      {
         ...
         if (conf)
            KINGSN_WARN("Config: using skeleton config \"%s\" as base for a new config file.\n", skeleton_conf);
         else
            conf = config_file_new_alloc();
         ...
         KINGSN_WARN("Config: Created new config file in: \"%s\".\n",
               conf_path);
      }
   }
#endif
   ...
   path_set(KINGSN_PATH_CONFIG, conf_path);
      ...
      const char *path, settings_t *settings)
{
   ...
   if (!path_is_empty(KINGSN_PATH_CONFIG_APPEND))
   {
      ...
      strlcpy(tmp_append_path, path_get(KINGSN_PATH_CONFIG_APPEND),
      ...
      while (extra_path)
      {
         ...
         KINGSN_LOG("Config: appending config \"%s\"\n", extra_path);

         if (!result)
            KINGSN_ERR("Config: failed to append config \"%s\"\n", extra_path);
   ...
   {
      KINGSN_LOG_OUTPUT("=== Config ===\n");
      config_file_dump_all(conf);
      KINGSN_LOG_OUTPUT("=== Config end ===\n");
   }
#endif

   /* Overrides */

   if (kingsn_ctl(KINGSN_CTL_HAS_SET_USERNAME, NULL))
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      ...
      snprintf(buf, sizeof(buf), "input_libks_device_p%u", i + 1);
      CONFIG_GET_INT_BASE(conf, settings, uints.input_libks_device[i], buf);
   }
   ...
   if (config_get_path(conf, "libks_directory", tmp_str, sizeof(tmp_str)))
      configuration_set_string(settings,
            settings->paths.directory_libks, tmp_str);
   ...
   if (kingsn_ctl(KINGSN_CTL_HAS_SET_USERNAME, NULL) && override_username)
   {
      configuration_set_string(settings,
   ...
   path_config = path_get(KINGSN_PATH_CONFIG);
   ...
   if (!string_is_empty(settings->paths.directory_screenshot))
   {
      ...
      {
         KINGSN_WARN("screenshot_directory is not an existing directory, ignoring ...\n");
         *settings->paths.directory_screenshot = '\0';
      }
   }
         ...
         config_get_path(conf, "savefile_directory", tmp_str, sizeof(tmp_str)))
   {
      ...
      {
         dir_set(KINGSN_DIR_SAVEFILE, tmp_str);
         ...
         if (global)
         {
            ...
            fill_pathname_dir(global->name.savefile,
                  path_get(KINGSN_PATH_BASENAME),
                  FILE_PATH_SRM_EXTENSION,
                  sizeof(global->name.savefile));
         }
      }
      else
         KINGSN_WARN("savefile_directory is not a directory, ignoring ...\n");
   }
         ...
         config_get_path(conf, "savestate_directory", tmp_str, sizeof(tmp_str)))
   {
      ...
      {
         dir_set(KINGSN_DIR_SAVESTATE, tmp_str);
         ...
         KINGSN_WARN("savestate_directory is not a directory, ignoring ...\n");
   }
...
bool config_load_override(void *data)
{
   ...
   kingsn_system_info_t *system            = (kingsn_system_info_t*)data;
   const char *core_name                  = system ?
      system->info.library_name : NULL;
   const char *kingsn_path_basename        = path_get(KINGSN_PATH_BASENAME);
   const char *game_name                  = path_basename(kingsn_path_basename);
   settings_t *settings                   = config_get_ptr();

   if (!string_is_empty(kingsn_path_basename))
      fill_pathname_parent_dir_name(content_dir_name,
            kingsn_path_basename, sizeof(content_dir_name));
   ...
   {
      KINGSN_LOG("[Overrides]: Core-specific overrides found at \"%s\".\n",
            core_path);

      path_set(KINGSN_PATH_CONFIG_APPEND, core_path);

      should_append = true;
   }
   else
      KINGSN_LOG("[Overrides]: No core-specific overrides found at \"%s\".\n",
            core_path);
   ...
   if (config_file_exists(content_path))
   {
      ...
      KINGSN_LOG("[Overrides]: Content dir-specific overrides found at \"%s\".\n",
            game_path);

      if (should_append)
      {
         KINGSN_LOG("[Overrides]: Content dir-specific overrides stacking on top of previous overrides.\n");
         strlcpy(temp_path, path_get(KINGSN_PATH_CONFIG_APPEND), sizeof(temp_path));
      ...
      path_set(KINGSN_PATH_CONFIG_APPEND, temp_path);

      should_append = true;
   }
   else
      KINGSN_LOG("[Overrides]: No content-dir-specific overrides found at \"%s\".\n",
         content_path);
   ...
   if (config_file_exists(game_path))
   {
      ...
      KINGSN_LOG("[Overrides]: Game-specific overrides found at \"%s\".\n",
            game_path);

      if (should_append)
      {
         KINGSN_LOG("[Overrides]: Game-specific overrides stacking on top of previous overrides.\n");
         strlcpy(temp_path, path_get(KINGSN_PATH_CONFIG_APPEND), sizeof(temp_path));
         strlcat(temp_path, "|", sizeof(temp_path));
      ...
      KINGSN_LOG("[Overrides]: No game-specific overrides found at \"%s\".\n",
            game_path);
   ...
   KingStation_override_setting_unset(KINGSN_OVERRIDE_SETTING_STATE_PATH, NULL);
   KingStation_override_setting_unset(KINGSN_OVERRIDE_SETTING_SAVE_PATH,  NULL);

   if (!config_load_file(global_get_ptr(),
            path_get(KINGSN_PATH_CONFIG), settings))
      return false;
   ...
   /* Reset save paths. */
   KingStation_override_setting_set(KINGSN_OVERRIDE_SETTING_STATE_PATH, NULL);
   KingStation_override_setting_set(KINGSN_OVERRIDE_SETTING_SAVE_PATH, NULL);

   path_clear(KINGSN_PATH_CONFIG_APPEND);
...
bool config_unload_override(void)
{
   ...
   if (!config_load_file(global_get_ptr(),
            path_get(KINGSN_PATH_CONFIG), config_get_ptr()))
      return false;

   KINGSN_LOG("[Overrides]: Configuration overrides unloaded, original configuration restored.\n");
      ...
      void *data)
{
   ...
   KINGSN_LOG("[Remaps]: Remap directory: \"%s\".\n", remap_directory);
   ...
   {
      KINGSN_LOG("[Remaps]: Game-specific remap found at \"%s\".\n", game_path);
      if (input_remapping_load_file(new_conf, game_path))
      {
         kingsn_ctl(KINGSN_CTL_SET_REMAPS_GAME_ACTIVE, NULL);
   ...
   {
      KINGSN_LOG("[Remaps]: Content-dir-specific remap found at \"%s\".\n", content_path);
      if (input_remapping_load_file(new_conf, content_path))
      {
         kingsn_ctl(KINGSN_CTL_SET_REMAPS_CONTENT_DIR_ACTIVE, NULL);
   ...
   {
      KINGSN_LOG("[Remaps]: Core-specific remap found at \"%s\".\n", core_path);
      if (input_remapping_load_file(new_conf, core_path))
      {
         kingsn_ctl(KINGSN_CTL_SET_REMAPS_CORE_ACTIVE, NULL);
...
static void config_parse_file(global_t *global)
{
   ...
   {
      KINGSN_LOG("[config] Loading default config.\n");
   }

   {
      const char *config_path = path_get(KINGSN_PATH_CONFIG);
      KINGSN_LOG("[config] loading config from: %s.\n", config_path);

      if (!config_load_file(global, config_path, config_get_ptr()))
      {
         KINGSN_ERR("[config] couldn't find config at path: \"%s\"\n",
               config_path);
      }
   }
}
      ...
      char *device_name, unsigned user)
{
   ...
   for (i = 0; i < KINGSN_FIRST_META_KEY; i++)
   {
      const struct ks_keybind *bind = &input_config_binds[user][i];
      if (bind->valid)
...
bool config_save_file(const char *path)
{
   ...
   if (!conf || kingsn_ctl(KINGSN_CTL_IS_OVERRIDES_ACTIVE, NULL))
   {
      if (conf)
         config_file_free(conf);
      return false;
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      ...
      snprintf(cfg, sizeof(cfg), "input_libks_device_p%u", i + 1);
   ...
   if (!KingStation_override_setting_is_set(KINGSN_OVERRIDE_SETTING_VERBOSITY, NULL))
      config_set_bool(conf, "log_verbosity",
            verbosity_is_enabled());
   config_set_bool(conf, "perfcnt_enable",
         kingsn_ctl(KINGSN_CTL_IS_PERFCNT_ENABLE, NULL));
...
bool config_save_overrides(enum override_type type, void *data)
{
   ...
   if (!string_is_empty(kingsn_path_basename))
      fill_pathname_parent_dir_name(content_dir_name, kingsn_path_basename, sizeof(content_dir_name));
   ...
   KINGSN_LOG("[Overrides]: Looking for changed settings... \n");
...
bool config_replace(bool config_replace_save_on_exit, char *path)
{
   ...
   const char *kingsn_path_config   = path_get(KINGSN_PATH_CONFIG);

   /* If config file to be replaced is the same as the
    * current config file, exit. */
   if (string_is_equal(path, kingsn_path_config))
      return false;

   if (config_replace_save_on_exit && !path_is_empty(KINGSN_PATH_CONFIG))
      config_save_file(kingsn_path_config);

   path_set(KINGSN_PATH_CONFIG, path);

   kingsn_ctl(KINGSN_CTL_UNSET_BLOCK_CONFIG_READ, NULL);

   /* Load core in new (salamander) config. */
   path_clear(KINGSN_PATH_CORE);
...
bool input_remapping_load_file(void *data, const char *path)
{
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      ...
      char btn_ident[KINGSN_FIRST_CUSTOM_BIND][128]       = {{0}};
      char key_ident[KINGSN_FIRST_CUSTOM_BIND][128]       = {{0}};
      char stk_ident[8][4096]                            = {{0}};

      char key_strings[KINGSN_FIRST_CUSTOM_BIND + 8][128] = {
         "b", "y", "select", "start",
         "up", "down", "left", "right",
         "a", "x", "l", "r", "l2", "r2",
      ...
      old_libks_device[i]  = settings->uints.input_libks_device[i];
      ...
      for (j = 0; j < KINGSN_FIRST_CUSTOM_BIND + 8; j++)
      {
         if (j < KINGSN_FIRST_CUSTOM_BIND)
         {
            int btn_remap = -1;
            int key_remap = -1;
      ...
      snprintf(s1, sizeof(s1), "input_libks_device_p%u", i + 1);
      CONFIG_GET_INT_BASE(conf, settings, uints.input_libks_device[i], s1);
   }
   ...
   for (i = 0; i < max_users; i++)
   {
      ...
      for (j = 0; j < KINGSN_FIRST_CUSTOM_BIND + 8; j++)
      {
         ...
         if (j < KINGSN_FIRST_CUSTOM_BIND)
         {
            ...
            if (remap_id != j && remap_id != KINGSN_UNMAPPED)
               config_set_int(conf, btn_ident[j],
                     settings->uints.input_remap_ids[i][j]);
            else if (remap_id != j && remap_id == KINGSN_UNMAPPED)
               config_set_int(conf, btn_ident[j], -1);
            else
               config_unset(conf, btn_ident[j]);

            if (keymap_id != KSK_UNKNOWN)
...
void input_remapping_set_defaults(bool deinit)
{
   ...
   if (deinit)
   {
      ...
      kingsn_ctl(KINGSN_CTL_UNSET_REMAPS_CORE_ACTIVE, NULL);
      kingsn_ctl(KINGSN_CTL_UNSET_REMAPS_CONTENT_DIR_ACTIVE, NULL);
      kingsn_ctl(KINGSN_CTL_UNSET_REMAPS_GAME_ACTIVE, NULL);
   }
   ...
   for (i = 0; i < MAX_USERS; i++)
   {
      for (j = 0; j < KINGSN_FIRST_CUSTOM_BIND + 8; j++)
      {
         if (j < KINGSN_FIRST_CUSTOM_BIND)
         {
            const struct  ks_keybind *keybind = &input_config_binds[i][j];
            if (keybind)
               configuration_set_uint(settings,
                     settings->uints.input_remap_ids[i][j], keybind->id);
            configuration_set_uint(settings,
                  settings->uints.input_keymapper_ids[i][j], KSK_UNKNOWN);
         }
         else
            configuration_set_uint(settings,
      ...
      if (old_libks_device[i])
         configuration_set_uint(settings,
               settings->uints.input_libks_device[i], old_libks_device[i]);
   }
}
#endif
 ...
 * entry (libks_path), which is linked to
 * KINGSN_PATH_CORE
...
void config_load_file_salamander(void)
{
   ...
   /* Read 'libks_path' value and update
    * KINGSN_PATH_CORE */
   KINGSN_LOG("[config] Loading salamander config from: \"%s\".\n",
         config_path);
   ...
   if (config_get_path(config, "libks_path",
      ...
      path_set(KINGSN_PATH_CORE, libks_path);
...
{
   config_file_t *config     = NULL;
   const char *libks_path = path_get(KINGSN_PATH_CORE);
   bool success              = false;
   char config_path[PATH_MAX_LENGTH];

   config_path[0] = '\0';

   if (string_is_empty(libks_path) ||
       string_is_equal(libks_path, "builtin"))
      return;
   ...
   /* Update config file */
   config_set_path(config, "libks_path", libks_path);
   ...
   if (success)
      KINGSN_LOG("[config] Saving salamander config to: \"%s\".\n",
            config_path);
   else
      KINGSN_ERR("[config] Failed to create new salamander config file in: \"%s\".\n",
            config_path);
