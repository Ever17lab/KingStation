...
#define THUMBNAIL_URL_HEADER "https://github.com/libks-thumbnails/"
...
void MainWindow::onThumbnailDownloadFinished()
{
   ...
   if (code != 200)
   {
      ...
      if (!redirectUrl.isEmpty())
      {
         ...
         KINGSN_LOG("[Qt]: Thumbnail download got redirect with HTTP code %d: %s\n", code, redirectUrlData);
      ...
      KINGSN_ERR("[Qt]: Thumbnail download failed with HTTP status code: %d\n", code);
   ...
   if (error == QNetworkReply::NoError)
   {
      ...
      if (newFile.exists() && !newFile.remove())
         KINGSN_ERR("[Qt]: Thumbnail download finished, but old file could not be deleted.\n");
      else
      {
         if (m_thumbnailDownloadFile.rename(newFileName))
         {
            KINGSN_LOG("[Qt]: Thumbnail download finished successfully.\n");
         ...
         {
            KINGSN_ERR("[Qt]: Thumbnail download finished, but temp file could not be renamed.\n");
   ...
   else
   {
      ...
      KINGSN_ERR("[Qt]: Thumbnail download ended prematurely: %s\n", errorData);
...
void MainWindow::downloadThumbnail(QString system, QString title, QUrl url)
{
   ...
   KINGSN_LOG("[Qt]: Starting thumbnail download...\n");
