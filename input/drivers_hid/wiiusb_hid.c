...
{
    if (devdesc.idVendor != VID_KSDE || devdesc.idProduct != PID_KSDE)
        return false;
...
static int wiiusb_hid_add_adapter(void *data, usb_device_entry *dev)
{
   ...
   {
       KINGSN_LOG("Retrode SNES mouse found (currently not supported)\n");
       goto error;
   }
   ...
   {
      KINGSN_ERR(" Interface not found.\n");
      goto error;
   }
   ...
   {
       /* Retrode port #1 */
       KINGSN_LOG("Interface Retrode1 gamepad slot: %d\n", adapter->slot);
       ...
       for (i = 2; i <= 4; i++)
       {
           ...
           if (slot1 == -1)
               KINGSN_LOG("No slot free for Retrode%d gamepad\n", i);
           else
           {
               KINGSN_LOG("Interface Retrode%d gamepad slot: %d\n", i, slot1);
...
static void *wiiusb_hid_init(void)
{
   ...
   if (!hid->poll_thread)
   {
      KINGSN_ERR("Error initializing poll thread.\n");
      goto error;
   }
