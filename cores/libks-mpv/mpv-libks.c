...
#define CORE_PREFIX(s) libks_mpv_##s
#else
...
static struct ks_log_callback logging;
static ks_log_printf_t log_cb;
...
static void process_mpv_events(mpv_event_id event_block)
{
	do
	{
		...
		else if(mp_event->event_id == MPV_EVENT_END_FILE)
		{
			 ...
			 */
			struct ks_message ra_msg = {
				"Finished playing file", 60 * 5, /* 5 seconds */
			};

			CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_MESSAGE, &ra_msg);KS_ENVIRONMENT_SHUTDOWN
#endif
		}
...
#pragma GCC diagnostic pop
	...
	if(proc_addr == NULL)
		log_cb(KS_LOG_ERROR, "Failure obtaining %s proc address\n", name);
...
void CORE_PREFIX(ks_init)(void)
{
	...
	{
		log_cb(KS_LOG_WARN, "libmpv version mismatch. Please update or "
				"recompile mpv-libks after updating libmpv.");
	}
...
{
	log_cb(KS_LOG_INFO, "Plugging device %u into port %u.\n", device, port);
	return;
}
...
void CORE_PREFIX(ks_get_system_info)(struct ks_system_info *info)
{
	...
	info->library_version  = LIBKS_MPV_VERSION;
...
void CORE_PREFIX(ks_get_system_av_info)(struct ks_system_av_info *info)
{
...
#if 0
	struct ks_variable var = { .key = "test_aspect" };
	CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &var);
...
#endif
	info->timing = (struct ks_system_timing) {
		.fps = 60.0,
	 ...
	 */
	info->geometry = (struct ks_game_geometry) {
		.base_width   = 256,
		.base_height  = 144,
		.max_width    = 1920,
		.max_height   = 1080,
...
void CORE_PREFIX(ks_set_environment)(ks_environment_t cb)
{
	...
	if (cb(KS_ENVIRONMENT_GET_LOG_INTERFACE, &logging))
		log_cb = logging.log;
	else
		log_cb = fallback_log;
}
...
static void context_reset(void)
{
	...
	if(!mpv)
	{
		log_cb(KS_LOG_ERROR, "failed creating context\n");
		exit(EXIT_FAILURE);
	}

	if((ret = mpv_initialize(mpv)) < 0)
	{
		log_cb(KS_LOG_ERROR, "mpv init failed: %s\n", mpv_error_string(ret));
		exit(EXIT_FAILURE);
	}

	if((ret = mpv_request_log_messages(mpv, "v")) < 0)
	{
		log_cb(KS_LOG_ERROR, "mpv logging failed: %s\n",
				mpv_error_string(ret));
	}
	...
	{
		log_cb(KS_LOG_ERROR, "failed to initialize mpv GL context: %s\n",
				mpv_error_string(ret));
		goto err;
	}
	...
	{
		log_cb(KS_LOG_ERROR, "failed to set hwdec option: %s\n",
				mpv_error_string(ret));
	}

	if((ret = mpv_command(mpv, cmd)) != 0)
	{
		log_cb(KS_LOG_ERROR, "mpv_command failed to load input file: %s\n",
				mpv_error_string(ret));
		goto err;
	}
	...
	log_cb(KS_LOG_INFO, "Context reset.\n");
...
static void context_destroy(void)
{
	...
	log_cb(KS_LOG_INFO, "Context destroyed.\n");
}

static bool ks_init_hw_context(void)
{
...
static void audio_callback(double fps)
{
    ...
    do
	{
        ...
        {
#if 0
            log_cb(KS_LOG_ERROR, "mpv encountered an error in audio "
                    "callback: %d.\n", ret);
#endif
            return;
        }
...
static void kspad_update_input(void)
{
	struct Input
	{
		unsigned int l : 1;
	 ...
	 */
	current.l = CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD,
			0, KS_DEVICE_ID_JOYPAD_L) != 0 ? 1 : 0;
	current.r = CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD,
			0, KS_DEVICE_ID_JOYPAD_R) != 0 ? 1 : 0;
	current.a = CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD,
			0, KS_DEVICE_ID_JOYPAD_A) != 0 ? 1 : 0;
	...
	if (CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD, 0,
			KS_DEVICE_ID_JOYPAD_LEFT))
		mpv_command_string(mpv, "seek -5");

	if (CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD, 0,
			KS_DEVICE_ID_JOYPAD_RIGHT))
		mpv_command_string(mpv, "seek 5");

	if (CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD, 0,
			KS_DEVICE_ID_JOYPAD_UP))
		mpv_command_string(mpv, "seek 60");

	if (CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD, 0,
			KS_DEVICE_ID_JOYPAD_DOWN))
		mpv_command_string(mpv, "seek -60");
...
void CORE_PREFIX(ks_run)(void)
{
	...
	if(updated_video_dimensions == false)
	{
		 ...
		 * ks_get_system_av_info is called, so we have to set it here for
		...
		log_cb(KS_LOG_INFO, "Setting fps to %f\n", container_fps);
		...
		struct ks_system_av_info av_info = {
			.geometry = geometry,
			.timing = timing,
		};

		if(width > 0 && height > 0)
			CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_SYSTEM_AV_INFO, &av_info);
	...
	if(frame_queue > 0)
	{
		...
		CORE_PREFIX(video_cb)(KS_HW_FRAME_BUFFER_VALID, width, height, 0);
		frame_queue--;
	}
	else
...
bool CORE_PREFIX(ks_serialize)(void *data_, size_t size)
{
	return true;
}

bool CORE_PREFIX(ks_unserialize)(const void *data_, size_t size)
{
	return true;
}
...
bool CORE_PREFIX(ks_load_game)(const struct ks_game_info *info)
{
	...
	struct ks_input_descriptor desc[] = {
		{ 0, KS_DEVICE_JOYPAD, 0, KS_DEVICE_ID_JOYPAD_A,  "Pause/Play" },
		{ 0, KS_DEVICE_JOYPAD, 0, KS_DEVICE_ID_JOYPAD_X,  "Show Progress" },
	...
	{
		log_cb(KS_LOG_ERROR, "Unable to allocate memory for filepath\n");
		return false;
	}
	...
	if(CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt) == false)
	{
		log_cb(KS_LOG_ERROR, "XRGB8888 is not supported.\n");

		/* Try RGB565 */
		fmt = KS_PIXEL_FORMAT_RGB565;
		CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt);
	}

	if(ks_init_hw_context() == false)
	{
		log_cb(KS_LOG_ERROR, "HW Context could not be initialized\n");
		return false;
	}
...
bool CORE_PREFIX(ks_load_game_special)(unsigned type, const struct ks_game_info *info,
		size_t num)
{
	return false;
}
