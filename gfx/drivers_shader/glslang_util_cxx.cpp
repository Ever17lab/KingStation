   ...
   for (i = 0; i < lines->size; i++)
   {
      ...
      if (string_starts_with_size(line, "#pragma", STRLEN_CONST("#pragma")))
      {
                  ...
                  STRLEN_CONST("#pragma name ")))
         {
            ...
            if (!meta->name.empty())
            {
               KINGSN_ERR("[slang]: Trying to declare multiple names for file.\n");
               return false;
            }
                  ...
                  STRLEN_CONST("#pragma parameter ")))
         {
            ...
            if (ret == 6)
            {
               ...
               if (parameter_found)
               {
                  ...
                  {
                     KINGSN_ERR("[slang]: Duplicate parameters found for \"%s\", but arguments do not match.\n", id);
                     return false;
                  }
               }
               else
            ...
            {
               KINGSN_ERR("[slang]: Invalid #pragma parameter line: \"%s\".\n",
                     line);
               return false;
            }
         }
                  ...
                  STRLEN_CONST("#pragma format ")))
         {
            ...
            {
               KINGSN_ERR("[slang]: Trying to declare format multiple times for file.\n");
               return false;
            }
            ...
            if (meta->rt_format == SLANG_FORMAT_UNKNOWN)
            {
               KINGSN_ERR("[slang]: Failed to find format \"%s\".\n", str);
               return false;
            }
         }
      }
...
bool glslang_compile_shader(const char *shader_path, glslang_output *output)
{
   ...
   KINGSN_LOG("[slang]: Compiling shader \"%s\".\n", shader_path);
   ...
   {
      KINGSN_ERR("Failed to compile vertex shader stage.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("Failed to compile fragment shader stage.\n");
      goto error;
   }
