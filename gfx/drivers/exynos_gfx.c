      ...
      struct exynos_device *dev, unsigned size)
{
   ...
   if (!buf)
   {
      KINGSN_ERR("[video_exynos]: failed to create temp buffer object\n");
      return NULL;
   }

   if (!exynos_bo_map(buf))
   {
      KINGSN_ERR("[video_exynos]: failed to map temp buffer object\n");
      ...
      enum exynos_buffer_type type, unsigned size)
{
   ...
   if (size > buf->size)
   {
...
#if (EXYNOS_GFX_DEBUG_LOG == 1)
      KINGSN_LOG("[video_exynos]: reallocating %s buffer (%u -> %u bytes)\n",
      ...
      if (!buf)
      {
         KINGSN_ERR("[video_exynos]: reallocation failed\n");
         return -1;
      }
...
static int exynos_clear_buffer(struct g2d_context *g2d, struct g2d_image *img)
{
   ...
   if (ret != 0)
      KINGSN_ERR("[video_exynos]: failed to clear buffer using G2D\n");
...
{
   KINGSN_LOG("[video_exynos]: debug: total memcpy calls: %u\n", p->memcpy_calls);
   KINGSN_LOG("[video_exynos]: debug: total g2d calls: %u\n", p->g2d_calls);

   KINGSN_LOG("[video_exynos]: debug: total memcpy time: %f seconds\n",
         (double)p->memcpy_time / 1000000.0);
   KINGSN_LOG("[video_exynos]: debug: total g2d time: %f seconds\n",
         (double)p->g2d_time / 1000000.0);

   KINGSN_LOG("[video_exynos]: debug: average time per memcpy call: %f microseconds\n",
         (double)p->memcpy_time / (double)p->memcpy_calls);
   KINGSN_LOG("[video_exynos]: debug: average time per g2d call: %f microseconds\n",
...
static int exynos_open(struct exynos_data *pdata)
{
   ...
   {
      KINGSN_ERR("[video_exynos]: no compatible DRM device found\n");
      return -1;
   }
   ...
   if (fd < 0)
   {
      KINGSN_ERR("[video_exynos]: can't open DRM device\n");
      return -1;
   }
   ...
   KINGSN_LOG("[video_exynos]: using DRM device \"%s\" with connector id %u.\n",
...
static int exynos_init(struct exynos_data *pdata, unsigned bpp)
{
   ...
   {
      KINGSN_ERR("[video_exynos]: failed to select sane resolution\n");
      goto fail;
   }
   ...
   KINGSN_LOG("[video_exynos]: selected %ux%u resolution with %u bpp\n",
...
static int exynos_alloc(struct exynos_data *pdata)
{
   ...
   if (!device)
   {
      KINGSN_ERR("[video_exynos]: failed to create device from fd\n");
      return -1;
   }
   ...
   if (!pages)
   {
      KINGSN_ERR("[video_exynos]: failed to allocate pages\n");
      goto fail_alloc;
   }
   ...
   for (i = 0; i < pdata->num_pages; ++i)
   {
      ...
      if (!bo)
      {
         KINGSN_ERR("[video_exynos]: failed to create buffer object\n");
         goto fail;
      }
   ...
   for (i = 0; i < pdata->num_pages; ++i)
   {
      ...
      {
         KINGSN_ERR("[video_exynos]: failed to add bo %u to fb\n", i);
         goto fail;
      }
   }
   ...
   {
      KINGSN_ERR("[video_exynos]: initial CRTC setup failed.\n");
      goto fail;
   }
...
static void exynos_free(struct exynos_data *pdata)
{
   ...
   if (drmModeSetCrtc(g_drm_fd, g_crtc_id, 0,
            0, 0, NULL, 0, NULL))
      KINGSN_WARN("[video_exynos]: failed to disable the CRTC.\n");
...
static void exynos_alloc_status(struct exynos_data *pdata)
{
   ...
   KINGSN_LOG("[video_exynos]: Allocated %u pages with %u bytes each (pitch = %u bytes)\n",
         pdata->num_pages, pdata->size, pdata->pitch);

   for (i = 0; i < pdata->num_pages; ++i)
   {
      KINGSN_LOG("[video_exynos]: page %u: BO at %p, buffer id = %u\n",
      ...
      unsigned src_pitch)
{
   ...
   if (g2d_copy_with_scale(pdata->g2d, src, pdata->dst, 0, 0,
         ...
         g2d_exec(pdata->g2d))
   {
      KINGSN_ERR("[video_exynos]: failed to blit frame.\n");
      return -1;
   }
                             ...
                             unsigned rotation)
{
   ...
   if (g2d_scale_and_blend(pdata->g2d, src, pdata->dst, 0, 0,
         ...
         g2d_exec(pdata->g2d))
   {
      KINGSN_ERR("[video_exynos]: failed to blend menu.\n");
      return -1;
   }
...
static int exynos_blend_font(struct exynos_data *pdata)
{
   ...
   if (g2d_scale_and_blend(pdata->g2d, src, pdata->dst, 0, 0, src->width,
         ...
         g2d_exec(pdata->g2d))
   {
      KINGSN_ERR("[video_exynos]: failed to blend font\n");
      return -1;
   }
...
static int exynos_flip(struct exynos_data *pdata, struct exynos_page *page)
{
   ...
   {
      KINGSN_ERR("[video_exynos]: failed to issue page flip\n");
      return -1;
   }
   else
   {
...
static int exynos_init_font(struct exynos_video *vid)
{
   ...
   {
      KINGSN_ERR("[video_exynos]: creating font renderer failed\n");
      return -1;
   }
      ...
      input_driver_t **input, void **input_data)
{
   ...
   {
      KINGSN_ERR("[video_exynos]: opening device failed\n");
      goto fail;
   }

   if (exynos_init(vid->data, fb_bpp) != 0)
   {
      KINGSN_ERR("[video_exynos]: initialization failed\n");
      goto fail_init;
   }

   if (exynos_alloc(vid->data) != 0)
   {
      KINGSN_ERR("[video_exynos]: allocation failed\n");
      goto fail_alloc;
   }

   if (exynos_g2d_init(vid->data) != 0)
   {
      KINGSN_ERR("[video_exynos]: G2D initialization failed\n");
      goto fail_g2d;
   }
   ...
   {
      KINGSN_ERR("[video_exynos]: font initialization failed\n");
      goto fail_font;
   }
      ...
      video_frame_info_t *video_info)
{
   ...
   if (frame)
   {
      if (width != vid->width || height != vid->height)
      {
         ...
         KINGSN_LOG("[video_exynos]: resolution changed by core: %ux%u -> %ux%u\n",
