      ...
      unsigned *new_out_rate)
{
   ...
   if (ioctl(ossaudio->fd, SNDCTL_DSP_SETFRAGMENT, &frag) < 0)
      KINGSN_WARN("Cannot set fragment sizes. Latency might not be as expected ...\n");
   ...
   {
      KINGSN_WARN("Requested sample rate not supported. Adjusting output rate to %d Hz.\n", new_rate);
...
static void oss_set_nonblock_state(void *data, bool state)
{
   ...
   if (state)
      ...
      rc =  fcntl(ossaudio->fd, F_SETFL,
            fcntl(ossaudio->fd, F_GETFL) & (~O_NONBLOCK));
   if (rc != 0)
      KINGSN_WARN("Could not set nonblocking on OSS file descriptor. Will not be able to fast-forward.\n");
}
...
static size_t oss_write_avail(void *data)
{
   ...
   {
      KINGSN_ERR("[OSS]: SNDCTL_DSP_GETOSPACE failed ...\n");
      return 0;
   }
