...
static void hidpad_psxadapter_packet_handler(void *data, uint8_t *packet, uint16_t size)
{
   ...
   if (device->data[2]==0x7F) /* digital mode detection */
   {
      ...
      device->buttons |= (device->data[4]==0x00) ? (1 << KS_DEVICE_ID_JOYPAD_LEFT) : 0;
      device->buttons |= (device->data[4]==0xFF) ? (1 << KS_DEVICE_ID_JOYPAD_RIGHT) : 0;
      device->buttons |= (device->data[5]==0x00) ? (1 << KS_DEVICE_ID_JOYPAD_UP) : 0;
      device->buttons |= (device->data[5]==0xFF) ? (1 << KS_DEVICE_ID_JOYPAD_DOWN) : 0;
   }
   else
   {
      /* We're in analog mode, process the hat values as if they were pad buttons */
      hat_value = PSX_H_GET(device->data[6]);
      device->buttons |= PSX_H_LEFT(hat_value) ? (1 << KS_DEVICE_ID_JOYPAD_LEFT) : 0;
      device->buttons |= PSX_H_RIGHT(hat_value) ? (1 << KS_DEVICE_ID_JOYPAD_RIGHT) : 0;
      device->buttons |= PSX_H_UP(hat_value) ? (1 << KS_DEVICE_ID_JOYPAD_UP) : 0;
      device->buttons |= PSX_H_DOWN(hat_value) ? (1 << KS_DEVICE_ID_JOYPAD_DOWN) : 0;
   }
}
