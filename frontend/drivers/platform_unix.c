...
#include "../../libks-common/lists/vector_list.c"
...
error:
   KINGSN_ERR("Could not create pipe.\n");
   return 0;
}
...
void android_app_write_cmd(struct android_app *android_app, int8_t cmd)
{
   ...
   if (write(android_app->msgwrite, &cmd, sizeof(cmd)) != sizeof(cmd))
      KINGSN_ERR("Failure writing android_app cmd: %s\n", strerror(errno));
}
...
static void android_app_free(struct android_app* android_app)
{
   ...
   KINGSN_LOG("Joined with KingStation native thread.\n");
      ...
      ANativeActivity* activity, size_t* outLen)
{
   ...
   KINGSN_LOG("SaveInstanceState: %p\n", activity);
...
{
   KINGSN_LOG("ConfigurationChanged: %p\n", activity);
...
{
   KINGSN_LOG("WindowFocusChanged: %p -- %d\n", activity, focused);
...
static void onNativeWindowCreated(ANativeActivity* activity,
      ANativeWindow* window)
{
   KINGSN_LOG("NativeWindowCreated: %p -- %p\n", activity, window);
...
static void onNativeWindowDestroyed(ANativeActivity* activity,
      ANativeWindow* window)
{
   KINGSN_LOG("NativeWindowDestroyed: %p -- %p\n", activity, window);
   android_app_set_window((struct android_app*)activity->instance, NULL);
}

static void onInputQueueCreated(ANativeActivity* activity, AInputQueue* queue)
{
   KINGSN_LOG("InputQueueCreated: %p -- %p\n", activity, queue);
...
static void onInputQueueDestroyed(ANativeActivity* activity,
      AInputQueue* queue)
{
   KINGSN_LOG("InputQueueDestroyed: %p -- %p\n", activity, queue);
      ...
      const ARect *rect)
{
   ...
   KINGSN_LOG("Content rect changed: %u x %u\n", width, height);
...
JNIEnv *jni_thread_getenv(void)
{
   ...
   if (status < 0)
   {
      KINGSN_ERR("jni_thread_getenv: Failed to attach current thread.\n");
      return NULL;
   }
...
static void jni_thread_destruct(void *value)
{
   ...
   struct android_app *android_app = (struct android_app*)g_android;
   KINGSN_LOG("jni_thread_destruct()\n");
        ...
        void* savedState, size_t savedStateSize)
{
   ...
   if (!android_app)
   {
      KINGSN_ERR("Failed to initialize android_app\n");
      return NULL;
   }
   ...
   if (pipe(msgpipe))
   {
      KINGSN_ERR("could not create pipe: %s.\n", strerror(errno));
      if (android_app->savedState)
...
void ANativeActivity_onCreate(ANativeActivity* activity,
      void* savedState, size_t savedStateSize)
{
   KINGSN_LOG("Creating Native Activity: %p\n", activity);
   ...
   if (pthread_key_create(&thread_key, jni_thread_destruct))
      KINGSN_ERR("Error initializing pthread_key\n");
      ...
      int *seconds, int *percent)
{
   ...
   bool have_ac        = false;
   bool charging       = false;
   struct RDIR *entry  = ks_opendir(proc_acpi_battery_path);
   if (!entry)
      return false;

   if (ks_dirent_error(entry))
   {
      ks_closedir(entry);
      return false;
   }

   while (ks_readdir(entry))
      check_proc_acpi_battery(ks_dirent_get_name(entry),
            &have_battery, &charging, seconds, percent);

   ks_closedir(entry);

   entry = ks_opendir(proc_acpi_ac_adapter_path);
   if (!entry)
      return false;

   while (ks_readdir(entry))
      check_proc_acpi_ac_adapter(
            ks_dirent_get_name(entry), &have_ac);
      ...
      int *seconds, int *percent)
{
   ...
   bool have_ac        = false;
   bool charging       = false;
   struct RDIR *entry  = ks_opendir(proc_acpi_sysfs_battery_path);
   if (!entry)
      goto error;
   ...
   while (ks_readdir(entry))
   {
      const char *node = ks_dirent_get_name(entry);
   ...
   entry = ks_opendir(proc_acpi_sysfs_ac_adapter_path);

   if (entry)
   {
      check_proc_acpi_sysfs_ac_adapter(ks_dirent_get_name(entry), &have_ac);
      ks_closedir(entry);
   }
   else
      have_ac = false;
      ...
      char *argv[], void *data, void *params_data)
{
   ...
   struct kingsn_main_wrap      *args  = NULL;
   JNIEnv                       *env  = NULL;
   jobject                       obj  = NULL;
   jstring                      jstr  = NULL;
   ...
   args = (struct kingsn_main_wrap*)params_data;
   ...
   if (android_app->getStringExtra && jstr)
   {
      ...
      *core_path = '\0';
      ...
      __android_log_print(ANDROID_LOG_INFO,
         "KingStation", "[ENV]: libks path: [%s]\n", core_path);
      if (args && *core_path)
         args->libks_path = core_path;
   }
   ...
   else if (strstr(device_model, "SHIELD"))
   {
      ...
      g_defaults.menu_controls_menu_btn_ok     = KS_DEVICE_ID_JOYPAD_B;
      g_defaults.menu_controls_menu_btn_cancel = KS_DEVICE_ID_JOYPAD_A;
#endif
   }
   ...
   fill_pathname_join(g_defaults.dirs[DEFAULT_DIR_REMAP],
         ...
         "System/Remaps", sizeof(g_defaults.dirs[DEFAULT_DIR_REMAP]));
   fill_pathname_join(g_defaults.dirs[DEFAULT_DIR_PLAYLIST], base_path,
         "System/Playlists", sizeof(g_defaults.dirs[DEFAULT_DIR_PLAYLIST]));
   fill_pathname_join(g_defaults.dirs[DEFAULT_DIR_RECORD_CONFIG], base_path,
         "System/Records_config", sizeof(g_defaults.dirs[DEFAULT_DIR_RECORD_CONFIG]));
   fill_pathname_join(g_defaults.dirs[DEFAULT_DIR_RECORD_OUTPUT], base_path,
         "System/Records", sizeof(g_defaults.dirs[DEFAULT_DIR_RECORD_OUTPUT]));
   ...
   fill_pathname_join(g_defaults.dirs[DEFAULT_DIR_OVERLAY], base_path,
         "System/Overlay", sizeof(g_defaults.dirs[DEFAULT_DIR_OVERLAY]));
#ifdef HAVE_VIDEO_LAYOUT
...
static void frontend_unix_init(void *data)
{
   ...
   KINGSN_LOG("Waiting for Android Native Window to be initialized ...\n");
   ...
   KINGSN_LOG("Android Native Window initialized.\n");
   ...
   switch (fork_mode)
   {
      ...
      case FRONTEND_FORK_RESTART:
...

         {
            ...
            path_set(KINGSN_PATH_CORE, executable_path);
         }
...
static void frontend_unix_watch_path_for_changes(struct string_list *list, int flags, path_change_data_t **change_data)
{
   ...
   if (uname(&buffer) != 0)
   {
      KINGSN_WARN("watch_path_for_changes: Failed to get current kernel version.\n");
      return;
   }
   ...
   if (major < 2)
   {
      KINGSN_WARN("watch_path_for_changes: inotify unsupported on this kernel version (%d.%d.%u).\n", major, minor, krel);
      return;
   }
   else if (major == 2)
   {
   ...
   if (fd < 0)
   {
      KINGSN_WARN("watch_path_for_changes: Could not initialize inotify.\n");
      return;
   }

   if (fcntl(fd, F_SETFL, fcntl(fd, F_GETFL) | O_NONBLOCK))
   {
      KINGSN_WARN("watch_path_for_changes: Could not set socket to non-blocking.\n");
      return;
   }
   ...
   for (i = 0; i < list->size; i++)
   {
      ...
      KINGSN_LOG("Watching file for changes: %s\n", list->elems[i].data);
         ...
         if (event->mask & inotify_data->flags)
         {
            ...
            for (j = 0; j < inotify_data->wd_list->count; j++)
            {
               if (inotify_data->wd_list->data[j] == event->wd)
               {
                  ...
                  KINGSN_LOG("file change detected: %s\n", path);
...
enum ks_language frontend_unix_get_user_language(void)
{
   enum ks_language lang = KS_LANGUAGE_ENGLISH;
   ...
   if (g_android->getUserLanguageString)
   {
      ...
      if (jstr)
      {
         ...
         lang = kingsn_get_language_from_iso(langStr);
   ...
   if (envvar)
      lang = kingsn_get_language_from_iso(envvar);
#endif
#endif
   return lang;
}
      ...
      const char* speak_text, int priority)
{
   ...
   {
      /* error */
      KINGSN_LOG("ERROR: could not fork for espeak.\n");
   }
   else if (pid > 0)
   {
      /* parent process */
