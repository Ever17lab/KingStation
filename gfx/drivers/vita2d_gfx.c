      ...
      input_driver_t **input, void **input_data)
{
   ...
   *input_data    = NULL;
   ...
   KINGSN_LOG("vita2d_gfx_init: w: %i  h: %i\n", video->width, video->height);
   KINGSN_LOG("KINGSN_SCALE_BASE: %i input_scale: %i = %i\n",
	 KINGSN_SCALE_BASE, video->input_scale, KINGSN_SCALE_BASE * video->input_scale);
   ...
   if (video->rgb32)
   {
      KINGSN_LOG("Format: SCE_GXM_TEXTURE_FORMAT_X8U8U8U8_1RGB\n");
      vita->format = SCE_GXM_TEXTURE_FORMAT_X8U8U8U8_1RGB;
   }
   else
   {
      KINGSN_LOG("Format: SCE_GXM_TEXTURE_FORMAT_R5G6B5\n");
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (frame)
   {
      if(!(vita->texture&&vita2d_texture_get_datap(vita->texture)==frame))
      {
         ...
         if (!vita->texture)
         {
            KINGSN_LOG("Creating texture: %ix%i\n", width, height);
            vita->width = width;
            vita->height = height;
      ...
      unsigned width, unsigned height, float alpha)
{
   ...
   if (!vita->menu.texture)
   {
      if (rgb32)
      {
         ...
         KINGSN_LOG("Creating Frame RGBA8 texture: w: %i  h: %i\n",
               width, height);
      }
      else
      {
         ...
         KINGSN_LOG("Creating Frame R5G6B5 texture: w: %i  h: %i\n",
               width, height);
      }
