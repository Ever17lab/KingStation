      ...
      )
{
   ...
   LPDIRECT3DDEVICE8 d3dr                  = (LPDIRECT3DDEVICE8)d3d->dev;
   d3d8_renderchain_t *chain              = (d3d8_renderchain_t*)d3d->renderchain_data;
   unsigned fmt                           = (rgb32) ? KS_PIXEL_FORMAT_XRGB8888 : KS_PIXEL_FORMAT_RGB565;
   struct video_viewport *custom_vp       = video_viewport_get_custom();

   video_driver_get_size(&width, &height);

   chain->dev                             = dev_data;
   chain->pixel_size                      = (fmt == KS_PIXEL_FORMAT_RGB565) 
      ? 2 
      : 4;
...
static bool d3d8_init_chain(d3d8_video_t *d3d, const video_info_t *video_info)
{
   ...
   link_info.pass  = NULL;
   link_info.tex_w = video_info->input_scale * KINGSN_SCALE_BASE;
   link_info.tex_h = video_info->input_scale * KINGSN_SCALE_BASE;
   ...
   {
      KINGSN_ERR("[D3D8]: Failed to init render chain.\n");
      return false;
   }

   KINGSN_LOG("[D3D8]: Renderchain driver: %s\n", "d3d8");
...
static bool d3d8_init_base(void *data, const video_info_t *info)
{
   ...
   if (!g_pD3D8)
   {
      KINGSN_ERR("[D3D8]: Failed to create D3D interface.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[D3D8]: Failed to initialize device.\n");
      return false;
   }
...
static bool d3d8_initialize(d3d8_video_t *d3d, const video_info_t *info)
{
   ...
   else if (d3d->needs_restore)
   {
      ...
      if (!d3d8_reset(d3d->dev, &d3dpp))
      {
         ...
         ret = d3d8_init_base(d3d, info);
         if (ret)
            KINGSN_LOG("[D3D8]: Recovered from dead state.\n");
      }
   ...
   {
      KINGSN_ERR("[D3D8]: Failed to initialize render chain.\n");
      return false;
   }
      ...
      void **input_data)
{
   ...
   KINGSN_LOG("[D3D8]: Monitor size: %dx%d.\n",
      ...
      input_driver_t **input, void **input_data)
{
   ...
   {
      KINGSN_ERR("[D3D8]: Failed to init D3D.\n");
      free(d3d);
      return NULL;
   }
      ...
      const void *image_data, unsigned num_images)
{
   ...
   for (i = 0; i < num_images; i++)
   {
      ...
      if (!overlay->tex)
      {
         KINGSN_ERR("[D3D8]: Failed to create overlay texture\n");
         return false;
      }
      ...
      const char *msg, video_frame_info_t *video_info)
{
   ...
   if (d3d->needs_restore)
   {
      ...
      if (!d3d8_restore(d3d))
      {
         KINGSN_ERR("[D3D8]: Failed to restore.\n");
         return false;
      }
   }
   ...
   {
      KINGSN_ERR("[D3D8]: Failed to render scene.\n");
      return false;
   }
      ...
      float alpha)
{
            ...
            d3d->menu->tex_h != height)
   {
      ...
      if (!d3d->menu->tex)
      {
         KINGSN_ERR("[D3D8]: Failed to create menu texture.\n");
         return;
      }
      ...
      uintptr_t *id)
{
   ...
   if (!tex)
   {
      KINGSN_ERR("[D3D8]: Failed to create texture\n");
      return;
   }
