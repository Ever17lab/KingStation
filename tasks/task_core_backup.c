...
static void task_core_backup_handler(ks_task_t *task);

static void task_core_restore_handler(ks_task_t *task);

static bool task_core_backup_finder(ks_task_t *task, void *user_data)
{
...
static void task_core_backup_handler(ks_task_t *task)
{
   ...
   switch (backup_handle->status)
   {
      case CORE_BACKUP_BEGIN:
         ...
         if (!backup_handle->core_file)
         {
            KINGSN_ERR("[core backup] Failed to open core file: %s\n",
                  backup_handle->core_path);
         ...
         {
            KINGSN_ERR("[core backup] Core file is empty/invalid: %s\n",
                  backup_handle->core_path);
            backup_handle->status = CORE_BACKUP_END;
            break;
         }
      ...
      case CORE_BACKUP_CHECK_CRC:
         ...
         if (backup_handle->core_crc == 0)
         {
            ...
            {
               KINGSN_ERR("[core backup] Failed to determine CRC of core file: %s\n",
                     backup_handle->core_path);
               backup_handle->status = CORE_BACKUP_END;
               break;
            }
         ...
         if (backup_handle->backup_list)
         {
            ...
            if (core_backup_list_get_crc(
                     ...
                     &entry))
            {
               KINGSN_LOG("[core backup] Current version of core is already backed up: %s\n",
                     entry->backup_path);
      ...
      case CORE_BACKUP_PRE_ITERATE:
         {
            ...
            {
               KINGSN_ERR("[core backup] Failed to generate backup path for core file: %s\n",
                     backup_handle->core_path);
               backup_handle->status = CORE_BACKUP_END;
               break;
            }
            ...
            backup_handle->backup_file = intfstream_open_rzip_file(
                  backup_handle->backup_path, KS_VFS_FILE_ACCESS_WRITE);
#else
            ...
            if (!backup_handle->backup_file)
            {
               KINGSN_ERR("[core backup] Failed to open core backup file: %s\n",
      ...
      case CORE_BACKUP_ITERATE:
         {
            ...
            if (data_read < 0)
            {
               KINGSN_ERR("[core backup] Failed to read from core file: %s\n",
                     backup_handle->core_path);
               backup_handle->status = CORE_BACKUP_END;
               break;
            }
            ...
            {
               KINGSN_ERR("[core backup] Failed to write to core backup file: %s\n",
                     backup_handle->backup_path);
               backup_handle->status = CORE_BACKUP_END;
               break;
            }
...
static void task_core_restore_handler(ks_task_t *task)
{
   ...
   switch (backup_handle->status)
   {
      case CORE_RESTORE_GET_CORE_CRC:
         {
            ...
            if (path_is_valid(backup_handle->core_path))
            {
               ...
               backup_handle->core_file = intfstream_open_file(
                     backup_handle->core_path, KS_VFS_FILE_ACCESS_READ,
                     KS_VFS_FILE_ACCESS_HINT_NONE);

               if (!backup_handle->core_file)
               {
                  KINGSN_ERR("[core restore] Failed to open core file: %s\n",
                        backup_handle->core_path);
               ...
               if (!intfstream_get_crc(backup_handle->core_file,
                     &backup_handle->core_crc))
               {
                  KINGSN_ERR("[core restore] Failed to determine CRC of core file: %s\n",
                        backup_handle->core_path);
                  backup_handle->status = CORE_RESTORE_END;
                  break;
               }
      ...
      case CORE_RESTORE_GET_BACKUP_CRC:
         ...
         {
            KINGSN_ERR("[core restore] Failed to determine CRC of core backup file: %s\n",
                  backup_handle->backup_path);
            backup_handle->status = CORE_RESTORE_END;
            break;
         }
      ...
      case CORE_RESTORE_CHECK_CRC:
         ...
         {
            KINGSN_LOG("[core restore] Selected backup core file is already installed: %s\n",
                  backup_handle->backup_path);
      ...
      case CORE_RESTORE_PRE_ITERATE:
         {
...
#if defined(HAVE_ZLIB)
            backup_handle->backup_file = intfstream_open_rzip_file(
                  backup_handle->backup_path, KS_VFS_FILE_ACCESS_READ);
#else
            backup_handle->backup_file = intfstream_open_file(
                  backup_handle->backup_path, KS_VFS_FILE_ACCESS_READ,
                  KS_VFS_FILE_ACCESS_HINT_NONE);
            ...
            if (!backup_handle->backup_file)
            {
               KINGSN_ERR("[core restore] Failed to open core backup file: %s\n",
                     backup_handle->backup_path);
               backup_handle->status = CORE_RESTORE_END;
               break;
            }
            ...
            {
               KINGSN_ERR("[core restore] Core backup file is empty/invalid: %s\n",
                     backup_handle->backup_path);
               backup_handle->status = CORE_RESTORE_END;
               break;
            }
            ...
            if (play_feature_delivery_enabled())
            {
               ...
               {
                  KINGSN_ERR("[core restore] Failed to delete existing play feature delivery core: %s\n",
                        backup_handle->core_path);
                  backup_handle->status = CORE_RESTORE_END;
                  break;
               }
            ...
            backup_handle->core_file = intfstream_open_file(
                  backup_handle->core_path, KS_VFS_FILE_ACCESS_WRITE,
                  KS_VFS_FILE_ACCESS_HINT_NONE);
      ...
      case CORE_RESTORE_ITERATE:
         {
            ...
            if (data_read < 0)
            {
               KINGSN_ERR("[core restore] Failed to read from core backup file: %s\n",
                     backup_handle->backup_path);
               backup_handle->status = CORE_RESTORE_END;
               break;
            }
            ...
            if (data_written != data_read)
            {
               KINGSN_ERR("[core restore] Failed to write to core file: %s\n",
                     backup_handle->core_path);
               backup_handle->status = CORE_RESTORE_END;
               break;
            }
...
bool task_push_core_restore(const char *backup_path, const char *dir_libks,
      bool *core_loaded)
{
   task_finder_data_t find_data;
   core_info_ctx_find_t core_info;
   ...
   if (string_is_empty(backup_path) ||
       ...
       string_is_empty(dir_libks) ||
       !core_loaded)
      goto error;

   /* Ensure core directory is valid */
   if (!path_is_directory(dir_libks))
   {
      if (!path_mkdir(dir_libks))
      {
         KINGSN_ERR("[core restore] Failed to create core directory: %s\n", dir_libks);
         goto error;
      }
   }
   ...
   backup_type = core_backup_get_core_path(
         backup_path, dir_libks, core_path, sizeof(core_path));
   ...
   if (backup_type == CORE_BACKUP_TYPE_INVALID)
   {
      ...
      KINGSN_ERR("[core restore] Invalid core file selected: %s\n", backup_path);
   ...
   if (core_info_get_core_lock(core_path, true))
   {
      ...
      KINGSN_ERR("[core restore] Restoration disabled - core is locked: %s\n", core_path);
      runloop_msg_queue_push(msg, 1, 100, true,
            NULL, MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
      goto error;
   }
