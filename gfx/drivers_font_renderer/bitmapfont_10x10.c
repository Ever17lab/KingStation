...
bitmapfont_lut_t *bitmapfont_10x10_load(unsigned language)
{
   ...
   {
      KINGSN_WARN("[bitmap 10x10] No font file found for specified language: %u\n", language);
      goto error;
   }
   ...
   {
      KINGSN_WARN("[bitmap 10x10] Failed to read font file: %s\n", font_path);
      goto error;
   }
   ...
   if (len != font_size)
   {
      KINGSN_WARN("[bitmap 10x10] Font file has invalid size: %s\n", font_path);
      goto error;
   }
