HAVE_LIBKS=             # Libks library used
HAVE_ASSETS_DIR=           # Assets install directory
HAVE_NVDA=yes              # NVDA support
HAVE_PATCH=no             # Softpatching support (BPS/IPS/UPS)
...
HAVE_LIBKSDB=yes        # Libksdb support
...
HAVE_RUNAHEAD=no          # Runahead support
...
HAVE_SSL=no              # SSL support
C89_SSL=no
HAVE_SYSTEMMBEDTLS=no    # Use system mbedTLS
C89_SYSTEMMBEDTLS=no
HAVE_BUILTINMBEDTLS=no   # Use builtin mbedTLS
C89_BUILTINMBEDTLS=no
HAVE_BUILTINBEARSSL=no   # Use builtin BearSSL
C89_BUILTINBEARSSL=no
CXX_BUILTINBEARSSL=no
HAVE_OVERLAY=yes           # Overlay support
HAVE_VIDEO_LAYOUT=yes      # Layout support
HAVE_DYNAMIC=yes           # Dynamic loading of libks library
...
HAVE_NETWORKING=no       # Networking features (recommended)
HAVE_NETWORKGAMEPAD=auto   # Networked game pad (plus baked-in core)
C89_NETWORKGAMEPAD=no
HAVE_NETPLAYDISCOVERY=no  # Add netplay discovery (room creation, etc.)
...
HAVE_ZLIB=yes             # zlib support (ZIP extract, PNG decoding/encoding)
...
CXX_CHD=no
HAVE_7ZIP=no              # Compile in 7z support
...
HAVE_SCREENSHOTS=no       # Screenshot support
...
HAVE_GIT_VERSION=no       # Git version support
