      ...
      unsigned *new_rate)
{
   ...
   {
      KINGSN_ERR("[ALSA QSA]: Audio open error: %s\n",
            snd_strerror(err));
      goto error;
   }
   ...
   {
      KINGSN_ERR("[ALSA QSA]: Can't set blocking mode: %s\n",
            snd_strerror(err));
      goto error;
   }
   ...
   {
      KINGSN_ERR("[ALSA QSA]: snd_pcm_channel_info failed: %s\n",
            snd_strerror(err));
      goto error;
   }
   ...
   KINGSN_LOG("Fragment size: %d\n", params.buf.block.frag_size);
   KINGSN_LOG("Min Fragment size: %d\n", params.buf.block.frags_min);
   KINGSN_LOG("Max Fragment size: %d\n", params.buf.block.frags_max);

   if ((err = snd_pcm_channel_params(alsa->pcm, &params)) < 0)
   {
      KINGSN_ERR("[ALSA QSA]: Channel Parameter Error: %s\n",
            snd_strerror(err));
      goto error;
   }
   ...
   {
      KINGSN_ERR("[ALSA QSA]: Channel Parameter Read Back Error: %s\n",
            snd_strerror(err));
      goto error;
   }
   ...
   KINGSN_LOG("[ALSA QSA]: buffer size: %u bytes\n", alsa->buf_size);
   ...
   {
      KINGSN_ERR("[ALSA QSA]: Channel Prepare Error: %s\n",
            snd_strerror(err));
      goto error;
   }
   ...
   alsa->can_pause = true;
   KINGSN_LOG("[ALSA QSA]: Can pause: %s.\n",
...
static int check_pcm_status(void *data, int channel_type)
{
   ...
   if ((ret = snd_pcm_channel_status(alsa->pcm, &status)) == 0)
   {
      ...
      {
         KINGSN_ERR("check_pcm_status got SND_PCM_STATUS_UNSECURE, aborting playback\n");
         ret = -EPROTO;
      }
      else if (status.status == SND_PCM_STATUS_UNDERRUN)
      {
         KINGSN_LOG("check_pcm_status: SNDP_CM_STATUS_UNDERRUN.\n");
         if ((ret = snd_pcm_channel_prepare(alsa->pcm, channel_type)) < 0)
         {
            KINGSN_ERR("Invalid state detected for underrun on snd_pcm_channel_prepare: %s\n",
                  snd_strerror(ret));
            ret = -EPROTO;
         }
      }
      ...
      {
         KINGSN_LOG("check_pcm_status: SNDP_CM_STATUS_OVERRUN.\n");
         if ((ret = snd_pcm_channel_prepare(alsa->pcm, channel_type)) < 0)
         {
            KINGSN_ERR("Invalid state detected for overrun on snd_pcm_channel_prepare: %s\n",
                  snd_strerror(ret));
            ret = -EPROTO;
         }
      }
      ...
      {
         KINGSN_LOG("check_pcm_status: SNDP_CM_STATUS_CHANGE.\n");
         if ((ret = snd_pcm_channel_prepare(alsa->pcm, channel_type)) < 0)
         {
            KINGSN_ERR("Invalid state detected for change on snd_pcm_channel_prepare: %s\n",
                  snd_strerror(ret));
            ret = -EPROTO;
         }
      }
   ...
   {
      KINGSN_ERR("check_pcm_status failed: %s\n", snd_strerror(ret));
      if (ret == -ESRCH)
         ret = -EBADF;
   }
...
static bool alsa_qsa_start(void *data, bool is_shutdown)
{
   ...
   if (alsa->can_pause && alsa->is_paused)
   {
      ...
      if (ret < 0)
      {
         KINGSN_ERR("[ALSA QSA]: Failed to unpause: %s.\n",
               snd_strerror(ret));
         return false;
      }
...
static void alsa_qsa_set_nonblock_state(void *data, bool state)
{
   ...
   {
      KINGSN_ERR("Can't set blocking mode to %d: %s\n", state,
            snd_strerror(err));
      return;
   }
