      ...
      const char *prog, bool path_is_file)
{
   ...
   KINGSN_LOG("[D3D9 Cg]: Vertex profile: %s\n",
         cgGetProfileString(vertex_profile));
   KINGSN_LOG("[D3D9 Cg]: Fragment profile: %s\n",
      ...
      struct shader_pass *pass)
{
   ...
   if (param)
   {
      ...
      KINGSN_LOG("[D3D9 Cg]: FVF POSITION semantic found.\n");
   }
   ...
   if (param)
   {
      ...
      KINGSN_LOG("[D3D9 Cg]: FVF TEXCOORD0 semantic found.\n");
      index           = cgGetParameterResourceIndex(param);
   ...
   if (param)
   {
      ...
      KINGSN_LOG("[D3D9 Cg]: FVF TEXCOORD1 semantic found.\n");
      index           = cgGetParameterResourceIndex(param);
   ...
   if (param)
   {
      ...
      stream_taken[3] = true;
      KINGSN_LOG("[D3D9 Cg]: FVF COLOR0 semantic found.\n");
      index           = cgGetParameterResourceIndex(param);
      decl[index]     = color;
      indices[index]  = true;
   }
...
static void d3d9_cg_deinit_progs(cg_renderchain_t *chain)
{
   ...
   KINGSN_LOG("[D3D9 Cg]: Destroying programs.\n");
...
static void d3d9_cg_deinit_context_state(cg_renderchain_t *chain)
{
   if (chain->cgCtx)
   {
      KINGSN_LOG("[D3D9 Cg]: Destroying context.\n");
      ...
      const struct LinkInfo *info, unsigned _fmt)
{
   ...
   unsigned fmt = (_fmt == KS_PIXEL_FORMAT_RGB565) ?
      ...
      bool rgb32)
{
   ...
   {
      KINGSN_ERR("[D3D9 Cg]: Failed to initialize shader subsystem.\n");
      return false;
   }
   ...
   chain->chain.pixel_size     = (fmt == KS_PIXEL_FORMAT_RGB565) ? 2 : 4;
