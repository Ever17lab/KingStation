      ...
      case KS_DEVICE_KEYBOARD:
         if (id < KINGSN_BIND_LIST_END)
            return (dos_key_state[DOS_KEYBOARD_PORT]
                  [kingsn_keysym_lut[binds[id].key]]);
         break;
   }
...
{
   input_keymaps_init_keyboard_lut(kingsn_key_map_dos);
...
{
   return UINT64_C(1) << KS_DEVICE_JOYPAD;
}
