      ...
      void *args, void *params_data)
{
   ...
   fill_pathname_join(g_defaults.dirs[DEFAULT_DIR_CHEATS], bundle_path_buf, "Contents/Resources/cht", sizeof(g_defaults.dirs[DEFAULT_DIR_CHEATS]));
...
#if TARGET_OS_IOS
    if (major > 8)
       strcpy_literal(g_defaults.path_buildbot_server_url, "http://buildbot.libks.com/nightly/apple/ios9/latest/");
#endif
    ...
    if (path_is_valid(assets_zip_path))
    {
       ...
       KINGSN_LOG("Assets ZIP found at [%s], setting up bundle assets extraction...\n", assets_zip_path);
       KINGSN_LOG("Extraction dir will be: %s\n", home_dir_buf);
   ...
   if (access(bundle_path_buf, 0755) != 0)
      KINGSN_ERR("Failed to create or access base directory: %s\n", bundle_path_buf);
   else
   {
      path_mkdir(g_defaults.dirs[DEFAULT_DIR_SYSTEM]);

      if (access(g_defaults.dirs[DEFAULT_DIR_SYSTEM], 0755) != 0)
         KINGSN_ERR("Failed to create or access system directory: %s.\n", g_defaults.dirs[DEFAULT_DIR_SYSTEM]);
   }
      ...
      const char* speak_text, int priority)
{
   ...
   {
      /* error */
      KINGSN_LOG("ERROR: could not fork for say command.\n");
   }
   else if (pid > 0)
   {
      /* parent process */
