      ...
      case WM_INITDIALOG:
         {
            ...
            core_info_list_get_supported_cores(core_info_list,
                  path_get(KINGSN_PATH_CONTENT), &core_info, &list_size);
                  ...
                  case LBN_SELCHANGE:
                     {
                        ...
                        path_set(KINGSN_PATH_CORE, info->path);
                     }
                     break;
               }
               return TRUE;
...
bool win32_load_content_from_gui(const char *szFilename)
{
   ...
   path_set(KINGSN_PATH_CONTENT, szFilename);

   if (!path_is_empty(KINGSN_PATH_CONTENT))
   {
      unsigned i;
      core_info_t *current_core = NULL;
      core_info_get_current_core(&current_core);

      /*we already have path for libks core */
      for (i = 0; i < list_size; i++)
      {
         const core_info_t *info = (const core_info_t*)&core_info[i];

         if (string_is_equal(path_get(KINGSN_PATH_CORE), info->path))
         {
   ...
   switch (mode)
   {
      case ID_M_LOAD_CORE:
         {
            ...
            char    *title_cp       = NULL;
            size_t converted        = 0;
            const char *extensions  = "Libks core (.dll)\0*.dll\0All Files\0*.*\0\0";
            const char *title       = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_CORE_LIST);
            const char *initial_dir = settings->paths.directory_libks;
            ...
            if (title_cp)
               free(title_cp);
            path_set(KINGSN_PATH_CORE, win32_file);
      ...
      default:
         if (mode >= ID_M_WINDOW_SCALE_1X && mode <= ID_M_WINDOW_SCALE_10X)
         {
            ...
            kingsn_ctl(KINGSN_CTL_SET_WINDOWED_SCALE, &idx);
         ...
         *quit = true;
         {
             ...
             */
            input_keyboard_event(true, KSK_UNKNOWN, wparam, mod,
                  KS_DEVICE_KEYBOARD);
         }
         return TRUE;
      case WM_CLOSE:
      ...
      unsigned height, bool fullscreen)
{
   ...
   if (!notification_handler)
      KINGSN_ERR("Error registering for notifications\n");
#endif

   video_driver_display_type_set(KINGSN_DISPLAY_WIN32);
      ...
      unsigned refresh, char *dev_name)
{
   ...
   KINGSN_LOG("Setting fullscreen to %ux%u @ %uHz on device %s.\n",
...
static unsigned int menu_id_to_meta_key(unsigned int menu_id)
{
   switch (menu_id)
   {
      case ID_M_RESET:
         return KINGSN_RESET;
      case ID_M_QUIT:
         return KINGSN_QUIT_KEY;
      case ID_M_MENU_TOGGLE:
         return KINGSN_MENU_TOGGLE;
      case ID_M_PAUSE_TOGGLE:
         return KINGSN_PAUSE_TOGGLE;
      case ID_M_LOAD_STATE:
         return KINGSN_LOAD_STATE_KEY;
      case ID_M_SAVE_STATE:
         return KINGSN_SAVE_STATE_KEY;
      case ID_M_DISK_CYCLE:
         return KINGSN_DISK_EJECT_TOGGLE;
      case ID_M_DISK_NEXT:
         return KINGSN_DISK_NEXT;
      case ID_M_DISK_PREV:
         return KINGSN_DISK_PREV;
      case ID_M_FULL_SCREEN:
         return KINGSN_FULLSCREEN_TOGGLE_KEY;
      case ID_M_MOUSE_GRAB:
         return KINGSN_GRAB_MOUSE_TOGGLE;
      case ID_M_TAKE_SCREENSHOT:
         return KINGSN_SCREENSHOT;
      case ID_M_MUTE_TOGGLE:
         return KINGSN_MUTE;
      default:
         break;
...
static const char *meta_key_to_name(unsigned int meta_key)
{
   ...
   {
      int i = 0;
      const struct ks_keybind* key = &input_config_binds[0][meta_key];
      ...
      bool fullscreen)
{
         ...
         && (res = GetMessage(&msg, main_window.hwnd, 0, 0)) != 0)
   {
      if (res == -1)
      {
         KINGSN_ERR("GetMessage error code %d\n", GetLastError());
         break;
      }
      ...
      bool fullscreen, const char *class_name)
{
   ...
   SetThreadUILanguage(win32_get_langid_from_ks_lang(
            (enum ks_language)
