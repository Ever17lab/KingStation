...
static void switch_joypad_poll(void)
{
   ...
   for (i = 0; i < DEFAULT_MAX_PADS; i++)
   {
      ...
      analog_state[i][KS_DEVICE_INDEX_ANALOG_LEFT]
         [KS_DEVICE_ID_ANALOG_X] = joy_position_left.dx;
      analog_state[i][KS_DEVICE_INDEX_ANALOG_LEFT]
         [KS_DEVICE_ID_ANALOG_Y] = -joy_position_left.dy;
      analog_state[i][KS_DEVICE_INDEX_ANALOG_RIGHT]
         [KS_DEVICE_ID_ANALOG_X] = joy_position_right.dx;
      analog_state[i][KS_DEVICE_INDEX_ANALOG_RIGHT]
         [KS_DEVICE_ID_ANALOG_Y] = -joy_position_right.dy;
   }
}
#else
static void switch_joypad_poll(void)
{
   ...
   analog_state[0][KS_DEVICE_INDEX_ANALOG_LEFT]
      [KS_DEVICE_ID_ANALOG_X]     = lsx;
   analog_state[0][KS_DEVICE_INDEX_ANALOG_LEFT]
      [KS_DEVICE_ID_ANALOG_Y]     = -lsy;
   analog_state[0][KS_DEVICE_INDEX_ANALOG_RIGHT]
      [KS_DEVICE_ID_ANALOG_X]     = rsx;
   analog_state[0][KS_DEVICE_INDEX_ANALOG_RIGHT]
      [KS_DEVICE_ID_ANALOG_Y]     = -rsy;
}
#endif

#ifdef HAVE_LIBNX
bool switch_joypad_set_rumble(unsigned pad,
      enum ks_rumble_effect type, uint16_t strength)
{
   u32* handle;
   float amp;
