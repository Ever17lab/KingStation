...
#define VITA_NUM_SCANCODES 115 /* size of kingsn_key_map_vita */
      ...
      unsigned id)
{
   ...
   switch (device)
   {
      ...
      case KS_DEVICE_KEYBOARD:
         return ((id < KSK_LAST) && 
               psp->keyboard_state[kingsn_keysym_lut[(enum ks_key)id]]);
...
#ifdef VITA
   caps |= (1 << KS_DEVICE_KEYBOARD) | (1 << KS_DEVICE_MOUSE);
#endif
...
static void *vita_input_initialize(const char *joypad_driver)
{
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_vita);
