      ...
      size_t offset, bool push_constant)
{
   ...
   if (active)
   {
      ...
      {
         KINGSN_ERR("[slang]: Vertex and fragment have"
      ...
      bool push_constant)
{
   ...
   if (active)
   {
      if (active_offset != offset)
      {
         KINGSN_ERR("[slang]: Vertex and fragment have different"
      ...
      bool push_constant)
{
   ...
   for (i = 0; i < ranges.size(); i++)
   {
      ...
      {
         KINGSN_ERR("[slang]: Non causal filter chain detected. "
      ...
      if (sem != SLANG_INVALID_SEMANTIC)
      {
         ...
         {
            KINGSN_ERR("[slang]: Underlying type of semantic is invalid.\n");
            return false;
         }
      ...
      else if (tex_sem != SLANG_INVALID_TEXTURE_SEMANTIC)
      {
         ...
         {
            KINGSN_ERR("[slang]: Underlying type of texture"
                  " semantic is invalid.\n");
            return false;
         }
      ...
      {
         KINGSN_ERR("[slang]: Unknown semantic found.\n");
         return false;
      }
   }
   return true;
      ...
      slang_reflection *reflection)
{
   ...
   {
      KINGSN_ERR("[slang]: Invalid resource type detected.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Vertex must have two attributes.\n");
      return false;
   }

   if (fragment.stage_outputs.size() != 1)
   {
      KINGSN_ERR("[slang]: Multiple render targets not supported.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Render target must use location = 0.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: The two vertex attributes do not"
   ...
   {
      KINGSN_ERR("[slang]: Vertex must use zero or one uniform buffer.\n");
      return false;
   }

   if (fragment.uniform_buffers.size() > 1)
   {
      KINGSN_ERR("[slang]: Fragment must use zero or one uniform buffer.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Vertex must use zero or one push constant buffers.\n");
      return false;
   }

   if (fragment.push_constant_buffers.size() > 1)
   {
      KINGSN_ERR("[slang]: Fragment must use zero or one push cosntant buffer.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Resources must use descriptor set #0.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Vertex and fragment uniform buffer must have same binding.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Binding %u is out of range.\n", ubo_binding);
      return false;
   }
   ...
   {
      KINGSN_ERR("[slang]: Exceeded maximum size of 128 bytes"
   ...
   for (i = 0; i < fragment.sampled_images.size(); i++)
   {
      ...
      {
         KINGSN_ERR("[slang]: Binding %u is already in use.\n", binding);
         return false;
      }
      ...
      {
         KINGSN_ERR("[slang]: Texture name '%s' not found in semantic map, "
...
#ifdef DEBUG
   KINGSN_LOG("[slang]: Reflection\n");
   KINGSN_LOG("[slang]:   Textures:\n");
   ...
   for (i = 0; i < SLANG_NUM_SEMANTICS; i++)
   {
      ...
      {
         KINGSN_LOG("[slang]:      %s (PushOffset: %u)\n",
               semantic_uniform_names[i],
   ...
   for (i = 0; i < SLANG_NUM_TEXTURE_SEMANTICS; i++)
   {
      ...
      for (auto &sem : reflection->semantic_textures[i])
      {
         ...
         {
            KINGSN_LOG("[slang]:      %s (#%u) (PushOffset: %u)\n",
                  texture_semantic_uniform_names[i],
                  index,
   ...
   for (i = 0; i < reflection->semantic_float_parameters.size(); i++)
   {
      ...
      if (param->uniform)
         KINGSN_LOG("[slang]:     #%u (Offset: %u)\n", i,
               (unsigned int)param->ubo_offset);
      if (param->push_constant)
         KINGSN_LOG("[slang]:     #%u (PushOffset: %u)\n", i,
      ...
      slang_reflection *reflection)
{
   try
   {
      ...
      if (!slang_reflect(vertex_compiler, fragment_compiler,
               ...
               reflection))
      {
         KINGSN_ERR("[slang]: Failed to reflect SPIR-V."
   ...
   {
      KINGSN_ERR("[slang]: SPIRV-Cross threw exception: %s.\n", e.what());
      return false;
   }
}
