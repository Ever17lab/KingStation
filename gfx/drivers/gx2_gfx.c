      ...
      input_driver_t **input, void **input_data)
{
   ...
   *input_data        = NULL;
   ...
   GX2SetPixelSampler(&wiiu->sampler_linear[KINGSN_WRAP_DEFAULT],
...

   {
      ...
      shader_preset               = KingStation_get_shader_preset();
...
static void gx2_render_overlay(void *data)
{
   ...
   for (i = 0; i < gx2->overlays; i++)
   {
      ...
      GX2SetPixelSampler(&gx2->sampler_linear[KINGSN_WRAP_EDGE], sprite_shader.ps.samplerVars[0].location);
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (texture)
   {
      ...
      GX2SetPixelSampler(wiiu->smooth ? &wiiu->sampler_linear[KINGSN_WRAP_DEFAULT] :
                         &wiiu->sampler_nearest[KINGSN_WRAP_DEFAULT],
                         frame_shader.ps.samplerVars[0].location);
   }
   ...
   GX2SetPixelSampler(&wiiu->sampler_linear[KINGSN_WRAP_EDGE],
                      sprite_shader.ps.samplerVars[0].location);
      ...
      enum kingsn_shader_type type, const char *path)
{
   ...
   {
      KINGSN_WARN("[GX2] Only Slang shaders are supported. Falling back to stock.\n");
      return false;
   }
