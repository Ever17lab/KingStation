...
#define WINRAW_KEYBOARD_PRESSED(wr, key) (wr->keyboard.keys[kingsn_keysym_lut[(enum ks_key)(key)]])
      ...
      winraw_mouse_t *mouse, RAWMOUSE *state)
{
   ...
   else if (state->lLastX || state->lLastY)
   {
      ...
      if (!GetCursorPos(&crs_pos))
      {
         KINGSN_WARN("[WINRAW]: GetCursorPos failed with error %lu.\n", GetLastError());
      }
      else if (!ScreenToClient((HWND)video_driver_window_get(), &crs_pos))
      {
         KINGSN_WARN("[WINRAW]: ScreenToClient failed with error %lu.\n", GetLastError());
      }
#else
      if (!GetCursorPos(&crs_pos)) { }
      else if (!ScreenToClient((HWND)video_driver_window_get(), &crs_pos)) { }
   ...
   switch (ri->data.keyboard.MakeCode)
   {
      ...
      case 29:
         ...
         input_keyboard_event(flags & RI_KEY_BREAK ? 0 : 1, keycode,
               0, KSKMOD_CTRL, KS_DEVICE_KEYBOARD);
         break;
      ...
      case 42:
         ...
         input_keyboard_event(flags & RI_KEY_BREAK ? 0 : 1, keycode,
               0, KSKMOD_SHIFT, KS_DEVICE_KEYBOARD);
         break;
      ...
      case 56:
         ...
         input_keyboard_event(flags & RI_KEY_BREAK ? 0 : 1, keycode,
               0, KSKMOD_ALT, KS_DEVICE_KEYBOARD);
         break;
   }
}
...
static void *winraw_init(const char *joypad_driver)
{
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_winraw);
      ...
      unsigned id)
{
   ...
   bool process_mouse    = 
      ...
      || (device == KS_DEVICE_MOUSE)
      || (device == KINGSN_DEVICE_MOUSE_SCREEN)
      || (device == KS_DEVICE_LIGHTGUN);
   ...
   switch (device)
   {
      ...
      case KS_DEVICE_KEYBOARD:
         return (id < KSK_LAST) && WINRAW_KEYBOARD_PRESSED(wr, id);
      case KS_DEVICE_MOUSE:
      case KINGSN_DEVICE_MOUSE_SCREEN:
         if (mouse)
         {
            bool abs = (device == KINGSN_DEVICE_MOUSE_SCREEN);
            switch (id)
            {
