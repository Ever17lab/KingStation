...
#define IMAGE_CORE_PREFIX(s) libks_imageviewer_##s
#else
#define IMAGE_CORE_PREFIX(s) s
#endif

static ks_log_printf_t IMAGE_CORE_PREFIX(log_cb);
static ks_video_refresh_t IMAGE_CORE_PREFIX(video_cb);
static ks_input_poll_t IMAGE_CORE_PREFIX(input_poll_cb);
static ks_input_state_t IMAGE_CORE_PREFIX(input_state_cb);
static ks_audio_sample_batch_t IMAGE_CORE_PREFIX(audio_batch_cb);
static ks_environment_t IMAGE_CORE_PREFIX(environ_cb);
...
void IMAGE_CORE_PREFIX(ks_get_system_info)(struct ks_system_info *info)
{
...
void IMAGE_CORE_PREFIX(ks_get_system_av_info)(struct ks_system_av_info *info)
{
...
void IMAGE_CORE_PREFIX(ks_init)(void)
{
   struct ks_log_callback log;

   if (IMAGE_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_LOG_INTERFACE, &log))
...
void IMAGE_CORE_PREFIX(ks_deinit)(void)
{
   imageviewer_free_image();
   imageviewer_reset();
}

void IMAGE_CORE_PREFIX(ks_set_environment)(ks_environment_t cb)
{
   static const struct ks_variable vars[] = {
      { NULL, NULL },
   };
#ifndef KINGSN_INTERNAL
   struct ks_vfs_interface_info vfs_iface_info = { 1, NULL };
#endif
...
#ifndef KINGSN_INTERNAL
   /* I don't trust filestream_vfs_init to work inside kingsn */
   if (environ_cb(KS_ENVIRONMENT_GET_VFS_INTERFACE, &vfs_iface_info))
      filestream_vfs_init(&vfs_iface_info);
#endif
}

void IMAGE_CORE_PREFIX(ks_set_video_refresh)(ks_video_refresh_t cb)
{
   IMAGE_CORE_PREFIX(video_cb) = cb;
}

void IMAGE_CORE_PREFIX(ks_set_audio_sample)(ks_audio_sample_t unused)
{
}

void IMAGE_CORE_PREFIX(ks_set_audio_sample_batch)(ks_audio_sample_batch_t cb)
{
   IMAGE_CORE_PREFIX(audio_batch_cb) = cb;
}

void IMAGE_CORE_PREFIX(ks_set_input_poll)(ks_input_poll_t cb)
{
   IMAGE_CORE_PREFIX(input_poll_cb) = cb;
}

void IMAGE_CORE_PREFIX(ks_set_input_state)(ks_input_state_t cb)
{
   IMAGE_CORE_PREFIX(input_state_cb) = cb;
}

void IMAGE_CORE_PREFIX(ks_set_controller_port_device)(unsigned a, unsigned b)
{
}

void IMAGE_CORE_PREFIX(ks_reset)(void)
{
   image_uploaded = false;
}

size_t IMAGE_CORE_PREFIX(ks_serialize_size)(void)
{
   return 0;
}

bool IMAGE_CORE_PREFIX(ks_serialize)(void *data, size_t size)
{
   (void)data;
   (void)size;
   return false;
...
bool IMAGE_CORE_PREFIX(ks_unserialize)(const void *data, size_t size)
{
   (void)data;
   (void)size;
   return false;
...
void IMAGE_CORE_PREFIX(ks_cheat_reset)(void)
{
}

void IMAGE_CORE_PREFIX(ks_cheat_set)(unsigned a, bool b, const char * c)
{
}
...
#ifdef STB_IMAGE_IMPLEMENTATION
   f = filestream_open(path, KS_VFS_FILE_ACCESS_READ, KS_VFS_FILE_ACCESS_HINT_NONE);
...
bool IMAGE_CORE_PREFIX(ks_load_game)(const struct ks_game_info *info)
{
   ...
   if (!IMAGE_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt))
   {
      if (IMAGE_CORE_PREFIX(log_cb))
         IMAGE_CORE_PREFIX(log_cb)(KS_LOG_INFO, "XRGB8888 is not supported.\n");
      return false;
   }
...
bool IMAGE_CORE_PREFIX(ks_load_game_special)(unsigned a, const struct ks_game_info *b, size_t c)
{
   return false;
}

void IMAGE_CORE_PREFIX(ks_unload_game)(void)
{
   imageviewer_free_image();
   image_width  = 0;
   image_height = 0;
...
unsigned IMAGE_CORE_PREFIX(ks_get_region)(void)
{
   return KS_REGION_NTSC;
}

void *IMAGE_CORE_PREFIX(ks_get_memory_data)(unsigned id)
{
   return NULL;
}

size_t IMAGE_CORE_PREFIX(ks_get_memory_size)(unsigned id)
{
   return 0;
}

void IMAGE_CORE_PREFIX(ks_run)(void)
{
   ...
   for (i=0;i<16;i++)
   {
      if (IMAGE_CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD, 0, i))
         realinput |= 1<<i;
   }
   ...
   if (input & (1<<KS_DEVICE_ID_JOYPAD_UP))
   {
   ...
   if (input & (1<<KS_DEVICE_ID_JOYPAD_DOWN))
   {
      if ((image_index - 5) > 0)
   ...
   if (input & (1<<KS_DEVICE_ID_JOYPAD_LEFT))
   {
      if (image_index > 0)
         prev_image = true;
   }
   if (input & (1<<KS_DEVICE_ID_JOYPAD_RIGHT))
   {
      if (image_index < (signed)(image_file_list->size - 1))
         next_image = true;
   }

   if (input & (1<<KS_DEVICE_ID_JOYPAD_Y))
   {
   ...
   if (load_image)
   {
      ...
      {
         IMAGE_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SHUTDOWN, NULL);
      }
   }
   ...
   if (process_new_image)
   {
      ...
      IMAGE_CORE_PREFIX(ks_get_system_av_info)(&info);

      IMAGE_CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_GEOMETRY, &info.geometry);
...
unsigned IMAGE_CORE_PREFIX(ks_api_version)(void)
{
