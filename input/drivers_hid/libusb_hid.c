      ...
      uint8_t* data_buf, size_t size)
{
   ...
   {
      KINGSN_WARN("adapter write buffer is full, cannot write send control\n");
   }
   slock_unlock(adapter->send_control_lock);
}
      ...
      struct libusb_adapter *adapter)
{
   ...
   if (desc_ret != 0)
   {
      KINGSN_ERR("Error %d getting libusb config descriptor\n", desc_ret);
      return;
   }
...
static int add_adapter(void *data, struct libusb_device *dev)
{
   ...
   {
      free(adapter);
      KINGSN_ERR("Allocation of adapter failed.\n");
      return -1;
   }
   ...
   {
      KINGSN_ERR("Error getting device descriptor.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("Could not find HID config for device.\n");
      goto error;
   }
   ...
   if (rc != LIBUSB_SUCCESS)
   {
      KINGSN_ERR("Error opening device 0x%p (VID/PID: %04x:%04x).\n",
...
#if 0
      KINGSN_ERR(" Adapter Manufacturer name: %s\n",
   ...
   {
      KINGSN_ERR("Error creating send control buffer.\n");
      goto error;
   }
   ...
   if (rc != LIBUSB_SUCCESS)
   {
      KINGSN_ERR("Error claiming interface %d .\n", adapter->interface_number);
      goto error;
   }

   KINGSN_LOG("Device 0x%p attached (VID/PID: %04x:%04x).\n",
   ...
   if (!adapter->thread)
   {
      KINGSN_ERR("Error initializing adapter thread.\n");
      goto error;
   }
...
static void libusb_hid_free(const void *data)
{
   ...
   while (adapters.next)
      if (remove_adapter(hid, adapters.next->device) == -1)
         KINGSN_ERR("could not remove device %p\n",
...
static void *libusb_hid_init(void)
{
   ...
   if (hid->can_hotplug)
   {
      ...
      if (ret != LIBUSB_SUCCESS)
      {
          ...
          */
         KINGSN_WARN("[libusb] Failed to create a hotplug callback.\n");
         hid->can_hotplug = 0;
      }
   }
   ...
   if (!hid->poll_thread)
   {
      KINGSN_ERR("Error creating polling thread");
      goto error;
   }
