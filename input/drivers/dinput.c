...
static void *dinput_init(const char *joypad_driver)
{
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_dinput);
      ...
      case KSKMOD_SHIFT:
         {
            ...
            {
               input_keyboard_event(vk_shift_l, KSK_LSHIFT,
            ...
            {
               input_keyboard_event(vk_shift_r, KSK_RSHIFT,
                     0, KSKMOD_SHIFT, KS_DEVICE_KEYBOARD);
      ...
      case KSKMOD_ALT:
         {
            ...
            {
               input_keyboard_event(vk_alt_l, KSK_LALT,
...
static void dinput_poll(void *data)
{
   ...
   if (di->keyboard)
   {
                  ...
                  di->keyboard, sizeof(di->state), di->state)))
      {
         ...
         dinput_keyboard_mods(di, KSKMOD_SHIFT);

      /* Left alt keyup when unfocused, to prevent alt-tab sticky */
      dinput_keyboard_mods(di, KSKMOD_ALT);
   }
   ...
   if (di->mouse)
   {
       ...
       * for KS_DEVICE_POINTER. Just use Win32 APIs. */
      GetCursorPos(&point);
      ...
      unsigned id)
{
   ...
   switch (device)
   {
                  ...
                  for (i = 0; i < KINGSN_FIRST_CUSTOM_BIND; i++)
                  {
                     if (binds[port][i].valid)
                     {
                        if ((binds[port][i].key < KSK_LAST) &&
                              di->state[kingsn_keysym_lut
                              [(enum ks_key)binds[port][i].key]] & 0x80)
                           ret |= (1 << i);
                     }
                  }
               }
            ...
            if (id < KINGSN_BIND_LIST_END)
            {
               if (binds[port][id].valid)
               {
                  if  (binds[port][id].key < KSK_LAST
                        && (di->state[kingsn_keysym_lut
                           [(enum ks_key)binds[port][id].key]] & 0x80)
      ...
      case KS_DEVICE_KEYBOARD:
         return (id < KSK_LAST) && 
            di->state[kingsn_keysym_lut[(enum ks_key)id]] & 0x80;
            ...
            case KS_DEVICE_ID_LIGHTGUN_PAUSE:
               {
                  ...
                  {
                     case KS_DEVICE_ID_LIGHTGUN_TRIGGER:
                        new_id = KINGSN_LIGHTGUN_TRIGGER;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_RELOAD:
                        new_id = KINGSN_LIGHTGUN_RELOAD;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_AUX_A:
                        new_id = KINGSN_LIGHTGUN_AUX_A;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_AUX_B:
                        new_id = KINGSN_LIGHTGUN_AUX_B;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_AUX_C:
                        new_id = KINGSN_LIGHTGUN_AUX_C;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_START:
                        new_id = KINGSN_LIGHTGUN_START;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_SELECT:
                        new_id = KINGSN_LIGHTGUN_SELECT;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_DPAD_UP:
                        new_id = KINGSN_LIGHTGUN_DPAD_UP;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_DPAD_DOWN:
                        new_id = KINGSN_LIGHTGUN_DPAD_DOWN;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_DPAD_LEFT:
                        new_id = KINGSN_LIGHTGUN_DPAD_LEFT;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_DPAD_RIGHT:
                        new_id = KINGSN_LIGHTGUN_DPAD_RIGHT;
                        break;
                     case KS_DEVICE_ID_LIGHTGUN_PAUSE:
                        new_id = KINGSN_LIGHTGUN_START;
                        break;
                  ...
                  if (binds[port][new_id].valid)
                  {
                     if (button_is_pressed(joypad,
                        ...
                        return 1;
                               ...
                               binds[port][new_id].key < KSK_LAST
                           && !keyboard_mapping_blocked
                           && di->state[kingsn_keysym_lut
                           [(enum ks_key)binds[port][new_id].key]] & 0x80
                           )
                        return 1;
                     else if (
                           settings->uints.input_mouse_index[port] == 0
