...
static void mainLoop(void* data)
{
   ...
   while (swa->running)
   {
      ...
      if (!released_out_buffer)
      {
         ...
         {
            swa->running = false;
            KINGSN_LOG("[Audio]: audoutGetReleasedAudioOutBuffer failed: %d\n", (int)rc);
            break;
         }
      ...
      if (released_out_buffer->data_size >= released_out_buffer->buffer_size / 2)
      {
         ...
         if (R_FAILED(rc))
         {
            KINGSN_LOG("[Audio]: audoutAppendAudioOutBuffer failed: %d\n", (int)rc);
         }
...
static void *switch_thread_audio_init(const char *device, unsigned rate, unsigned latency, unsigned block_frames, unsigned *new_rate)
{
   ...
   if (R_FAILED(rc))
   {
      KINGSN_LOG("[Audio]: audio init failed %d\n", (int)rc);
      free(swa);
      return NULL;
   }
   ...
   if (R_FAILED(rc))
   {
      KINGSN_LOG("[Audio]: audio start init failed: %d\n", (int)rc);
      goto fail_audio_ipc;
   }
   ...
   if (num_names != 1)
   {
	   KINGSN_ERR("[Audio]: got back more than one AudioOut\n");
	   goto fail_audio_ipc;
   }
   ...
   {
      KINGSN_ERR("expected %d channels, got %d\n", 2,
   ...
   {
      KINGSN_ERR("expected PCM_INT16, got %d\n", swa->output.sample_format);
      goto fail_audio_output;
   }
   ...
   *new_rate     = swa->sampleRate;
   ...
   KINGSN_LOG("[Audio]: switch_thread_audio_init device %s requested rate %hu rate %hu latency %hu block_frames %hu fifoSize %lu\n",
   ...
   if (R_FAILED(rc))
   {
      KINGSN_LOG("[Audio]: thread creation failed create %u\n", swa->thread.handle);
      swa->running = false;
      return NULL;
   }
   ...
   {
      KINGSN_LOG("[Audio]: thread creation failed start %u\n", swa->thread.handle);
...
{
   /* KINGSN_LOG("[Audio]: switch_thread_audio_start\n"); */
   switch_thread_audio_t *swa = (switch_thread_audio_t *)data;
