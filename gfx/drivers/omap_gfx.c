...
static const char *omapfb_get_fb_device(void)
{
   ...
   KINGSN_LOG("[video_omap]: Using %s as framebuffer device.\n", fbname);
   return fbname;
}
...
static int omapfb_detect_screen(omapfb_data_t *pdata)
{
   ...
   if (ret != 0)
   {
      KINGSN_ERR("[video_omap]: can't stat %s.\n", pdata->fbname);
      return -1;
   }
   ...
   if (!f)
   {
      KINGSN_ERR("[video_omap]: can't open %s.\n", buff);
      return -1;
   }
   ...
   if (ret != 1)
   {
      KINGSN_ERR("[video_omap]: can't parse %s.\n", buff);
      return -1;
   }
   ...
   if (ret < 0)
   {
      KINGSN_ERR("[video_omap]: can't read manager name.\n");
      return -1;
   }
   ...
   for (i = 0; ; i++)
   {
      ...
      if (string_is_equal(manager_name, buff))
      {
         ...
         if (ret < 0)
         {
            KINGSN_ERR("[video_omap]: can't read display name.\n");
            return -1;
         }
   ...
   if (ret < 0)
   {
      KINGSN_ERR("[video_omap]: couldn't find manager.\n");
      return -1;
   }
   ...
   if (display_id < 0)
   {
      KINGSN_ERR("[video_omap]: couldn't find display.\n");
      return -1;
   }
   ...
   if (ret != 2)
   {
      KINGSN_ERR("[video_omap]: can't parse %s (%d).\n", buff, ret);
      return -1;
   }

   if (w <= 0 || h <= 0)
   {
      KINGSN_ERR("[video_omap]: unsane dimensions detected (%dx%d).\n", w, h);
      return -1;
   }

   KINGSN_LOG("[video_omap]: detected %dx%d '%s' (%d) display attached to fb %d and overlay %d.\n",
...
static int omapfb_setup_pages(omapfb_data_t *pdata)
{
   ...
   if (!pdata->pages)
   {
      ...
      if (!pdata->pages)
      {
         KINGSN_ERR("[video_omap]: pages allocation failed.\n");
         return -1;
      }
   }
...
static int omapfb_mmap(omapfb_data_t *pdata)
{
   ...
   {
      pdata->fb_mem = NULL;
      KINGSN_ERR("[video_omap]: framebuffer mmap failed\n");
...
static int omapfb_backup_state(omapfb_data_t *pdata)
{
   ...
   {
      KINGSN_ERR("[video_omap]: backup layer (plane) failed\n");
      return -1;
   }

   if (ioctl(pdata->fd, OMAPFB_QUERY_MEM, &pdata->saved_state->mi) != 0)
   {
      KINGSN_ERR("[video_omap]: backup layer (mem) failed\n");
      return -1;
   }

   if (ioctl(pdata->fd, FBIOGET_VSCREENINFO, &pdata->saved_state->si) != 0)
   {
      KINGSN_ERR("[video_omap]: backup layer (screeninfo) failed\n");
      return -1;
   }
   ...
   {
      KINGSN_ERR("[video_omap]: backup layer (mem backup) failed\n");
...
static int omapfb_alloc_mem(omapfb_data_t *pdata)
{
   ...
   struct omapfb_mem_info mi;
   void                              *mem = NULL;
   const struct ks_game_geometry *geom = NULL;
   ...
   {
      KINGSN_ERR("[video_omap]: alloc mem (query plane) failed\n");
      goto error;
   }

   if (ioctl(pdata->fd, OMAPFB_QUERY_MEM, &mi) != 0)
   {
      KINGSN_ERR("[video_omap]: alloc mem (query mem) failed\n");
      goto error;
   }
   ...
   if (pi.enabled)
   {
      ...
      {
         KINGSN_ERR("[video_omap]: alloc mem (disable plane) failed\n");
         goto error;
      }
   }
   ...
   if (mi.size < mem_size)
   {
      ...
      {
         KINGSN_ERR("[video_omap]: allocation of %u bytes of VRAM failed\n", mem_size);
         goto error;
      }
   }
   ...
   if (mem == MAP_FAILED)
   {
      KINGSN_ERR("[video_omap]: zeroing framebuffer failed\n");
      goto error;
   }
...
static int omapfb_setup_screeninfo(omapfb_data_t *pdata, int width, int height)
{
   ...
   {
      KINGSN_ERR("[video_omap]: setup screeninfo failed\n");
      return -1;
   }
...
static int omapfb_setup_plane(omapfb_data_t *pdata, int width, int height)
{
   ...
   KINGSN_LOG("omap_video: scaling %dx%d to %dx%d\n", width, height, w, h);
   ...
   {
      KINGSN_ERR("omap_video: fb dimensions too large for allocated buffer\n");
      return -1;
   }

   if (ioctl(pdata->fd, OMAPFB_QUERY_PLANE, &pi) != 0)
   {
      KINGSN_ERR("[video_omap]: setup plane (query) failed\n");
      return -1;
   }
   ...
   if (ioctl(pdata->fd, OMAPFB_SETUP_PLANE, &pi) != 0)
   {
      KINGSN_ERR("[video_omap]: setup plane (param = %d %d %d %d) failed\n", x, y, w, h);
      return -1;
   }
...
static int omapfb_enable_plane(omapfb_data_t *pdata)
{
   ...
   if (ioctl(pdata->fd, OMAPFB_QUERY_PLANE, &pi) != 0)
   {
      KINGSN_ERR("[video_omap]: enable plane (query) failed\n");
      return -1;
   }
   ...
   if (ioctl(pdata->fd, OMAPFB_SETUP_PLANE, &pi) != 0)
   {
      KINGSN_ERR("[video_omap]: enable plane failed\n");
      return -1;
   }
...
static int omapfb_init(omapfb_data_t *pdata, unsigned bpp)
{
   ...
   if (fd == -1)
   {
      KINGSN_ERR("[video_omap]: can't open framebuffer device\n");
      return -1;
   }
...
static void omap_init_font(omap_video_t *vid, const char *font_path, unsigned font_size)
{
   ...
   {
      KINGSN_LOG("[video_omap]: font init failed\n");
      return;
   }
...
fail:
   free(vid);
   KINGSN_ERR("[video_omap]: initialization failed\n");
   return NULL;
}
      ...
      video_frame_info_t *video_info)
{
   ...
   {
      KINGSN_LOG("[video_omap]: mode set (resolution changed by core)\n");

      if (omapfb_set_mode(vid->omap, width, height) != 0)
      {
         KINGSN_ERR("[video_omap]: mode set failed\n");
         return false;
      }
