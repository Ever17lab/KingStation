...
static ks_vulkan_destroy_device_t cached_destroy_device_vk;
      ...
      void *pUserData)
{
   ...
   {
      KINGSN_ERR("[Vulkan]: Error: %s: %s\n",
            pLayerPrefix, pMessage);
   }
#if 0
   else if (flags & VK_DEBUG_REPORT_WARNING_BIT_EXT)
   {
      KINGSN_WARN("[Vulkan]: Warning: %s: %s\n",
            pLayerPrefix, pMessage);
   }
   else if (flags & VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT)
   {
      KINGSN_LOG("[Vulkan]: Performance warning: %s: %s\n",
            pLayerPrefix, pMessage);
   }
   else
   {
      KINGSN_LOG("[Vulkan]: Information: %s: %s\n",
            pLayerPrefix, pMessage);
   }
#endif
      ...
      uint32_t device_reqs, uint32_t host_reqs)
{
   ...
   KINGSN_ERR("[Vulkan]: Failed to find valid memory type. This should never happen.");
   abort();
}
...
void vulkan_log_textures(void)
{
   ...
   {
      KINGSN_WARN("[Vulkan]: Found leaked texture %llu.\n",
...
static void vulkan_track_dealloc(VkImage image)
{
   ...
   ks_assert(0 && "Couldn't find VkImage in dealloc!");
}
#endif
      ...
      enum vk_texture_type type)
{
   ...
   if (type == VULKAN_TEXTURE_STREAMED)
   {
      ...
      {
         KINGSN_LOG("[Vulkan]: GPU does not support using linear images as textures. Falling back to copy path.\n");
   ...
   switch (type)
   {
      case VULKAN_TEXTURE_STATIC:
         ...
         tex.mipmap         = true;
         ks_assert(initial && "Static textures must have initial data.\n");
      ...
      case VULKAN_TEXTURE_DYNAMIC:
         ks_assert(!initial && "Dynamic textures must not have initial data.\n");
         info.tiling        = VK_IMAGE_TILING_OPTIMAL;
         info.usage         = VK_IMAGE_USAGE_SAMPLED_BIT |
                              VK_IMAGE_USAGE_TRANSFER_DST_BIT |
                              VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
...
#ifdef VULKAN_DEBUG
            KINGSN_LOG("[Vulkan]: GPU supports linear images as textures, but not DEVICE_LOCAL. Falling back to copy path.\n");
#endif
            type = VULKAN_TEXTURE_STAGING;
...
void vulkan_transition_texture(vk_t *vk, VkCommandBuffer cmd, struct vk_texture *texture)
{
   ...
   switch (texture->type)
   {
      ...
      default:
         ks_assert(0 && "Attempting to transition invalid texture type.\n");
         break;
   }
   texture->layout = VK_IMAGE_LAYOUT_GENERAL;
}
      ...
      VkDevice device, struct vk_descriptor_manager *manager)
{
   ...
   ks_assert(manager->current->next);
      ...
      VkDescriptorSetLayout set_layout)
{
   ...
   ks_assert(num_sizes <= VULKAN_MAX_DESCRIPTOR_POOL_SIZES);
      ...
      size_t size, struct vk_buffer_range *range)
{
   ...
   ks_assert(vulkan_buffer_chain_suballoc(chain, size, range));
   return true;
}
...
static bool vulkan_find_instance_extensions(const char **exts, unsigned num_exts)
{
   ...
   {
      KINGSN_ERR("[Vulkan]: Could not find instance extensions. Will attempt without them.\n");
      ret = false;
      goto end;
   }
      ...
      const char **optional_exts, unsigned num_optional_exts)
{
   ...
   if (!vulkan_find_extensions(exts, num_exts, properties, property_count))
   {
      KINGSN_ERR("[Vulkan]: Could not find device extension. Will attempt without it.\n");
      ret = false;
      goto end;
   }
...
static bool vulkan_context_init_gpu(gfx_ctx_vulkan_data_t *vk)
{
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to enumerate physical devices.\n");
      return false;
   }
   ...
   if (gpu_count < 1)
   {
      KINGSN_ERR("[Vulkan]: Failed to enumerate Vulkan physical device.\n");
      free(gpus);
      return false;
   }
   ...
   for (i = 0; i < gpu_count; i++)
   {
      ...
      KINGSN_LOG("[Vulkan]: Found GPU at index %d: %s\n", i, gpu_properties.deviceName);
   ...
   {
      KINGSN_LOG("[Vulkan]: Using GPU index %d.\n", gpu_index);
      vk->context.gpu = gpus[gpu_index];
   }
   else
   {
      KINGSN_WARN("[Vulkan]: Invalid GPU index %d, using first device found.\n", gpu_index);
...
static bool vulkan_context_init_device(gfx_ctx_vulkan_data_t *vk)
{
   ...
   struct ks_hw_render_context_negotiation_interface_vulkan *iface =
      (struct ks_hw_render_context_negotiation_interface_vulkan*)video_driver_get_context_negotiation_interface();

   if (iface && iface->interface_type != KS_HW_RENDER_CONTEXT_NEGOTIATION_INTERFACE_VULKAN)
   {
      KINGSN_WARN("[Vulkan]: Got HW context negotiation interface, but it's the wrong API.\n");
      iface = NULL;
   }

   if (iface && iface->interface_version != KS_HW_RENDER_CONTEXT_NEGOTIATION_INTERFACE_VULKAN_VERSION)
   {
      KINGSN_WARN("[Vulkan]: Got HW context negotiation interface, but it's the wrong interface version.\n");
      iface = NULL;
   }

   if (!cached_device_vk && iface && iface->create_device)
   {
      struct ks_vulkan_context context = { 0 };
      ...
      if (!ret)
      {
         KINGSN_WARN("[Vulkan]: Failed to create device with negotiation interface. Falling back to default path.\n");
      }
      else
      {
         ...
         {
            KINGSN_ERR("[Vulkan]: Present queue != graphics queue. This is currently not supported.\n");
            return false;
         }
      }
   }
   ...
   if (!vk->emulate_mailbox)
   {
      if (vk->context.gpu_properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU)
      {
         ...
         KINGSN_LOG("[Vulkan]: Using semaphores for WSI acquire.\n");
      }
      else
      {
         vk->use_wsi_semaphore = false;
         KINGSN_LOG("[Vulkan]: Using fences for WSI acquire.\n");
      }
   }

   KINGSN_LOG("[Vulkan]: Using GPU: %s\n", vk->context.gpu_properties.deviceName);
   ...
   if (vk->context.device == VK_NULL_HANDLE)
   {
      ...
      if (queue_count < 1)
      {
         KINGSN_ERR("[Vulkan]: Invalid number of queues detected.\n");
         return false;
      }
      ...
      for (i = 0; i < queue_count; i++)
      {
         ...
         if (supported && ((queue_properties[i].queueFlags & required) == required))
         {
            ...
            KINGSN_LOG("[Vulkan]: Queue family %u supports %u sub-queues.\n",
      ...
      if (!found_queue)
      {
         KINGSN_ERR("[Vulkan]: Did not find suitable graphics queue.\n");
         return false;
      }
      ...
      if (!use_device_ext)
      {
          KINGSN_ERR("[Vulkan]: Could not find required device extensions.\n");
          return false;
      }
      ...
      if (cached_device_vk)
      {
         ...
         KINGSN_LOG("[Vulkan]: Using cached Vulkan context.\n");
      }
      else if (vkCreateDevice(vk->context.gpu, &device_info,
               NULL, &vk->context.device) != VK_SUCCESS)
      {
         KINGSN_ERR("[Vulkan]: Failed to create device.\n");
         return false;
      }
   }
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to load device symbols.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to create queue lock.\n");
      return false;
   }
#endif
      ...
      enum vulkan_wsi_type type)
{
   ...
   if (!vulkan_library)
   {
      KINGSN_ERR("[Vulkan]: Failed to open Vulkan loader.\n");
      return false;
   }

   KINGSN_LOG("[Vulkan]: Vulkan dynamic library loaded.\n");
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to load vkGetInstanceProcAddr symbol, broken loader?\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to load global Vulkan symbols, broken loader?\n");
      return false;
   }
   ...
   if (iface && iface->get_application_info)
   {
      ...
      {
         KINGSN_LOG("[Vulkan]: App: %s (version %u)\n",
      ...
      {
         KINGSN_LOG("[Vulkan]: Engine: %s (version %u)\n",
   ...
   KINGSN_LOG("[Vulkan]: Enabling Vulkan debug layers.\n");
#endif
   ...
   if (res != VK_SUCCESS)
   {
      KINGSN_ERR("Failed to create Vulkan instance (%d).\n", res);
      return false;
   }

   if (!vulkan_load_instance_symbols(vk))
   {
      KINGSN_ERR("[Vulkan]: Failed to load instance symbols.\n");
      return false;
   }
...
#define GOTO_FAIL() do { \
   ...
   {
      KINGSN_WARN("Monitor index is out of range, using automatic display.\n");
      monitor_index = 0;
   }
...
out:
   ...
   {
      KINGSN_WARN("Could not find suitable surface for monitor index: %u.\n",
            monitor_index);
      KINGSN_WARN("Retrying first suitable monitor.\n");
      monitor_index = 0;
      best_mode = VK_NULL_HANDLE;
      *width = saved_width;
      *height = saved_height;
      ...
      case VULKAN_WSI_ANDROID:
#ifdef ANDROID
         {
            ...
            if (create(vk->context.instance,
                     &surf_info, NULL, &vk->vk_surface) != VK_SUCCESS)
            {
               KINGSN_ERR("[Vulkan]: Failed to create Android surface.\n");
               return false;
            }
            KINGSN_LOG("[Vulkan]: Created Android surface: %llu\n",
...
void vulkan_present(gfx_ctx_vulkan_data_t *vk, unsigned index)
{
   ...
   {
      KINGSN_LOG("[Vulkan]: QueuePresent failed, destroying swapchain.\n");
...
retry:
   if (vk->swapchain == VK_NULL_HANDLE)
   {
      ...
      {
#ifdef VULKAN_DEBUG
         KINGSN_ERR("[Vulkan]: Failed to create new swapchain.\n");
#endif
         ks_sleep(20);
         return;
      }
   ...
   ks_assert(!vk->context.has_acquired_swapchain);
   ...
   if (err == VK_SUCCESS || err == VK_SUBOPTIMAL_KHR)
   {
      ...
      if (vk->context.swapchain_acquire_semaphore)
      {
         ...
         slock_lock(vk->context.queue_lock);
#endif
         KINGSN_LOG("[Vulkan]: Destroying stale acquire semaphore.\n");
         vkDeviceWaitIdle(vk->context.device);
   ...
   switch (err)
   {
      ...
      case VK_SUBOPTIMAL_KHR:
         ...
         if (is_retrying)
         {
            KINGSN_ERR("[Vulkan]: Swapchain is out of date, trying to create new one. Have tried multiple times ...\n");
            ks_sleep(10);
         }
         else
            KINGSN_ERR("[Vulkan]: Swapchain is out of date, trying to create new one.\n");
         is_retrying = true;
         vulkan_acquire_clear_fences(vk);
         goto retry;
      default:
         if (err != VK_SUCCESS)
         {
            ...
            vulkan_destroy_swapchain(vk);
            KINGSN_ERR("[Vulkan]: Failed to acquire from swapchain (err = %d).\n",
                  (int)err);
            if (err == VK_ERROR_SURFACE_LOST_KHR)
               KINGSN_ERR("[Vulkan]: Got VK_ERROR_SURFACE_LOST_KHR.\n");
      ...
      unsigned swap_interval)
{
...
#ifdef VULKAN_DEBUG
      KINGSN_LOG("[Vulkan]: Do not need to re-create swapchain.\n");
#endif
   ...
   {
      KINGSN_ERR("[Vulkan]: Bogus present modes found.\n");
      return false;
   }
   vkGetPhysicalDeviceSurfacePresentModesKHR(
         vk->context.gpu, vk->vk_surface,
   ...
   {
      KINGSN_LOG("[Vulkan]: Swapchain supports present mode: %u.\n",
            present_modes[i]);
   }
   ...
   KINGSN_LOG("[Vulkan]: Creating swapchain with present mode: %u\n",
   ...
   else
   {
      if (format_count == 0)
      {
         KINGSN_ERR("[Vulkan]: Surface has no formats.\n");
         return false;
      }
   ...
   if (swapchain_size.width == 0 && swapchain_size.height == 0)
   {
      ...
      memset(vk->context.swapchain_images, 0, sizeof(vk->context.swapchain_images));
      KINGSN_LOG("[Vulkan]: Cannot create a swapchain yet. Will try again later ...\n");
      return true;
   }

   KINGSN_LOG("[Vulkan]: Using swapchain size %u x %u.\n",
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to create swapchain.\n");
      return false;
   }
   ...
   KINGSN_LOG("[Vulkan]: Got %u swapchain images.\n",
         vk->context.num_swapchain_images);
