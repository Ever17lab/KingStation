...
#if defined(VITA)
   extern int ks_epoll_fd;
   SceNetEpollEvent ev = {0};

   ev.events = SCE_NET_EPOLLIN | SCE_NET_EPOLLHUP;
   ev.data.fd = fd + 1;

   if((sceNetEpollControl(ks_epoll_fd, SCE_NET_EPOLL_CTL_ADD, fd + 1, &ev)))
   {
      int ret = sceNetEpollWait(ks_epoll_fd, &ev, 1, 0);
      sceNetEpollControl(ks_epoll_fd, SCE_NET_EPOLL_CTL_DEL, fd + 1, NULL);
#else
