   ...
   for (i = 0; i < MAX_KB_PORT_NUM; i++)
   {
      ...
      for (j = 0; j < last_kbdata[i].nb_keycode; j++)
      {
         ...
         if (newly_depressed)
         {
            ...
            input_keyboard_event(false, keyboardcode, keyboardcode, mod, KS_DEVICE_KEYBOARD);
         }
      }
      ...
      for (j = 0; j < ps3->kbdata[i].nb_keycode; j++)
      {
         ...
         {
            unsigned keyboardcode = input_keymaps_translate_keysym_to_rk(code);
            input_keyboard_event(true, keyboardcode, keyboardcode, mod, KS_DEVICE_KEYBOARD);
         }
      }
   }
}
...
static void* ps3_input_init(const char *joypad_driver)
{
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_psl1ght);
...
static bool ps3_input_set_sensor_state(void *data, unsigned port,
      enum ks_sensor_action action, unsigned event_rate) { return false; }
...
{
   return (
         (data->BTN_CROSS << KS_DEVICE_ID_JOYPAD_B)
         | (data->BTN_SQUARE << KS_DEVICE_ID_JOYPAD_Y)
         | (data->BTN_SELECT << KS_DEVICE_ID_JOYPAD_SELECT)
         | (data->BTN_START << KS_DEVICE_ID_JOYPAD_START)
         | (data->BTN_UP << KS_DEVICE_ID_JOYPAD_UP)
         | (data->BTN_DOWN << KS_DEVICE_ID_JOYPAD_DOWN)
         | (data->BTN_LEFT << KS_DEVICE_ID_JOYPAD_LEFT)
         | (data->BTN_RIGHT << KS_DEVICE_ID_JOYPAD_RIGHT)
         | (data->BTN_CIRCLE << KS_DEVICE_ID_JOYPAD_A)
         | (data->BTN_TRIANGLE << KS_DEVICE_ID_JOYPAD_X)
         | (data->BTN_L1 << KS_DEVICE_ID_JOYPAD_L)
         | (data->BTN_R1 << KS_DEVICE_ID_JOYPAD_R)
         | (data->BTN_L2 << KS_DEVICE_ID_JOYPAD_L2)
         | (data->BTN_R2 << KS_DEVICE_ID_JOYPAD_R2)
         | (data->BTN_L3 << KS_DEVICE_ID_JOYPAD_L3)
         | (data->BTN_R3 << KS_DEVICE_ID_JOYPAD_R3)
         );
}
...
static bool ps3_joypad_rumble(unsigned pad,
      enum ks_rumble_effect effect, uint16_t strength) { return true; }
