      ...
      uint16_t *buf, uint16_t joykey)
{
   switch (key)
   {
      case KS_DEVICE_ID_JOYPAD_A:
         return buf[DOSKEY_x];
      case KS_DEVICE_ID_JOYPAD_B:
         return buf[DOSKEY_z];
      case KS_DEVICE_ID_JOYPAD_X:
         return buf[DOSKEY_s];
      case KS_DEVICE_ID_JOYPAD_Y:
         return buf[DOSKEY_a];
      case KS_DEVICE_ID_JOYPAD_SELECT:
         return buf[DOSKEY_RSHIFT];
      case KS_DEVICE_ID_JOYPAD_START:
   ...
   for (i = 0; i <= DEFAULT_MAX_PADS; i++)
   {
      ...
      for (key = 0; key < LAST_KEYCODE; key++)
      {
         if (cur_state[key] != normal_keys[key])
         {
            ...
            input_keyboard_event(normal_keys[key], code, code, 0, KS_DEVICE_KEYBOARD);
         }
      }
