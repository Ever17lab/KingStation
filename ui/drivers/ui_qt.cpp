...
void ThumbnailWidget::dropEvent(QDropEvent *event)
{
   ...
   if (data->hasUrls())
   {
      ...
      else
      {
         ...
         KINGSN_ERR("[Qt]: Could not read image: %s\n", string_data);
      }
   }
}
...
static void* ui_companion_qt_init(void)
{
   ...
   mainwindow->setWindowTitle("KingStation");
   ...
   updaterMenu->addAction(msg_hash_to_str(MENU_ENUM_LABEL_VALUE_QT_UPDATE_KingStation_NIGHTLY), mainwindow, SLOT(updateKingStationNightly()));
#endif
#endif
...
#if defined(HAVE_CG) || defined(HAVE_GLSL) || defined(HAVE_SLANG) || defined(HAVE_HLSL)
         KINGSN_LOG("[Qt]: Reloading shader parameters.\n");
