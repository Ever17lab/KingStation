...
joypad_connection_t *pad_connection_init(unsigned pads)
{
   ...
   if(pads > MAX_USERS)
   {
     KINGSN_WARN("[joypad] invalid number of pads requested (%d), using default (%d)\n",
      pads, MAX_USERS);
     pads = MAX_USERS;
   }
   ...
   void *data, hid_driver_t *driver)
{
   ...
   } pad_map[] =
   {
      ...
      { "Retrode",                       0,     0,  &pad_connection_ksde },
   ...
   pad_map[10].vid        = 1406;
   pad_map[10].pid        = 774;
   pad_map[11].vid        = VID_KSDE;
   pad_map[11].pid        = PID_KSDE;
...
#if 0
         KINGSN_LOG("[connect] %s\n", pad_map[i].name);
         KINGSN_LOG("[connect] VID: Expected: %04x got: %04x\n",
                   pad_map[i].vid, vid);
         KINGSN_LOG("[connect] PID: Expected: %04x got: %04x\n",
                   pad_map[i].pid, pid);
#endif
...
void pad_connection_pad_deinit(joypad_connection_t *joyconn, uint32_t pad)
{
   ...
   if (joyconn->iface)
   {
      joyconn->iface->set_rumble(joyconn->data, KS_RUMBLE_STRONG, 0);
      joyconn->iface->set_rumble(joyconn->data, KS_RUMBLE_WEAK, 0);
