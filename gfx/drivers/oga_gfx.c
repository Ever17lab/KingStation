...
static bool oga_create_display(oga_video_t* vid)
{
   ...
   if (vid->fd < 0)
   {
      KINGSN_ERR("open /dev/dri/card0 failed.\n");
      return false;
   }
   ...
   if (!resources)
   {
      KINGSN_ERR("drmModeGetResources failed: %s\n", strerror(errno));
      goto err_01;
   }
   ...
   if (!connector)
   {
      KINGSN_ERR("DRM_MODE_CONNECTED not found.\n");
      goto err_02;
   }
   ...
   if (!mode)
   {
      KINGSN_ERR("DRM_MODE_TYPE_PREFERRED not found.\n");
      goto err_03;
   }
   ...
   if (!encoder)
   {
      KINGSN_ERR("could not find encoder!\n");
      goto err_03;
   }
      ...
      int width, int height, int rk_format)
{
   ...
   if (!surface)
   {
      KINGSN_ERR("Error allocating surface\n");
      return NULL;
   }
   ...
   {
      KINGSN_ERR("DRM_IOCTL_MODE_CREATE_DUMB failed.\n");
      goto out;
   }
   ...
   {
      KINGSN_ERR("drmPrimeHandleToFD failed.\n");
      goto out;
   }
...
static void oga_destroy_surface(oga_surface_t* surface)
{
   ...
   if (io < 0)
      KINGSN_ERR("DRM_IOCTL_MODE_DESTROY_DUMB failed.\n");
...
static oga_framebuf_t* oga_create_framebuf(oga_surface_t* surface)
{
   ...
   if (!framebuf)
   {
      KINGSN_ERR("Error allocating framebuf\n");
      return NULL;
   }
   ...
   if (ret)
   {
      KINGSN_ERR("drmModeAddFB2 failed.\n");
      free(framebuf);
      return NULL;
   }
      ...
      input_driver_t **input, void **input_data)
{
   ...
   struct ks_game_geometry  *geom    = &av_info->geometry;
   ...
   if (!vid)
   {
      KINGSN_ERR("Error allocating vid\n");
      return NULL;
   }

   if (!oga_create_display(vid))
   {
      KINGSN_ERR("Error initializing drm\n");
      return NULL;
   }
   ...
   KINGSN_LOG("oga_gfx_init video %dx%d rgb32 %d smooth %d ctx_scaling %d"
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (unlikely(drmModeSetCrtc(vid->fd, vid->crtc_id, page->fb_id, 0, 0, &vid->connector_id, 1, &vid->mode) != 0))
      KINGSN_ERR("drmModeSetCrtc failed.\n");
...
static bool oga_gfx_set_shader(void *data, enum kingsn_shader_type type, const char *path)
{
   (void)data;
   (void)type;
   (void)path;
...
static void oga_set_rotation(void *data, unsigned rotation)
{
   ...
   case 3:
      ...
      break;
   default:
      KINGSN_ERR("Unhandled rotation %hu\n", rotation);
      break;
   }
}

static bool oga_get_current_software_framebuffer(void *data, struct ks_framebuffer *framebuffer)
{
   oga_video_t *vid = (oga_video_t*)data;
   if (!vid)
      return false;
