...
static int generic_action_start_performance_counters(struct ks_perf_counter **counters,
      ...
      unsigned type, size_t idx, size_t entry_idx)
{
   ...
   {
      const struct ks_keybind *keyptr = &input_config_binds[user_idx][btn_idx];
      ...
      unsigned type, size_t idx, size_t entry_idx)
{
   ...
   settings->uints.input_keymapper_ids[user_idx][btn_idx] = KSK_FIRST;
...
#endif
   if (type >= MENU_SETTINGS_LIBKS_PERF_COUNTERS_BEGIN &&
         type <= MENU_SETTINGS_LIBKS_PERF_COUNTERS_END)
   {
