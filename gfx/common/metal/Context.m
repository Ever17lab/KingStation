...
- (bool)_initClearState
{
   ...
   if (err != nil)
   {
      KINGSN_ERR("[Metal]: error creating clear pipeline state %s\n", err.localizedDescription.UTF8String);
      return NO;
   }
...
- (bool)_initConversionFilters
{
   ...
   if (err)
   {
      KINGSN_LOG("[Metal]: unable to create 'convert_bgra4444_to_bgra8888' conversion filter: %s\n",
                err.localizedDescription.UTF8String);
      return NO;
   }
   ...
   if (err)
   {
      KINGSN_LOG("[Metal]: unable to create 'convert_rgb565_to_bgra8888' conversion filter: %s\n",
                err.localizedDescription.UTF8String);
      return NO;
   }
...
- (bool)readBackBuffer:(uint8_t *)buffer
{
   ...
   {
      KINGSN_WARN("[Metal]: unexpected pixel format %d\n", _backBuffer.pixelFormat);
      return NO;
   }
...
- (bool)allocRange:(BufferRange *)range length:(NSUInteger)length
{
   ...
   [self _nextNode];
   ks_assert([self _subAllocRange:range length:length]);
   return YES;
}
