...
/* ARGB*/
static const unsigned long KingStation_icon_data[] = {
   16, 16,
...
static int x_log_error_handler(Display *dpy, XErrorEvent *event)
{
   ...
   KINGSN_WARN("[GLX]: X error message: %s, request code: %d, minor code: %d\n",
...
static void gfx_ctx_x_swap_interval(void *data, int interval)
{
   ...
   if (x->swap_mode)
   {
      ...
      {
         KINGSN_LOG("[GLX]: glXSwapInterval(%i)\n", x->interval);
         if (g_pglSwapInterval(x->interval) != 0)
            KINGSN_WARN("[GLX]: glXSwapInterval() failed.\n");
      }
      else if (g_pglSwapIntervalEXT)
      {
         KINGSN_LOG("[GLX]: glXSwapIntervalEXT(%i)\n", x->interval);
         g_pglSwapIntervalEXT(g_x11_dpy, x->glx_win, x->interval);
      }
      else if (g_pglSwapIntervalSGI)
      {
         KINGSN_LOG("[GLX]: glXSwapIntervalSGI(%i)\n", x->interval);
         if (g_pglSwapIntervalSGI(x->interval) != 0)
            KINGSN_WARN("[GLX]: glXSwapIntervalSGI() failed.\n");
      }
   }
   else
   {
...
static void *gfx_ctx_x_init(void *data)
{
   ...
   switch (x_api)
   {
      case GFX_CTX_OPENGL_API:
         ...
         {
            KINGSN_LOG("[GLX]: GLX_EXT_swap_control_tear supported.\n");
      ...
      bool fullscreen)
{
   ...
   if (fullscreen && !windowed_full)
   {
      if (x11_enter_fullscreen(g_x11_dpy, width, height))
      {
         ...
         KINGSN_ERR("[GLX]: Entering true fullscreen failed. Will attempt windowed mode.\n");
   }

   wm_name = x11_get_wm_name(g_x11_dpy);
   if (wm_name)
   {
      KINGSN_LOG("[GLX]: Window manager is %s.\n", wm_name);

      if (true_full && strcasestr(wm_name, "xfwm"))
      {
         KINGSN_LOG("[GLX]: Using override-redirect workaround.\n");
   ...
   if (fullscreen || g_x11_screen != 0)
   {
      ...
      if (xinerama_get_coord(g_x11_dpy, g_x11_screen,
         ...
         KINGSN_LOG("[GLX]: Using Xinerama on screen #%u.\n", g_x11_screen);
      else
         KINGSN_LOG("[GLX]: Xinerama is not active on screen.\n");
   ...
   KINGSN_LOG("[GLX]: X = %d, Y = %d, W = %u, H = %u.\n",
   ...
   XChangeProperty(g_x11_dpy, g_x11_win, net_wm_icon, cardinal, 32, PropModeReplace, (const unsigned char*)KingStation_icon_data, sizeof(KingStation_icon_data) / sizeof(*KingStation_icon_data));
   ...
   if (fullscreen && disable_composition)
   {
      ...
      KINGSN_LOG("[GLX]: Requesting compositor bypass.\n");
   ...
   if (true_full)
   {
      KINGSN_LOG("[GLX]: Using true fullscreen.\n");
   ...
   else if (fullscreen)
   {
      ...
      XMapRaised(g_x11_dpy, g_x11_win);
      KINGSN_LOG("[GLX]: Using windowed fullscreen.\n");
   ...
   switch (x_api)
   {
      ...
      case GFX_CTX_OPENGL_ES_API:
            ...
            if (x->core_es || x->debug)
            {
               ...
               *aptr = None;
                ...
                */
               {
                   ...
                   */
                  KINGSN_LOG("[GLX]: Creating context for requested version %u.%u.\n", g_major, g_minor);
                  ...
                  if (x->ctx)
                  {
                     ...
                     if (x->use_hw_ctx)
                     {
                        KINGSN_LOG("[GLX]: Creating shared HW context.\n");
                        x->hw_ctx = glx_create_context_attribs(g_x11_dpy,
                              x->fbc, x->ctx, True, attribs);

                        if (!x->hw_ctx)
                           KINGSN_ERR("[GLX]: Failed to create new shared context.\n");
                     }
                     ...
                     KINGSN_LOG("[GLX]: Not running Mesa, trying higher versions...\n");
                  }
                  else
                  {
                     KINGSN_ERR("[GLX]: Failed to create new context.\n");
                     goto error;
                  }
                  ...
                  for (i = 0; i < version_rows; i++)
                  {
                     if (x->core_es)
                     {
                        ...
                        KINGSN_LOG("[GLX]: Creating context for version %d.%d.\n", versions[i][0], versions[i][1]);
                     }
                     else
                        KINGSN_LOG("[GLX]: Creating context for version %u.%u.\n", g_major, g_minor);
         ...
         else
         {
            ...
            KINGSN_LOG("[GLX]: Using cached GL context.\n");
         }
      ...
      case GFX_CTX_OPENGL_ES_API:
         ...
         if (x->is_double)
         {
            ...
            if (!g_pglSwapInterval && !g_pglSwapIntervalEXT && !g_pglSwapIntervalSGI)
               KINGSN_WARN("[GLX]: Cannot find swap interval call.\n");
            else
               KINGSN_LOG("[GLX]: Found swap function: %s.\n", swap_func);
         }
         else
            KINGSN_WARN("[GLX]: Context is not double buffered!.\n");
#endif
         break;
