      ...
      )
{
   ...
   if (!font_renderer_create_default(
            ...
            *path_font ? path_font : NULL,
            video_font_size))
   {
      KINGSN_LOG("[SDL]: Could not initialize fonts.\n");
      return;
   }
      ...
      input_driver_t **input, void **input_data)
{
   ...
   ks_assert(video_info);
   full_x = video_info->current_w;
   full_y = video_info->current_h;
   KINGSN_LOG("[SDL]: Detecting desktop resolution %ux%u.\n", full_x, full_y);
   ...
   if (!vid->screen)
   {
      KINGSN_ERR("[SDL1]: Failed to init SDL surface: %s\n", SDL_GetError());
      goto error;
   }
   ...
   if (!vid->menu.frame)
   {
      KINGSN_ERR("[SDL1]: Failed to init menu surface: %s\n", SDL_GetError());
      goto error;
   }
