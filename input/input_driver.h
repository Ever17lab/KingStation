...
struct kingsn_joypad_info
{
   const struct ks_keybind *auto_binds;
...
struct kingsn_joypad_driver
{
   void *(*init)(void *data);
   bool (*query_pad)(unsigned);
   void (*destroy)(void);
   int16_t (*button)(unsigned, uint16_t);
   int16_t (*state)(kingsn_joypad_info_t *joypad_info,
         const struct ks_keybind *binds, unsigned port);
   void (*get_buttons)(unsigned, input_bits_t *);
   int16_t (*axis)(unsigned, uint32_t);
   void (*poll)(void);
   bool (*set_rumble)(unsigned, enum ks_rumble_effect, uint16_t);
 ...
 * Used by KS_ENVIRONMENT_GET_RUMBLE_INTERFACE.
 **/
 ...
 * Used by KS_ENVIRONMENT_GET_SENSOR_INTERFACE.
 **/
 ...
 *                            E.g.:
 *                            - KS_DEVICE_INDEX_ANALOG_LEFT
 *                            - KS_DEVICE_INDEX_ANALOG_RIGHT
 * @ident                   : Analog key identifier.
 *                            E.g.:
 *                            - KS_DEVICE_ID_ANALOG_X
 *                            - KS_DEVICE_ID_ANALOG_Y
...
#define input_conv_analog_id_to_bind_id(idx, ident, ident_minus, ident_plus) \
   switch ((idx << 1) | ident) \
   { \
      case (KS_DEVICE_INDEX_ANALOG_LEFT << 1) | KS_DEVICE_ID_ANALOG_X: \
         ident_minus = KINGSN_ANALOG_LEFT_X_MINUS; \
         ident_plus  = KINGSN_ANALOG_LEFT_X_PLUS; \
         break; \
      case (KS_DEVICE_INDEX_ANALOG_LEFT << 1) | KS_DEVICE_ID_ANALOG_Y: \
         ident_minus = KINGSN_ANALOG_LEFT_Y_MINUS; \
         ident_plus  = KINGSN_ANALOG_LEFT_Y_PLUS; \
         break; \
      case (KS_DEVICE_INDEX_ANALOG_RIGHT << 1) | KS_DEVICE_ID_ANALOG_X: \
         ident_minus = KINGSN_ANALOG_RIGHT_X_MINUS; \
         ident_plus  = KINGSN_ANALOG_RIGHT_X_PLUS; \
         break; \
      case (KS_DEVICE_INDEX_ANALOG_RIGHT << 1) | KS_DEVICE_ID_ANALOG_Y: \
         ident_minus = KINGSN_ANALOG_RIGHT_Y_MINUS; \
         ident_plus  = KINGSN_ANALOG_RIGHT_Y_PLUS; \
         break; \
   }
 ...
 *
 * Registers a newly connected pad with KingStation.
 **/
 ...
 * This interfaces with the global driver struct and libks callbacks.
 **/
void input_keyboard_event(bool down, unsigned code, uint32_t character,
      uint16_t mod, unsigned device);

extern struct ks_keybind input_config_binds[MAX_USERS][KINGSN_BIND_LIST_END];
extern struct ks_keybind input_autoconf_binds[MAX_USERS][KINGSN_BIND_LIST_END];
...
uint8_t input_config_bind_map_get_ks_key(unsigned i);

/* auto_bind can be NULL. */
void input_config_get_bind_string(char *buf,
      const struct ks_keybind *bind,
      const struct ks_keybind *auto_bind, size_t size);
 ...
 **/
enum ks_key input_config_translate_str_to_rk(const char *str);
 ...
 * KINGSN_BIND_LIST_END on not found.
 **/
...
const struct ks_keybind *input_config_get_bind_auto(unsigned port, unsigned id);

void input_config_save_keybinds_user(void *data, unsigned user);

void input_config_save_keybind(void *data, const char *prefix,
      const char *base, const struct ks_keybind *bind,
      bool save_empty);
