...
static void hidpad_ps2adapter_packet_handler(void *data, uint8_t *packet, uint16_t size)
{
   ...
   device->buttons |= PS2_H_LEFT(hat_value) ? (UINT64_C(1) << KS_DEVICE_ID_JOYPAD_LEFT) : 0;
   device->buttons |= PS2_H_RIGHT(hat_value) ? (UINT64_C(1) << KS_DEVICE_ID_JOYPAD_RIGHT) : 0;
   device->buttons |= PS2_H_UP(hat_value) ? (UINT64_C(1) << KS_DEVICE_ID_JOYPAD_UP) : 0;
   device->buttons |= PS2_H_DOWN(hat_value) ? (UINT64_C(1) << KS_DEVICE_ID_JOYPAD_DOWN) : 0;
}
