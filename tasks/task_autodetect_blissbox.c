...
static const blissbox_pad_type_t* input_autoconfigure_get_blissbox_pad_type_win32(int vid, int pid)
{
   ...
   {
      KINGSN_ERR("[Autoconf]: Error in SetupDiGetClassDevs: %d.\n",
            GetLastError());
      goto done;
   }
   ...
   while (!success)
   {
         ...
         &deviceInterfaceData)); i++)
      {
         ...
         if (!bResult)
         {
            KINGSN_ERR("[Autoconf]: Error in SetupDiEnumDeviceInterfaces: %d.\n", GetLastError());
            goto done;
         }
         ...
         if (!bResult)
         {
                  ...
                  && (required_length > 0))
            {
               ...
               if (!pInterfaceDetailData)
               {
                  KINGSN_ERR("[Autoconf]: Error allocating memory for the device detail buffer.\n");
                  goto done;
               }
            }
            else
            {
               KINGSN_ERR("[Autoconf]: Other error: %d.\n", GetLastError());
               goto done;
            }
         }
   ...
   if (!lp_device_path)
   {
      KINGSN_ERR("[Autoconf]: No devicepath. Error %d.", GetLastError());
      goto done;
   }
...
found:
   ...
   if (hDeviceHandle == INVALID_HANDLE_VALUE)
   {
      ...
      if (hDeviceHandle == INVALID_HANDLE_VALUE)
      {
         KINGSN_ERR("[Autoconf]: Can't open device for reading and writing: %d.", GetLastError());
   ...
   if (!bResult)
      KINGSN_ERR("[Autoconf]: Could not destroy device info list.\n");
   ...
   KINGSN_LOG("[Autoconf]: Could not find connected pad in Bliss-Box port#%d.\n", pid - BLISSBOX_PID);
#endif
...
static const blissbox_pad_type_t* input_autoconfigure_get_blissbox_pad_type_libusb(int vid, int pid)
{
   ...
   if (ret < 0)
   {
      KINGSN_ERR("[Autoconf]: Could not initialize libusb.\n");
      return NULL;
   }
   ...
   {
      KINGSN_ERR("[Autoconf]: Could not find or open libusb device %d:%d.\n", vid, pid);
      goto error;
   }
   ...
   if (ret < 0)
   {
      KINGSN_ERR("[Autoconf]: Error during libusb_set_configuration.\n");
      goto error;
   }
   ...
   if (ret < 0)
   {
      KINGSN_ERR("[Autoconf]: Error during libusb_claim_interface.\n");
      goto error;
   }
   ...
   if (ret < 0)
      KINGSN_ERR("[Autoconf]: Error during libusb_control_transfer.\n");
      ...
      int vid, int pid, char *device_name, size_t len)
{
   if (pid == BLISSBOX_UPDATE_MODE_PID)
      KINGSN_LOG("[Autoconf]: Bliss-Box in update mode detected. Ignoring.\n");
   else if (pid == BLISSBOX_OLD_PID)
      KINGSN_LOG("[Autoconf]: Bliss-Box 1.0 firmware detected. Please update to 2.0 or later.\n");
   else if (pid >= BLISSBOX_PID && pid <= BLISSBOX_PID + BLISSBOX_MAX_PAD_INDEX)
   {
      const blissbox_pad_type_t *pad;
      int index      = pid - BLISSBOX_PID;

      KINGSN_LOG("[Autoconf]: Bliss-Box detected. Getting pad type...\n");
      ...
      if (pad && !string_is_empty(pad->name))
      {
         KINGSN_LOG("[Autoconf]: Found Bliss-Box pad type: %s (%d) in port#%d\n", pad->name, pad->index, index);
