...
static INLINE bool vg_query_extension(const char *ext)
{
   ...
   KINGSN_LOG("[VG]: Querying VG extension: %s => %s\n",
      ...
      input_driver_t **input, void **input_data)
{
   ...
   KINGSN_LOG("[VG]: Detecting screen resolution %ux%u.\n", temp_width, temp_height);
   ...
   if (temp_width != 0 && temp_height != 0)
   {
      KINGSN_LOG("[VG]: Verified window resolution %ux%u.\n",
            temp_width, temp_height);
      video_driver_set_size(temp_width, temp_height);
   }
   ...
   vg->mTextureWidth = vg->mTextureHeight = video->input_scale * KINGSN_SCALE_BASE;
         ...
         && vg->ctx_driver->image_buffer_init(vg->ctx_data, (void*)video))
   {
      ...
      {
         KINGSN_LOG("[VG] Using EGLImage buffer\n");
   ...
   if (ext)
      KINGSN_LOG("[VG] Supported extensions: %s\n", ext);
#endif
      ...
      unsigned width, unsigned height, unsigned pitch)
{
   ...
   if (vg->mEglImageBuf)
   {
      ...
      ks_assert(img != EGL_NO_IMAGE_KHR);
