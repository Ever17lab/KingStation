#!/bin/sh
###############
# Build script which builds and packages KingStation for MinGW 32/64-bit.
...
do_phoenix_build()
{
   ...
   ### Build Phoenix GUI
   if [ ! -d "Phoenix" ]; then
      git clone git://github.com/Themaister/KingStation-Phoenix.git Phoenix
      cd Phoenix
   else
      cd Phoenix
   ...
   touch KingStation-phoenix.cfg
   cp KingStation-phoenix.cfg KingStation-phoenix.exe ../ || die "Failed to copy ..."
...
do_build()
{
   ...
   if [ ! -d "$KingStation_DIR" ]; then
      git clone git://github.com/libks/KingStation.git "$KingStation_DIR"
      cd "$KingStation_DIR"
   else
      cd "$KingStation_DIR"
      git pull origin master
   ...
   if [ -z "`find . | grep "KingStation-win"`" ]; then
   ...
   ZIP_BASE="`find . | grep "KingStation-win" | head -n1`"
   ZIP_SLIM="`echo $ZIP_BASE | sed -e 's|\.zip|-slim.zip|'`"
   ZIP_FULL="`echo $ZIP_BASE | sed -e 's|\.zip|-full.zip|'`"

   if [ "$BUILD_PHOENIX_GUI" = "yes" ]; then
      zip "$ZIP_BASE" KingStation-phoenix.exe KingStation-phoenix.cfg
   fi
   ...
   mv -v "$ZIP_BASE" ..
   zip "../$ZIP_BASE" *.dll KingStation-redist-version || die "Failed to build full/redist ..."
...
if [ "$BUILD_32BIT" = yes ]; then
   ...
   do_build "KingStation-w32" "KingStation-win32-libs.zip" "x86"
fi
...
if [ "$BUILD_64BIT" = yes ]; then
   ...
   do_build "KingStation-w64" "KingStation-win64-libs.zip" "x86_64"
fi
