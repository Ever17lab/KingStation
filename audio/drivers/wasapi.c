...
static IMMDevice *wasapi_init_device(const char *id)
{
   ...
   if (id)
   {
      KINGSN_LOG("[WASAPI]: Initializing device %s ...\n", id);
   }
   else
   {
      KINGSN_LOG("[WASAPI]: Initializing default device.. \n");
   }
      ...
      if (list)
      {
         if (list->elems)
         {
            ...
            if (idx_found == -1 && isdigit(id[0]))
            {
               ...
               KINGSN_LOG("[WASAPI]: Fallback, device index is a single number index instead: %d.\n", idx_found);
...
error:
   ...
   if (id)
   {
      KINGSN_WARN("[WASAPI]: Failed to initialize device.\n");
   }
   else
   {
      KINGSN_ERR("[WASAPI]: Failed to initialize device.\n");
   }
      ...
      bool *float_fmt, unsigned *rate, unsigned latency)
{
   ...
   for (i = 0; i < 2; ++i)
   {
      ...
      for (j = 0; rate_res; ++j)
      {
         KINGSN_LOG("[WASAPI]: Initializing client (shared, %s, %uHz, %ums) ...\n",
         ...
         KINGSN_WARN("[WASAPI]: Unsupported format.\n");
      ...
      bool *float_fmt, unsigned *rate, unsigned latency)
{
   ...
   for (i = 0; i < 2; ++i)
   {
      ...
      for (j = 0; rate_res; ++j)
      {
         KINGSN_LOG("[WASAPI]: Initializing client (exclusive, %s, %uHz, %ums) ...\n",
               float_fmt_res ? "float" : "pcm", rate_res, latency);
      ...
      bool *float_fmt, unsigned *rate, unsigned latency)
{
   ...
   if (FAILED(hr))
   {
      KINGSN_WARN("[WASAPI]: IAudioClient::GetDevicePeriod failed with error 0x%.8X.\n", hr);
   }
   ...
   if (!*exclusive)
   {
      ...
      if (FAILED(hr))
      {
         KINGSN_WARN("[WASAPI]: IAudioClient::GetStreamLatency failed with error 0x%.8X.\n", hr);
      }
   }
   ...
   if (FAILED(hr))
   {
      KINGSN_WARN("[WASAPI]: IAudioClient::GetBufferSize failed with error 0x%.8X.\n", hr);
   }
   ...
   KINGSN_LOG("[WASAPI]: Client initialized (%s, %s, %uHz, %.1fms).\n",
         *exclusive ? "exclusive" : "shared",
         *float_fmt ? "float" : "pcm", *rate, latency_res);

   KINGSN_LOG("[WASAPI]: Client's buffer length is %u frames (%.1fms).\n",
         buffer_length, (double)buffer_length * 1000.0 / (*rate));

   KINGSN_LOG("[WASAPI]: Device period is %.1fms (%lld frames).\n",
      ...
      unsigned u1, unsigned *u2)
{
   ...
   if (w->exclusive)
   {
      ...
      KINGSN_LOG("[WASAPI]: Intermediate buffer length is %u frames (%.1fms).\n",
   ...
   {
      KINGSN_LOG("[WASAPI]: Intermediate buffer is off. \n");
   }
...
static void wasapi_free(void *wh)
{
   ...
   if (ir == WAIT_FAILED)
   {
      KINGSN_ERR("[WASAPI]: WaitForSingleObject failed with error %d.\n", GetLastError());
   }
