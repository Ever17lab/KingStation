...
static void alsa_worker_thread(void *data)
{
   ...
   if (!buf)
   {
      KINGSN_ERR("failed to allocate audio buffer");
      goto end;
   }
   ...
   while (!alsa->thread_dead)
   {
            ...
            frames == -ESTRPIPE)
      {
         ...
         {
            KINGSN_ERR("[ALSA]: (#2) Failed to recover from error (%s)\n",
                  snd_strerror(frames));
            break;
         }
      ...
      {
         KINGSN_ERR("[ALSA]: Unknown error occurred (%s).\n",
               snd_strerror(frames));
         break;
      }
   }
      ...
      snd_pcm_hw_params_t *params)
{
   ...
   {
      KINGSN_LOG("ALSA: Using floating point format.\n");
      return true;
   }
   KINGSN_LOG("ALSA: Using signed 16-bit format.\n");
   return false;
}
      ...
      unsigned *new_rate)
{
   ...
   if (!alsa->worker_thread)
   {
      KINGSN_ERR("error initializing worker thread");
      goto error;
   }
...
error:
   KINGSN_ERR("ALSA: Failed to initialize...\n");
   if (params)
      snd_pcm_hw_params_free(params);
