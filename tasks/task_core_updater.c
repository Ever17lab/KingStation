...
#if defined(KINGSN_INTERNAL) && defined(HAVE_MENU)
...
typedef struct core_updater_download_handle
{
   ...
   char *display_name;
   ks_task_t *http_task;
   ks_task_t *decompress_task;
   ks_task_t *backup_task;
...
{
   char *path_dir_libks;
   char *path_libks_info;
   char *error_msg;
   core_updater_list_t* core_list;
   ks_task_t *install_task;
   size_t list_size;
   size_t list_index;
   size_t installed_index;
...
static uint32_t task_core_updater_get_core_crc(const char *core_path)
{
   ...
   if (path_is_valid(core_path))
   {
      ...
      intfstream_t *core_file = intfstream_open_file(
            core_path, KS_VFS_FILE_ACCESS_READ,
...
finish:
   ...
   if (!success)
      KINGSN_ERR("[core updater] Download of core list '%s' failed: %s\n",
...
static void task_core_updater_get_list_handler(ks_task_t *task)
{
   core_updater_list_handle_t *list_handle = NULL;
      ...
      case CORE_UPDATER_LIST_BEGIN:
         {
            ...
            list_handle->http_task = (ks_task_t*)task_push_http_transfer_file(
...
static bool task_core_updater_get_list_finder(ks_task_t *task, void *user_data)
{
   core_updater_list_handle_t *list_handle = NULL;
      ...
      void *user_data, const char *err)
{
   ...
   if (path_is_compressed_file(transf->path))
   {
      download_handle->decompress_task = (ks_task_t*)task_push_decompress(
...
finish:
   ...
   /* Log any error messages */
   if (!string_is_empty(err))
      KINGSN_ERR("[core updater] Download of '%s' failed: %s\n",
...
static void free_core_updater_download_handle(core_updater_download_handle_t *download_handle)
{
   ...
   if (download_handle->path_dir_libks)
      free(download_handle->path_dir_libks);
...
static void task_core_updater_download_handler(ks_task_t *task)
{
   core_updater_download_handle_t *download_handle = NULL;
   ...
   switch (download_handle->status)
   {
      ...
      case CORE_UPDATER_DOWNLOAD_START_BACKUP:
         {
            /* Request core backup */
            download_handle->backup_task = (ks_task_t*)task_push_core_backup(
                  download_handle->local_core_path,
            ...
            else
            {
                ...
                *   download */
               KINGSN_ERR("[core updater] Failed to backup core: %s\n",
                     download_handle->local_core_path);
      ...
      case CORE_UPDATER_DOWNLOAD_START_TRANSFER:
         {
            ...
            /* Push HTTP transfer task */
            download_handle->http_task = (ks_task_t*)task_push_http_transfer_file(
...
static bool task_core_updater_download_finder(ks_task_t *task, void *user_data)
{
   core_updater_download_handle_t *download_handle = NULL;
      ...
      const char *path_dir_core_assets)
{
   ...
   {
      KINGSN_ERR("[core updater] Update disabled - core is locked: %s\n",
   ...
   if (string_is_empty(path_dir_libks))
      goto error;
   ...
   download_handle->path_dir_libks        = strdup(path_dir_libks);
      ...
      update_installed_cores_handle_t *update_installed_handle)
{
   ...
   if (update_installed_handle->path_dir_libks)
      free(update_installed_handle->path_dir_libks);
...
static void task_update_installed_cores_handler(ks_task_t *task)
{
   ...
   switch (update_installed_handle->status)
   {
      case UPDATE_INSTALLED_CORES_BEGIN:
         ...
         update_installed_handle->list_task = (ks_task_t*)
      ...
      case UPDATE_INSTALLED_CORES_UPDATE_CORE:
         {
            ...
            if (core_info_get_core_lock(list_entry->local_core_path, false))
            {
               KINGSN_LOG("[core updater] Skipping locked core: %s\n",
             ...
             * > Request download */
            update_installed_handle->download_task = (ks_task_t*)
                  task_push_core_updater_download(
                        ...
                        update_installed_handle->path_dir_libks,
...
static bool task_update_installed_cores_finder(ks_task_t *task, void *user_data)
{
   if (!task)
      return false;
...
static void task_play_feature_delivery_core_install_handler(ks_task_t *task)
{
      ...
      bool mute)
{
   ...
   if (kingsn_ctl(KINGSN_CTL_IS_CORE_LOADED, (void*)list_entry->local_core_path))
      ...
      play_feature_delivery_switch_cores_handle_t *pfd_switch_cores_handle)
{
   ...
   if (pfd_switch_cores_handle->path_dir_libks)
      free(pfd_switch_cores_handle->path_dir_libks);

   if (pfd_switch_cores_handle->path_libks_info)
      free(pfd_switch_cores_handle->path_libks_info);
...
static void task_play_feature_delivery_switch_cores_handler(ks_task_t *task)
{
      ...
      case PLAY_FEATURE_DELIVERY_SWITCH_CORES_BEGIN:
         {
            ...
            success = core_updater_list_parse_pfd_data(
                  ...
                  pfd_switch_cores_handle->path_dir_libks,
                  pfd_switch_cores_handle->path_libks_info,
                  available_cores);
      ...
      case PLAY_FEATURE_DELIVERY_SWITCH_CORES_INSTALL_CORE:
         {
            ...
            pfd_switch_cores_handle->install_task = (ks_task_t*)
      ...
      const char *path_libks_info)
{
   ...
   /* Sanity check */
   if (string_is_empty(path_dir_libks) ||
       string_is_empty(path_libks_info) ||
   ...
   /* Configure handle */
   pfd_switch_cores_handle->path_dir_libks  = strdup(path_dir_libks);
   pfd_switch_cores_handle->path_libks_info = strdup(path_libks_info);
