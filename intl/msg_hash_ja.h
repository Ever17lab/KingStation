...
MSG_HASH(
   ...
   "キーが押されている間, 現在のコンテンツを巻き戻します. 注意: [巻き戻しサポート[を有効にする必要があります."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "不揮発性セーブファイルをアーカイブ形式で書き込みます. 保存/読み込み時間の増加を犠牲にして, ファイルサイズを大幅に削減します.  注:標準のlibksセーブインタフェースを介して保存をするコアにのみ適用されます. "
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Libks buildbot上のコアアップデータディレクトリを示すURL。"
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Libks buildbotのアセットアップデータディレクトリを示すURL。"
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "｢最終プレイ日時｣情報を表示する日付と時刻のスタイルを指定します. 注意: ｢(午前/午後)｣オプションは, 一部のプラットフォームではパフォーマンスに少し影響します."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "RetroAchievements アカウントのユーザー名を入力してください."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "仮想ディスクトレイを開き、現在ロードされているディスクを取り出します.  [メニューがアクティブなときにコンテンツを一時停止]が有効な場合, いくつかのコアでは, ディスク操作の後に数秒間おいてコンテンツを再開しないと、変更が反映されないかもしれません."
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "Libksコアはコンテンツを必要としますが, 何も提供されませんでした."
   )
MSG_HASH(
   MSG_ERROR_LIBKS_CORE_REQUIRES_SPECIAL_CONTENT,
   "Libks コアは特別なコンテンツを必要としていますが, それらが供給されていません。"
   )
MSG_HASH(
...
MSG_HASH(
   ...
   "スキャンが成功しました。<br><br>\nコンテンツを正確にスキャンするには:\n<ul><li>互換機コアがもうダウンロードされた</li>\n<li>オンラインアップデータで｢コア情報｣がもうアップデートされた</li>\n<li>オンラインアップデータで｢データベース｣がもうアップデートされた</li>\n<li>上記タスクの後にKingStationを再起動してください。</li></ul>\nコンテンツが<a href=\"https://docs.libks.com/guides/roms-playlists-thumbnails/#sources\">存在データベース</a>の一致が必要です。\nまだちゃんと動けないとバグを<a href=\"https://www.github.com/libks/KingStation/issues\">報告</a>できます。"
   )
#endif
