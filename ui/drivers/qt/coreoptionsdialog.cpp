...
void CoreOptionsDialog::onCoreOptionComboBoxCurrentIndexChanged(int index)
{
   ...
   {
      kingsn_ctl(KINGSN_CTL_GET_CORE_OPTION_SIZE, &opts);
...
void CoreOptionsDialog::buildLayout()
{
   ...
   bool has_core_options = kingsn_ctl(KINGSN_CTL_HAS_CORE_OPTIONS, NULL);
   ...
   if (has_core_options)
   {
      ...
      if (opts)
      {
         ...
         if (settings->bools.game_specific_options)
         {
            ...
            if (system)
               contentLabel = QFileInfo(path_get(KINGSN_PATH_BASENAME)).completeBaseName();
