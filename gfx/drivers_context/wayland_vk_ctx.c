...
static bool gfx_ctx_wl_set_resize(void *data, unsigned width, unsigned height)
{
   ...
   {
      KINGSN_ERR("[Wayland/Vulkan]: Failed to update swapchain.\n");
      return false;
   }
...
static void *gfx_ctx_wl_init(void *video_driver)
{
   ...
   if (!wl->input.dpy)
   {
      KINGSN_ERR("[Wayland]: Failed to connect to Wayland server.\n");
      goto error;
   }
   ...
   if (!wl->compositor)
   {
      KINGSN_ERR("[Wayland]: Failed to create compositor.\n");
      goto error;
   }

   if (!wl->shm)
   {
      KINGSN_ERR("[Wayland]: Failed to create shm.\n");
      goto error;
   }

   if (!wl->xdg_shell && !!wl->zxdg_shell)
   {
      KINGSN_LOG("[Wayland]: Using zxdg_shell_v6 interface.\n");
   }

   if (!wl->xdg_shell && !wl->zxdg_shell)
   {
	   KINGSN_ERR("[Wayland]: Failed to create shell.\n");
	   goto error;
   }

   if (!wl->idle_inhibit_manager)
   {
	   KINGSN_WARN("[Wayland]: Compositor doesn't support zwp_idle_inhibit_manager_v1 protocol!\n");
   }

   if (!wl->deco_manager)
   {
	   KINGSN_WARN("[Wayland]: Compositor doesn't support zxdg_decoration_manager_v1 protocol!\n");
   }
...
static bool gfx_ctx_wl_suppress_screensaver(void *data, bool state)
{
    ...
    if (state)
    {
        KINGSN_LOG("[Wayland]: Enabling idle inhibitor\n");
        struct zwp_idle_inhibit_manager_v1 *mgr = wl->idle_inhibit_manager;
        wl->idle_inhibitor = zwp_idle_inhibit_manager_v1_create_inhibitor(mgr, wl->surface);
    }
    else
    {
        KINGSN_LOG("[Wayland]: Disabling the idle inhibitor\n");
        zwp_idle_inhibitor_v1_destroy(wl->idle_inhibitor);
