...
#define OSMESA_BPP            4
#define OSMESA_FIFO_PATH      "/tmp/osmesa-KingStation.sock"
...
static void osmesa_fifo_open(gfx_ctx_osmesa_data_t *osmesa)
{
   ...
   KINGSN_ERR("[osmesa] Frame size is %ix%ix%i\n",
         osmesa->width, osmesa->height, osmesa->pixsize);
   KINGSN_ERR("[osmesa] Please connect to unix:%s\n",
         OSMESA_FIFO_PATH);
}
...
static void osmesa_fifo_accept(gfx_ctx_osmesa_data_t *osmesa)
{
   ...
   else if (res > 0)
   {
      ...
      KINGSN_LOG("[osmesa] Client %i connected.\n", osmesa->client);
   }
}
...
static void osmesa_fifo_write(gfx_ctx_osmesa_data_t *osmesa)
{
   ...
   for (i = osmesa->height -1; i >= 0; --i)
   {
      ...
      if (res < 0)
      {
         KINGSN_LOG("[osmesa] Lost connection to %i: %s\n", osmesa->client, strerror(errno));
         close(osmesa->client);
         osmesa->client = -1;
         break;
      }
...
static void *osmesa_ctx_init(void *video_driver)
{
   ...
   if (!osmesa->ctx)
   {
      ...
      KINGSN_WARN("[osmesa]: Falling back to standard context creation.\n");
#endif
...
error:
   if (osmesa)
      free(osmesa);
   KINGSN_WARN("[omesa]: Failed to initialize the context driver.\n");
   return NULL;
}
...
static void osmesa_ctx_destroy(void *data)
{
   ...
   unlink("/tmp/KingStation-osmesa.fifo");
