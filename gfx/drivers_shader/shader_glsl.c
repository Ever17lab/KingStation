      ...
      const char *define, const char *program)
{
   ...
   if (existing_version)
   {
      ...
      KINGSN_LOG("[GLSL]: Using GLSL version %u%s.\n", version_no, version_extra);
   }
   else if (glsl_core)
   {
      ...
      KINGSN_LOG("[GLSL]: Using GLSL version %u.\n", version_no);
   }
      ...
      struct shader_program_info *program_info)
{
   ...
   {
      KINGSN_LOG("[GLSL]: Found GLSL vertex shader.\n");
      ...
      {
         KINGSN_ERR("Failed to compile vertex shader #%u\n", idx);
         goto error;
      }
   ...
   {
      KINGSN_LOG("[GLSL]: Linking GLSL program.\n");
...
error:
   KINGSN_ERR("Failed to link program #%u.\n", idx);
   program->id = 0;
   return false;
}
      ...
      glsl_shader_data_t *glsl, struct shader_program_glsl_data *program)
{
   ...
   for (i = 0; i < glsl->shader->passes; i++)
   {
      ...
      {
         KINGSN_ERR("Failed to load GLSL shader: %s.\n",
               pass->source.path);
         return false;
      }
      ...
      if (!gl_glsl_compile_program(glsl, i,
            ...
            &shader_prog_info))
      {
         KINGSN_ERR("Failed to create GL program #%u.\n", i);
         return false;
      }
   }
...
static void gl_glsl_reset_attrib(glsl_shader_data_t *glsl)
{
   ...
   ks_assert(glsl->attribs_index <= ARRAY_SIZE(glsl->attribs_elems));
      ...
      const struct glsl_attrib *attrs, size_t num_attrs)
{
   ...
   for (i = 0; i < num_attrs; i++)
   {
      if (glsl->attribs_index < ARRAY_SIZE(glsl->attribs_elems))
      {
         ...
         KINGSN_WARN("Attrib array buffer was overflown!\n");
   }
...
static void gl_glsl_init_menu_shaders(void *data)
{
   ...
   KINGSN_LOG("[GLSL]: Compiling ribbon shader..\n");
   ...
   KINGSN_LOG("[GLSL]: Compiling simple ribbon shader..\n");
   gl_glsl_compile_program(
         glsl,
         VIDEO_SHADER_MENU_2,
   ...
   KINGSN_LOG("[GLSL]: Compiling snow shader..\n");
   gl_glsl_compile_program(
         glsl,
         VIDEO_SHADER_MENU_3,
   ...
   KINGSN_LOG("[GLSL]: Compiling modern snow shader..\n");
   gl_glsl_compile_program(
         glsl,
         VIDEO_SHADER_MENU_4,
   ...
   KINGSN_LOG("[GLSL]: Compiling bokeh shader..\n");
   gl_glsl_compile_program(
         glsl,
         VIDEO_SHADER_MENU_5,
   ...
   KINGSN_LOG("[GLSL]: Compiling snowflake shader..\n");
   gl_glsl_compile_program(
         glsl,
         VIDEO_SHADER_MENU_6,
...
static void *gl_glsl_init(void *data, const char *path)
{
   ...
   KINGSN_LOG("[GLSL]: Checking GLSL shader support ...\n");
   ...
   if (!shader_support)
   {
      KINGSN_ERR("GLSL shaders aren't supported by your OpenGL driver.\n");
      goto error;
   }
#endif
...

   {
      ...
      if (!string_is_empty(path) && type != KINGSN_SHADER_GLSL)
      {
         KINGSN_ERR("[GL]: Invalid shader type, falling back to stock.\n");
         path = NULL;
      }
      ...
      if (!string_is_empty(path))
      {
         ...
         if (!ret)
         {
            KINGSN_ERR("[GL]: Failed to parse GLSL shader.\n");
            goto error;
         }
      }
      else
      {
         KINGSN_WARN("[GL]: Stock GLSL shaders will be used.\n");
         glsl->shader->passes = 1;
#if defined(VITA)
   ...
   {
      KINGSN_ERR("[GL]: GLES context is used, but shader is not modern. Cannot use it.\n");
      goto error;
   }
#else
   if (glsl_core && !glsl->shader->modern)
   {
      KINGSN_ERR("[GL]: GL core context is used, but shader is not core compatible. Cannot use it.\n");
      goto error;
   }
#endif
   ...
   *glsl->alias_define = '\0';
   ...
   {
      KINGSN_ERR("GLSL stock programs failed to compile.\n");
      goto error;
   }
   ...
   {
      KINGSN_ERR("[GL]: Failed to load LUTs.\n");
      goto error;
   }
   ...
   if (!gl_check_error(&error_string))
   {
      KINGSN_ERR("%s\n", error_string);
      free(error_string);
      KINGSN_WARN("Detected GL error in GLSL.\n");
   }
#endif
...
static bool gl_glsl_filter_type(void *data, unsigned idx, bool *smooth)
{
   glsl_shader_data_t *glsl = (glsl_shader_data_t*)data;
   if (glsl && idx
         && (glsl->shader->pass[idx - 1].filter != KINGSN_FILTER_UNSPEC)
      )
   {
      *smooth = (glsl->shader->pass[idx - 1].filter == KINGSN_FILTER_LINEAR);
      return true;
   }
   return false;
}
