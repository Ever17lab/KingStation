      ...
      unsigned font_size)
{
   ...
   {
      KINGSN_WARN("[SDL]: Could not initialize fonts.\n");
      return;
   }
   ...
   if (vid->font.tex)
   {
      ...
      KINGSN_WARN("[SDL]: Failed to initialize font texture: %s\n", SDL_GetError());
...
static void sdl2_init_renderer(sdl2_video_t *vid)
{
   ...
   if (!vid->renderer)
   {
      KINGSN_ERR("[SDL2]: Failed to initialize renderer: %s", SDL_GetError());
      return;
   }
      ...
      unsigned width, unsigned height, unsigned pitch)
{
       ...
       || target->rgb32 != rgb32 || target->pitch != pitch)
   {
      ...
      if (!target->tex)
      {
         KINGSN_ERR("[SDL2]: Failed to create %s texture: %s\n", menu ? "menu" : "main",
                   SDL_GetError());
         return;
      }
      ...
      input_driver_t **input, void **input_data)
{
   ...
   KINGSN_LOG("[SDL2]: Available renderers (change with $SDL_RENDER_DRIVER):\n");
   ...
   KINGSN_LOG("[SDL2]: Available displays:\n");
   for (i = 0; i < SDL_GetNumVideoDisplays(); ++i)
   {
      SDL_DisplayMode mode;

      if (SDL_GetCurrentDisplayMode(i, &mode) < 0)
         KINGSN_LOG("\tDisplay #%i mode: unknown.\n", i);
      else
         KINGSN_LOG("\tDisplay #%i mode: %ix%i@%ihz.\n", i, mode.w, mode.h,
                   mode.refresh_rate);
   }

   if (!video->fullscreen)
      KINGSN_LOG("[SDL]: Creating window @ %ux%u\n", video->width, video->height);
   ...
   if (!vid->window)
   {
      KINGSN_ERR("[SDL2]: Failed to init SDL window: %s\n", SDL_GetError());
      goto error;
   }
...
#if defined(_WIN32)
   sdl2_set_handles(vid->window, KINGSN_DISPLAY_WIN32);
#elif defined(HAVE_COCOA)
   sdl2_set_handles(vid->window, KINGSN_DISPLAY_OSX);
#else
   ...
   if (strcmp(video_driver, "x11") == 0)
      sdl2_set_handles(vid->window, KINGSN_DISPLAY_X11);
   else
#endif
#ifdef HAVE_WAYLAND
   if (strcmp(video_driver, "wayland") == 0)
      sdl2_set_handles(vid->window, KINGSN_DISPLAY_WAYLAND);
   else
#endif
      sdl2_set_handles(vid->window, KINGSN_DISPLAY_NONE);
#endif
...
static bool sdl2_gfx_read_viewport(void *data, uint8_t *buffer, bool is_idle)
{
   ...
   if (!bgr24)
   {
      KINGSN_WARN("Failed to convert viewport data to BGR24: %s", SDL_GetError());
      return false;
   }
