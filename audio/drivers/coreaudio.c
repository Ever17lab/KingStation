      ...
      unsigned *new_rate)
{
   ...
   KINGSN_LOG("[CoreAudio]: Using output sample rate of %.1f Hz\n",
   ...
   *new_rate = real_desc.mSampleRate;
   ...
   KINGSN_LOG("[CoreAudio]: Using buffer size of %u bytes: (latency = %u ms)\n",
...
error:
   KINGSN_ERR("[CoreAudio]: Failed to initialize driver ...\n");
   coreaudio_free(dev);
   return NULL;
}
