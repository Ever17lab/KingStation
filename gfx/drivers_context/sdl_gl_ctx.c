...
static void *sdl_ctx_init(void *video_driver)
{
   ...
   KINGSN_LOG("[SDL_GL] SDL %i.%i.%i gfx context driver initialized.\n",
         SDL_MAJOR_VERSION, SDL_MINOR_VERSION, SDL_PATCHLEVEL);

   return sdl;

error:
   KINGSN_WARN("[SDL_GL]: Failed to initialize SDL gfx context driver: %s\n",
         SDL_GetError());
...
#ifdef HAVE_SDL2
#if defined(_WIN32)
   sdl2_set_handles(sdl->win, KINGSN_DISPLAY_WIN32);
#elif defined(HAVE_X11)
   sdl2_set_handles(sdl->win, KINGSN_DISPLAY_X11);
#elif defined(HAVE_COCOA)
   sdl2_set_handles(sdl->win, KINGSN_DISPLAY_OSX);
#endif
...
error:
   KINGSN_WARN("[SDL_GL]: Failed to set video mode: %s\n", SDL_GetError());
   return false;
}
   ...
   *height = sdl->height;
   ...
   if (!sdl->win)
   {
      ...
      if (SDL_GetCurrentDisplayMode(i, &mode) < 0)
         KINGSN_WARN("[SDL_GL]: Failed to get display #%i mode: %s\n", i,
                    SDL_GetError());
#else
      ...
      if (!modes)
         KINGSN_WARN("[SDL_GL]: Failed to detect available video modes: %s\n",
                    SDL_GetError());
      else if (*modes)
         mode = **modes;
#endif
