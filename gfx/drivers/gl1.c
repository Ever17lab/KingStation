      ...
      input_driver_t **input, void **input_data)
{
   ...
   *input_data                          = NULL;
   ...
   KINGSN_LOG("[GL1]: Found GL1 context: %s\n", ctx_driver->ident);
   ...
   KINGSN_LOG("[GL1]: Detecting screen resolution %ux%u.\n", full_x, full_y);
   ...
   KINGSN_LOG("[GL1]: Using resolution %ux%u\n", temp_width, temp_height);
   ...
   KINGSN_LOG("[GL1]: Vendor: %s, Renderer: %s.\n", vendor, renderer);
   KINGSN_LOG("[GL1]: Version: %s.\n", version);
   KINGSN_LOG("[GL1]: Extensions: %s\n", extensions);
      ...
      unsigned pitch, const char *msg, video_frame_info_t *video_info)
{
   ...
   if (  !frame || frame == KS_HW_FRAME_BUFFER_VALID || (
         frame_width  == 4 &&
         frame_height == 4 &&
...
static unsigned gl1_wrap_type_to_enum(enum gfx_wrap_type type)
{
   switch (type)
   {
      ...
      case KINGSN_WRAP_MIRRORED_REPEAT: /* mirrored not actually supported */
         return GL_REPEAT;
      default:
         return GL_CLAMP;
   }
      ...
      const void *frame, unsigned base_size)
{
   ...
   glTexImage2D(GL_TEXTURE_2D,
         0,
         (use_rgba || !rgb32) ? GL_RGBA : KINGSN_GL1_INTERNAL_FORMAT32,
         width, height, 0,
         (use_rgba || !rgb32) ? GL_RGBA : KINGSN_GL1_TEXTURE_TYPE32,
         (rgb32) ? KINGSN_GL1_FORMAT32 : GL_UNSIGNED_BYTE, frame);
}
