      ...
      if (list && list->elems)
      {
         if (list->elems)
         {
            ...
            if (idx_found == -1)
            {
               if (isdigit(device[0]))
               {
                  KINGSN_LOG("[XAudio2]: Fallback, device index is a single number index instead: %d.\n", idx_found);
      ...
      unsigned *new_rate)
{
   ...
   if (!xa->xa)
   {
      KINGSN_ERR("[XAudio2] Failed to init driver.\n");
      free(xa);
      return NULL;
   }

   KINGSN_LOG("[XAudio2]: Requesting %u ms latency, using %d ms latency.\n",
