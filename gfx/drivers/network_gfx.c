      ...
      input_driver_t **input, void **input_data)
{
   ...
   *input_data                          = NULL;
   ...
   KINGSN_LOG("[network] Connecting to host %s:%d\n", network->address, network->port);
try_connect:
   ...
   if (network->fd > 0)
      KINGSN_LOG("[network]: Connected to host.\n");
   else
   {
      KINGSN_LOG("[network]: Could not connect to host, retrying...\n");
      ks_sleep(1000);
      goto try_connect;
   }

   KINGSN_LOG("[network]: Init complete.\n");
