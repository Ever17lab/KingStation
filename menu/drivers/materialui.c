...
static void materialui_context_reset_textures(materialui_handle_t *mui)
{
   ...
   for (i = 0; i < MUI_TEXTURE_LAST; i++)
   {
      if (!gfx_display_reset_textures_list(
            ...
            TEXTURE_FILTER_MIPMAP_LINEAR, NULL, NULL))
      {
         KINGSN_WARN("[GLUI] Asset missing: %s%s%s\n", icon_path, 
      ...
      float menu_framebuffer_opacity)
{
   ...
   if (mui->textures.bg && !libks_running)
   {
...
static void materialui_frame(void *data, video_frame_info_t *video_info)
{
   ...
   mui->show_selection_marker_shadow = libks_running &&
   ...
   materialui_render_background(mui, userdata,
         video_width, video_height, libks_running,
      ...
      materialui_handle_t *mui, const char *label, settings_t *settings)
{
   ...
   mui->nav_bar.resume_tab.enabled = !kingsn_ctl(KINGSN_CTL_IS_DUMMY_CORE, NULL);
      ...
      unsigned type)
{
   ...
   if (!node)
   {
      KINGSN_ERR("GLUI node could not be allocated.\n");
      return;
   }
      ...
      switch (type)
      {
         ...
         default:
             ...
             * all this manually inside this menu driver */
                  ...
                  string_is_equal(label, msg_hash_to_str(MENU_ENUM_LABEL_KS_ACHIEVEMENTS_SETTINGS)) ||
                  ...
                  string_is_equal(label, msg_hash_to_str(MENU_ENUM_LABEL_ACCOUNTS_KS_ACHIEVEMENTS)) ||
