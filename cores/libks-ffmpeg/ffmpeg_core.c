...
#define CORE_PREFIX(s) libks_ffmpeg_##s
#else
#define CORE_PREFIX(s) s
#endif

#define PRINT_VERSION(s) log_cb(KS_LOG_INFO, "[FFMPEG] lib%s version:\t%d.%d.%d\n", #s, \
   s ##_version() >> 16 & 0xFF, \
   s ##_version() >> 8 & 0xFF, \
   s ##_version() & 0xFF);

static bool reset_triggered;
static bool libks_supports_bitmasks = false;

static void fallback_log(enum ks_log_level level, const char *fmt, ...)
{
   va_list va;
   va_start(va, fmt);
...
ks_log_printf_t log_cb;
static ks_video_refresh_t CORE_PREFIX(video_cb);
static ks_audio_sample_t CORE_PREFIX(audio_cb);
static ks_audio_sample_batch_t CORE_PREFIX(audio_batch_cb);
static ks_environment_t CORE_PREFIX(environ_cb);
static ks_input_poll_t CORE_PREFIX(input_poll_cb);
static ks_input_state_t CORE_PREFIX(input_state_cb);
...
static bool use_gl;
static struct ks_hw_render_callback hw_render;
static GLuint prog;
static GLuint vbo;
...
void CORE_PREFIX(ks_init)(void)
{
   reset_triggered = false;

   av_register_all();

   if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_INPUT_BITMASKS, NULL))
      libks_supports_bitmasks = true;
}

void CORE_PREFIX(ks_deinit)(void)
{
   libks_supports_bitmasks = false;
...
unsigned CORE_PREFIX(ks_api_version)(void)
{
   return KS_API_VERSION;
}

void CORE_PREFIX(ks_set_controller_port_device)(unsigned port, unsigned device)
{
   (void)port;
   (void)device;
}

void CORE_PREFIX(ks_get_system_info)(struct ks_system_info *info)
{
...
void CORE_PREFIX(ks_get_system_av_info)(struct ks_system_av_info *info)
{
...
void CORE_PREFIX(ks_set_environment)(ks_environment_t cb)
{
   static const struct ks_variable vars[] = {
#if ENABLE_HW_ACCEL
   ...
   struct ks_log_callback log;

   CORE_PREFIX(environ_cb) = cb;

   cb(KS_ENVIRONMENT_SET_VARIABLES, (void*)vars);

   if (cb(KS_ENVIRONMENT_GET_LOG_INTERFACE, &log))
      log_cb = log.log;
   else
      log_cb = fallback_log;
}

void CORE_PREFIX(ks_set_audio_sample)(ks_audio_sample_t cb)
{
   CORE_PREFIX(audio_cb) = cb;
}

void CORE_PREFIX(ks_set_audio_sample_batch)(ks_audio_sample_batch_t cb)
{
   CORE_PREFIX(audio_batch_cb) = cb;
}

void CORE_PREFIX(ks_set_input_poll)(ks_input_poll_t cb)
{
   CORE_PREFIX(input_poll_cb) = cb;
}

void CORE_PREFIX(ks_set_input_state)(ks_input_state_t cb)
{
   CORE_PREFIX(input_state_cb) = cb;
}

void CORE_PREFIX(ks_set_video_refresh)(ks_video_refresh_t cb)
{
   CORE_PREFIX(video_cb) = cb;
}

void CORE_PREFIX(ks_reset)(void)
{
...
{
   struct ks_variable hw_var  = {0};
   struct ks_variable sw_threads_var = {0};
   struct ks_variable color_var  = {0};
#if defined(HAVE_OPENGL) || defined(HAVE_OPENGLES)
   struct ks_variable var        = {0};
#endif
#ifdef HAVE_GL_FFT
   struct ks_variable fft_var    = {0};
   struct ks_variable fft_ms_var = {0};
#endif

#if defined(HAVE_OPENGL) || defined(HAVE_OPENGLES)
   var.key = "ffmpeg_temporal_interp";

   if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
   ...
   fft_height      = 720;
   fft_multisample = 1;
   if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &fft_var) && fft_var.value)
   {
      unsigned w, h;
   ...
   if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &fft_ms_var) && fft_ms_var.value)
   ...
   if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &color_var) && color_var.value)
   {
   ...
   if (firststart)
   {
      ...
      if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &hw_var) && hw_var.value)
      {
   ...
   if (firststart)
   {
      ...
      if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE, &sw_threads_var) && sw_threads_var.value)
      {
...
{
   char msg[256];
   struct ks_message_ext msg_obj = {0};
   ...
   msg_obj.type     = KS_MESSAGE_TYPE_PROGRESS;
   msg_obj.progress = seek_progress;
   CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_MESSAGE_EXT, &msg_obj);

   if (seek_frames_capped < 0)
   {
      log_cb(KS_LOG_INFO, "[FFMPEG] Resetting PTS.\n");
      frames[0].pts = 0.0;
      frames[1].pts = 0.0;
   }
...
void CORE_PREFIX(ks_run)(void)
{
   ...
   if (CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_GET_VARIABLE_UPDATE, &updated) && updated)
   ...
   if (fft_width != old_fft_width || fft_height != old_fft_height)
   {
      ...
      CORE_PREFIX(ks_get_system_av_info)(&info);
      if (!CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_SYSTEM_AV_INFO, &info))
      {
   ...
   if (libks_supports_bitmasks)
      ret = CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD,
            0, KS_DEVICE_ID_JOYPAD_MASK);
   else
   {
      unsigned i;
      for (i = KS_DEVICE_ID_JOYPAD_B; i <= KS_DEVICE_ID_JOYPAD_R; i++)
         if (CORE_PREFIX(input_state_cb)(0, KS_DEVICE_JOYPAD, 0, i))
            ret |= (1 << i);
   }
   ...
   up    = ret & (1 << KS_DEVICE_ID_JOYPAD_UP);
   down  = ret & (1 << KS_DEVICE_ID_JOYPAD_DOWN);
   l     = ret & (1 << KS_DEVICE_ID_JOYPAD_L);
   ...
   if (l && !last_l && audio_streams_num > 0)
   {
      ...
      msg_obj.type     = KS_MESSAGE_TYPE_NOTIFICATION;
      msg_obj.progress = -1;
      CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_MESSAGE_EXT, &msg_obj);
   }
   ...
   {
      CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SHUTDOWN, NULL);
      return;
   }
   ...
   if (audio_streams_num > 0)
   {
      ...
      {
         log_cb(KS_LOG_INFO, "[FFMPEG] Resetting PTS (bias).\n");
         frames[0].pts = 0.0;
         frames[1].pts = 0.0;
      }
   ...
   if (video_stream_index >= 0)
   {
      ...
      if (use_gl)
      {
         ...
         CORE_PREFIX(video_cb)(KS_HW_FRAME_BUFFER_VALID,
                                    ...
                                    const enum AVPixelFormat *pix_fmts)
{
   ...
   for (int i = 0;; i++)
   {
      ...
      if (!config)
      {
         log_cb(KS_LOG_ERROR, "[FFMPEG] Codec %s is not supported by HW video decoder %s.\n",
      ...
      {
         log_cb(KS_LOG_INFO, "[FFMPEG] Selected HW decoder %s.\n",
                  av_hwdevice_get_type_name(type));
         log_cb(KS_LOG_INFO, "[FFMPEG] Selected HW pixel format %s.\n",
         ...
         if (pix_fmts != NULL)
         {
            ...
            log_cb(KS_LOG_ERROR, "[FFMPEG] Codec %s does not support device pixel format %s.\n",
...
exit:
   if (decoder_pix_fmt != AV_PIX_FMT_NONE)
   {
      ...
      {
#ifdef __cplusplus
         log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to create specified HW device: %d\n", ret);
#else
         log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to create specified HW device: %s\n", av_err2str(ret));
#endif
                                    ...
                                    const enum AVPixelFormat *pix_fmts)
{
   ...
   if (format == AV_PIX_FMT_NONE)
   {
      ...
      log_cb(KS_LOG_INFO, "[FFMPEG] Using SW decoding.\n");

      ctx->thread_type = FF_THREAD_FRAME;
      ctx->thread_count = sw_decoder_threads;
      log_cb(KS_LOG_INFO, "[FFMPEG] Configured software decoding threads: %d\n", sw_decoder_threads);
...
static bool open_codec(AVCodecContext **ctx, enum AVMediaType type, unsigned index)
{
   ...
   if (!codec)
   {
      log_cb(KS_LOG_ERROR, "[FFMPEG] Couldn't find suitable decoder\n");
      return false;
   }
   ...
   *ctx = fctx->streams[index]->codec;
   ...
   {
#ifdef __cplusplus
      log_cb(KS_LOG_ERROR, "[FFMPEG] Could not open codec: %d\n", ret);
#else
      log_cb(KS_LOG_ERROR, "[FFMPEG] Could not open codec: %s\n", av_err2str(ret));
#endif
      return false;
   }
...
static bool init_media_info(void)
{
   ...
   if (fctx)
   {
      ...
      else
      {
         ...
         log_cb(KS_LOG_ERROR, "[FFMPEG] Could not determine media duration\n");
      }
   }
...
static void sws_worker_thread(void *arg)
{
   ...
   {
#ifdef __cplusplus
      log_cb(KS_LOG_ERROR, "[FFMPEG] Error while scaling image: %d\n", ret);
#else
      log_cb(KS_LOG_ERROR, "[FFMPEG] Error while scaling image: %s\n", av_err2str(ret));
#endif
   }
...
#endif
{
   ...
   while (!decode_thread_dead && !video_buffer_has_open_slot(video_buffer))
   {
      ...
      {
         if (!do_seek)
            log_cb(KS_LOG_ERROR, "[FFMPEG] Thread: Video deadlock detected.\n");
         tpool_wait(tpool);
   ...
   {
#ifdef __cplusplus
      log_cb(KS_LOG_ERROR, "[FFMPEG] Can't decode video packet: %d\n", ret);
#else
      log_cb(KS_LOG_ERROR, "[FFMPEG] Can't decode video packet: %s\n", av_err2str(ret));
#endif
      return;
   }
   ...
   while (!decode_thread_dead && video_buffer_has_open_slot(video_buffer))
   {
      ...
      {
#ifdef __cplusplus
         log_cb(KS_LOG_ERROR, "[FFMPEG] Error while reading video frame: %d\n", ret);
#else
         log_cb(KS_LOG_ERROR, "[FFMPEG] Error while reading video frame: %s\n", av_err2str(ret));
#endif
         goto end;
      }
      ...
      if (hw_decoding_enabled)
         ...
         {
#ifdef __cplusplus
               log_cb(KS_LOG_ERROR, "[FFMPEG] Error transferring the data to system memory: %d\n", ret);
#else
               log_cb(KS_LOG_ERROR, "[FFMPEG] Error transferring the data to system memory: %s\n", av_err2str(ret));
#endif
               goto end;
         }
#endif
      ...
      SwrContext *swr)
{
   ...
   {
#ifdef __cplusplus
      log_cb(KS_LOG_ERROR, "[FFMPEG] Can't decode audio packet: %d\n", ret);
#else
      log_cb(KS_LOG_ERROR, "[FFMPEG] Can't decode audio packet: %s\n", av_err2str(ret));
#endif
      return buffer;
   }
   ...
   for (;;)
   {
      ...
      {
#ifdef __cplusplus
         log_cb(KS_LOG_ERROR, "[FFMPEG] Error while reading audio frame: %d\n", ret);
#else
         log_cb(KS_LOG_ERROR, "[FFMPEG] Error while reading audio frame: %s\n", av_err2str(ret));
#endif
         break;
      }
            ...
            FIFO_WRITE_AVAIL(audio_decode_fifo) < required_buffer)
      {
         ...
         {
            log_cb(KS_LOG_ERROR, "[FFMPEG] Thread: Audio deadlock detected.\n");
            fifo_clear(audio_decode_fifo);
            break;
         }
      }
...
static void decode_thread_seek(double time)
{
   ...
   if(avformat_seek_file(fctx, -1, INT64_MIN, seek_to, INT64_MAX, 0) < 0)
      log_cb(KS_LOG_ERROR, "[FFMPEG] av_seek_frame() failed.\n");
...
static void decode_thread(void *data)
{
   ...
   if (video_stream_index >= 0)
   {
      ...
      log_cb(KS_LOG_INFO, "[FFMPEG] Configured worker threads: %d\n", sw_sws_threads);
   }
   ...
   while (!decode_thread_dead)
   {
      ...
      else if (pkt->stream_index == subtitle_stream && sctx_active)
      {
         ...
         while (!finished)
         {
            ...
            {
               log_cb(KS_LOG_ERROR, "[FFMPEG] Decode subtitles failed.\n");
               break;
            }
         }
#ifdef HAVE_SSA
...
void CORE_PREFIX(ks_unload_game)(void)
{
   unsigned i;
...
bool CORE_PREFIX(ks_load_game)(const struct ks_game_info *info)
{
   int ret = 0;
   bool is_fft = false;
   ...
   CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_INPUT_DESCRIPTORS, desc);

   if (!CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_PIXEL_FORMAT, &fmt))
   {
      log_cb(KS_LOG_ERROR, "[FFMPEG] Cannot set pixel format.");
      goto error;
   }
   ...
   {
#ifdef __cplusplus
      log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to open input: %d\n", ret);
#else
      log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to open input: %s\n", av_err2str(ret));
#endif
      goto error;
   }
   ...
   {
#ifdef __cplusplus
      log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to find stream info: %d\n", ret);
#else
      log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to find stream info: %s\n", av_err2str(ret));
#endif
      goto error;
   }

   log_cb(KS_LOG_INFO, "[FFMPEG] Media information:\n");
   av_dump_format(fctx, 0, info->path, 0);

   if (!open_codecs())
   {
      log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to find codec.");
      goto error;
   }

   if (!init_media_info())
   {
      log_cb(KS_LOG_ERROR, "[FFMPEG] Failed to init media info.");
      goto error;
   }
...
#if defined(HAVE_OPENGLES)
      hw_render.context_type = KS_HW_CONTEXT_OPENGLES2;
#else
      hw_render.context_type = KS_HW_CONTEXT_OPENGL;
#endif
      if (!CORE_PREFIX(environ_cb)(KS_ENVIRONMENT_SET_HW_RENDER, &hw_render))
      {
         use_gl = false;
         log_cb(KS_LOG_ERROR, "[FFMPEG] Cannot initialize HW render.\n");
      }
#endif
   }
   if (audio_streams_num > 0)
...
error:
   CORE_PREFIX(ks_unload_game)();
   return false;
}

unsigned CORE_PREFIX(ks_get_region)(void)
{
   return KS_REGION_NTSC;
}

bool CORE_PREFIX(ks_load_game_special)(unsigned type, const struct ks_game_info *info, size_t num)
{
   (void)type;
   (void)info;
   (void)num;
...
size_t CORE_PREFIX(ks_serialize_size)(void)
{
   return 0;
}

bool CORE_PREFIX(ks_serialize)(void *data, size_t size)
{
   (void)data;
   (void)size;
   return false;
...
bool CORE_PREFIX(ks_unserialize)(const void *data, size_t size)
{
   (void)data;
   (void)size;
   return false;
...
void *CORE_PREFIX(ks_get_memory_data)(unsigned id)
{
   (void)id;
   return NULL;
}

size_t CORE_PREFIX(ks_get_memory_size)(unsigned id)
{
   (void)id;
   return 0;
}

void CORE_PREFIX(ks_cheat_reset)(void)
{}

void CORE_PREFIX(ks_cheat_set)(unsigned index, bool enabled, const char *code)
{
   (void)index;
   (void)enabled;
   (void)code;
