...
typedef struct kingsn_CC_resampler
{
   void (*process)(void *re, struct resampler_data *data);
   audio_frame_float_t buffer[4];
   float distance;
} kingsn_CC_resampler_t;
...
{
   kingsn_CC_resampler_t *re     = (kingsn_CC_resampler_t*)re_;
...
size_t resampler_CC_downsample_neon(float *outp, const float *inp,
      kingsn_CC_resampler_t* re_, size_t input_frames, float ratio);
...
{
   int i;
   kingsn_CC_resampler_t *re = (kingsn_CC_resampler_t*)
      memalign_alloc(32, sizeof(kingsn_CC_resampler_t));
