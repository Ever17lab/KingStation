...
#define LIBKS_MESSAGE_FADE_DURATION MSG_QUEUE_ANIMATION_DURATION

/* Widget state */

enum gfx_widget_libks_message_status
{
   GFX_WIDGET_LIBKS_MESSAGE_IDLE = 0,
   GFX_WIDGET_LIBKS_MESSAGE_SLIDE_IN,
   GFX_WIDGET_LIBKS_MESSAGE_FADE_IN,
   GFX_WIDGET_LIBKS_MESSAGE_WAIT,
   GFX_WIDGET_LIBKS_MESSAGE_FADE_OUT
};

struct gfx_widget_libks_message_state
{
   unsigned bg_width;
   unsigned bg_height;
   ...
   enum gfx_widget_libks_message_status status;

   char message[512];
   bool message_updated;
};

typedef struct gfx_widget_libks_message_state gfx_widget_libks_message_state_t;

static gfx_widget_libks_message_state_t p_w_libks_message_st = {
   ...
   GFX_WIDGET_LIBKS_MESSAGE_IDLE,   /* status */
...
static void gfx_widget_libks_message_reset(bool cancel_pending)
{
   gfx_widget_libks_message_state_t *state = &p_w_libks_message_st;
   ...
   /* Reset status */
   state->status             = GFX_WIDGET_LIBKS_MESSAGE_IDLE;
   if (cancel_pending)
...
static void gfx_widget_libks_message_fade_out_cb(void *userdata)
{
   gfx_widget_libks_message_reset(false);
}

static void gfx_widget_libks_message_wait_cb(void *userdata)
{
   gfx_widget_libks_message_state_t *state = (gfx_widget_libks_message_state_t*)userdata;
   uintptr_t alpha_tag                        = (uintptr_t)&state->alpha;
   ...
   animation_entry.duration     = LIBKS_MESSAGE_FADE_DURATION;
   animation_entry.target_value = 0.0f;
   animation_entry.subject      = &state->alpha;
   animation_entry.cb           = gfx_widget_libks_message_fade_out_cb;
   animation_entry.userdata     = NULL;

   gfx_animation_push(&animation_entry);
   state->status = GFX_WIDGET_LIBKS_MESSAGE_FADE_OUT;
}

static void gfx_widget_libks_message_slide_in_cb(void *userdata)
{
   gfx_widget_libks_message_state_t *state = (gfx_widget_libks_message_state_t*)userdata;
   ...
   timer.cb       = gfx_widget_libks_message_wait_cb;
   timer.userdata = state;

   gfx_animation_timer_start(&state->timer, &timer);
   state->status = GFX_WIDGET_LIBKS_MESSAGE_WAIT;
}

/* Widget interface */

void gfx_widget_set_libks_message(void *data,
...
static void gfx_widget_libks_message_layout(
      void *data,
...
static void gfx_widget_libks_message_iterate(void *user_data,
      ...
      bool is_threaded)
{
   ...
   {
      enum gfx_widget_libks_message_status current_status = state->status;
      uintptr_t alpha_tag                                    = (uintptr_t)&state->alpha;
      gfx_animation_ctx_entry_t animation_entry;
      ...
      {
         case GFX_WIDGET_LIBKS_MESSAGE_IDLE:
            ...
            animation_entry.subject      = &state->alpha;
            animation_entry.cb           = gfx_widget_libks_message_slide_in_cb;
            animation_entry.userdata     = state;

            gfx_animation_push(&animation_entry);
            state->status = GFX_WIDGET_LIBKS_MESSAGE_SLIDE_IN;
            break;
         case GFX_WIDGET_LIBKS_MESSAGE_FADE_IN:
         case GFX_WIDGET_LIBKS_MESSAGE_FADE_OUT:
            {
               ...
               unsigned fade_duration = (unsigned)(((1.0f - state->alpha) *
                     (float)LIBKS_MESSAGE_FADE_DURATION) + 0.5f);
               fade_duration = (fade_duration > LIBKS_MESSAGE_FADE_DURATION) ?
                     LIBKS_MESSAGE_FADE_DURATION : fade_duration;
               ...
               if ((state->alpha >= 1.0f) || (fade_duration < 1))
                  gfx_widget_libks_message_slide_in_cb(state);
               else
               {
                  animation_entry.easing_enum  = EASING_OUT_QUAD;
                  animation_entry.tag          = alpha_tag;
                  ...
                  gfx_animation_push(&animation_entry);
                  state->status = GFX_WIDGET_LIBKS_MESSAGE_FADE_IN;
               }
            }
            break;
         case GFX_WIDGET_LIBKS_MESSAGE_WAIT:
...
static void gfx_widget_libks_message_frame(void *data, void *user_data)
{
   gfx_widget_libks_message_state_t *state = &p_w_libks_message_st;

   if (state->status != GFX_WIDGET_LIBKS_MESSAGE_IDLE)
   {
      ...
      switch (state->status)
      {
         case GFX_WIDGET_LIBKS_MESSAGE_SLIDE_IN:
...
static void gfx_widget_libks_message_free(void)
{
   gfx_widget_libks_message_reset(true);
}

/* Widget definition */

const gfx_widget_t gfx_widget_libks_message = {
   NULL, /* init */
   gfx_widget_libks_message_free,
   NULL, /* context_reset*/
   NULL, /* context_destroy */
   gfx_widget_libks_message_layout,
   gfx_widget_libks_message_iterate,
   gfx_widget_libks_message_frame
};
