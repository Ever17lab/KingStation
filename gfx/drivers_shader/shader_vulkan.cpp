      ...
      video_shader *shader)
{
   ...
   for (i = 0; i < shader->luts; i++)
   {
      ...
      if (!image)
      {
         KINGSN_ERR("[Vulkan]: Failed to load LUT \"%s\".\n", shader->lut[i].path);
         goto error;
      }
...
bool vulkan_filter_chain::init_history()
{
   ...
   {
#ifdef VULKAN_DEBUG
      KINGSN_LOG("[Vulkan filter chain]: Not using frame history.\n");
#endif
      return true;
   }
...
#ifdef VULKAN_DEBUG
   KINGSN_LOG("[Vulkan filter chain]: Using history of %u frames.\n", unsigned(required_images));
#endif
...
bool vulkan_filter_chain::init_feedback()
{
   ...
   for (i = 0; i < passes.size() - 1; i++)
   {
      ...
      if (use_feedback)
         KINGSN_LOG("[Vulkan filter chain]: Using framebuffer feedback for pass #%u.\n", i);
   }

   if (!use_feedbacks)
   {
#ifdef VULKAN_DEBUG
      KINGSN_LOG("[Vulkan filter chain]: Not using framebuffer feedback.\n");
#endif
      return true;
   }
...
#ifdef VULKAN_DEBUG
      KINGSN_LOG("[Vulkan]: Push Constant Block: %u bytes.\n", (unsigned int)reflection.push_constant_size);
#endif
...
Framebuffer::Framebuffer(
   ...
   device(device)
{
   KINGSN_LOG("[Vulkan filter chain]: Creating framebuffer %u x %u (max %u level(s)).\n",
...
void Framebuffer::set_size(DeferredDisposer &disposer, const Size2D &size, VkFormat format)
{
   ...
   KINGSN_LOG("[Vulkan filter chain]: Updating framebuffer size %u x %u (format: %u).\n",
      ...
      const char *path, glslang_filter_chain_filter filter)
{
   ...
   for (i = 0; i < shader->passes; i++)
   {
      ...
      {
         KINGSN_ERR("[Vulkan]: Failed to compile shader: \"%s\".\n",
               pass->source.path);
         goto error;
      }
      ...
      for (auto &meta_param : output.meta.parameters)
      {
         ...
         {
            KINGSN_ERR("[Vulkan]: Exceeded maximum number of parameters (%u).\n", GFX_MAX_PARAMETERS);
            goto error;
         }
         ...
         if (itr != shader->parameters + shader->num_parameters)
         {
            ...
            {
               KINGSN_ERR("[Vulkan]: Duplicate parameters found for \"%s\", but arguments do not match.\n",
                     itr->id);
               goto error;
            }
      ...
      pass_info.mip_filter = 
         (pass->filter != KINGSN_FILTER_NEAREST && pass_info.max_levels > 1)
         ? GLSLANG_FILTER_CHAIN_LINEAR 
         : GLSLANG_FILTER_CHAIN_NEAREST;
