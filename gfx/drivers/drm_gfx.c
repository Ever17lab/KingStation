      ...
      struct drm_surface **sp)
{
   ...
   *sp = calloc (1, sizeof(struct drm_surface));
   ...
   for (i = 0; i < surface->numpages; i++)
   {
      ...
      if (ret)
      {
         KINGSN_ERR ("DRM: can't create fb\n");
      }
   }
...
static void drm_page_flip(struct drm_surface *surface)
{
   ...
   if (ret < 0)
   {
      KINGSN_ERR ("DRM: failed to add atomic property for pageflip\n");
   }
   ...
   if (ret < 0)
   {
      KINGSN_ERR ("DRM: failed to commit for pageflip: %s\n", strerror(errno));
   }
...
static uint32_t get_plane_prop_id(uint32_t obj_id, const char *name)
{
   ...
   for (i = 0; i < plane_resources->count_planes; i++)
   {
      ...
      KINGSN_ERR ("DRM: plane %d fb property ID with name %s not found\n",
            plane->plane_id, name);
   }
   return (0);
}
...
static void drm_plane_setup(struct drm_surface *surface)
{
   ...
   if (!plane_resources)
   {
      KINGSN_ERR ("DRM: No scaling planes available!\n");
   }

   KINGSN_LOG ("DRM: Number of planes on FD %d is %d\n",
   ...
   for (i = 0; i < (unsigned int)drm.resources->count_crtcs; i++)
   {
      ...
      {
         crtc_index = i;
         KINGSN_LOG ("DRM: CRTC index found %d with ID %d\n", crtc_index, drm.crtc_id);
         break;
      }
   }
   ...
   for (i = 0; i < plane_resources->count_planes; i++)
   {
      ...
      if (!(plane->possible_crtcs & (1 << crtc_index))){
         KINGSN_LOG ("DRM: plane with ID %d can't be used with current CRTC\n",
               plane->plane_id);
         continue;
      }
      ...
      {
         KINGSN_LOG ("DRM: plane with ID %d is not an overlay. May be primary or cursor. Not usable.\n",
               plane->plane_id);
         continue;
      }
      ...
      {
         KINGSN_LOG ("DRM: plane with ID %d does not support framebuffer format\n", plane->plane_id);
         continue;
      }
   ...
   if (!drm.plane_id)
   {
      KINGSN_LOG ("DRM: couldn't find an usable overlay plane for current CRTC and framebuffer pixel formal.\n");
      deinit_drm();
      exit (0);
   }
   else
   {
      KINGSN_LOG ("DRM: using plane/overlay ID %d\n", drm.plane_id);
   }
   ...
   {
      KINGSN_LOG("[DRM]: Can't get the FB property ID for plane(%u)\n", drm.plane_id);
   }
   ...
   {
      KINGSN_ERR("[DRM]: failed to enable plane: %s\n", strerror(errno));
   }

   KINGSN_LOG("[DRM]: src_w %d, src_h %d, plane_w %d, plane_h %d\n",
         src_w, src_h, plane_w, plane_h);

   /* Report what plane (of overlay type) we're using. */
   drm_format_name(surface->pixformat, fmt_name);
   KINGSN_LOG("[DRM]: Using plane with ID %d on CRTC ID %d format %s\n",
      ...
      int bpp, uint32_t pixformat)
{
   ...
   {
      KINGSN_ERR ("DRM: cannot mmap dumb buffer\n");
      return 0;
   }
...
static bool init_drm(void)
{
   ...
   if (drm.fd < 0)
   {
      KINGSN_LOG ("DRM: could not open drm device\n");
      return false;
   }
   ...
   if (ret)
      KINGSN_ERR ("DRM: can't set UNIVERSAL PLANES cap.\n");
   else
      KINGSN_LOG ("DRM: UNIVERSAL PLANES cap set\n");
   ...
   if (ret)
   {
      ...
      KINGSN_ERR ("DRM: can't set ATOMIC caps: %s\n", strerror(errno));
   }
   else
      KINGSN_LOG ("DRM: ATOMIC caps set\n");

   drm.resources = drmModeGetResources(drm.fd);
   if (!drm.resources)
   {
      KINGSN_ERR ("DRM: drmModeGetResources failed\n");
      return false;
   }
   ...
   if (!connector)
   {
      KINGSN_ERR ("DRM: no connected connector found\n");
      return false;
   }
   ...
   if (!drm.encoder)
   {
      KINGSN_ERR ("DRM: no encoder found.\n");
      return false;
   }
      ...
      input_driver_t **input, void **input_data)
{
   ...
   if (!init_drm())
   {
      KINGSN_ERR ("DRM: Failed to initialize DRM\n");
      free(_drmvars);
      return NULL;
   }
   else
   {
      KINGSN_LOG ("DRM: Init successful.\n");
   }
