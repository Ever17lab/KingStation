...
struct ff_audio_info
{
    ...
    */
   const ks_resampler_t *resampler;
   void *resampler_data;
      ...
      const AVCodec *codec)
{
   ...
   if (ffmpeg_codec_has_sample_format(AV_SAMPLE_FMT_FLTP, codec->sample_fmts))
   {
      ...
      KINGSN_LOG("[FFmpeg]: Using sample format FLTP.\n");
   }
   else if (ffmpeg_codec_has_sample_format(AV_SAMPLE_FMT_FLT, codec->sample_fmts))
   {
      ...
      KINGSN_LOG("[FFmpeg]: Using sample format FLT.\n");
   }
   else if (ffmpeg_codec_has_sample_format(AV_SAMPLE_FMT_S16P, codec->sample_fmts))
   {
      ...
      audio->is_planar         = true;
      KINGSN_LOG("[FFmpeg]: Using sample format S16P.\n");
   }
   else if (ffmpeg_codec_has_sample_format(AV_SAMPLE_FMT_S16, codec->sample_fmts))
   {
      ...
      audio->use_float         = false;
      audio->is_planar         = false;
      KINGSN_LOG("[FFmpeg]: Using sample format S16.\n");
   }
      ...
      const AVCodec *codec)
{
   ...
   if (codec->supported_samplerates && !params->sample_rate)
   {
      ...
      KINGSN_LOG("[FFmpeg]: Using output sampling rate: %d.\n", best_rate);
   }
}
...
static bool ffmpeg_init_audio(ffmpeg_t *handle, const char *audio_resampler)
{
   ...
   if (!codec)
   {
      KINGSN_ERR("[FFmpeg]: Cannot find acodec %s.\n",
            *params->acodec ? params->acodec : "flac");
      return false;
   }
...
#if 0
   KINGSN_LOG("[FFmpeg]: Audio frame size: %d.\n", audio->codec->frame_size);
#endif
...
static bool ffmpeg_init_video(ffmpeg_t *handle)
{
   ...
   if (!codec)
   {
      KINGSN_ERR("[FFmpeg]: Cannot find vcodec %s.\n",
      ...
      const char *config)
{
   ...
   KINGSN_LOG("[FFmpeg] Loading FFmpeg config \"%s\".\n", config);

   if (!(params->conf = config_file_new_from_path_to_string(config)))
   {
      KINGSN_ERR("[FFmpeg] Failed to load FFmpeg config \"%s\".\n", config);
      return false;
   }
   ...
   if (config_get_array(params->conf, "pix_fmt", pix_fmt, sizeof(pix_fmt)))
   {
      ...
      if (params->out_pix_fmt == PIX_FMT_NONE)
      {
         KINGSN_ERR("[FFmpeg] Cannot find pix_fmt \"%s\".\n", pix_fmt);
         return false;
      }
   }
...
static bool init_thread(ffmpeg_t *handle)
{
   ...
   ks_assert(handle->lock && handle->cond_lock &&
      ...
      const struct record_video_data *vid)
{
    ...
    * libks tends to use a very large pitch.
    */
   attr_data = *vid;
...
static bool encode_video(ffmpeg_t *handle, AVFrame *frame)
{
   ...
   {
#ifdef __cplusplus
      KINGSN_ERR("[FFmpeg]: Cannot send video frame. Error code: %d.\n", ret);
#else
      KINGSN_ERR("[FFmpeg]: Cannot send video frame. Error code: %s.\n", av_err2str(ret));
#endif
      return false;
   }
   ...
   while (ret >= 0)
   {
      ...
      {
#ifdef __cplusplus
         KINGSN_ERR("[FFmpeg]: Cannot receive video packet. Error code: %d.\n", ret);
#else
         KINGSN_ERR("[FFmpeg]: Cannot receive video packet. Error code: %s.\n", av_err2str(ret));
#endif
         return false;
      }
      ...
      {
#ifdef __cplusplus
         KINGSN_ERR("[FFmpeg]: Cannot write video packet to output file. Error code: %d.\n", ret);
#else
         KINGSN_ERR("[FFmpeg]: Cannot write video packet to output file. Error code: %s.\n", av_err2str(ret));
#endif
         return false;
      }
   }
...
static bool encode_audio(ffmpeg_t *handle, bool dry)
{
   ...
   if (ret < 0)
   {
      av_frame_free(&frame);
#ifdef __cplusplus
      KINGSN_ERR("[FFmpeg]: Cannot send audio frame. Return code: %d.\n", ret);
#else
      KINGSN_ERR("[FFmpeg]: Cannot send audio frame. Return code: %s.\n", av_err2str(ret));
#endif
      return false;
   }
   ...
   while (ret >= 0) 
   {
      ...
      else if (ret < 0)
      {
         av_frame_free(&frame);
#ifdef __cplusplus
         KINGSN_ERR("[FFmpeg]: Cannot receive audio packet. Return code: %d.\n", ret);
#else
         KINGSN_ERR("[FFmpeg]: Cannot receive audio packet. Return code: %s.\n", av_err2str(ret));
#endif
         return false;
      }
