...
{
   KINGSN_LOG("[Wayland]: Seat name: %s.\n", name);
}
      ...
      struct wl_output *output)
{
    ...
    wl_list_for_each(oi, &wl->all_outputs, link)
    {
       if (oi->output == output)
       {
          KINGSN_LOG("[Wayland]: Entering output #%d, scale %d\n", oi->global_id, oi->scale);
      ...
      int transform)
{
   ...
   KINGSN_LOG("[Wayland]: Physical width: %d mm x %d mm.\n",
      ...
      int refresh)
{
   ...
   KINGSN_LOG("[Wayland]: Video mode: %d x %d @ %.4f Hz.\n",
      ...
      int32_t factor)
{
   ...
   KINGSN_LOG("[Wayland]: Display scale factor %d.\n", factor);
   oi->scale = factor;
}
