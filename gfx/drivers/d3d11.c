      ...
      bool smooth, bool ctx_scaling)
{
   ...
   for (i = 0; i < KINGSN_WRAP_MAX; i++)
   {
      if (smooth)
         d3d11->samplers[KINGSN_FILTER_UNSPEC][i] = d3d11->samplers[KINGSN_FILTER_LINEAR][i];
      else
         d3d11->samplers[KINGSN_FILTER_UNSPEC][i] = d3d11->samplers[KINGSN_FILTER_NEAREST][i];
   }
}
...
static bool d3d11_gfx_set_shader(void* data, enum kingsn_shader_type type, const char* path)
{
#if defined(HAVE_SLANG) && defined(HAVE_SPIRV_CROSS)
   unsigned         i;
   ...
   {
      KINGSN_WARN("[D3D11] Only Slang shaders are supported. Falling back to stock.\n");
      return false;
   }
   ...
   for (i = 0; i < d3d11->shader_preset->passes; source = &d3d11->pass[i++].rt)
   {
      ...
      if (!slang_process(
               d3d11->shader_preset, i, KINGSN_SHADER_HLSL, 40, &semantics_map,
...
static void d3d11_gfx_free(void* data)
{
   ...
   for (i = 0; i < KINGSN_WRAP_MAX; i++)
   {
      Release(d3d11->samplers[KINGSN_FILTER_LINEAR][i]);
      Release(d3d11->samplers[KINGSN_FILTER_NEAREST][i]);
   }
      ...
      void *corewindow)
{
   ...
   {
      KINGSN_WARN("[D3D11]: Failed to create swapchain with flip model, try non-flip model.\n");
      ...
      input_driver_t** input, void** input_data)
{
   ...
   {
      KINGSN_ERR("[D3D11]: win32_set_video_mode failed.\n");
      goto error;
   }
...

   {
      ...
      for (i = 0; i < KINGSN_WRAP_MAX; i++)
      {
         ...
         D3D11CreateSamplerState(d3d11->device, &desc, &d3d11->samplers[KINGSN_FILTER_LINEAR][i]);

         desc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
         D3D11CreateSamplerState(d3d11->device, &desc, &d3d11->samplers[KINGSN_FILTER_NEAREST][i]);
      }
   }
...
#include "d3d_shaders/snowflake_sm4.hlsl.h"
   ...
   if (video_driver_get_hw_context()->context_type == KS_HW_CONTEXT_DIRECT3D &&
         video_driver_get_hw_context()->version_major == 11)
   {
      d3d11->hw.enable                  = true;
      d3d11->hw.iface.interface_type    = KS_HW_RENDER_INTERFACE_D3D11;
      d3d11->hw.iface.interface_version = KS_HW_RENDER_INTERFACE_D3D11_VERSION;
...

   {
      ...
      for (;;)
      {
         ...
         KINGSN_LOG("[D3D11]: Found GPU at index %d: %s\n", i, str);
      ...
      if (0 <= gpu_index && gpu_index <= i && gpu_index < D3D11_MAX_GPU_COUNT)
      {
         ...
         KINGSN_LOG("[D3D11]: Using GPU index %d.\n", gpu_index);
         video_driver_set_gpu_device_string(d3d11->gpu_list->elems[gpu_index].data);
      }
      else
      {
         KINGSN_WARN("[D3D11]: Invalid GPU index %d, using first device found.\n", gpu_index);
...
static void d3d11_init_render_targets(d3d11_video_t* d3d11, unsigned width, unsigned height)
{
   ...
   for (i = 0; i < d3d11->shader_preset->passes; i++)
   {
      ...
      KINGSN_LOG("[D3D11]: Updating framebuffer size %u x %u.\n", width, height);
      ...
      video_frame_info_t* video_info)
{
   ...
   if (frame && width && height)
   {
      ...
      if (frame == KS_HW_FRAME_BUFFER_VALID)
      {
   ...
   if (texture)
   {
      ...
      D3D11SetPShaderSamplers(
            context, 0, 1, &d3d11->samplers[KINGSN_FILTER_UNSPEC][KINGSN_WRAP_DEFAULT]);
      ...
      void* data, const void* frame, bool rgb32, unsigned width, unsigned height, float alpha)
{
   ...
   d3d11->menu.texture.sampler = d3d11->samplers
      [menu_linear_filter
         ? KINGSN_FILTER_LINEAR
         : KINGSN_FILTER_NEAREST][KINGSN_WRAP_DEFAULT];
}
      ...
      void* video_data, void* data, bool threaded, enum texture_filter_type filter_type)
{
   ...
   switch (filter_type)
   {
      ...
      case TEXTURE_FILTER_LINEAR:
         texture->sampler = d3d11->samplers[KINGSN_FILTER_LINEAR][KINGSN_WRAP_EDGE];
         break;
      case TEXTURE_FILTER_MIPMAP_NEAREST:
         texture->desc.MiscFlags = D3D11_RESOURCE_MISC_GENERATE_MIPS;
         /* fallthrough */
      case TEXTURE_FILTER_NEAREST:
         texture->sampler = d3d11->samplers[KINGSN_FILTER_NEAREST][KINGSN_WRAP_EDGE];
         break;
   }
...
static bool d3d11_get_hw_render_interface(
      void* data, const struct ks_hw_render_interface** iface)
{
   d3d11_video_t* d3d11 = (d3d11_video_t*)data;
   *iface               = (const struct ks_hw_render_interface*)
      &d3d11->hw.iface;
