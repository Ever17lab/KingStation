...
static bool gl_core_init_pbo_readback(gl_core_t *gl)
{
   ...
   if (!scaler_ctx_gen_filter(scaler))
   {
      ...
      KINGSN_ERR("[GLCore]: Failed to initialize pixel conversion for PBO.\n");
      ...
      float w, float h)
{
   ...
   {
      KINGSN_ERR("[GLCore]: Invalid overlay id: %u\n", image);
      return;
   }
...
static bool gl_core_init_hw_render(gl_core_t *gl, unsigned width, unsigned height)
{
   ...
   KINGSN_LOG("[GLCore]: Initializing HW render (%u x %u).\n", width, height);
   glGetIntegerv(GL_MAX_TEXTURE_SIZE, &max_fbo_size);
   glGetIntegerv(GL_MAX_RENDERBUFFER_SIZE, &max_rb_size);
   KINGSN_LOG("[GLCore]: Max texture size: %d px, renderbuffer size: %d px.\n",
   ...
   {
      KINGSN_ERR("[GLCore]: Framebuffer is not complete.\n");
      gl_core_context_bind_hw_render(gl, false);
      return false;
   }
...
static const gfx_ctx_driver_t *gl_core_get_context(gl_core_t *gl)
{
   ...
   major = 3;
   minor = 0;
   if (hwr && hwr->context_type == KS_HW_CONTEXT_OPENGLES_VERSION)
   {
   ...
   if (hwr && hwr->context_type != KS_HW_CONTEXT_NONE)
   {
      major = hwr->version_major;
      minor = hwr->version_minor;
      gl_query_core_context_set(hwr->context_type == KS_HW_CONTEXT_OPENGL_CORE);
   ...
   if (hwr)
      gl->use_shared_context = hwr->context_type != KS_HW_CONTEXT_NONE;
...
static bool gl_core_init_default_filter_chain(gl_core_t *gl)
{
   ...
   if (!gl->filter_chain)
   {
      KINGSN_ERR("Failed to create filter chain.\n");
      return false;
   }
...
static bool gl_core_init_filter_chain_preset(gl_core_t *gl, const char *shader_path)
{
   ...
   if (!gl->filter_chain)
   {
      KINGSN_ERR("[GLCore]: Failed to create preset: \"%s\".\n", shader_path);
      return false;
   }
...
static bool gl_core_init_filter_chain(gl_core_t *gl)
{
   ...
   enum kingsn_shader_type type = video_shader_parse_type(shader_path);

   if (string_is_empty(shader_path))
   {
      KINGSN_LOG("[GLCore]: Loading stock shader.\n");
      return gl_core_init_default_filter_chain(gl);
   }

   if (type != KINGSN_SHADER_SLANG)
   {
      KINGSN_WARN("[GLCore]: Only Slang shaders are supported, falling back to stock.\n");
      return gl_core_init_default_filter_chain(gl);
   }
...
static void gl_core_begin_debug(gl_core_t *gl)
{
   if (gl_check_capability(GL_CAPS_DEBUG))
   {
      ...
      KINGSN_ERR("[GLCore]: Neither GL_KHR_debug nor GL_ARB_debug_output are implemented. Cannot start GL debugging.\n");
}
#endif
      ...
      input_driver_t **input, void **input_data)
{
   ...
   KINGSN_LOG("[GLCore]: Found GL context: %s\n", ctx_driver->ident);
   ...
   KINGSN_LOG("[GLCore]: Detecting screen resolution %ux%u.\n", full_x, full_y);
   ...
   if (hwr && hwr->context_type != KS_HW_CONTEXT_NONE)
      gl_core_init_hw_render(gl, KINGSN_SCALE_BASE * video->input_scale, KINGSN_SCALE_BASE * video->input_scale);
   ...
   KINGSN_LOG("[GLCore]: Vendor: %s, Renderer: %s.\n", vendor, renderer);
   KINGSN_LOG("[GLCore]: Version: %s.\n", version);
   ...
   {
      KINGSN_ERR("[GLCore]: Failed to cross-compile menu pipelines.\n");
      goto error;
   }
   ...
   KINGSN_LOG("[GLCore]: Using resolution %ux%u\n", temp_width, temp_height);
   ...
   {
      KINGSN_ERR("[GLCore]: Failed to init filter chain.\n");
      goto error;
   }
   ...
   {
      KINGSN_LOG("[GLCore]: Async PBO readback enabled.\n");
   }
                               ...
                               enum kingsn_shader_type type, const char *path)
{
   ...
   if (!string_is_empty(path) && type != KINGSN_SHADER_SLANG)
   {
      KINGSN_WARN("[GLCore]: Only Slang shaders are supported. Falling back to stock.\n");
      path = NULL;
   }
   ...
   {
      KINGSN_ERR("[GLCore]: Failed to create filter chain: \"%s\". Falling back to stock.\n", path);
      gl_core_init_default_filter_chain(gl);
      gl_core_context_bind_hw_render(gl, true);
      return false;
   }
...
static ks_proc_address_t gl_core_get_proc_address(
