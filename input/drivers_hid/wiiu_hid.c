      ...
      unsigned port)
{
   unsigned i;
   int16_t ret                          = 0;
   const struct ks_keybind *binds    = (const struct ks_keybind*)
      binds_data;
...
{
   KINGSN_LOG("[hid]: initializing HID subsystem\n");
   ...
   KINGSN_LOG("[hid]: init success\n");
   return hid;

error:
   KINGSN_LOG("[hid]: initialization failed. cleaning up.\n");
...
static void wiiu_hid_send_control(void *data, uint8_t *buf, size_t size)
{
   ...
   if (!adapter)
   {
      KINGSN_ERR("[hid]: send_control: bad adapter.\n");
      return;
   }
   ...
   if (result < 0)
   {
      ...
      KINGSN_LOG("[hid]: write failed: %08x (%d:%d)\n", result, r2, r1);
   }
}
...
static void start_polling_thread(wiiu_hid_t *hid)
{
   ...
   KINGSN_LOG("[hid]: starting polling thread.\n");

   if (!thread || !stack)
   {
      KINGSN_LOG("[hid]: allocation failed, aborting thread start.\n");
      goto error;
   }
   ...
   if (!OSCreateThread(thread,
            ...
            attributes))
   {
      KINGSN_LOG("[hid]: OSCreateThread failed.\n");
      goto error;
   }
...
{
   int thread_result = 0;
   KINGSN_LOG("[hid]: stopping polling thread.\n");
...
static void log_device(HIDDevice *device)
{
   if (!device)
   {
      KINGSN_LOG("NULL device.\n");
   }

   KINGSN_LOG("                handle: %d\n", device->handle);
   KINGSN_LOG("  physical_device_inst: %d\n", device->physical_device_inst);
   KINGSN_LOG("                   vid: 0x%x\n", device->vid);
   KINGSN_LOG("                   pid: 0x%x\n", device->pid);
   KINGSN_LOG("       interface_index: %d\n", device->interface_index);
   KINGSN_LOG("             sub_class: %d\n", device->sub_class);
   KINGSN_LOG("              protocol: %d\n", device->protocol);
   KINGSN_LOG("    max_packet_size_rx: %d\n", device->max_packet_size_rx);
   KINGSN_LOG("    max_packet_size_tx: %d\n", device->max_packet_size_tx);
}
...
static uint8_t try_init_driver(wiiu_adapter_t *adapter)
{
   ...
   {
     KINGSN_ERR("[hid]: Failed to initialize driver: %s\n",
        adapter->driver->name);
   ...
   for (adapter = adapters.list; adapter != NULL; adapter = adapter_next)
   {
     ...
     switch(adapter->state)
     {
       ...
       case ADAPTER_STATE_GC:
          ...
          break;
       default:
          KINGSN_ERR("[hid]: Invalid adapter state: %d\n", adapter->state);
          break;
     }
      ...
      HIDDevice *device, uint32_t attach)
{
   ...
   if (attach)
   {
      KINGSN_LOG("[hid]: Device attach event generated.\n");
      log_device(device);
   }
   else
   {
      KINGSN_LOG("[hid]: Device detach event generated.\n");
   }
...
static void wiiu_hid_attach(wiiu_hid_t *hid, wiiu_attach_event *event)
{
   ...
   if (!adapter)
   {
      KINGSN_ERR("[hid]: Failed to allocate adapter.\n");
      goto error;
   }
              ...
              uint8_t *buffer, uint32_t buffer_size, void *userdata)
{
   ...
   if (!adapter)
   {
      KINGSN_ERR("read_loop_callback: bad userdata\n");
      return;
   }
...
static void report_hid_error(const char *msg, wiiu_adapter_t *adapter, int32_t error)
{
   ...
   {
      case -100:
         KINGSN_ERR("[hid]: Invalid RM command (%s)\n", device);
         break;
      case -102:
         KINGSN_ERR("[hid]: Invalid IOCTL command (%s)\n", device);
         break;
      case -103:
         KINGSN_ERR("[hid]: bad vector count (%s)\n", device);
         break;
      case -104:
         KINGSN_ERR("[hid]: invalid memory bank (%s)\n", device);
         break;
      case -105:
         KINGSN_ERR("[hid]: invalid memory alignment (%s)\n", device);
         break;
      case -106:
         KINGSN_ERR("[hid]: invalid data size (%s)\n", device);
         break;
      case -107:
         KINGSN_ERR("[hid]: request cancelled (%s)\n", device);
         break;
      case -108:
         KINGSN_ERR("[hid]: request timed out (%s)\n", device);
         break;
      case -109:
         KINGSN_ERR("[hid]: request aborted (%s)\n", device);
         break;
      case -110:
         KINGSN_ERR("[hid]: client priority error (%s)\n", device);
         break;
      case -111:
         KINGSN_ERR("[hid]: invalid device handle (%s)\n", device);
         break;
      default:
#if 0
         KINGSN_ERR("[hid]: Unknown error (%d:%d: %s)\n",
...
static void wiiu_hid_polling_thread_cleanup(OSThread *thread, void *stack)
{
   ...
   KINGSN_LOG("Waiting for in-flight reads to finish.\n");
   ...
   do
   {
      ...
      if (incomplete == 0)
      {
         KINGSN_LOG("All in-flight reads complete.\n");
         while (adapters.list)
         {
            KINGSN_LOG("[hid]: shutting down adapter..\n");
      ...
      if (++retries >= 1000)
      {
         KINGSN_WARN("[hid]: timed out waiting for in-flight read to finish.\n");
         incomplete = 0;
      }
   } while (incomplete);
}
...
static int wiiu_hid_polling_thread(int argc, const char **argv)
{
   ...
   KINGSN_LOG("[hid]: polling thread is starting\n");
   ...
   KINGSN_LOG("[hid]: polling thread is stopping\n");
   return 0;
}
...
{
   KINGSN_LOG("[hid]: Initializing events list\n");
   memset(&events, 0, sizeof(events));
   OSFastMutex_Init(&(events.lock), "attach_events");
   KINGSN_LOG("[hid]: Initializing adapters list\n");
...
static wiiu_attach_event *new_attach_event(HIDDevice *device)
{
   ...
   if (!driver)
   {
      KINGSN_ERR("[hid]: Failed to locate driver for device vid=%04x pid=%04x\n",
