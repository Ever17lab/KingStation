...
static void netplay_crc_scan_callback(ks_task_t *task,
      void *task_data,
      void *user_data, const char *error)
{
   ...
   if (!string_is_empty(state->subsystem_name) && !string_is_equal(state->subsystem_name, "N/A"))
   {
      ...
      if (!content_set_subsystem_by_name(state->subsystem_name))
         KINGSN_LOG("[Lobby]: Subsystem not found in implementation\n");
      ...
      && !state->contentless && !state->current)
   {
      ...
      KINGSN_LOG("[Lobby]: Loading core %s with content file %s\n",
      ...
      && state->contentless)
   {
      ...
      KINGSN_LOG("[Lobby]: Loading contentless core %s\n", state->core_path);
   ...
   /* regular core with current content */
      ...
      && state->current)
   {
      KINGSN_LOG("[Lobby]: Loading core %s with current content\n", state->core_path);
   ...
   {
      KINGSN_LOG("[Lobby]: Couldn't find a suitable %s\n",
...
static void begin_task(ks_task_t *task, const char *title)
{
...
static void finish_task(ks_task_t *task, const char *title)
{
 ...
 */
static void task_netplay_crc_scan_handler(ks_task_t *task)
{
   size_t i, j, k;
   ...
   if (have_crc && content_get_crc() > 0)
   {
      ...
      KINGSN_LOG("[Lobby]: Testing CRC matching for: %s\n", state->content_crc);

      snprintf(current, sizeof(current), "%X|crc", content_get_crc());
      KINGSN_LOG("[Lobby]: Current content CRC: %s\n", current);

      if (string_is_equal(current, state->content_crc))
      {
         KINGSN_LOG("[Lobby]: CRC match %s with currently loaded content\n", current);
   ...
   if (string_is_empty(state->subsystem_name) || string_is_equal(state->subsystem_name, "N/A"))
   {
      for (i = 0; i < state->lpl_list->size; i++)
      {
         ...
         KINGSN_LOG("[Lobby]: Searching playlist: %s\n", lpl_path);
         ...
         for (j = 0; j < playlist_size; j++)
         {
            ...
            */
            ...
            {
               KINGSN_LOG("[Lobby]: Filename match %s\n", playlist_path);
   ...
   else
   {
      ...
      for (i = 0; i < game_list->size; i++)
      {
         ...
         for (j = 0; j < state->lpl_list->size && !found[i]; j++)
         {
            ...
            KINGSN_LOG("[Lobby]: Searching content %d/%d (%s) in playlist: %s\n", i + 1, game_list->size, game_list->elems[i].data, lpl_path);
            playlist_config_set_path(&state->playlist_config, lpl_path);
            playlist      = playlist_init(&state->playlist_config);
            playlist_size = playlist_get_size(playlist);
      ...
      if (state->found)
      {
         KINGSN_LOG("[Lobby]: Subsystem matching set found %s\n", state->content_path);
         task_set_data(task, state);
         finish_task(task, msg_hash_to_str(MENU_ENUM_LABEL_VALUE_NETPLAY_COMPAT_CONTENT_FOUND));
      }
