...
/*
 * https://github.com/libks/KingStation/pull/11590
...
bool gfx_ctx_drm_load_mode(drmModeModeInfoPtr modeInfo)
{
   ...
   if(modeInfo != NULL && !string_is_empty(crt_switch_timings)) {
      ...
      if (ret != 17) {
         KINGSN_ERR("[DRM]: malformed mode requested: %s\n", crt_switch_timings);
         return false;
      }
...
static struct drm_fb *drm_fb_get_from_bo(struct gbm_bo *bo)
{
   ...
   KINGSN_LOG("[KMS]: New FB: %ux%u (stride: %u).\n",
         width, height, stride);
...
error:
   KINGSN_ERR("[KMS]: Failed to create FB: %s\n", strerror(errno));
   free(fb);
   return NULL;
}
...
static void gfx_ctx_drm_swap_interval(void *data, int interval)
{
   ...
   if (interval > 1)
      KINGSN_WARN("[KMS]: Swap intervals > 1 currently not supported. Will use swap interval of 1.\n");
}
      ...
      unsigned sec, unsigned usec, void *data)
{
   ...
   if (last_page_flip)
   {
      ...
      if (missed)
         KINGSN_LOG("[KMS]: Missed %u VBlank(s) (Frame: %u, DRM frame: %u).\n",
...
nextgpu:
   ...
   {
      KINGSN_ERR("[KMS]: Couldn't find a suitable DRM device.\n");
      goto error;
   }
   ...
   if (drm->fd < 0)
   {
      KINGSN_WARN("[KMS]: Couldn't open DRM device.\n");
      goto nextgpu;
   }
   ...
   if (!drm->gbm_dev)
   {
      KINGSN_WARN("[KMS]: Couldn't create GBM device.\n");
      goto nextgpu;
   }
      ...
      bool fullscreen)
{
   ...
   else
   {
      ...
      {
         KINGSN_LOG("[DRM]: custom mode requested: %s\n", gfx_ctx_crt_switch_mode.name);
   ...
   if (!g_drm_mode)
   {
      KINGSN_ERR("[KMS/EGL]: Did not find suitable video mode for %u x %u.\n",
            width, height);
      goto error;
   }
   ...
   if (!drm->gbm_surface)
   {
      KINGSN_ERR("[KMS/EGL]: Couldn't create GBM surface.\n");
      goto error;
   }
