...
STBTT_DEF int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
   ...
   {
      ks_assert(0); /* @TODO: high-byte mapping for japanese/chinese/korean */
      return 0;
   }
   else if (format == 4)
   {
...

      {
         ...
         ks_assert(unicode_codepoint <= ttUSHORT(data + endCount + 2*item));
...
{
   ks_assert(h_oversample <= STBTT_MAX_OVERSAMPLE);
   ks_assert(v_oversample <= STBTT_MAX_OVERSAMPLE);
...
static void stbtt__v_prefilter(unsigned char *pixels, int w, int h, int stride_in_bytes, unsigned int kernel_width)
{
   ...
   for (j=0; j < w; ++j)
   {
      ...
      for (; i < h; ++i)
      {
         ks_assert(pixels[i*stride_in_bytes] == 0);
         total -= buffer[i & STBTT__OVER_MASK];
         pixels[i*stride_in_bytes] = (unsigned char) (total / kernel_width);
      }
