...
static void hidpad_ps4_get_buttons(void *data, input_bits_t* state)
{
   ...
   if (rpt->btn.r3)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_R3 );
   if (rpt->btn.l3)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_L3 );
   if (rpt->btn.options)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_START );
   if ( rpt->btn.share)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_SELECT );
   if ( rpt->btn.r2)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_R2 );
   if (rpt->btn.l2)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_L2 );
   if (rpt->btn.r1)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_R );
   if (rpt->btn.l1)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_L );
   if (rpt->btn.triangle)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_X );
   if (rpt->btn.circle)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_A );
   if (rpt->btn.cross)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_B );
   if (rpt->btn.square)
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_Y );
   if ((hidpad_ps4_check_dpad(rpt, KS_DEVICE_ID_JOYPAD_LEFT)))
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_LEFT );
   if ((hidpad_ps4_check_dpad(rpt, KS_DEVICE_ID_JOYPAD_DOWN)))
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_DOWN );
   if ((hidpad_ps4_check_dpad(rpt, KS_DEVICE_ID_JOYPAD_RIGHT)))
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_RIGHT );
   if ((hidpad_ps4_check_dpad(rpt, KS_DEVICE_ID_JOYPAD_UP)))
      BIT256_SET_PTR( state, KS_DEVICE_ID_JOYPAD_UP );
   if (rpt->btn.ps)
      BIT256_SET_PTR( state, KINGSN_MENU_TOGGLE );
}
...
#if 0
   struct hidpad_ps4_data *device = (struct hidpad_ps4_data*)data;
   unsigned idx = (effect == KS_RUMBLE_STRONG) ? 0 : 1;
