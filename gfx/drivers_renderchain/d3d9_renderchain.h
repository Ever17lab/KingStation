      ...
      const char *id, const char *path, bool smooth)
{
   ...
   KINGSN_LOG("[D3D9]: LUT texture loaded: %s.\n", path);
      ...
      unsigned index, unsigned i)
{
   ...
   d3d9_set_sampler_magfilter(chain->dev, index,
         d3d_translate_filter(chain->luts->data[i].smooth ? KINGSN_FILTER_LINEAR : KINGSN_FILTER_NEAREST));
      ...
      d3d9_video_t *d3d)
{
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to set pass size.\n");
      return;
   }
