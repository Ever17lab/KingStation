...
{
   ks_assert(rsound != NULL);
   ks_assert(rsound->rate > 0);
   ks_assert(rsound->channels > 0);
   ks_assert(rsound->host != NULL);
   ks_assert(rsound->port != NULL);
...
{
   ks_assert(rd);
   ks_assert(rd->rate > 0 && rd->channels > 0);
...
void rsd_set_callback(rsound_t *rsound, rsd_audio_callback_t audio_cb, rsd_error_callback_t err_cb, size_t max_size, void *userdata)
{
   ...
   {
      ks_assert(rsound->error_callback);
   }
}
