...
static void wiiu_gca_update_pad_state(hid_wiiu_gca_instance_t *instance)
{
   ...
   for (i = 1; i < 37; i += 9)
   {
      ...
      if (port_connected > GCA_PORT_POWERED)
      {
         if (!pad)
         {
            KINGSN_LOG("[gca]: Gamepad at port %d connected.\n", port+1);
            instance->pads[port] = hid_pad_register(
                  instance, &wiiu_gca_pad_connection);
            pad                  = instance->pads[port];
            if (!pad)
            {
               KINGSN_ERR("[gca]: Failed to register pad.\n");
               break;
            }
         }
      ...
      else
      {
         if (pad)
         {
            KINGSN_LOG("[gca]: Gamepad at port %d disconnected.\n", port+1);
...
static void wiiu_gca_handle_packet(void *data, uint8_t *buffer, size_t size)
{
   ...
   {
      KINGSN_WARN("[gca]: packet size %d is too big for buffer of size %d\n",
...
static void wiiu_gca_update_analog_state(gca_pad_t *pad)
{
    ...
    * by 256 to get the 16-bit range KingStation expects. */
   for (pad_axis = 0; pad_axis < 4; pad_axis++)
   {
      axis         = (pad_axis % 2) ? 0 : 1;
      stick        = pad_axis / 2;
      interpolated = pad->data[3 + pad_axis];
      /* libks requires "up" to be negative, so we invert the y axis */
      interpolated = (axis) ?
...
static void wiiu_gca_set_rumble(void *data, enum ks_rumble_effect effect, uint16_t strength) { }
