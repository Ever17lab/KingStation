...
public:
   ...
   ThumbnailWidget(const ThumbnailWidget& other) { ks_assert(false && "DONT EVER USE THIS"); }
...
signals:
   ...
   void extractArchiveDeferred(QString path, QString extractionDir, QString tempExtension, ks_task_callback_t cb);
   void itemChanged();
...
public slots:
   ...
   void showAbout();
   void showDocs();
   void updateKingStationNightly();
   void onUpdateKingStationFinished(bool success);
...
private slots:
   ...
   int onExtractArchive(QString path, QString extractionDir, QString tempExtension, ks_task_callback_t cb);

   void onUpdateNetworkError(QNetworkReply::NetworkError code);
   void onUpdateNetworkSslErrors(const QList<QSslError> &errors);
   void onKingStationUpdateDownloadFinished();
