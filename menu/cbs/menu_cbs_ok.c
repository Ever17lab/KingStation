...
static enum msg_hash_enums action_ok_dl_to_enum(unsigned lbl)
{
   switch (lbl)
   {
      ...
      case ACTION_OK_DL_KS_ACHIEVEMENTS_SETTINGS_LIST:
         return MENU_ENUM_LABEL_DEFERRED_KS_ACHIEVEMENTS_SETTINGS_LIST;
      ...
      unsigned action_type)
{
   ...
   const char *dir_libks                = settings->paths.directory_libks;
   ...
   switch (action_type)
   {
      ...
      case ACTION_OK_DL_DISK_IMAGE_APPEND_LIST:
         {
            filebrowser_clear_type();
            strlcpy(tmp, path_get(KINGSN_PATH_CONTENT), sizeof(tmp));
            path_basedir(tmp);
      ...
      char *new_core_path, size_t len)
{
   ...
   if (load_content_with_current_core)
   {
      ...
      {
#if 0
         KINGSN_LOG("[lobby] use the current core (%s) to load this content...\n",
               info->path);
#endif
         supported = 1;
      }
...
#ifdef HAVE_LIBKSDB
void handle_dbscan_finished(ks_task_t *task,
...
#ifdef HAVE_CONFIGFILE
         {
            ...
            if (conf)
               if (input_remapping_load_file(conf, action_path))
               {
                  ...
                  {
                     snprintf(conf_key, sizeof(conf_key), "input_libks_device_p%u", port + 1);
...
static int default_action_ok_load_content_with_core_from_menu(const char *_path, unsigned _type)
{
   ...
   if (!task_push_load_content_with_core_from_menu(
            _path, &content_info,
            (enum kingsn_core_type)_type, NULL, NULL))
      return -1;
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (!string_is_empty(entry->subsystem_ident))
   {
      ...
      {
         KINGSN_LOG("[playlist] subsystem not found in implementation\n");
         goto error;
      }
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   system = runloop_get_libks_system_info();
   ...
   if (system && !string_is_empty(system->library_name))
   {
      ...
      KINGSN_LOG("[CDROM]: Loading disc from path: %s\n", cdrom_path);

      path_clear(KINGSN_PATH_CONTENT);
      path_set(KINGSN_PATH_CONTENT, cdrom_path);
   ...
   {
      KINGSN_LOG("[CDROM]: Cannot load disc without a core.\n");
      ...
      case ACTION_OK_REMAP_FILE_REMOVE_GAME:
         if (!string_is_empty(core_name))
            fill_pathname_join(file, core_name,
                  path_basename(path_get(KINGSN_PATH_BASENAME)), sizeof(file));
         break;
      case ACTION_OK_REMAP_FILE_SAVE_CONTENT_DIR:
      case ACTION_OK_REMAP_FILE_REMOVE_CONTENT_DIR:
         if (!string_is_empty(core_name))
         {
            fill_pathname_parent_dir_name(content_dir, path_get(KINGSN_PATH_BASENAME), sizeof(content_dir));
            fill_pathname_join(file, core_name,
      ...
      const char *content_label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   menu_handle_t            *menu          = menu_driver_get_ptr();
   settings_t *settings                    = config_get_ptr();
   const char *path_libks_info          = settings->paths.path_libks_info;
...
DEFAULT_ACTION_OK_START_BUILTIN_CORE(action_ok_start_net_kspad_core, CORE_TYPE_NETKSPAD)
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   bool          refresh = false;
   unsigned int new_size = cheat_manager_get_size() + 1;
   cheat_manager_realloc(new_size, CHEAT_HANDLER_TYPE_KS);
...
#ifdef HAVE_ZLIB
static void cb_decompressed(ks_task_t *task,
      void *task_data, void *user_data, const char *err)
{
...
static void cb_net_generic_subdir(ks_task_t *task,
      void *task_data, void *user_data, const char *err)
{
...
static void cb_net_generic(ks_task_t *task,
      void *task_data, void *user_data, const char *err)
{
   bool refresh                   = false;
   http_transfer_data_t *data     = (http_transfer_data_t*)task_data;
      ...
      enum msg_hash_enums enum_idx)
{
   ...
   ks_task_callback_t callback          = NULL;
   bool refresh                            = true;
...
static void cb_generic_dir_download(ks_task_t *task,
      void *task_data,
...
void cb_generic_download(ks_task_t *task,
      void *task_data,
      void *user_data, const char *err)
{
   ...
   switch (transf->enum_idx)
   {
      ...
      case MENU_ENUM_LABEL_CB_UPDATE_CORE_INFO_FILES:
         dir_path = settings->paths.path_libks_info;
         break;
      case MENU_ENUM_LABEL_CB_UPDATE_ASSETS:
      ...
      case MENU_ENUM_LABEL_CB_DISCORD_AVATAR:
         ...
         break;
      default:
         KINGSN_WARN("Unknown transfer type '%s' bailing out.\n",
   ...
   if (path_is_compressed_file(output_path))
   {
      ks_task_t *decompress_task = NULL;
      void *frontend_userdata       = task->frontend_userdata;
      task->frontend_userdata       = NULL;

      decompress_task = (ks_task_t*)task_push_decompress(
...
finish:
   if (err)
   {
      KINGSN_ERR("Download of '%s' failed: %s\n",
      ...
      enum msg_hash_enums enum_idx)
{
   ...
   file_transfer_t *transf      = NULL;
   bool suppress_msg            = false;
   ks_task_callback_t cb     = cb_generic_download;
   settings_t *settings         = config_get_ptr();
   const char *network_buildbot_assets_url =
      settings->paths.network_buildbot_assets_url;
   ...
   switch (enum_idx)
   {
      ...
      case MENU_ENUM_LABEL_CB_UPDATE_SHADERS_CG:
         ...
         break;
      case MENU_ENUM_LABEL_CB_CORE_THUMBNAILS_DOWNLOAD:
         strcpy_literal(s, "http://thumbnailpacks.libks.com");
         break;
      default:
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (play_feature_delivery_enabled())
      ...
      task_push_core_updater_download(
            ...
            path_dir_libks, path_dir_core_assets);
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   task_push_play_feature_delivery_switch_installed_cores(
         path_dir_libks, path_libks_info);
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   task_push_core_restore(backup_path, dir_libks, &core_loaded);
...
int action_ok_close_content(const char *path, const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (type == MENU_SETTING_ACTION_CLOSE)
   {
       ...
       * KINGSN_MENU_CTL_SET_PREVENT_POPULATE in awkward
       * places, which can cause breakage here when flushing
       * the menu stack. We therefore have to force a
       * KINGSN_MENU_CTL_UNSET_PREVENT_POPULATE */
      menu_driver_ctl(KINGSN_MENU_CTL_UNSET_PREVENT_POPULATE, NULL);
   }
...
static int action_ok_add_to_favorites(const char *path,
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   const char *content_path = path_get(KINGSN_PATH_CONTENT);
   int ret = 0;
   ...
   if (!string_is_empty(content_path))
   {
      ...
      if (system)
      {
         if (!string_is_empty(path_get(KINGSN_PATH_CORE)))
         {
            core_info_ctx_find_t core_info;

            /* >> core_path */
            strlcpy(core_path, path_get(KINGSN_PATH_CORE), sizeof(core_path));
...
DEFAULT_ACTION_OK_FUNC(action_ok_ks_achievements_list, ACTION_OK_DL_KS_ACHIEVEMENTS_SETTINGS_LIST)
...
#ifdef HAVE_NETWORKING
static void wifi_menu_refresh_callback(ks_task_t *task,
      void *task_data,
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_DATA_INITED, NULL))
      generic_action_ok_command(CMD_EVENT_NETPLAY_DEINIT);
   netplay_driver_ctl(KINGSN_NETPLAY_CTL_ENABLE_CLIENT, NULL);
...
#if 0
   KINGSN_LOG("[lobby] connecting to: %s with game: %s/%08x\n",
         tmp_hostname,
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (!netplay_discovery_driver_ctl(KINGSN_NETPLAY_DISCOVERY_CTL_LAN_GET_RESPONSES, &hosts))
      return -1;
...
#ifdef HAVE_NETWORKING
static void netplay_refresh_rooms_cb(ks_task_t *task,
      void *task_data, void *user_data, const char *err)
{
   char *new_data                = NULL;
...
#ifndef KINGSN_CONSOLE
         netplay_discovery_driver_ctl(KINGSN_NETPLAY_DISCOVERY_CTL_LAN_GET_RESPONSES, &lan_hosts);
         if (lan_hosts)
...
#ifndef KINGSN_CONSOLE
static void netplay_lan_scan_callback(ks_task_t *task,
      void *task_data,
      void *user_data, const char *error)
{
   ...
   if (!netplay_discovery_driver_ctl(
            KINGSN_NETPLAY_DISCOVERY_CTL_LAN_GET_RESPONSES,
            (void *) &netplay_hosts))
      return;
...
static int action_ok_push_netplay_refresh_rooms(const char *path,
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   char url [2048] = "http://lobby.libks.com/list/";
#ifndef KINGSN_CONSOLE
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (!settings ||
       ...
       (key_id >= (KINGSN_MAX_KEYS + MENU_SETTINGS_INPUT_DESC_KBD_BEGIN)))
      return menu_cbs_exit();

   /* Determine user/button indices */
   user_idx = (entry_type - MENU_SETTINGS_INPUT_DESC_KBD_BEGIN) 
      / KINGSN_FIRST_CUSTOM_BIND;
   btn_idx  = (entry_type - MENU_SETTINGS_INPUT_DESC_KBD_BEGIN) 
      - KINGSN_FIRST_CUSTOM_BIND * user_idx;
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   path_clear(KINGSN_PATH_BASENAME);
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
    ...
    *   confusing/ugly) */
   if (task_push_core_restore(backup_path, dir_libks, &core_loaded) &&
       core_loaded)
      menu_entries_flush_stack(NULL, 0);
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   if (kingsn_ctl(KINGSN_CTL_IS_CORE_LOADED, (void*)core_path))
      ...
      const char *label, unsigned type, size_t idx, size_t entry_idx)
{
   ...
   menu_driver_ctl(KINGSN_MENU_CTL_ENVIRONMENT, &menu_environ);
      ...
      const char *label)
{
   ...
   if (cbs->enum_idx != MSG_UNKNOWN)
   {
      ...
      temp_ok_list_t ok_list[] = {
         ...
         {MENU_ENUM_LABEL_START_NET_KSPAD,                  action_ok_start_net_kspad_core},
         ...
         {MENU_ENUM_LABEL_ACCOUNTS_KS_ACHIEVEMENTS,         action_ok_push_accounts_cheevos_list},
         ...
         {MENU_ENUM_LABEL_KS_ACHIEVEMENTS_SETTINGS,         action_ok_ks_achievements_list},
