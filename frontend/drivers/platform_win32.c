...
const struct win32_lang_pair win32_lang_pairs[] =
{
   ...
   {0x7c04, KS_LANGUAGE_CHINESE_TRADITIONAL}, /* neutral */
   {0x1404, KS_LANGUAGE_CHINESE_TRADITIONAL}, /* MO */
   {0x1004, KS_LANGUAGE_CHINESE_SIMPLIFIED}, /* SG */
   {0xC04, KS_LANGUAGE_CHINESE_TRADITIONAL}, /* HK/PRC */
   {0x816, KS_LANGUAGE_PORTUGUESE_PORTUGAL},
   {0x416, KS_LANGUAGE_PORTUGUESE_BRAZIL},
   {0x2a, KS_LANGUAGE_VIETNAMESE},
   {0x19, KS_LANGUAGE_RUSSIAN},
   {0x16, KS_LANGUAGE_PORTUGUESE_PORTUGAL},
   {0x15, KS_LANGUAGE_POLISH},
   {0x13, KS_LANGUAGE_DUTCH},
   {0x12, KS_LANGUAGE_KOREAN},
   {0x11, KS_LANGUAGE_JAPANESE},
   {0x10, KS_LANGUAGE_ITALIAN},
   {0xc, KS_LANGUAGE_FRENCH},
   {0xa, KS_LANGUAGE_SPANISH},
   {0x9, KS_LANGUAGE_ENGLISH},
   {0x8, KS_LANGUAGE_GREEK},
   {0x7, KS_LANGUAGE_GERMAN},
   {0x4, KS_LANGUAGE_CHINESE_SIMPLIFIED}, /* neutral */
   {0x1, KS_LANGUAGE_ARABIC},
   /* MS does not support Esperanto */
   /*{0x0, KS_LANGUAGE_ESPERANTO},*/
};

unsigned short win32_get_langid_from_ks_lang(enum ks_language lang)
{
   unsigned i;
...
enum ks_language win32_get_ks_lang_from_langid(unsigned short langid)
{
   unsigned i;
...
static bool gfx_init_dwm(void)
{
   ...
   if (!shell32lib)
   {
      KINGSN_WARN("Did not find shell32.dll.\n");
   }

   dwmlib = dylib_load("dwmapi.dll");
   if (!dwmlib)
   {
      KINGSN_WARN("Did not find dwmapi.dll.\n");
      return false;
   }
...
static void gfx_set_dwm(void)
{
   ...
   {
      KINGSN_ERR("Did not find DwmEnableComposition ...\n");
      return;
   }

   ret = composition_enable(!disable_composition);
   if (FAILED(ret))
      KINGSN_ERR("Failed to set composition state ...\n");
...
enum ks_language frontend_win32_get_user_language(void)
{
   enum ks_language lang = KS_LANGUAGE_ENGLISH;
#if defined(HAVE_LANGEXTRA) && !defined(_XBOX)
#if (defined(_WIN32_WINNT) && _WIN32_WINNT >= 0x0500) || !defined(_MSC_VER)
   LANGID langid = GetUserDefaultUILanguage();
   lang = win32_get_ks_lang_from_langid(langid);
#endif
#endif
   return lang;
}
...
static void frontend_win32_respawn(char *s, size_t len, char *args)
{
   ...
   KINGSN_LOG("Restarting KingStation with commandline: %s and %s\n",
   ...
   {
      KINGSN_LOG("Failed to restart KingStation\n");
   }
}
...
static bool is_narrator_running_windows(void)
{
   ...
   else if (USE_NVDA)
   {
      ...
      if (res != 0) 
      {
         ...
         */
         KINGSN_LOG("Error communicating with NVDA\n");
         USE_POWERSHELL = true;
         USE_NVDA       = false;
	 return false;
      }
