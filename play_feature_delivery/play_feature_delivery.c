...
#include "com_KingStation_browser_ksactivity_RetroActivityCommon.h"
...
/*
 * Class:     com_KingStation_browser_ksactivity_RetroActivityCommon
 * Method:    coreInstallInitiated
 * Signature: (Ljava/lang/String;Z)V
 */
JNIEXPORT void JNICALL Java_com_KingStation_browser_ksactivity_RetroActivityCommon_coreInstallInitiated
 ...
 */
JNIEXPORT void JNICALL Java_com_KingStation_browser_ksactivity_RetroActivityCommon_coreInstallStatusChanged
...
void play_feature_delivery_init(void)
{
   ...
   ks_assert(state->enabled_lock);

   if (!state->status_lock)
      state->status_lock = slock_new();

   ks_assert(state->status_lock);
#endif
      ...
      const char *core_file, char *core_name, size_t len)
{
    ...
    * suffix */
   if (!string_ends_with_size(core_file, "_libks_android.so",
         core_file_len, STRLEN_CONST("_libks_android.so")))
      return false;

   /* Copy core_file and remove suffix */
   strlcpy(core_name, core_file, len);
   core_name[core_file_len - STRLEN_CONST("_libks_android.so")] = '\0';
...
struct string_list *play_feature_delivery_available_cores(void)
{
   ...
   for (i = 0; i < num_cores; i++)
   {
      ...
      if (!string_is_empty(core_name))
      {
         ...
         strlcat(core_file, "_libks_android.so", sizeof(core_file));
