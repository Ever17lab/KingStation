      ...
      unsigned *new_rate)
{
   ...
   KINGSN_LOG("[Audio]: Using libnx_audren_thread driver\n");

   aud = (libnx_audren_thread_t*)calloc(1, sizeof(libnx_audren_thread_t));

   if (!aud)
   {
      KINGSN_ERR("[Audio]: struct alloc failed\n");
      goto fail;
   }

   real_latency     = MAX(latency, 5);
   KINGSN_LOG("[Audio]: real_latency is %u\n", real_latency);
   ...
   if (!aud->mempool)
   {
      KINGSN_ERR("[Audio]: mempool alloc failed\n");
      goto fail;
   }
   ...
   if (R_FAILED(rc))
   {
      KINGSN_ERR("[Audio]: audrenInitialize: %x\n", rc);
      goto fail;
   }
   ...
   if (R_FAILED(rc))
   {
      KINGSN_ERR("[Audio]: audrvCreate: %x\n", rc);
      goto fail_init;
   }
   ...
   if (R_FAILED(rc))
   {
      KINGSN_ERR("[Audio]: audrenStartAudioRenderer: %x\n", rc);
   }
   ...
   if (!aud->fifo)
   {
      KINGSN_ERR("[Audio]: fifo alloc failed\n");
      goto fail_drv;
   }
   ...
   if (R_FAILED(rc))
   {
      KINGSN_ERR("[Audio]: threadCreate: %x\n", rc);
      goto fail_drv;
   }
   ...
   if (R_FAILED(rc))
   {
      KINGSN_ERR("[Audio]: threadStart: %x\n", rc);
