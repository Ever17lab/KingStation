...
/* Forward declaration */
static void apple_kingsn_exited(void);

static void kingsn_enable_ui(void)
{
   bool boolean = true;
...
static void kingsn_disable_ui(void)
{
   bool boolean = false;
...
static void kingsn_draw_observer(CFRunLoopObserverRef observer,
    CFRunLoopActivity activity, void *info)
{
   ...
   if (kingsn_ctl(KINGSN_CTL_IS_IDLE, NULL))
      return;
         ...
         switch (eventType)
         {
            ...
            case GSEVENT_TYPE_KEYUP:
               apple_input_keyboard_event(eventType == GSEVENT_TYPE_KEYDOWN,
                     *(uint16_t*)&eventMem[0x3C], 0, 0, KS_DEVICE_KEYBOARD);
               break;
         }
      }
   }
...
- (void)applicationDidFinishLaunching:(UIApplication *)application
{
   ...
   if (kingsn_main(argc, argv, NULL))
      apple_kingsn_exited();

   iterate_observer = CFRunLoopObserverCreate(0, kCFRunLoopBeforeWaiting,
         true, 0, kingsn_draw_observer, 0);
