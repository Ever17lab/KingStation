            ...
            uint16_t keycode          = event.keyCode;
               ...
               for (i = 1; i < ch.length; i++)
                  apple_input_keyboard_event(event_type == NSEventTypeKeyDown,
                        0, inputTextUTF8[i], mod, KS_DEVICE_KEYBOARD);

            apple_input_keyboard_event(event_type == NSEventTypeKeyDown,
                  keycode, character, mod, KS_DEVICE_KEYBOARD);
         }
         break;
#if defined(HAVE_COCOA_METAL)
        ...
        case NSFlagsChanged:
#endif
         {
            ...
            apple_input_keyboard_event(down, keycode,
                  0, (uint32_t)new_flags, KS_DEVICE_KEYBOARD);
         }
         break;
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
   ...
   if (kingsn_main(waiting_argc, waiting_argv, NULL))
   ...
   [self performSelectorOnMainThread:@selector(kingsn_main) withObject:nil waitUntilDone:NO];
}
...
- (NSApplicationTerminateReply)applicationShouldTerminate:(NSApplication *)sender
{
   ...
   if (kingsn_ctl(KINGSN_CTL_IS_INITED, NULL))
...
- (void)application:(NSApplication *)sender openFiles:(NSArray *)filenames
{
   if ((filenames.count == 1) && [filenames objectAtIndex:0])
   {
      ...
      if (core_name)
      {
         ...
         path_set(KINGSN_PATH_CONTENT, __core.UTF8String);
...
{
   kingsn_system_info_t *info        = runloop_get_system_info();
   ...
   path_set(KINGSN_PATH_CORE, state->result);
      ...
      ui_browser_window_state_t *state, bool result)
{
   ...
   path_set(KINGSN_PATH_CONTENT, state->result);
...
- (IBAction)openCore:(id)sender
{
   ...
   if (browser)
   {
      ...
      browser_state.startdir        = strdup(path_dir_libks);
...
- (IBAction)showCoresDirectory:(id)sender
{
   settings_t          *settings = config_get_ptr();
   const char *path_dir_libks = settings->paths.directory_libks;
   [[NSWorkspace sharedWorkspace] openFile:BOXSTRING(path_dir_libks)];
}
