      ...
      void *data, void *params_data)
{
   ...
   if(!string_is_empty(workdir))
   {
      ...
      snprintf(user_path, sizeof(user_path),
            "%s/shared/misc/KingStation", workdir);
   ...
   else
   {
      ...
      snprintf(user_path, sizeof(user_path), "shared/misc/KingStation");
   ...
   if (!filestream_exists(data_assets_path))
   {
      ...
      KINGSN_LOG( "Copying application assets to data directory...\n" );
      ...
      if(system(copy_command) == -1)
         KINGSN_LOG( "Asset copy failed: Shell could not be run.\n" );
      else
         KINGSN_LOG( "Asset copy successful.\n");
   }
