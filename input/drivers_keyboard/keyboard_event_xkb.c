...
int init_xkb(int fd, size_t size)
{
   ...
   if (xkb_map)
   {
      ...
      *map_idx                 = xkb_keymap_mod_get_index(
      ...
      map_idx++;
      *map_bit                 = KSKMOD_CAPSLOCK;
      map_bit++;
      *map_idx                 = xkb_keymap_mod_get_index(
            xkb_map, XKB_MOD_NAME_SHIFT);
      map_idx++;
      *map_bit                 = KSKMOD_SHIFT;
      map_bit++;
      *map_idx                 = xkb_keymap_mod_get_index(
...
int handle_xkb(int code, int value)
{
   ...
   input_keyboard_event(value, input_keymaps_translate_keysym_to_rk(code),
         num_syms ? xkb_keysym_to_utf32(syms[0]) : 0, mod, KS_DEVICE_KEYBOARD);

   for (i = 1; i < num_syms; i++)
      input_keyboard_event(value, KSK_UNKNOWN,
            xkb_keysym_to_utf32(syms[i]), mod, KS_DEVICE_KEYBOARD);
