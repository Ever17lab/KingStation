...
static void underrun_update_cb(pa_stream *s, void *data)
{
   ...
   KINGSN_LOG("[PulseAudio]: Underrun (Buffer: %u, Writable size: %u).\n",
...
#if 0
   KINGSN_LOG("[PulseAudio]: Got new buffer size %u.\n", (unsigned)pa->buffer_size);
#endif
}
      ...
      unsigned *new_rate)
{
   ...
   pa->context = pa_context_new(pa_threaded_mainloop_get_api(pa->mainloop), "KingStation");
   if (!pa->context)
      goto error;
   ...
   {
      pa->buffer_size = server_attr->tlength;
      KINGSN_LOG("[PulseAudio]: Requested %u bytes buffer, got %u.\n",
...
static bool pulse_start(void *data, bool is_shutdown)
{
   ...
   KINGSN_LOG("[PulseAudio]: Unpausing.\n");
