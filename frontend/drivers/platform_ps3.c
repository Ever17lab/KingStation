...
#ifndef IS_SALAMANDER
   ks_main_log_file_init("/dev_hdd0/game/" EMULATOR_CONTENT_DIR "/USRDIR/KingStation-log.txt", false);
#else
   ks_main_log_file_init("/dev_hdd0/game/" EMULATOR_CONTENT_DIR "/USRDIR/KingStation-log-salamander.txt", false);
#endif
#endif
      ...
      void *args, void *params_data)
{
   ...
   logger_init();
#elif defined(HAVE_FILE_LOGGER)
   ks_main_log_file_init("/dev_hdd0/KingStation-log.txt", false);
#endif
#endif
         ...
         && (string_is_equal(argv[2], EMULATOR_CONTENT_DIR)))
   {
      ...
      KINGSN_LOG("Started from multiMAN, auto-game start enabled.\n");
   }
   else
#endif
#ifndef IS_SALAMANDER
      if (*argc > 1 && !string_is_empty(argv[1]))
      {
         ...
         struct kingsn_main_wrap      *args = (struct kingsn_main_wrap*)params_data;
         ...
         KINGSN_WARN("Started from Salamander, auto-game start disabled.\n");
#endif
   ...
   if (ret < 0)
   {
      KINGSN_ERR("cellGameBootCheck() Error: 0x%x.\n", ret);
   }
   else
   {
      char content_info_path[PATH_MAX_LENGTH] = {0};

      KINGSN_LOG("cellGameBootCheck() OK.\n");
      KINGSN_LOG("Directory name: [%s].\n", dirName);
      KINGSN_LOG(" HDD Free Size (in KB) = [%d] Size (in KB) = [%d] System Size (in KB) = [%d].\n",
            size.hddFreeSizeKB, size.sizeKB, size.sysSizeKB);

      switch(get_type)
      {
         case CELL_GAME_GAMETYPE_DISC:
            KINGSN_LOG("KingStation was launched on Optical Disc Drive.\n");
            break;
         case CELL_GAME_GAMETYPE_HDD:
            KINGSN_LOG("KingStation was launched on HDD.\n");
            break;
      }

      if ((get_attributes & CELL_GAME_ATTRIBUTE_APP_HOME)
            == CELL_GAME_ATTRIBUTE_APP_HOME)
         KINGSN_LOG("KingStation was launched from host machine (APP_HOME).\n");
      ...
      if (ret < 0)
         KINGSN_ERR("cellGameContentPermit() Error: 0x%x\n", ret);
      else
      {
         KINGSN_LOG("cellGameContentPermit() OK.\n");
         KINGSN_LOG("content_info_path : [%s].\n", content_info_path);
         KINGSN_LOG("usrDirPath : [%s].\n", g_defaults.dirs[DEFAULT_DIR_PORT]);
      }
...
static void frontend_ps3_init(void *data)
{
   (void)data;
#ifdef HAVE_SYSUTILS
   KINGSN_LOG("Registering system utility callback...\n");
      ...
      char const *argv[], char const *envp[])
{
   ...
   if (ret <  0)
   {
      KINGSN_WARN("SELF file is not of NPDRM type, trying another approach to boot it...\n");
...
static void frontend_ps3_exec(const char *path, bool should_load_game)
{
   ...
   {
      char game_path[256];
      strlcpy(game_path, path_get(KINGSN_PATH_CONTENT), sizeof(game_path));
