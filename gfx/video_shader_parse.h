...
enum kingsn_shader_type
{
   KINGSN_SHADER_NONE = 0,
   KINGSN_SHADER_CG,
   KINGSN_SHADER_HLSL,
   KINGSN_SHADER_GLSL,
   KINGSN_SHADER_SLANG,
...
enum gfx_scale_type
{
   KINGSN_SCALE_INPUT = 0,
   KINGSN_SCALE_ABSOLUTE,
   KINGSN_SCALE_VIEWPORT
};

enum
{
   KINGSN_FILTER_UNSPEC = 0,
   KINGSN_FILTER_LINEAR,
   KINGSN_FILTER_NEAREST,
   KINGSN_FILTER_MAX
};

enum gfx_wrap_type
{
   KINGSN_WRAP_BORDER = 0, /* Kinda deprecated, but keep as default.
                             Will be translated to EDGE in GLES. */
   KINGSN_WRAP_DEFAULT = KINGSN_WRAP_BORDER,
   KINGSN_WRAP_EDGE,
   KINGSN_WRAP_REPEAT,
   KINGSN_WRAP_MIRRORED_REPEAT,
   KINGSN_WRAP_MAX
};
