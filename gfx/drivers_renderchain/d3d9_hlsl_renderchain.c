...
#define KINGSN_HLSL_MAX_SHADERS 16
      ...
      const char *prog)
{
   ...
   {
      KINGSN_ERR("Could not compile fragment shader program (%s)..\n", prog);
      goto error;
   }
   if (!d3d9x_compile_shader_from_file(prog, NULL, NULL,
            "main_vertex", "vs_3_0", 0, &code_v, &listing_v, &pass->vtable))
   {
      KINGSN_ERR("Could not compile vertex shader program (%s)..\n", prog);
      goto error;
   }
      ...
      const char *prog)
{
   ...
   if (!d3d9x_compile_shader(prog, strlen(prog), NULL, NULL,
            ...
            &pass->ftable ))
   {
      KINGSN_ERR("Could not compile stock fragment shader..\n");
      goto error;
   }
   if (!d3d9x_compile_shader(prog, strlen(prog), NULL, NULL,
            ...
            &pass->vtable ))
   {
      KINGSN_ERR("Could not compile stock vertex shader..\n");
      goto error;
   }
...
{
   KINGSN_LOG("[D3D9 HLSL]: Destroying programs.\n");
...
static bool hlsl_d3d9_renderchain_init_shader(d3d9_video_t *d3d,
      hlsl_renderchain_t *chain)
{
   KINGSN_LOG("[D3D9]: Using HLSL shader backend.\n");
      ...
      )
{
   ...
   {
      KINGSN_ERR("[D3D9 HLSL]: Failed to initialize shader subsystem.\n");
      return false;
   }
