      ...
      )
{
   ...
   if (list_size > 0)
   {
      for (i = 0; i < list_size; i++)
      {
         ...
         {
            case KINGSN_DIRECTORY:
               file_type = FILE_TYPE_DIRECTORY;
               break;
            case KINGSN_COMPRESSED_ARCHIVE:
               file_type = FILE_TYPE_CARCHIVE;
               break;
            case KINGSN_COMPRESSED_FILE_IN_ARCHIVE:
               file_type = FILE_TYPE_IN_CARCHIVE;
               break;
            case KINGSN_PLAIN_FILE:
            default:
         ...
         {
            enum kingsn_content_type path_type = path_is_media_type(path);

            if (filebrowser_types == FILEBROWSER_SELECT_COLLECTION)
               file_type = FILE_TYPE_PLAYLIST_COLLECTION;

            if (path_type == KINGSN_CONTENT_MUSIC)
...
static unsigned menu_displaylist_parse_system_info(file_list_t *list)
{
   ...
   KingStation_get_capabilities(KINGSN_CAPABILITIES_COMPILER, tmp, sizeof(tmp));
   if (menu_entries_append_enum(list, tmp, "",
         MENU_ENUM_LABEL_SYSTEM_INFO_ENTRY, MENU_SETTINGS_CORE_INFO_NONE, 0, 0))
      count++;
...

   {
      ...
      KingStation_get_capabilities(KINGSN_CAPABILITIES_CPU,
   ...
   if (frontend)
   {
      ...
      if (frontend->get_rating)
      {
         snprintf(tmp, sizeof(tmp), "%s : %d",
               msg_hash_to_str(MENU_ENUM_LABEL_VALUE_SYSTEM_INFO_KSRATING_LEVEL),
               frontend->get_rating());
         if (menu_entries_append_enum(list, tmp, "",
               MENU_ENUM_LABEL_SYSTEM_INFO_ENTRY,
               MENU_SETTINGS_CORE_INFO_NONE, 0, 0))
...

   {
      ...
      menu_features_info_t info_list[] = {
         {SUPPORTS_LIBKSDB, MENU_ENUM_LABEL_VALUE_SYSTEM_INFO_LIBKSDB_SUPPORT},
      ...
      menu_displaylist_info_t *info)
{
   ...
   for (i = 0; i < db_info->count; i++)
   {
          ...
          !string_is_equal(menu_driver, "glui"))
      {
         ...
         menu_driver_ctl(KINGSN_MENU_CTL_UPDATE_THUMBNAIL_PATH, NULL);
         menu_driver_ctl(KINGSN_MENU_CTL_UPDATE_THUMBNAIL_IMAGE, NULL);
      }
      ...
      file_list_t *list, bool horizontal)
{
   ...
   if (!kingsn_ctl(KINGSN_CTL_IS_DUMMY_CORE, NULL))
   {
#ifdef HAVE_LAKKA
      bool show_advanced_settings         = settings->bools.menu_show_advanced_settings;
#endif
      ...
      menu_displaylist_info_t *info)
{
   ...
   playlist_t *playlist            = playlist_get_cached();
   settings_t *settings            = config_get_ptr();
   const char *fullpath            = path_get(KINGSN_PATH_CONTENT);
   ...
   content_loaded = !kingsn_ctl(KINGSN_CTL_IS_DUMMY_CORE, NULL)
         ...
         !settings->bools.kiosk_mode_enable)
   {
      ...
      if (content_loaded)
      {
         const char *core_path = path_get(KINGSN_PATH_CORE);
...
static unsigned menu_displaylist_parse_information_list(file_list_t *info_list)
{
   ...
   if (kingsn_ctl(KINGSN_CTL_IS_PERFCNT_ENABLE, NULL))
   {
      if (menu_entries_append_enum(info_list,
      ...
      menu_displaylist_info_t *info)
{
   ...
   unsigned idx                        = menu->rpl_entry_selection_ptr;
   const struct playlist_entry *entry  = NULL;
   const char *loaded_content_path     = path_get(KINGSN_PATH_CONTENT);
   const char *loaded_core_path        = path_get(KINGSN_PATH_CORE);
...
static int menu_displaylist_parse_input_description_list(menu_displaylist_info_t *info)
{
   ...
   user_idx = (info->type - MENU_SETTINGS_INPUT_DESC_BEGIN) / (KINGSN_FIRST_CUSTOM_BIND + 8);
   btn_idx  = (info->type - MENU_SETTINGS_INPUT_DESC_BEGIN) - (KINGSN_FIRST_CUSTOM_BIND + 8) * user_idx;

   if ((user_idx >= MAX_USERS) || (btn_idx >= KINGSN_CUSTOM_BIND_LIST_END))
      goto end;

   /* Get current mapping for selected button */
   current_remap_idx = settings->uints.input_remap_ids[user_idx][btn_idx];

   if (current_remap_idx >= KINGSN_CUSTOM_BIND_LIST_END)
      current_remap_idx = KINGSN_UNMAPPED;
   ...
   for (i = 0; i < KINGSN_CUSTOM_BIND_LIST_END; i++)
   {
      ...
      if (!string_is_empty(input_desc_btn))
      {
         ...
         /* > Up to KINGSN_FIRST_CUSTOM_BIND, inputs
          *   are buttons - description can be used
          *   directly
          * > Above KINGSN_FIRST_CUSTOM_BIND, inputs
          *   are analog axes - have to add +/-
          *   indicators */
         if (i < KINGSN_FIRST_CUSTOM_BIND)
...
static int menu_displaylist_parse_input_description_kbd_list(menu_displaylist_info_t *info)
{
   ...
   /* Determine user/button indices */
   user_idx = (info->type - MENU_SETTINGS_INPUT_DESC_KBD_BEGIN) / KINGSN_FIRST_CUSTOM_BIND;
   btn_idx  = (info->type - MENU_SETTINGS_INPUT_DESC_KBD_BEGIN) - KINGSN_FIRST_CUSTOM_BIND * user_idx;
   ...
   if (current_key_id >= (KINGSN_MAX_KEYS + MENU_SETTINGS_INPUT_DESC_KBD_BEGIN))
      current_key_id = KSK_FIRST;
...
static void bluetooth_scan_callback(ks_task_t *task,
      void *task_data,
...
#ifdef HAVE_NETWORKING
static void wifi_scan_callback(ks_task_t *task,
      void *task_data,
      void *user_data, const char *error)
{
   bool refresh = false;
...
bool menu_displaylist_process(menu_displaylist_info_t *info)
{
   ...
   if (info->push_builtin_cores)
   {
      ...
      menu_entries_append_enum(info->list,
            msg_hash_to_str(MENU_ENUM_LABEL_VALUE_START_NET_KSPAD),
            msg_hash_to_str(MENU_ENUM_LABEL_START_NET_KSPAD),
...
static unsigned menu_displaylist_populate_subsystem(
      const struct ks_subsystem_info* subsystem, file_list_t *list)
{
   char star_char[16];
   unsigned count           = 0;
   settings_t *settings     = config_get_ptr();
            ...
            if (content_get_subsystem_rom_id() < subsystem->num_roms)
            {
               ...
               if (is_rgui && !menu_show_sublabels)
               {
                  ...
                  if ((n < 0) || (n >= PATH_MAX_LENGTH))
                  {
                     ...
                     {
                        KINGSN_WARN("Menu subsystem entry: Description label truncated.\n");
                     }
                  }
      ...
      bool include_everything)
{
      ...
      case DISPLAYLIST_INPUT_HOTKEY_BINDS_LIST:
         {
            ...
            {
               if (input_config_bind_map_get_ks_key(i) == KINGSN_ENABLE_HOTKEY)
               {
      ...
      case DISPLAYLIST_OPTIONS_REMAPPINGS:
         {
            ...
            if (kingsn_ctl(KINGSN_CTL_IS_REMAPS_CORE_ACTIVE, NULL))
               if (menu_entries_append_enum(list,
            ...
            if (kingsn_ctl(KINGSN_CTL_IS_REMAPS_GAME_ACTIVE, NULL))
               if (menu_entries_append_enum(list,
            ...
            if (kingsn_ctl(KINGSN_CTL_IS_REMAPS_CONTENT_DIR_ACTIVE, NULL))
               if (menu_entries_append_enum(list,
 ...
 /*        if (MENU_DISPLAYLIST_PARSE_SETTINGS_ENUM(list,
      ...
      case DISPLAYLIST_PERFCOUNTERS_FRONTEND:
         {
            unsigned i;
            struct ks_perf_counter **counters =
               (type == DISPLAYLIST_PERFCOUNTERS_CORE)
               ? ks_get_perf_counter_libks()
               : ks_get_perf_counter_kingsn();
            unsigned num                         =
               (type == DISPLAYLIST_PERFCOUNTERS_CORE)
               ?   ks_get_perf_count_libks()
               : ks_get_perf_count_kingsn();
            unsigned id                          =
               (type == DISPLAYLIST_PERFCOUNTERS_CORE)
               ? MENU_SETTINGS_LIBKS_PERF_COUNTERS_BEGIN
      ...
      case DISPLAYLIST_CHEAT_SEARCH_SETTINGS_LIST:
#ifdef HAVE_CHEATS
         {
            ...
            {
               kingsn_setting_t *setting = menu_setting_find_enum(MENU_ENUM_LABEL_CHEAT_DELETE_MATCH);
               if (setting)
      ...
      case DISPLAYLIST_CHEAT_DETAILS_SETTINGS_LIST:
#ifdef HAVE_CHEATS
         {
            ...
            {
               kingsn_setting_t *setting = menu_setting_find_enum(MENU_ENUM_LABEL_CHEAT_ADDRESS);
               if (setting )
      ...
      case DISPLAYLIST_ACCOUNTS_YOUTUBE_LIST:
         ...
         break;
      case DISPLAYLIST_KS_ACHIEVEMENTS_SETTINGS_LIST:
         {
            settings_t *settings      = config_get_ptr();
      ...
      case DISPLAYLIST_USER_INTERFACE_SETTINGS_LIST:
         {
            ...
            menu_displaylist_build_info_selective_t build_list[] = {
               ...
               {MENU_ENUM_LABEL_PAUSE_LIBKS,                                        PARSE_ONLY_BOOL,   true},
      ...
      case DISPLAYLIST_MENU_VIEWS_SETTINGS_LIST:
         {
            menu_displaylist_build_info_selective_t build_list[] = {
               ...
               {MENU_ENUM_LABEL_MENU_SHOW_QUIT_KingStation,                              PARSE_ONLY_BOOL, true  },
               {MENU_ENUM_LABEL_MENU_SHOW_RESTART_KingStation,                           PARSE_ONLY_BOOL, true  },
      ...
      case DISPLAYLIST_ACCOUNTS_LIST:
         {
            menu_displaylist_build_info_t build_list[] = {
               {MENU_ENUM_LABEL_ACCOUNTS_KS_ACHIEVEMENTS,            PARSE_ACTION},
      ...
      case DISPLAYLIST_SETTINGS_ALL:
         {
            ...
            menu_displaylist_build_info_selective_t build_list[] = {
               ...
               {MENU_ENUM_LABEL_ONLINE_UPDATER,               PARSE_ACTION, true},
               {MENU_ENUM_LABEL_INFORMATION_LIST,               PARSE_ACTION, true},
               {MENU_ENUM_LABEL_CONFIGURATIONS_LIST,               PARSE_ACTION, true},
               {MENU_ENUM_LABEL_HELP_LIST,               PARSE_ACTION, true},			   
            };
            ...
            for (i = 0; i < ARRAY_SIZE(build_list); i++)
            {
               switch (build_list[i].enum_idx)
               {
                  ...
                  case MENU_ENUM_LABEL_POWER_MANAGEMENT_SETTINGS:
                     ...
                     break;
                  case MENU_ENUM_LABEL_KS_ACHIEVEMENTS_SETTINGS:
...
#ifdef HAVE_CHEEVOS
               {MENU_ENUM_LABEL_KS_ACHIEVEMENTS_SETTINGS, PARSE_ACTION, true},
#endif
      ...
      case DISPLAYLIST_DIRECTORY_SETTINGS_LIST:
         {
            menu_displaylist_build_info_t build_list[] = {
               ...
               {MENU_ENUM_LABEL_LIBKS_DIR_PATH,               PARSE_ONLY_DIR},
               {MENU_ENUM_LABEL_LIBKS_INFO_PATH,              PARSE_ONLY_DIR},
      ...
      case DISPLAYLIST_LOGGING_SETTINGS_LIST:
         {
            ...
            menu_displaylist_build_info_selective_t build_list[] = {
               ...
               {MENU_ENUM_LABEL_LIBKS_LOG_LEVEL,    PARSE_ONLY_UINT, false},
            ...
            for (i = 0; i < ARRAY_SIZE(build_list); i++)
            {
               switch (build_list[i].enum_idx)
               {
                  ...
                  case MENU_ENUM_LABEL_LIBKS_LOG_LEVEL:
                     if (verbosity_is_enabled())
                        build_list[i].checked = true;
                     break;
...
static bool history_needs_navigation_clear(menu_handle_t *menu, playlist_t *playlist)
{
   ...
   if (!kingsn_ctl(KINGSN_CTL_IS_DUMMY_CORE, NULL))
      return string_is_equal(menu->deferred_path, path_get(KINGSN_PATH_CONTENT));
...
unsigned menu_displaylist_netplay_refresh_rooms(file_list_t *list)
{
   ...
   if (netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_ENABLED, NULL) &&
      !netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_SERVER, NULL) &&
      netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_CONNECTED, NULL))
   {
      if (menu_entries_append_enum(list,
   ...
   switch (type)
   {
      case DISPLAYLIST_NETWORK_HOSTING_SETTINGS_LIST:
#ifdef HAVE_NETWORKING
         {
            ...
            else if (netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_ENABLED, NULL) &&
                  !netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_SERVER, NULL) &&
                  netplay_driver_ctl(KINGSN_NETPLAY_CTL_IS_CONNECTED, NULL))
            {
            }
      ...
      case DISPLAYLIST_OPTIONS_REMAPPINGS_PORT:
         {
...

            {
               ...
               snprintf(key_type, sizeof(key_type),
                     msg_hash_to_str(MENU_ENUM_LABEL_INPUT_LIBKS_DEVICE), val);
            ...
            {
               unsigned ks_id;
               unsigned device  = settings->uints.input_libks_device[p];
               device &= KS_DEVICE_MASK;

               if (device == KS_DEVICE_JOYPAD || device == KS_DEVICE_ANALOG)
               {
                  for (ks_id = 0; ks_id < KINGSN_ANALOG_BIND_LIST_END; ks_id++)
                  {
                     char desc_label[400];
                     char descriptor[300];
                     const struct ks_keybind *keybind   =
                        &input_config_binds[p][ks_id];
                     const struct ks_keybind *auto_bind =
                        (const struct ks_keybind*)
                        input_config_get_bind_auto(p, ks_id);
                     ...
                     {
                        const struct ks_keybind *keyptr =
                           &input_config_binds[p][ks_id];
                     ...
                     if (menu_entries_append_enum(list, descriptor, "",
                              ...
                              (p * (KINGSN_FIRST_CUSTOM_BIND + 8)) +  ks_id, 0, 0))
                        count++;
                  }
               }
               else if (device == KS_DEVICE_KEYBOARD)
               {
                  for (ks_id = 0; ks_id < KINGSN_FIRST_CUSTOM_BIND; ks_id++)
                  {
                     char desc_label[400];
                     char descriptor[300];
                     const struct ks_keybind *keybind   =
                     ...
                     if (menu_entries_append_enum(list, descriptor, "",
                              ...
                              (p * KINGSN_FIRST_CUSTOM_BIND) + ks_id, 0, 0))
                        count++;
                  }
               }
            }
      ...
      case DISPLAYLIST_CDROM_DETAIL_INFO:
      {
         ...
         if (cdrom_drive_has_media(drive))
         {
            ...
            file = filestream_open(file_path, KS_VFS_FILE_ACCESS_READ, 0);

            if (file)
            {
               const cdrom_toc_t *toc = ks_vfs_file_get_cdrom_toc();
               ...
               if (media_detect_cd_info(file_path, 0, &cd_info))
               {
                  ...
                  KINGSN_ERR("[CDROM]: Could not detect any disc info.\n");
            }
            else
         ...
         {
            KINGSN_LOG("[CDROM]: No media is inserted or drive is not ready.\n");
 ...
 /*              if (menu_entries_append_enum(info->list,
      ...
      case DISPLAYLIST_CORES_SUPPORTED:
...

         {
            ...
            if (cores_names_size == 0)
            {
               ...
               if (!path_is_empty(KINGSN_PATH_CORE))
               {

                  if (menu_entries_append_enum(info->list,
                        path_get(KINGSN_PATH_CORE),
                        path_get(KINGSN_PATH_CORE),
            ...
            if (cores_names_size != 0)
            {
               ...
               for (i = 0; i < cores_names_size; i++)
               {
                  ...
                  if (     !path_is_empty(KINGSN_PATH_CORE) &&
                        string_is_equal(core_path, path_get(KINGSN_PATH_CORE)))
                  {
      ...
      case DISPLAYLIST_CORE_OPTIONS:
         {
            ...
            if (kingsn_ctl(KINGSN_CTL_HAS_CORE_OPTIONS, NULL))
            {
               size_t               num_opts = 0;
               settings_t      *settings     = config_get_ptr();
               bool game_specific_options    = settings->bools.game_specific_options;

               kingsn_ctl(KINGSN_CTL_GET_CORE_OPTION_SIZE, &num_opts);

               if (game_specific_options)
               {
                  if (!kingsn_ctl(KINGSN_CTL_IS_GAME_OPTIONS_ACTIVE, NULL))
                  {
                     if (menu_entries_append_enum(info->list,
                           msg_hash_to_str(
               ...
               if (num_opts != 0)
               {
                  ...
                  if (kingsn_ctl(KINGSN_CTL_CORE_OPTIONS_LIST_GET, &coreopts))
                  {
      ...
      case DISPLAYLIST_OPTIONS:
         menu_entries_ctl(MENU_ENTRIES_CTL_CLEAR, info->list);
         {
...
#if defined(HAVE_CG) || defined(HAVE_GLSL) || defined(HAVE_SLANG) || defined(HAVE_HLSL)
            if (video_shader_is_supported(KINGSN_SHADER_CG))
            {
               if (menu_entries_append_enum(info->list,
            ...
            if (video_shader_is_supported(KINGSN_SHADER_GLSL))
            {
               if (menu_entries_append_enum(info->list,
            ...
            if (video_shader_is_supported(KINGSN_SHADER_SLANG))
            {
               if (menu_entries_append_enum(info->list,
      ...
      case DISPLAYLIST_MAIN_MENU:
         menu_entries_ctl(MENU_ENTRIES_CTL_CLEAR, info->list);
         {
            ...
            if (kingsn_ctl(KINGSN_CTL_CORE_IS_RUNNING, NULL))
            {
               if (!kingsn_ctl(KINGSN_CTL_IS_DUMMY_CORE, NULL))
                  if (MENU_DISPLAYLIST_PARSE_SETTINGS_ENUM(info->list,
      ...
      case DISPLAYLIST_DROPDOWN_LIST:
         {
                     ...
                     STRLEN_CONST("core_option_")))
            {
               ...
               if (tmp_str_list.size > 0)
               {
                  ...
                  kingsn_ctl(KINGSN_CTL_CORE_OPTIONS_LIST_GET, &coreopts);
            ...
            else
            {
               ...
               kingsn_setting_t     *setting = menu_setting_find_enum(enum_idx);
