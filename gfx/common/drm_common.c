...
bool drm_get_resources(int fd)
{
   ...
   if (!g_drm_resources)
   {
      KINGSN_WARN("[DRM]: Couldn't get device resources.\n");
      return false;
   }
...
bool drm_get_connector(int fd, unsigned monitor_index)
{
   ...
   KINGSN_LOG("[DRM]: Found %d connectors.\n", g_drm_resources->count_connectors);
   ...
   for (i = 0; (int)i < g_drm_resources->count_connectors; i++)
   {
      ...
      if (conn)
      {
         ...
         KINGSN_LOG("[DRM]: Connector %d connected: %s\n", i, connected ? "yes" : "no");
         KINGSN_LOG("[DRM]: Connector %d has %d modes.\n", i, conn->count_modes);
         if (connected && conn->count_modes > 0)
         {
            monitor_index_count++;
            KINGSN_LOG("[DRM]: Connector %d assigned to monitor index: #%u.\n", i, monitor_index_count);
         }
   ...
   if (!g_drm_connector)
   {
      KINGSN_WARN("[DRM]: Couldn't get device connector.\n");
      return false;
   }
   return true;
}
...
bool drm_get_encoder(int fd)
{
   ...
   if (!g_drm_encoder)
   {
      KINGSN_WARN("[DRM]: Couldn't find DRM encoder.\n");
      return false;
   }

   for (i = 0; (int)i < g_drm_connector->count_modes; i++)
   {
      KINGSN_LOG("[DRM]: Mode %d: (%s) %d x %d, %u Hz\n",
            i,
...
void drm_setup(int fd)
{
   ...
   if (!g_orig_crtc)
      KINGSN_WARN("[DRM]: Cannot find original CRTC.\n");
}
