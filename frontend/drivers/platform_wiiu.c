...
#endif
static const char *elf_path_cst = WIIU_SD_PATH "KingStation/KingStation.elf";
...
static void frontend_wiiu_exec(const char *path, bool should_load_content)
{
   ...
   if (!path || !*path)
   {
      KINGSN_LOG("No executable path provided, cannot Restart\n");
   }
...
#ifndef IS_SALAMANDER
   if (should_load_content && !path_is_empty(KINGSN_PATH_CONTENT))
   {
      strcpy(param->args + len, path_get(KINGSN_PATH_CONTENT));
      param->argv[param->argc] = param->args + len;
      len += strlen(param->args + len) + 1;
      param->argc++;
...

   {
      if (HBL_loadToMemory(path, (u32)param->args - (u32)param + len) < 0)
         KINGSN_LOG("Failed to load core\n");
      else
      {
