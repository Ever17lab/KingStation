...
#include "vulkan_shaders/pipeline_bokeh.frag.inc"
   ...
   for (i = 0; i < ARRAY_SIZE(vk->display.pipelines) - 4; i++)
   {
      switch (i >> 1)
      {
         ...
         default:
            ks_assert(0 && "No shader for menu pipeline.");
            break;
      }
...
static bool vulkan_init_filter_chain_preset(vk_t *vk, const char *shader_path)
{
   ...
   if (!vk->filter_chain)
   {
      KINGSN_ERR("[Vulkan]: Failed to create preset: \"%s\".\n", shader_path);
      return false;
   }
...
static bool vulkan_init_filter_chain(vk_t *vk)
{
   ...
   {
      KINGSN_LOG("[Vulkan]: Loading stock shader.\n");
      return vulkan_init_default_filter_chain(vk);
   }

   if (type != KINGSN_SHADER_SLANG)
   {
      KINGSN_LOG("[Vulkan]: Only Slang shaders are supported, falling back to stock.\n");
      return vulkan_init_default_filter_chain(vk);
   }
      ...
      uint32_t src_queue_family)
{
   ...
   if (num_semaphores > 0)
   {
      ...
      ks_assert(stage_flags && new_semaphores);
...
{
   (void)handle;
   /* no-op. KingStation already waits for this
...
{
   struct ks_hw_render_interface_vulkan *iface   =
      &vk->hw.iface;
   struct ks_hw_render_callback *hwr =
      video_driver_get_hw_context();

   if (hwr->context_type != KS_HW_CONTEXT_VULKAN)
      return;

   vk->hw.enable               = true;

   iface->interface_type       = KS_HW_RENDER_INTERFACE_VULKAN;
   iface->interface_version    = KS_HW_RENDER_INTERFACE_VULKAN_VERSION;
...
static void vulkan_init_readback(vk_t *vk)
{
   ...
   if (!scaler_ctx_gen_filter(&vk->readback.scaler_bgr))
   {
      ...
      KINGSN_ERR("[Vulkan]: Failed to initialize scaler context.\n");
   }
      ...
      void **input_data)
{
   ...
   if (!ctx_driver)
   {
      KINGSN_ERR("[Vulkan]: Failed to get Vulkan context.\n");
      goto error;
   }
   ...
   KINGSN_LOG("[Vulkan]: Found vulkan context: %s\n", ctx_driver->ident);
   ...
   KINGSN_LOG("[Vulkan]: Detecting screen resolution %ux%u.\n", full_x, full_y);
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to set video mode.\n");
      goto error;
   }
   ...
   KINGSN_LOG("[Vulkan]: Using resolution %ux%u\n", temp_width, temp_height);

   if (!vk->ctx_driver || !vk->ctx_driver->get_context_data)
   {
      KINGSN_ERR("[Vulkan]: Failed to get context data.\n");
      goto error;
   }
   ...
   *(void**)&vk->context = vk->ctx_driver->get_context_data(vk->ctx_data);
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to init filter chain.\n");
      goto error;
   }
...
static void vulkan_update_filter_chain(vk_t *vk)
{
   ...
   if (!vulkan_filter_chain_update_swapchain_info((vulkan_filter_chain_t*)vk->filter_chain, &info))
      KINGSN_ERR("Failed to update filter chain info. This will probably lead to a crash ...\n");
}
      ...
      enum kingsn_shader_type type, const char *path)
{
   ...
   {
      KINGSN_WARN("[Vulkan]: Only Slang shaders are supported. Falling back to stock.\n");
      path = NULL;
   }
   ...
   {
      KINGSN_ERR("[Vulkan]: Failed to create filter chain: \"%s\". Falling back to stock.\n", path);
      vulkan_init_default_filter_chain(vk);
      return false;
   }
...
{
   vk_t *vk = (vk_t*)data;
   *iface   = (const struct ks_hw_render_interface*)&vk->hw.iface;
   return vk->hw.enable;
}
...
static bool vulkan_read_viewport(void *data, uint8_t *buffer, bool is_idle)
{
   ...
   if (vk->readback.streamed)
   {
      ...
      switch (vk->context->swapchain_format)
      {
         ...
         default:
            KINGSN_ERR("[Vulkan]: Unexpected swapchain format. Cannot readback.\n");
            break;
      }
   ...
   else
   {
...

      {
         ...
         switch (vk->context->swapchain_format)
         {
            ...
            default:
               KINGSN_ERR("[Vulkan]: Unexpected swapchain format.\n");
               break;
         }
      }
      vulkan_destroy_texture(
