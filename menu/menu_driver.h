...
enum menu_settings_type
{
   ...
   MENU_SETTINGS_BIND_LAST = MENU_SETTINGS_BIND_BEGIN + KINGSN_ANALOG_RIGHT_Y_MINUS,
   MENU_SETTINGS_BIND_ALL_LAST = MENU_SETTINGS_BIND_BEGIN + KINGSN_MENU_TOGGLE,
   ...
   MENU_SETTINGS_LIBKS_PERF_COUNTERS_END = MENU_SETTINGS_LIBKS_PERF_COUNTERS_BEGIN + (MAX_COUNTERS - 1),
   ...
   MENU_SETTINGS_INPUT_END = MENU_SETTINGS_INPUT_BEGIN + KINGSN_CUSTOM_BIND_LIST_END + 6,
   MENU_SETTINGS_INPUT_DESC_BEGIN,
   MENU_SETTINGS_INPUT_DESC_END = MENU_SETTINGS_INPUT_DESC_BEGIN + ((KINGSN_FIRST_CUSTOM_BIND + 8) * MAX_USERS),
   MENU_SETTINGS_INPUT_DESC_KBD_BEGIN,
   MENU_SETTINGS_INPUT_DESC_KBD_END = MENU_SETTINGS_INPUT_DESC_KBD_BEGIN + (KINGSN_MAX_KEYS * MAX_USERS),
   ...
   MENU_SETTINGS_SUBSYSTEM_LAST = MENU_SETTINGS_SUBSYSTEM_ADD + KINGSN_MAX_SUBSYSTEMS,
...
typedef struct
{
   ...
   struct
   {
      enum kingsn_shader_type preset_type;
      enum kingsn_shader_type pass_type;
...
bool menu_driver_ctl(enum kingsn_menu_ctl_state state, void *data);
...
ks_time_t menu_driver_get_current_time(void);

size_t menu_navigation_get_selection(void);

void menu_navigation_set_selection(size_t val);

void menu_display_handle_thumbnail_upload(ks_task_t *task,
      void *task_data,
      void *user_data, const char *err);

void menu_display_handle_left_thumbnail_upload(ks_task_t *task,
      void *task_data,
      void *user_data, const char *err);

void menu_display_handle_savestate_thumbnail_upload(ks_task_t *task,
      void *task_data,
      void *user_data, const char *err);
...
void menu_display_handle_wallpaper_upload(ks_task_t *task,
      void *task_data,
      void *user_data, const char *err);
...
enum kingsn_shader_type menu_driver_get_last_shader_preset_type(void);
enum kingsn_shader_type menu_driver_get_last_shader_pass_type(void);
