...
static inline void add_sublabel_and_whats_this(
      QWidget *widget, kingsn_setting_t *setting)
{
...
static inline QString form_label(kingsn_setting_t *setting)
{
...
void FormLayout::addCheckBox(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(new CheckBox(setting));
}

void FormLayout::addUIntRadioButtons(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(new UIntRadioButtons(setting));
}

void FormLayout::addUIntComboBox(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new UIntComboBox(setting));
}

void FormLayout::addStringComboBox(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new StringComboBox(setting));
}

void FormLayout::addStringLineEdit(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new StringLineEdit(setting));
}

void FormLayout::addPasswordLineEdit(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new PasswordLineEdit(setting));
}

void FormLayout::addUIntSpinBox(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new UIntSpinBox(setting));
}

void FormLayout::addSizeSpinBox(kingsn_setting_t *setting, unsigned scale)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new SizeSpinBox(setting, scale));
}

void FormLayout::addFloatSpinBox(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new FloatSpinBox(setting));
}

void FormLayout::addDirectorySelector(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new DirectorySelector(setting));
}

void FormLayout::addFileSelector(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
      addRow(form_label(setting), new FileSelector(setting));
}

void FormLayout::addFloatSliderAndSpinBox(kingsn_setting_t *setting)
{
   if (setting && setting->short_description)
...
{
   kingsn_setting_t   *red = menu_setting_find_enum(r);
   kingsn_setting_t *green = menu_setting_find_enum(g);
   kingsn_setting_t  *blue = menu_setting_find_enum(b);

   if (red && green && blue)
      addRow(title, new UIntColorButton(red, green, blue));
}

bool FormLayout::addBindButton(kingsn_setting_t *setting)
{
   if (!setting || !setting->short_description)
      return false;
...
void SettingsGroup::addCheckBox(kingsn_setting_t *setting)
{
   m_layout->addCheckBox(setting);
}

void SettingsGroup::addDirectorySelector(kingsn_setting_t *setting)
{
   m_layout->addDirectorySelector(setting);
}

void SettingsGroup::addFileSelector(kingsn_setting_t *setting)
{
   m_layout->addFileSelector(setting);
}

void SettingsGroup::addStringLineEdit(kingsn_setting_t *setting)
{
   m_layout->addStringLineEdit(setting);
}

void SettingsGroup::addPasswordLineEdit(kingsn_setting_t *setting)
{
   m_layout->addPasswordLineEdit(setting);
}

void SettingsGroup::addStringComboBox(kingsn_setting_t *setting)
{
   m_layout->addStringComboBox(setting);
}

void SettingsGroup::addUIntComboBox(kingsn_setting_t *setting)
{
   m_layout->addUIntComboBox(setting);
}

void SettingsGroup::addUIntRadioButtons(kingsn_setting_t *setting)
{
   m_layout->addUIntRadioButtons(setting);
}

void SettingsGroup::addUIntSpinBox(kingsn_setting_t *setting)
{
   m_layout->addUIntSpinBox(setting);
}

void SettingsGroup::addFloatSpinBox(kingsn_setting_t *setting)
{
   m_layout->addFloatSpinBox(setting);
}

void SettingsGroup::addFloatSliderAndSpinBox(kingsn_setting_t *setting)
{
...
void SettingsGroup::addBindButton(kingsn_setting_t *setting)
{
   m_layout->addBindButton(setting);
}

CheckBox::CheckBox(kingsn_setting_t *setting, QWidget *parent) :
...
CheckableSettingsGroup::CheckableSettingsGroup(kingsn_setting_t *setting, QWidget *parent) :
...
CheckableIcon::CheckableIcon(kingsn_setting_t *setting, const QIcon &icon, QWidget *parent) :
   QToolButton(parent)
   ,m_setting(setting)
   ,m_value(setting->value.target.boolean)
{
...
StringLineEdit::StringLineEdit(kingsn_setting_t *setting, QWidget *parent) :
...
PasswordLineEdit::PasswordLineEdit(kingsn_setting_t *setting, QWidget *parent) :
   StringLineEdit(setting)
{
   setEchoMode(QLineEdit::Password);
}

StringComboBox::StringComboBox(kingsn_setting_t *setting, QWidget *parent) :
   QComboBox(parent)
   ,m_setting(setting)
   ,m_value(setting->value.target.string)
{
...
UIntComboBox::UIntComboBox(kingsn_setting_t *setting, QWidget *parent) :
   QComboBox(parent)
   ,m_setting(setting)
...
UIntComboBox::UIntComboBox(kingsn_setting_t *setting, double min, double max, QWidget *parent) :
   QComboBox(parent)
   ,m_setting(setting)
   ,m_value(setting->value.target.unsigned_integer)
{
...
UIntSpinBox::UIntSpinBox(kingsn_setting_t *setting, QWidget *parent) :
   QSpinBox(parent)
   ,m_setting(setting)
   ,m_value(setting->value.target.unsigned_integer)
{
...
SizeSpinBox::SizeSpinBox(kingsn_setting_t *setting, unsigned scale, QWidget *parent) :
   QSpinBox(parent)
   ,m_setting(setting)
...
UIntRadioButton::UIntRadioButton(const QString &text, kingsn_setting_t *setting, unsigned value, QWidget *parent) :
...
UIntRadioButtons::UIntRadioButtons(kingsn_setting_t *setting, QWidget *parent) :
...
IntSpinBox::IntSpinBox(kingsn_setting_t *setting, QWidget *parent) :
   QSpinBox(parent)
   ,m_setting(setting)
...
FloatSpinBox::FloatSpinBox(kingsn_setting_t *setting, QWidget *parent) :
...
PathButton::PathButton(kingsn_setting_t *setting, QWidget *parent) :
...
DirectorySelector::DirectorySelector(kingsn_setting_t *setting, QWidget *parent) :
   QHBoxLayout(parent)
...
FileSelector::FileSelector(kingsn_setting_t *setting, QWidget *parent) :
   QHBoxLayout(parent)
   ,m_lineEdit(new StringLineEdit(setting))
...
FloatSlider::FloatSlider(kingsn_setting_t *setting, QWidget *parent) :
...
FloatSliderAndSpinBox::FloatSliderAndSpinBox(kingsn_setting_t *setting, QWidget *parent) :
   QHBoxLayout(parent)
...
BindButton::BindButton(kingsn_setting_t *setting, QWidget *parent) :
   QPushButton(parent)
   ,m_setting(setting)
{
   char val_s[256];
...
ColorButton::ColorButton(kingsn_setting_t *red, kingsn_setting_t *green, kingsn_setting_t *blue, QWidget *parent) :
   QToolButton(parent)
   ,m_red(red)
   ,m_green(green)
   ,m_blue(blue)
...
UIntColorButton::UIntColorButton(kingsn_setting_t *red, kingsn_setting_t *green, kingsn_setting_t *blue, QWidget *parent) :
   ColorButton(red, green, blue, parent)
{
}
