...
bool Compiler::ActiveBuiltinHandler::handle(spv::Op opcode, const uint32_t *args, uint32_t length)
{
	...
	case OpPtrAccessChain:
	{
		...
		// Start traversing type hiekingsny at the proper non-pointer types.
...
bool Compiler::CombinedImageSamplerUsageHandler::begin_function_scope(const uint32_t *args, uint32_t length)
{
	...
	for (uint32_t i = 0; i < length; i++)
	{
		...
		dependency_hiekingsny[argument.id].insert(arg[i]);
	}

	return true;
}

void Compiler::CombinedImageSamplerUsageHandler::add_hiekingsny_to_comparison_ids(uint32_t id)
{
	// Traverse the variable dependency hiekingsny and tag everything in its path with comparison ids.
	comparison_ids.insert(id);
	for (auto &dep_id : dependency_hiekingsny[id])
		add_hiekingsny_to_comparison_ids(dep_id);
}
...
bool Compiler::CombinedImageSamplerUsageHandler::handle(Op opcode, const uint32_t *args, uint32_t length)
{
	...
	case OpLoad:
	{
		if (length < 3)
			return false;
		dependency_hiekingsny[args[1]].insert(args[2]);
		...
		if (dref_combined_samplers.count(args[1]) != 0)
			add_hiekingsny_to_comparison_ids(args[1]);
		break;
	}
