...
DEF_FLAGS :=
ASFLAGS :=
DEFINES := -DHAVE_CONFIG_H -DKINGSN_INTERNAL -D_FILE_OFFSET_BITS=64
...
KINGSN_OBJ := $(addprefix $(OBJDIR)/,$(OBJ))
...
$(TARGET): $(KINGSN_OBJ)
	...
	$(Q)$(LINK) -o $@ $(KINGSN_OBJ) $(LIBS) $(LDFLAGS) $(LIBRARY_DIRS)
...
install: $(TARGET)
	...
	cp tools/cg2glsl.py $(DESTDIR)$(BIN_DIR)/KingStation-cg2glsl
	cp KingStation.cfg $(DESTDIR)$(GLOBAL_CONFIG_DIR)
	cp com.libks.KingStation.appdata.xml $(DESTDIR)$(DATA_DIR)/metainfo
	cp KingStation.desktop $(DESTDIR)$(DATA_DIR)/applications
	cp docs/KingStation.6 $(DESTDIR)$(MAN_DIR)/man6
	cp docs/KingStation-cg2glsl.6 $(DESTDIR)$(MAN_DIR)/man6
	cp media/KingStation.svg $(DESTDIR)$(DATA_DIR)/pixmaps
	cp COPYING $(DESTDIR)$(DOC_DIR)
	cp README.md $(DESTDIR)$(DOC_DIR)
	chmod 755 $(DESTDIR)$(BIN_DIR)/$(TARGET)
	chmod 755 $(DESTDIR)$(BIN_DIR)/KingStation-cg2glsl
	chmod 644 $(DESTDIR)$(GLOBAL_CONFIG_DIR)/KingStation.cfg
	chmod 644 $(DESTDIR)$(DATA_DIR)/applications/KingStation.desktop
	chmod 644 $(DESTDIR)$(DATA_DIR)/metainfo/com.libks.KingStation.appdata.xml
	chmod 644 $(DESTDIR)$(MAN_DIR)/man6/KingStation.6
	chmod 644 $(DESTDIR)$(MAN_DIR)/man6/KingStation-cg2glsl.6
	chmod 644 $(DESTDIR)$(DATA_DIR)/pixmaps/KingStation.svg
...
uninstall:
	...
	rm -f $(DESTDIR)$(BIN_DIR)/KingStation-cg2glsl
	rm -f $(DESTDIR)$(GLOBAL_CONFIG_DIR)/KingStation.cfg
	rm -f $(DESTDIR)$(DATA_DIR)/applications/KingStation.desktop
	rm -f $(DESTDIR)$(DATA_DIR)/metainfo/com.libks.KingStation.appdata.xml
	rm -f $(DESTDIR)$(DATA_DIR)/pixmaps/KingStation.svg
	rm -f $(DESTDIR)$(DOC_DIR)/COPYING
	rm -f $(DESTDIR)$(DOC_DIR)/COPYING.assets
	rm -f $(DESTDIR)$(DOC_DIR)/README.md
	rm -f $(DESTDIR)$(MAN_DIR)/man6/KingStation.6
	rm -f $(DESTDIR)$(MAN_DIR)/man6/KingStation-cg2glsl.6
