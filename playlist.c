      ...
      const struct playlist_entry *entry)
{
   ...
   {
      KINGSN_ERR("cannot push NULL or empty core path into the playlist.\n");
      return false;
   }
      ...
      const struct playlist_entry *entry)
{
   ...
   if (string_is_empty(core_name))
   {
      ...
      if (string_is_empty(core_name))
      {
         KINGSN_ERR("cannot push NULL or empty core name into the playlist.\n");
         return false;
      }
   }
...
void playlist_write_runtime_file(playlist_t *playlist)
{
   ...
   if (!file)
   {
      KINGSN_ERR("Failed to write to playlist file: %s\n", playlist->config.path);
      return;
   }
   ...
   KINGSN_LOG("[Playlist]: Written to playlist file: %s\n", playlist->config.path);
end:
   ...
   if (pCtx->in_items && pCtx->object_depth == 2)
   {
      if ((pCtx->array_depth == 1) && !pCtx->capacity_exceeded)
      {
         ...
         else
         {
            ...
            KINGSN_WARN("JSON file contains more entries than current playlist capacity. Excess entries will be discarded.\n");
...
static bool playlist_read_file(playlist_t *playlist)
{
...
#if 0
      KINGSN_LOG("[Playlist]: New playlist format detected.\n");
#endif
      playlist->old_format = false;
   }
   else
...
#if 0
      KINGSN_LOG("[Playlist]: Old playlist format detected.\n");
#endif
      playlist->old_format = true;
   }
   ...
   if (!playlist->old_format)
   {
            ...
            != RJSON_DONE)
      {
         ...
         {
            KINGSN_WARN("Ran out of memory while parsing JSON playlist\n");
            res = false;
         }
         else
         {
            KINGSN_WARN("Error parsing chunk:\n---snip---\n%.*s\n---snip---\n",
                  rjson_get_source_context_len(parser),
                  rjson_get_source_context_buf(parser));
            KINGSN_WARN("Error: Invalid JSON at line %d, column %d - %s.\n",
