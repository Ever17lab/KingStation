...
#define gl2_bind_fb(id)      glBindFramebufferOES(KINGSN_GL_FRAMEBUFFER, id)
...
#define gl2_bind_fb(id)      glBindFramebufferEXT(KINGSN_GL_FRAMEBUFFER, id)
...
#define gl2_bind_fb(id)      glBindFramebuffer(KINGSN_GL_FRAMEBUFFER, id)
      ...
      )
{
   ...
   gl2_load_texture_image(GL_TEXTURE_2D,
         0, KINGSN_GL_INTERNAL_FORMAT32,
         fbo_rect->width,
         fbo_rect->height,
         0, KINGSN_GL_TEXTURE_TYPE32,
         KINGSN_GL_FORMAT32, NULL);

   gl2_fb_texture_2d(KINGSN_GL_FRAMEBUFFER,
         KINGSN_GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
         *texture, 0);

   if (gl2_check_fb_status(KINGSN_GL_FRAMEBUFFER)
         == KINGSN_GL_FRAMEBUFFER_COMPLETE)
      return true;

   KINGSN_WARN("Failed to reinitialize FBO texture.\n");
   return false;
}
...
#if 0
   KINGSN_LOG("Setting viewport @ %ux%u\n", viewport_width, viewport_height);
#endif
}
...
static bool gl2_create_fbo_targets(gl_t *gl, gl2_renderchain_data_t *chain)
{
   ...
   for (i = 0; i < (unsigned)chain->fbo_pass; i++)
   {
      ...
      gl2_bind_fb(chain->fbo[i]);
      gl2_fb_texture_2d(KINGSN_GL_FRAMEBUFFER,
            KINGSN_GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, chain->fbo_texture[i], 0);

      status = gl2_check_fb_status(KINGSN_GL_FRAMEBUFFER);
      if (status != KINGSN_GL_FRAMEBUFFER_COMPLETE)
         goto error;
   }
...
error:
   ...
   if (gl->fbo_feedback)
      gl2_delete_fb(1, &gl->fbo_feedback);
   KINGSN_ERR("[GL]: Failed to set up frame buffer objects. Multi-pass shading will not work.\n");
   return false;
}
...
static unsigned gl2_wrap_type_to_enum(enum gfx_wrap_type type)
{
   switch (type)
   {
      ...
      case KINGSN_WRAP_BORDER: /* GL_CLAMP_TO_BORDER: Available since GL 1.3 */
         return GL_CLAMP_TO_BORDER;
#else
      case KINGSN_WRAP_BORDER:
#endif
      case KINGSN_WRAP_EDGE:
         return GL_CLAMP_TO_EDGE;
      case KINGSN_WRAP_REPEAT:
         return GL_REPEAT;
      case KINGSN_WRAP_MIRRORED_REPEAT:
      ...
      unsigned i, GLuint texture)
{
   ...
   {
      if (!chain->has_fp_fbo)
         KINGSN_ERR("[GL]: Floating-point FBO was requested, but is not supported. Falling back to UNORM. Result may band/clip/etc.!\n");
   }

#if !defined(HAVE_OPENGLES2)
   if (fp_fbo && chain->has_fp_fbo)
   {
      KINGSN_LOG("[GL]: FBO pass #%d is floating-point.\n", i);
...
#endif
   {
      ...
      {
         if (!chain->has_srgb_fbo)
               KINGSN_ERR("[GL]: sRGB FBO was requested, but it is not supported. Falling back to UNORM. Result may have banding!\n");
      }
      ...
      {
         KINGSN_LOG("[GL]: FBO pass #%d is sRGB.\n", i);
#ifdef HAVE_OPENGLES2
...
#endif
      {
         ...
         gl2_load_texture_image(GL_TEXTURE_2D,
            0, KINGSN_GL_INTERNAL_FORMAT32,
            gl->fbo_rect[i].width, gl->fbo_rect[i].height, 0,
            KINGSN_GL_TEXTURE_TYPE32, KINGSN_GL_FORMAT32, NULL);
#endif
      }
   }
}
      ...
      unsigned vp_width, unsigned vp_height)
{
   ...
   for (i = 0; i < (unsigned)chain->fbo_pass; i++)
   {
      ...
      if (size_modified)
         KINGSN_WARN("FBO textures exceeded maximum size of GPU (%dx%d). Resizing to fit.\n", max_size, max_size);
      ...
      unsigned fbo_width, unsigned fbo_height)
{
   ...
   if (!gl->has_fbo)
   {
      KINGSN_ERR("[GL]: Failed to locate FBO functions. Won't be able to use render-to-texture.\n");
      return;
   }
   ...
   if (!scale.valid)
   {
      scale.scale_x = 1.0f;
      scale.scale_y = 1.0f;
      scale.type_x  = scale.type_y = KINGSN_SCALE_INPUT;
      scale.valid   = true;
   }
   ...
   for (i = 0; i < chain->fbo_pass; i++)
   {
      ...
      KINGSN_LOG("[GL]: Creating FBO %d @ %ux%u\n", i,
            gl->fbo_rect[i].width, gl->fbo_rect[i].height);
   }
   ...
   {
      KINGSN_LOG("[GL]: Creating feedback FBO %d @ %ux%u\n", i,
            gl->fbo_rect[gl->fbo_feedback_pass].width,
            gl->fbo_rect[gl->fbo_feedback_pass].height);
   }
   else if (gl->fbo_feedback_enable)
   {
      KINGSN_WARN("[GL]: Tried to create feedback FBO of pass #%u, but there are only %d FBO passes. Will use input texture as feedback texture.\n",
   ...
   {
      glDeleteTextures(chain->fbo_pass, chain->fbo_texture);
      KINGSN_ERR("[GL]: Failed to create FBO targets. Will continue without FBO.\n");
      return;
   }
      ...
      unsigned width, unsigned height)
{
   ...
   struct ks_hw_render_callback *hwr =
   ...
   KINGSN_LOG("[GL]: Initializing HW render (%u x %u).\n", width, height);
   glGetIntegerv(GL_MAX_TEXTURE_SIZE, &max_fbo_size);
   glGetIntegerv(KINGSN_GL_MAX_RENDERBUFFER_SIZE, &max_renderbuffer_size);
   KINGSN_LOG("[GL]: Max texture size: %d px, renderbuffer size: %d px.\n",
         max_fbo_size, max_renderbuffer_size);

   if (!gl->has_fbo)
      return false;

   KINGSN_LOG("[GL]: Supports FBO (render-to-texture).\n");
   ...
   for (i = 0; i < gl->textures; i++)
   {
      ...
      gl2_fb_texture_2d(KINGSN_GL_FRAMEBUFFER,
            KINGSN_GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gl->texture[i], 0);

      if (depth)
      {
         gl2_bind_rb(KINGSN_GL_RENDERBUFFER, chain->hw_render_depth[i]);
         gl2_rb_storage(KINGSN_GL_RENDERBUFFER,
               stencil ? KINGSN_GL_DEPTH24_STENCIL8 : GL_DEPTH_COMPONENT16,
               width, height);
         gl2_bind_rb(KINGSN_GL_RENDERBUFFER, 0);
         ...
         if (stencil)
         {
            ...
            gl2_fb_rb(KINGSN_GL_FRAMEBUFFER,
                  KINGSN_GL_DEPTH_ATTACHMENT,
                  KINGSN_GL_RENDERBUFFER,
                  chain->hw_render_depth[i]);
            gl2_fb_rb(KINGSN_GL_FRAMEBUFFER,
                  KINGSN_GL_STENCIL_ATTACHMENT,
                  KINGSN_GL_RENDERBUFFER,
                  chain->hw_render_depth[i]);
#else
      ...
      if (status != KINGSN_GL_FRAMEBUFFER_COMPLETE)
      {
         KINGSN_ERR("[GL]: Failed to create HW render FBO #%u, error: 0x%04x.\n",
               i, status);
         return false;
      }
   }
      ...
      uint8_t *buffer, bool is_idle)
{
   ...
   if (gl->pbo_readback_enable)
   {
      ...
      if (!ptr)
      {
         KINGSN_ERR("[GL]: Failed to map pixel unpack buffer.\n");
         goto error;
      }
      ...
      unsigned width, unsigned height, unsigned pitch)
{
   ...
   if (chain->egl_images)
   {
      ...
      {
         KINGSN_ERR("[GL]: Failed to create EGL image.\n");
         return;
      }
      ...
      const void *frame, unsigned base_size)
{
   ...
   glTexImage2D(GL_TEXTURE_2D,
         0,
         (use_rgba || !rgb32) ? GL_RGBA : KINGSN_GL_INTERNAL_FORMAT32,
         width, height, 0,
         (use_rgba || !rgb32) ? GL_RGBA : KINGSN_GL_TEXTURE_TYPE32,
         (rgb32) ? KINGSN_GL_FORMAT32 : GL_UNSIGNED_SHORT_4_4_4_4, frame);
      ...
      unsigned i, GLuint *textures_lut)
{
   ...
   {
      KINGSN_ERR("[GL]: Failed to load texture image from: \"%s\"\n",
            lut_path);
      return false;
   }

   KINGSN_LOG("[GL]: Loaded texture image from: \"%s\" ...\n",
         lut_path);

   if (lut_filter == KINGSN_FILTER_NEAREST)
      filter_type = TEXTURE_FILTER_NEAREST;
      ...
      float w, float h)
{
   ...
   {
      KINGSN_ERR("[GL]: Invalid overlay id: %u\n", image);
      return;
   }
 ...
 *
 * gl2_get_fallback_shader_type(KINGSN_SHADER_NONE) returns a default shader type.
 * if gl2_get_fallback_shader_type(type) != type, type was not supported.
 * 
 * Returns: A supported shader type.
 *  If KINGSN_SHADER_NONE is returned, no shader backend is supported.
 **/
static enum kingsn_shader_type gl2_get_fallback_shader_type(enum kingsn_shader_type type)
{
#if defined(HAVE_GLSL) || defined(HAVE_CG)
   unsigned i;

   if (type != KINGSN_SHADER_CG && type != KINGSN_SHADER_GLSL)
   {
      type = DEFAULT_SHADER_TYPE;

      if (type != KINGSN_SHADER_CG && type != KINGSN_SHADER_GLSL)
         type = KINGSN_SHADER_GLSL;
   }
   ...
   for (i = 0; i < 2; i++)
   {
      switch (type)
      {
         ...
         case KINGSN_SHADER_GLSL:
#ifdef HAVE_GLSL
            if (video_shader_is_supported(type))
               return type;
#endif
            type = KINGSN_SHADER_CG;
            break;

         default:
            return KINGSN_SHADER_NONE;
      }
   }
#endif
   return KINGSN_SHADER_NONE;
...
static const shader_backend_t *gl_shader_driver_set_backend(
      enum kingsn_shader_type type)
{
   enum kingsn_shader_type fallback = gl2_get_fallback_shader_type(type);
   if (fallback != type)
      KINGSN_ERR("[Shader driver]: Shader backend %d not supported, falling back to %d.", type, fallback);

   switch (fallback)
   {
#ifdef HAVE_CG
      case KINGSN_SHADER_CG:
         KINGSN_LOG("[Shader driver]: Using Cg shader backend.\n");
         return &gl_cg_backend;
#endif
#ifdef HAVE_GLSL
      case KINGSN_SHADER_GLSL:
         KINGSN_LOG("[Shader driver]: Using GLSL shader backend.\n");
         return &gl_glsl_backend;
#endif
      default:
         KINGSN_LOG("[Shader driver]: No supported shader backend.\n");
         return NULL;
   }
}
...
static bool gl_shader_driver_init(video_shader_ctx_init_t *init)
{
   ...
   {
      KINGSN_LOG("Setting up menu pipeline shaders for XMB ... \n");
      ...
      )
{
   ...
   enum kingsn_shader_type parse_type = video_shader_parse_type(shader_path);
   enum kingsn_shader_type type;

   type = gl2_get_fallback_shader_type(parse_type);

   if (type == KINGSN_SHADER_NONE)
   {
      KINGSN_ERR("[GL]: Couldn't find any supported shader backend! Continuing without shaders.\n");
      return true;
   }
   ...
   if (type != parse_type)
   {
      if (!string_is_empty(shader_path))
         KINGSN_WARN("[GL] Shader preset %s is using unsupported shader type %s, falling back to stock %s.\n",
   ...
   KINGSN_ERR("[GL]: Failed to initialize shader, falling back to stock.\n");
...
static void gl2_init_textures(gl_t *gl)
{
   ...
   if (gl->hw_render_use && gl->base_size == sizeof(uint32_t))
   {
      ...
      {
         KINGSN_WARN("[GL]: 32-bit FBO not supported. Falling back to 16-bit.\n");
         internal_fmt = GL_RGB;
         texture_type = GL_RGB;
...
static void gl2_video_layout_fbo_init(gl_t *gl, unsigned width, unsigned height)
{
   ...
   gl2_load_texture_image(GL_TEXTURE_2D, 0, KINGSN_GL_INTERNAL_FORMAT32,
      width, height, 0, GL_RGBA, GL_FLOAT, NULL);

   gl2_gen_fb(1, &gl->video_layout_fbo);
   gl2_bind_fb(gl->video_layout_fbo);

   gl2_fb_texture_2d(KINGSN_GL_FRAMEBUFFER, KINGSN_GL_COLOR_ATTACHMENT0,
      GL_TEXTURE_2D, gl->video_layout_fbo_texture, 0);

   if (gl2_check_fb_status(KINGSN_GL_FRAMEBUFFER) != 
         KINGSN_GL_FRAMEBUFFER_COMPLETE)
      KINGSN_LOG("Unable to create FBO for video_layout\n");
...
static void gl2_video_layout_init(gl_t *gl)
{
   ...
   gl_load_texture_data(gl->video_layout_white_texture,
      KINGSN_WRAP_EDGE, TEXTURE_FILTER_NEAREST,
...
static void *gl2_video_layout_take_image(void *video_driver_data, struct texture_image image)
{
   ...
   gl_load_texture_data(tex,
      KINGSN_WRAP_EDGE, TEXTURE_FILTER_MIPMAP_LINEAR,
      ...
      video_frame_info_t *video_info)
{
         ...
         for (i = 0; i < (unsigned)chain->fbo_pass; i++)
         {
            ...
            if (fbo_rect)
            {
                     ...
                     (img_height > fbo_rect->height))
               {
                  ...
                  KINGSN_LOG("[GL]: Recreating FBO texture #%d: %ux%u\n",
   ...
   /* Have to reset rendering state which libks core
   ...
   /* Reset state which could easily mess up libks core. */
   if (gl->hw_render_fbo_init)
   {
...
static bool gl2_resolve_extensions(gl_t *gl, const char *context_ident, const video_info_t *video)
{
   ...
   if (gl->have_sync && video_hard_sync)
      KINGSN_LOG("[GL]: Using ARB_sync to reduce latency.\n");
   ...
   if (!gl_check_capability(GL_CAPS_BGRA8888))
   {
      ...
      KINGSN_WARN("[GL]: GLES implementation does not have BGRA8888 extension.\n"
   ...
   KINGSN_LOG("[GL]: Supported extensions:\n");
...
{
   gl->internal_fmt = KINGSN_GL_INTERNAL_FORMAT16;
   gl->texture_type = KINGSN_GL_TEXTURE_TYPE16;
   gl->texture_fmt  = KINGSN_GL_FORMAT16;
   ...
   if (rgb32)
   {
      ...
      gl->internal_fmt = KINGSN_GL_INTERNAL_FORMAT32;
      gl->texture_type = KINGSN_GL_TEXTURE_TYPE32;
      gl->texture_fmt  = KINGSN_GL_FORMAT32;
   ...
   {
      KINGSN_LOG("[GL]: Using GL_RGB565 for texture uploads.\n");
      gl->internal_fmt = KINGSN_GL_INTERNAL_FORMAT16_565;
      gl->texture_type = KINGSN_GL_TEXTURE_TYPE16_565;
      gl->texture_fmt  = KINGSN_GL_FORMAT16_565;
   }
#endif
}
...
static bool gl2_init_pbo_readback(gl_t *gl)
{
...
#ifndef HAVE_OPENGLES3
   {
      ...
      if (!scaler_ctx_gen_filter(scaler))
      {
         ...
         KINGSN_ERR("[GL]: Failed to initialize pixel conversion for PBO.\n");
         glDeleteBuffers(4, gl->pbo_readback);
         return false;
      }
   }
...
static const gfx_ctx_driver_t *gl2_get_context(gl_t *gl)
{
   ...
   void                      *ctx_data  = NULL;
   settings_t                 *settings = config_get_ptr();
   struct ks_hw_render_callback *hwr = video_driver_get_hw_context();
   ...
   if (hwr->context_type == KS_HW_CONTEXT_OPENGLES3)
   {
      major                             = 3;
      minor                             = 0;
   }
   ...
   gl->shared_context_use                = video_shared_context
      && hwr->context_type != KS_HW_CONTEXT_NONE;

   if (     (libks_get_shared_context())
         && (hwr->context_type != KS_HW_CONTEXT_NONE))
      ...
      const GLchar *message, void *userParam)
{
   ...
   switch (severity)
   {
      case GL_DEBUG_SEVERITY_HIGH:
         KINGSN_ERR("[GL debug (High, %s, %s)]: %s\n", src, typestr, message);
         break;
      case GL_DEBUG_SEVERITY_MEDIUM:
         KINGSN_WARN("[GL debug (Medium, %s, %s)]: %s\n", src, typestr, message);
         break;
      case GL_DEBUG_SEVERITY_LOW:
         KINGSN_LOG("[GL debug (Low, %s, %s)]: %s\n", src, typestr, message);
         break;
   }
}
...
static void gl2_begin_debug(gl_t *gl)
{
   if (gl_check_capability(GL_CAPS_DEBUG))
   {
      ...
      KINGSN_ERR("[GL]: Neither GL_KHR_debug nor GL_ARB_debug_output are implemented. Cannot start GL debugging.\n");
}
#endif
      ...
      input_driver_t **input, void **input_data)
{
   ...
   struct ks_hw_render_callback *hwr = NULL;
   ...
   KINGSN_LOG("[GL]: Found GL context: %s\n", ctx_driver->ident);
   ...
   KINGSN_LOG("[GL]: Detecting screen resolution %ux%u.\n", full_x, full_y);
...
#if !defined(KINGSN_CONSOLE) || defined(HAVE_LIBNX)
   ...
   KINGSN_LOG("[GL]: Vendor: %s, Renderer: %s.\n", vendor, renderer);
   KINGSN_LOG("[GL]: Version: %s.\n", version);
   ...
   if (hwr->context_type == KS_HW_CONTEXT_OPENGL_CORE)
   {
      ...
      KINGSN_LOG("[GL]: Using Core GL context, setting up VAO...\n");
      if (!gl_check_capability(GL_CAPS_VAO))
      {
         KINGSN_ERR("[GL]: Failed to initialize VAOs.\n");
         goto error;
      }
   }
   ...
   {
      KINGSN_ERR("[GL]: Renderchain could not be initialized.\n");
      goto error;
   }
   ...
   if (gl->has_fbo && hwr->context_type != KS_HW_CONTEXT_NONE)
   ...
   KINGSN_LOG("[GL]: Using resolution %ux%u\n", temp_width, temp_height);
   ...
   if (!gl->shader)
   {
      KINGSN_ERR("[GL:]: Shader driver initialization failed.\n");
      goto error;
   }

   KINGSN_LOG("[GL]: Default shader backend found: %s.\n", gl->shader->ident);

   if (!gl2_shader_init(gl, ctx_driver, hwr))
   {
      KINGSN_ERR("[GL]: Shader initialization failed.\n");
      goto error;
   }
   ...
   if (!gl2_shader_info(gl, &shader_info))
   {
      KINGSN_ERR("[GL]: Shader driver info check failed.\n");
      goto error;
   }

   KINGSN_LOG("[GL]: Using %u textures.\n", gl->textures);
   KINGSN_LOG("[GL]: Loaded %u program(s).\n",
         shader_info.num);

   gl->tex_w = gl->tex_h = (KINGSN_SCALE_BASE * video->input_scale);
   ...
   if (gl->has_fbo)
   {
      ...
      {
         KINGSN_ERR("[GL]: Hardware rendering context initialization failed.\n");
         goto error;
      }
   }
   ...
   {
      KINGSN_LOG("[GL]: Async PBO readback enabled.\n");
   }
...
static bool gl2_set_shader(void *data,
      enum kingsn_shader_type type, const char *path)
{
#if defined(HAVE_GLSL) || defined(HAVE_CG)
   unsigned textures;
   video_shader_ctx_init_t init_data;
   enum kingsn_shader_type fallback;
   gl_t *gl = (gl_t*)data;
   ...
   if (fallback == KINGSN_SHADER_NONE)
   {
      KINGSN_ERR("[GL]: No supported shader backend found!\n");
      goto error;
   }
   ...
   if (type != fallback)
   {
      KINGSN_ERR("[GL]: %s shader not supported, falling back to stock %s\n",
   ...
   if (!gl_shader_driver_init(&init_data))
   {
      ...
      KINGSN_WARN("[GL]: Failed to set multipass shader. Falling back to stock.\n");
      ...
      const void *image_data, unsigned num_images)
{
   ...
   for (i = 0; i < num_images; i++)
   {
      ...
      gl_load_texture_data(gl->overlay_tex[i],
            KINGSN_WRAP_EDGE, TEXTURE_FILTER_LINEAR,
            alignment,
...
static ks_proc_address_t gl2_get_proc_address(void *data, const char *sym)
{
   gl_t *gl = (gl_t*)data;
