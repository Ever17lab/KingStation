...
typedef struct kingsn_setting_info
{
   int index;
   int size;
} kingsn_setting_info_t;
...
static void menu_input_st_uint_cb(void *userdata, const char *str)
{
   if (str && *str)
   {
      ...
      kingsn_setting_t *setting = menu_setting_find(label);
...
static void *setting_get_ptr(kingsn_setting_t *setting)
{
   if (!setting)
      return NULL;
...
static int setting_generic_action_ok_linefeed(
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
...
static void setting_add_special_callbacks(
      kingsn_setting_t **list,
      kingsn_setting_info_t *list_info,
      unsigned values)
{
      ...
      kingsn_setting_info_t *list_info)
{
   ...
   kingsn_setting_t *list_settings = (kingsn_setting_t*)
      realloc(*list, sizeof(kingsn_setting_t) * new_size);
...
unsigned setting_get_bind_type(kingsn_setting_t *setting)
{
   if (!setting)
      return 0;
...
static int setting_bind_action_start(kingsn_setting_t *setting)
{
   unsigned bind_type;
   struct ks_keybind *keybind   = NULL;
   struct ks_keybind *def_binds = (struct ks_keybind *)ks_keybinds_1;

   if (!setting)
      return -1;

   keybind = (struct ks_keybind*)setting->value.target.keybind;
   if (!keybind)
      return -1;
   ...
   if (setting->index_offset)
      def_binds = (struct ks_keybind*)ks_keybinds_rest;
...
#if 0
static void setting_get_string_representation_hex(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
      snprintf(s, len, "%08x",
...
void setting_get_string_representation_hex_and_uint(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
void setting_get_string_representation_uint(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
      snprintf(s, len, "%u",
...
void setting_get_string_representation_size(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
void setting_get_string_representation_size_in_mb(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
      snprintf(s, len, "%" PRI_SIZET,
...
static float recalc_step_based_on_length_of_action(kingsn_setting_t *setting)
{
...
void setting_generic_handle_change(kingsn_setting_t *setting)
{
   settings_t *settings = config_get_ptr();
...
static void setting_get_string_representation_int_gpu_index(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
   {
...
static void setting_get_string_representation_int(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
      snprintf(s, len, "%d", *setting->value.target.integer);
 ...
 **/
int setting_set_with_string_representation(kingsn_setting_t* setting,
      const char* value)
{
   double min, max;
   uint64_t flags;
 ...
 **/
static void setting_reset_setting(kingsn_setting_t* setting)
{
   if (!setting)
      return;
...
int setting_generic_action_start_default(kingsn_setting_t *setting)
{
   bool refresh                = false;
   if (!setting)
      return -1;
...
static void setting_get_string_representation_default(kingsn_setting_t *setting,
      char *s, size_t len)
{
   strcpy_literal(s, "...");
}
 ...
 **/
static void setting_get_string_representation_st_bool(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
 ...
 **/
static void setting_get_string_representation_st_float(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_st_dir(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
      strlcpy(s,
...
static void setting_get_string_representation_st_path(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_st_string(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_st_bind(kingsn_setting_t *setting,
      char *s, size_t len)
{
   unsigned index_offset                 = 0;
   const struct ks_keybind* keybind   = NULL;
   const struct ks_keybind* auto_bind = NULL;

   if (!setting)
      return;

   index_offset = setting->index_offset;
   keybind      = (const struct ks_keybind*)setting->value.target.keybind;
   auto_bind    = (const struct ks_keybind*)
 ...
 **/
static kingsn_setting_t setting_action_setting(const char* name,
      const char* short_description,
      const char *group, const char *subgroup,
      const char *parent_group,
      bool dont_use_enum_idx)
{
   kingsn_setting_t result;
 ...
 **/
static kingsn_setting_t setting_group_setting(
 ...
 **/
static kingsn_setting_t setting_float_setting(const char* name,
 ...
 **/
static kingsn_setting_t setting_uint_setting(const char* name,
 ...
 **/
static kingsn_setting_t setting_size_setting(const char* name,
 ...
 **/
#if 0
static kingsn_setting_t setting_hex_setting(const char* name,
      const char* short_description, unsigned int* target,
      unsigned int default_value,
      const char *group, const char *subgroup, const char *parent_group,
      change_handler_t change_handler, change_handler_t read_handler,
 ...
 **/
static kingsn_setting_t setting_bind_setting(const char* name,
      const char* short_description, struct ks_keybind* target,
      uint32_t idx, uint32_t idx_offset,
      const struct ks_keybind* default_value,
      const char *group, const char *subgroup,
      const char *parent_group,
      bool dont_use_enum_idx)
{
...
int setting_string_action_start_generic(kingsn_setting_t *setting)
{
   if (!setting)
      return -1;
 ...
 **/
static kingsn_setting_t setting_string_setting(enum setting_type type,
 ...
 **/
static kingsn_setting_t setting_string_setting_options(enum setting_type type,
      const char* name, const char* short_description, char* target,
      ...
      bool dont_use_enum_idx)
{
  kingsn_setting_t result = setting_string_setting(type, name,
 ...
 **/
static kingsn_setting_t setting_subgroup_setting(enum setting_type type,
 ...
 **/
static kingsn_setting_t setting_bool_setting(const char* name,
 ...
 **/
static kingsn_setting_t setting_int_setting(const char* name,
...
static void config_bool_alt(
      ...
      enum msg_hash_enums off_enum_idx,
      enum msg_hash_enums on_enum_idx,
      kingsn_setting_group_info_t *group_info,
      kingsn_setting_group_info_t *subgroup_info,
      const char *parent_group,
      change_handler_t change_handler,
      change_handler_t read_handler,
      uint32_t flags)
...
static void START_GROUP(kingsn_setting_t **list, kingsn_setting_info_t *list_info,
      kingsn_setting_group_info_t *group_info,
...
static void end_group(kingsn_setting_t **list,
      kingsn_setting_info_t *list_info,
      const char *parent_group)
{
...
static bool start_sub_group(kingsn_setting_t **list,
      kingsn_setting_info_t *list_info, const char *name,
      kingsn_setting_group_info_t *group_info,
      kingsn_setting_group_info_t *subgroup_info,
      const char *parent_group)
{
      ...
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
   ...
   struct ks_keybind *target          = NULL;
   const struct ks_keybind *def_binds = NULL;
   ...
   def_binds =  (setting->index_offset) ?
      ks_keybinds_rest : ks_keybinds_1;
      ...
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
            ...
            &deviation, &sample_points))
   {
      ...
      driver_ctl(KINGSN_DRIVER_CTL_SET_REFRESH_RATE, &video_refresh_rate_float);
      ...
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
   ...
   driver_ctl(KINGSN_DRIVER_CTL_SET_REFRESH_RATE, &refresh_rate);
   /* Incase refresh rate update forced non-block video. */
   command_event(CMD_EVENT_VIDEO_SET_BLOCKING_STATE, NULL);
...
static void setting_get_string_representation_video_record_quality(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static void setting_get_string_representation_video_filter(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static void setting_get_string_representation_state_slot(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static void setting_get_string_representation_percentage(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static void setting_get_string_representation_float_video_msg_color(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static void setting_get_string_representation_max_users(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
      ...
      char *s, size_t len)
{
   ...
   switch (*setting->value.target.unsigned_integer)
   {
      ...
      case XMB_ICON_THEME_FLATUI:
         ...
         break;
      case XMB_ICON_THEME_KSACTIVE:
         strlcpy(s,
               msg_hash_to_str(MENU_ENUM_LABEL_VALUE_XMB_ICON_THEME_KSACTIVE), len);
         break;
      case XMB_ICON_THEME_KSSYSTEM:
         strlcpy(s,
               msg_hash_to_str(MENU_ENUM_LABEL_VALUE_XMB_ICON_THEME_KSSYSTEM), len);
         break;
...
static void setting_get_string_representation_uint_video_monitor_index(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static void setting_get_string_representation_uint_custom_viewport_width(kingsn_setting_t *setting,
      char *s, size_t len)
{
   struct ks_game_geometry  *geom    = NULL;
   struct ks_system_av_info *av_info = NULL;
   unsigned int rotation                = KingStation_get_rotation();
   if (!setting)
      return;

   av_info = video_viewport_get_system_av_info();
   geom    = (struct ks_game_geometry*)&av_info->geometry;
...
static void setting_get_string_representation_uint_custom_viewport_height(kingsn_setting_t *setting,
      char *s, size_t len)
{
   struct ks_game_geometry  *geom    = NULL;
   struct ks_system_av_info *av_info = NULL;
...
#ifdef HAVE_WASAPI
static void setting_get_string_representation_int_audio_wasapi_sh_buffer_length(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (!setting)
      return;
...
static bool setting_is_protected_driver(kingsn_setting_t *setting)
{
   if (!setting)
      return false;
...
static unsigned libks_device_get_size(unsigned *devices, size_t devices_size, unsigned port)
{
   unsigned types                           = 0;
   const struct ks_controller_info *desc = NULL;
   kingsn_system_info_t              *system = runloop_get_system_info();

   devices[types++]                         = KS_DEVICE_NONE;
   devices[types++]                         = KS_DEVICE_JOYPAD;

   if (system)
   {
      /* Only push KS_DEVICE_ANALOG as default if we use an
       * older core which doesn't use SET_CONTROLLER_INFO. */
      if (!system->ports.size)
         devices[types++] = KS_DEVICE_ANALOG;
...
static int setting_action_left_libks_device_type(
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
   bool refresh = false;
   ks_ctx_controller_info_t pad;
   ...
   port           = setting->index_offset;
   types          = libks_device_get_size(devices, ARRAY_SIZE(devices), port);
...
{
   video_viewport_t vp;
   struct ks_system_av_info *av_info = video_viewport_get_system_av_info();
   video_viewport_t            *custom  = video_viewport_get_custom();
   settings_t                 *settings = config_get_ptr();
   struct ks_game_geometry     *geom = (struct ks_game_geometry*)
      &av_info->geometry;
      ...
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
   ...
   success = driver_ctl(KINGSN_DRIVER_CTL_FIND_PREV, &drv);
   ...
   if (!success)
   {
      ...
      if (menu_navigation_wraparound_enable)
      {
         ...
         drv.s     = setting->value.target.string;
         drv.len   = setting->size;
         success = driver_ctl(KINGSN_DRIVER_CTL_FIND_LAST, &drv);
         if (setting_is_protected_driver(setting))
         {
            while (success &&
                   string_is_equal(drv.s, "null") &&
      ...
      else if (setting_is_protected_driver(setting))
      {
         ...
         success = driver_ctl(KINGSN_DRIVER_CTL_FIND_NEXT, &drv);
         while (success &&
                string_is_equal(drv.s, "null") &&
                (success = driver_ctl(KINGSN_DRIVER_CTL_FIND_NEXT, &drv)));
      }
      ...
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
   ...
   if (!success)
   {
      ...
      if (menu_navigation_wraparound_enable)
      {
         ...
         drv.s     = setting->value.target.string;
         drv.len   = setting->size;
         success = driver_ctl(KINGSN_DRIVER_CTL_FIND_FIRST, &drv);
         if (setting_is_protected_driver(setting))
         {
            while (success &&
                   string_is_equal(drv.s, "null") &&
...
#ifdef HAVE_CHEATS
static void setting_get_string_representation_uint_cheat_exact(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_lt(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_gt(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_lte(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_gte(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_eq(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_neq(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_eqplus(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_cheat_eqminus(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
static void setting_get_string_representation_uint_video_rotation(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
   {
...
static void setting_get_string_representation_uint_screen_orientation(kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
   {
...
static void setting_get_string_representation_uint_libks_device(
      kingsn_setting_t *setting,
      char *s, size_t len)
{
   unsigned index_offset, device;
   const struct ks_controller_description *desc = NULL;
   const char *name            = NULL;
   kingsn_system_info_t *system = runloop_get_system_info();
   ...
   if (system)
   {
      if (index_offset < system->ports.size)
         desc = libks_find_controller_description(
   ...
   if (!name)
   {
      ...
      switch (device)
      {
         ...
         case KS_DEVICE_JOYPAD:
            name = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_KSPAD);
            break;
         case KS_DEVICE_ANALOG:
            name = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_KSPAD_WITH_ANALOG);
            break;
         default:
      ...
      char *s, size_t len)
{
   ...
   switch (*setting->value.target.unsigned_integer)
   {
      case KINGSN_NETPLAY_SHARE_ANALOG_NO_PREFERENCE:
         strlcpy(s, msg_hash_to_str(MENU_ENUM_LABEL_VALUE_NETPLAY_SHARE_NO_PREFERENCE), len);
         break;

      case KINGSN_NETPLAY_SHARE_ANALOG_MAX:
         strlcpy(s, msg_hash_to_str(MENU_ENUM_LABEL_VALUE_NETPLAY_SHARE_ANALOG_MAX), len);
         break;

      case KINGSN_NETPLAY_SHARE_ANALOG_AVERAGE:
...
static void setting_get_string_representation_uint_user_language(
      ...
      char *s, size_t len)
{
   const char *modes[KS_LANGUAGE_LAST];

   modes[KS_LANGUAGE_ENGLISH]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_ENGLISH);
   modes[KS_LANGUAGE_JAPANESE]               = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_JAPANESE);
   modes[KS_LANGUAGE_FRENCH]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_FRENCH);
   modes[KS_LANGUAGE_SPANISH]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_SPANISH);
   modes[KS_LANGUAGE_GERMAN]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_GERMAN);
   modes[KS_LANGUAGE_ITALIAN]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_ITALIAN);
   modes[KS_LANGUAGE_DUTCH]                  = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_DUTCH);
   modes[KS_LANGUAGE_PORTUGUESE_BRAZIL]      = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_PORTUGUESE_BRAZIL);
   modes[KS_LANGUAGE_PORTUGUESE_PORTUGAL]    = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_PORTUGUESE_PORTUGAL);
   modes[KS_LANGUAGE_RUSSIAN]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_RUSSIAN);
   modes[KS_LANGUAGE_KOREAN]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_KOREAN);
   modes[KS_LANGUAGE_CHINESE_TRADITIONAL]    = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_CHINESE_TRADITIONAL);
   modes[KS_LANGUAGE_CHINESE_SIMPLIFIED]     = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_CHINESE_SIMPLIFIED);
   modes[KS_LANGUAGE_ESPERANTO]              = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_ESPERANTO);
   modes[KS_LANGUAGE_POLISH]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_POLISH);
   modes[KS_LANGUAGE_VIETNAMESE]             = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_VIETNAMESE);
   modes[KS_LANGUAGE_ARABIC]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_ARABIC);
   modes[KS_LANGUAGE_GREEK]                  = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_GREEK);
   modes[KS_LANGUAGE_TURKISH]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_TURKISH);
   modes[KS_LANGUAGE_SLOVAK]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_SLOVAK);
   modes[KS_LANGUAGE_PERSIAN]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_PERSIAN);
   modes[KS_LANGUAGE_HEBREW]                 = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_HEBREW);
   modes[KS_LANGUAGE_ASTURIAN]               = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_ASTURIAN);
   modes[KS_LANGUAGE_FINNISH]                = msg_hash_to_str(MENU_ENUM_LABEL_VALUE_LANG_FINNISH);
   strlcpy(s, modes[*msg_hash_get_uint(MSG_HASH_USER_LANGUAGE)], len);
}
#endif

static void setting_get_string_representation_uint_libks_log_level(
      kingsn_setting_t *setting,
      char *s, size_t len)
{
   if (setting)
...
enum setting_type menu_setting_get_browser_selection_type(kingsn_setting_t *setting)
{
   if (!setting)
      return ST_NONE;
...
int menu_setting_generic(kingsn_setting_t *setting, size_t idx, bool wraparound)
{
...
int menu_action_handle_setting(kingsn_setting_t *setting,
            ...
            switch (action)
            {
               ...
               case MENU_ACTION_LEFT:
                  if (setting->action_left)
                  {
                     ...
                     menu_driver_ctl(
                           KINGSN_MENU_CTL_UPDATE_SAVESTATE_THUMBNAIL_PATH,
                           NULL);
                     menu_driver_ctl(
                           KINGSN_MENU_CTL_UPDATE_SAVESTATE_THUMBNAIL_IMAGE,
                           NULL);
                  }
                  break;
 ...
 **/
kingsn_setting_t *menu_setting_find(const char *label)
{
   kingsn_setting_t *setting = NULL;
   kingsn_setting_t **list   = &setting;
...
kingsn_setting_t *menu_setting_find_enum(enum msg_hash_enums enum_idx)
{
   kingsn_setting_t *setting = NULL;
   kingsn_setting_t **list   = &setting;
 ...
 **/
static int setting_action_start_bind_device(kingsn_setting_t *setting)
{
   settings_t      *settings = config_get_ptr();
...
static int setting_action_start_custom_viewport_width(kingsn_setting_t *setting)
{
   video_viewport_t vp;
   struct ks_system_av_info *av_info = video_viewport_get_system_av_info();
   video_viewport_t            *custom  = video_viewport_get_custom();
...
static int setting_action_start_custom_viewport_height(kingsn_setting_t *setting)
{
   video_viewport_t vp;
   struct ks_system_av_info *av_info = video_viewport_get_system_av_info();
   video_viewport_t            *custom  = video_viewport_get_custom();
...
static int setting_action_start_analog_dpad_mode(kingsn_setting_t *setting)
{
   if (!setting)
      return -1;
...
static int setting_action_start_libks_device_type(kingsn_setting_t *setting)
{
   ks_ctx_controller_info_t pad;
   unsigned port = 0;
   ...
   input_config_set_device(port, KS_DEVICE_JOYPAD);

   pad.port         = port;
   pad.device       = KS_DEVICE_JOYPAD;
...
static int setting_action_right_libks_device_type(
      kingsn_setting_t *setting, size_t idx, bool wraparound)
{
   bool refresh = false;
   ks_ctx_controller_info_t pad;
...
setting_get_string_representation_st_float_video_refresh_rate_polled(
      kingsn_setting_t *setting, char *s, size_t len)
{
...
#ifdef HAVE_LIBNX
static void get_string_representation_split_joycon(kingsn_setting_t *setting, char *s,
      size_t len)
{
   settings_t      *settings = config_get_ptr();
...
static void get_string_representation_bind_device(kingsn_setting_t *setting, char *s,
      size_t len)
{
...
static void general_read_handler(kingsn_setting_t *setting)
{
   settings_t      *settings = config_get_ptr();
...
static void general_write_handler(kingsn_setting_t *setting)
{
   enum event_command kingsn_cmd = CMD_EVENT_NONE;
   settings_t *settings         = config_get_ptr();
   ...
   if (setting->cmd_trigger_idx != CMD_EVENT_NONE)
   {
      ...
      if (setting->cmd_trigger_event_triggered ||
         ...
         kingsn_cmd = setting->cmd_trigger_idx;
   }
   ...
   switch (setting->enum_idx)
   {
      ...
      case MENU_ENUM_LABEL_AUDIO_RATE_CONTROL_DELTA:
         ...
         break;
      case MENU_ENUM_LABEL_VIDEO_REFRESH_RATE_AUTO:
         driver_ctl(KINGSN_DRIVER_CTL_SET_REFRESH_RATE, setting->value.target.fraction);

         /* In case refresh rate update forced non-block video. */
         kingsn_cmd = CMD_EVENT_VIDEO_SET_BLOCKING_STATE;
         break;
      ...
      case MENU_ENUM_LABEL_LOG_VERBOSITY:
         ...
         {
            verbosity_disable();
            kingsn_log_file_deinit();
         }
         KingStation_override_setting_unset(KINGSN_OVERRIDE_SETTING_VERBOSITY, NULL);
         break;
      case MENU_ENUM_LABEL_LOG_TO_FILE:
         ...
         KingStation_override_setting_unset(KINGSN_OVERRIDE_SETTING_LOG_TO_FILE, NULL);
         break;
      ...
      case MENU_ENUM_LABEL_AUDIO_WASAPI_SH_BUFFER_LENGTH:
         kingsn_cmd = CMD_EVENT_AUDIO_REINIT;
         break;
      ...
      case MENU_ENUM_LABEL_REWIND_BUFFER_SIZE_STEP:
         {
            kingsn_setting_t *buffer_size_setting = menu_setting_find_enum(MENU_ENUM_LABEL_REWIND_BUFFER_SIZE);
...
#ifdef HAVE_CHEATS
         {
            kingsn_setting_t *setting = menu_setting_find_enum(MENU_ENUM_LABEL_CHEAT_VALUE);
            if (setting)
            {
               *(setting->value.target.unsigned_integer) = 0;
...
#ifdef HAVE_CHEATS
         {
            kingsn_setting_t *setting = menu_setting_find_enum(MENU_ENUM_LABEL_CHEAT_SEARCH_EXACT);
            if (setting)
            {
               *setting->value.target.unsigned_integer = 0;
         ...
         if (!*setting->value.target.boolean)
         {
            ...
            for (i = 0; i < DEFAULT_MAX_PADS; i++)
            {
               ...
               input_sensor_set_state(i,
                     KS_SENSOR_ACCELEROMETER_DISABLE, 0);
               input_sensor_set_state(i,
                     KS_SENSOR_GYROSCOPE_DISABLE, 0);
               input_sensor_set_state(i,
                     KS_SENSOR_ILLUMINANCE_DISABLE, 0);
            }
         }
         break;
   ...
   if (kingsn_cmd || setting->cmd_trigger_event_triggered)
      command_event(kingsn_cmd, NULL);
}

static void frontend_log_level_change_handler(kingsn_setting_t *setting)
{
   if (!setting)
      return;
...
#ifdef HAVE_OVERLAY
static void overlay_enable_toggle_change_handler(kingsn_setting_t *setting)
{
   settings_t *settings  = config_get_ptr();
...
static void overlay_auto_rotate_toggle_change_handler(kingsn_setting_t *setting)
{
   settings_t *settings  = config_get_ptr();
...
#ifdef HAVE_VIDEO_LAYOUT
static void change_handler_video_layout_enable(kingsn_setting_t *setting)
{
   if (*setting->value.target.boolean)
   {
      settings_t *settings = config_get_ptr();
...
static void change_handler_video_layout_path(kingsn_setting_t *setting)
{
   settings_t *settings = config_get_ptr();
   configuration_set_uint(settings,
...
static void change_handler_video_layout_selected_view(kingsn_setting_t *setting)
{
...
#ifdef HAVE_CHEEVOS
static void achievement_hardcore_mode_write_handler(kingsn_setting_t *setting)
{
   rcheevos_hardcore_enabled_changed();
}

static void achievement_leaderboards_enabled_write_handler(kingsn_setting_t* setting)
{
   rcheevos_leaderboards_enabled_changed();
}

static void achievement_leaderboards_get_string_representation(kingsn_setting_t* setting, char* s, size_t len)
{
...
static void update_streaming_url_write_handler(kingsn_setting_t *setting)
{
      ...
      unsigned user)
{
   ...
   settings_t *settings                       = config_get_ptr();
   kingsn_system_info_t *system                = runloop_get_system_info();
   const struct ks_keybind* const defaults =
      (user == 0) ? ks_keybinds_1 : ks_keybinds_rest;
...

   {
      ...
      snprintf(key_type[user], sizeof(key_type[user]),
               msg_hash_to_str(MENU_ENUM_LABEL_INPUT_LIBKS_DEVICE),
               user + 1);
      ...
      (*list)[list_info->index - 1].action_left   = &setting_action_left_libks_device_type;
      (*list)[list_info->index - 1].action_right  = &setting_action_right_libks_device_type;
      (*list)[list_info->index - 1].action_select = &setting_action_right_libks_device_type;
      (*list)[list_info->index - 1].action_start  = &setting_action_start_libks_device_type;
      (*list)[list_info->index - 1].get_string_representation =
         &setting_get_string_representation_uint_libks_device;
      MENU_SETTINGS_LIST_CURRENT_ADD_ENUM_IDX_PTR(list, list_info,
            (enum msg_hash_enums)(MENU_ENUM_LABEL_INPUT_LIBKS_DEVICE + user));
   ...
   for (i = 0; i < KINGSN_BIND_LIST_END; i ++)
   {
      char label[255];
      char name[255];
      ...
      if (
            ...
            && (i < KINGSN_FIRST_META_KEY)
            && core_has_set_input_descriptor()
            && (i != KINGSN_TURBO_ENABLE)
         )
      {
...
static int directory_action_start_generic(kingsn_setting_t *setting)
{
   if (!setting)
      return -1;
      ...
      const char *parent_group)
{
   ...
   switch (type)
   {
      case SETTINGS_LIST_MAIN_MENU:
         ...
         CONFIG_ACTION(
               list, list_info,
               MENU_ENUM_LABEL_START_NET_KSPAD,
...
#endif
         {
            ...
            if (frontend_driver_get_core_extension(ext_name, sizeof(ext_name)))
            {
               CONFIG_ACTION(
                     ...
                     &subgroup_info,
                     parent_group);
               (*list)[list_info->index - 1].size                = (uint32_t)path_get_realsize(KINGSN_PATH_CORE);
               (*list)[list_info->index - 1].value.target.string = path_get_ptr(KINGSN_PATH_CORE);
        ...
        CONFIG_ACTION(
         ...
         CONFIG_ACTION(
               list, list_info,
               MENU_ENUM_LABEL_KS_ACHIEVEMENTS_SETTINGS,
      ...
      case SETTINGS_LIST_LOGGING:
         {
            ...
            (*list)[list_info->index - 1].get_string_representation =
               &setting_get_string_representation_uint_libks_log_level;

            CONFIG_UINT(
                  list, list_info,
                  &settings->uints.libks_log_level,
                  MENU_ENUM_LABEL_LIBKS_LOG_LEVEL,
                  MENU_ENUM_LABEL_VALUE_LIBKS_LOG_LEVEL,
                  DEFAULT_LIBKS_LOG_LEVEL,
                  &group_info,
                  &subgroup_info,
                  parent_group,
                  general_write_handler,
            ...
            kingsn_ctl(KINGSN_CTL_GET_PERFCNT, &tmp_b);
      ...
      case SETTINGS_LIST_CHEAT_DETAILS:
#ifdef HAVE_CHEATS
         {
            ...
            CONFIG_UINT_CBS(cheat_manager_state.working_cheat.handler, CHEAT_HANDLER,
                  ...
                  CHEAT_HANDLER_TYPE_EMU,CHEAT_HANDLER_TYPE_KS,1);
            (*list)[list_info->index - 1].action_ok = &setting_action_ok_uint;
...
#if !defined(KINGSN_CONSOLE) && !defined(KINGSN_MOBILE)
            CONFIG_BOOL(
                  list, list_info,
      ...
      case SETTINGS_LIST_AUDIO:
         ...
         CONFIG_UINT(
               ...
               general_read_handler);
         (*list)[list_info->index - 1].action_ok     = &setting_action_ok_uint_special;
         menu_settings_list_current_add_range(list, list_info, 1000, 192000, 50.0, true, true);
         SETTINGS_DATA_LIST_CURRENT_ADD_FLAGS(list, list_info, SD_FLAG_ADVANCED);
      ...
      case SETTINGS_LIST_INPUT_HOTKEY:
         {
            ...
            for (i = 0; i < KINGSN_BIND_LIST_END; i ++)
            {
               if (!input_config_bind_map_get_meta(i))
                  continue;
#ifndef HAVE_QT
               if (i == KINGSN_UI_COMPANION_TOGGLE)
                  continue;
#endif
               CONFIG_BIND_ALT(
                     list, list_info,
      ...
      case SETTINGS_LIST_CHEEVOS:
         ...
         START_GROUP(list, list_info, &group_info,
               ...
               parent_group);
         parent_group = msg_hash_to_str(MENU_ENUM_LABEL_KS_ACHIEVEMENTS_SETTINGS);
         START_SUB_GROUP(list, list_info, "State", &group_info, &subgroup_info, parent_group);
      ...
      case SETTINGS_LIST_NETPLAY:
         ...
         (*list)[list_info->index - 1].get_string_representation =
            ...
            menu_settings_list_current_add_range(list, list_info, 0, KINGSN_NETPLAY_SHARE_DIGITAL_LAST-1, 1, true, true);
            (*list)[list_info->index - 1].ui_type   = ST_UI_TYPE_UINT_COMBOBOX;
            ...
            menu_settings_list_current_add_range(list, list_info, 0, KINGSN_NETPLAY_SHARE_ANALOG_LAST-1, 1, true, true);
            (*list)[list_info->index - 1].ui_type   = ST_UI_TYPE_UINT_COMBOBOX;
      ...
      case SETTINGS_LIST_USER_ACCOUNTS:
         ...
         CONFIG_ACTION(
               list, list_info,
               MENU_ENUM_LABEL_ACCOUNTS_KS_ACHIEVEMENTS,
      ...
      case SETTINGS_LIST_DIRECTORY:
         ...
         CONFIG_DIR(
               list, list_info,
               settings->paths.directory_libks,
               sizeof(settings->paths.directory_libks),
               MENU_ENUM_LABEL_LIBKS_DIR_PATH,
         ...
         CONFIG_DIR(
               list, list_info,
               settings->paths.path_libks_info,
               sizeof(settings->paths.path_libks_info),
               MENU_ENUM_LABEL_LIBKS_INFO_PATH,
         ...
         CONFIG_DIR(
               list, list_info,
               dir_get_ptr(KINGSN_DIR_SAVEFILE),
               dir_get_size(KINGSN_DIR_SAVEFILE),
         ...
         CONFIG_DIR(
               list, list_info,
               dir_get_ptr(KINGSN_DIR_SAVESTATE),
               dir_get_size(KINGSN_DIR_SAVESTATE),
...
void menu_setting_free(kingsn_setting_t *setting)
{
   unsigned values, n;
   kingsn_setting_t **list = NULL;

   if (!setting)
      return;

   list                   = (kingsn_setting_t**)&setting;
...
static kingsn_setting_t *menu_setting_new_internal(kingsn_setting_info_t *list_info)
{
   unsigned i;
   kingsn_setting_t* resized_list        = NULL;
   ...
   kingsn_setting_t **list_ptr           = NULL;
   kingsn_setting_t *list                = (kingsn_setting_t*)
   ...
   resized_list = (kingsn_setting_t*)realloc(list,
         list_info->index * sizeof(kingsn_setting_t));
   if (!resized_list)
   {
      free(list);
      return NULL;
 ...
 **/
static kingsn_setting_t *menu_setting_new(void)
{
   kingsn_setting_t* list           = NULL;
   kingsn_setting_info_t *list_info = (kingsn_setting_info_t*)
      ...
      case MENU_SETTING_CTL_IS_OF_PATH_TYPE:
         {
            ...
            kingsn_setting_t *setting = (kingsn_setting_t*)data;
      ...
      case MENU_SETTING_CTL_ACTION_RIGHT:
         {
            ...
            if (setting->action_right)
            {
               ...
               menu_driver_ctl(
                     KINGSN_MENU_CTL_UPDATE_SAVESTATE_THUMBNAIL_PATH, NULL);
               menu_driver_ctl(
                     KINGSN_MENU_CTL_UPDATE_SAVESTATE_THUMBNAIL_IMAGE, NULL);
               if (ret == -1)
                  return false;
            }
            else
...
{
#ifdef HAVE_MENU
   kingsn_setting_t **list                    = (kingsn_setting_t**)list_data;
   kingsn_setting_info_t *list_info           = (kingsn_setting_info_t*)list_info_data;
   kingsn_setting_group_info_t *group_info    = (kingsn_setting_group_info_t*)group_data;
   kingsn_setting_group_info_t *subgroup_info = (kingsn_setting_group_info_t*)subgroup_data;
   global_t                        *global   = global_get_ptr();
