...
struct ks_perf_counter **ks_get_perf_counter_kingsn(void);

struct ks_perf_counter **ks_get_perf_counter_libks(void);

unsigned ks_get_perf_count_kingsn(void);

unsigned ks_get_perf_count_libks(void);

void kingsn_perf_register(struct ks_perf_counter *perf);

#define performance_counter_init(perf, name) \
   perf.ident = name; \
   if (!perf.registered) \
      kingsn_perf_register(&perf)
