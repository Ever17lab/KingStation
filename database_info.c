...
static int database_cursor_iterate(libksdb_cursor_t *cur,
      database_info_t *db_info)
{
   ...
   if (libksdb_cursor_read_item(cur, &item) != 0)
      return -1;
...
static int database_cursor_open(libksdb_t *db,
      libksdb_cursor_t *cur, const char *path, const char *query)
{
   const char *error     = NULL;
   libksdb_query_t *q = NULL;

   if ((libksdb_open(path, db)) != 0)
      return -1;

   if (query)
      q = (libksdb_query_t*)libksdb_query_compile(db, query,
      strlen(query), &error);

   if (error)
      goto error;
   if ((libksdb_cursor_open(db, cur, q)) != 0)
      goto error;
...
static int database_cursor_close(libksdb_t *db, libksdb_cursor_t *cur)
{
...
database_info_handle_t *database_info_dir_init(const char *dir,
      enum database_type type, ks_task_t *task,
      ...
      const char *rdb_path, const char *query)
{
   ...
   libksdb_t *db                         = libksdb_new();
   libksdb_cursor_t *cur                 = libksdb_cursor_new();
