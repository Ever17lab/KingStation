...
static void qnx_process_joystick_event(qnx_input_t *qnx, screen_event_t screen_event, int type)
{
    ...
    if(displacement != 0)
    {
        ...
        if(abs(qnx->trackpad_acc[0]) > TRACKPAD_THRESHOLD)
        {
            ...
            {
                input_keyboard_event(true, KSK_LEFT, 0, 0, KS_DEVICE_KEYBOARD);
                input_keyboard_event(false, KSK_LEFT, 0, 0, KS_DEVICE_KEYBOARD);
            }
            else if(qnx->trackpad_acc > 0)
            {
                input_keyboard_event(true, KSK_RIGHT, 0, 0, KS_DEVICE_KEYBOARD);
                input_keyboard_event(false, KSK_RIGHT, 0, 0, KS_DEVICE_KEYBOARD);
            }
        ...
        if(abs(qnx->trackpad_acc[1]) > TRACKPAD_THRESHOLD)
        {
            if(qnx->trackpad_acc < 0)
            {
                input_keyboard_event(true, KSK_UP, 0, 0, KS_DEVICE_KEYBOARD);
                input_keyboard_event(false, KSK_UP, 0, 0, KS_DEVICE_KEYBOARD);
            }
            else if(qnx->trackpad_acc > 0)
            {
                input_keyboard_event(true, KSK_DOWN, 0, 0, KS_DEVICE_KEYBOARD);
                input_keyboard_event(false, KSK_DOWN, 0, 0, KS_DEVICE_KEYBOARD);
            }
    ...
    input_keyboard_event(buttons != 0, KSK_RETURN, 0, 0, KS_DEVICE_KEYBOARD);
}
...
#ifdef DEBUG
   if (controller->type == SCREEN_EVENT_GAMEPAD)
      KINGSN_LOG("Gamepad Device Connected:\n");
   else if (controller->type == SCREEN_EVENT_JOYSTICK)
      KINGSN_LOG("Joystick Device Connected:\n");
   else if (controller->type == SCREEN_EVENT_KEYBOARD)
      KINGSN_LOG("Keyboard Device Connected:\n");

   KINGSN_LOG("\tID: %s\n", controller->id);
   KINGSN_LOG("\tVendor  ID: %s\n", controller->vid);
   KINGSN_LOG("\tProduct ID: %s\n", controller->pid);
   KINGSN_LOG("\tButton Count: %d\n", controller->buttonCount);
   KINGSN_LOG("\tAnalog Count: %d\n", controller->analogCount);
#endif
}
...
static int qnx_discover_controllers(qnx_input_t *qnx)
{
   ...
   if (ret < 0)
   {
     KINGSN_ERR("Error querying SCREEN_PROPERTY_DEVICE_COUNT: [%d] %s\n",
   ...
   if (!devices_found)
   {
     KINGSN_ERR("Error allocating devices_found, deviceCount=%d\n",
	       deviceCount);
     return false;
   }
   ...
   if (ret < 0)
   {
     KINGSN_ERR("Error querying SCREEN_PROPERTY_DEVICES: [%d] %s\n",
	       errno, strerror(errno));
     return false;
   }
      ...
      screen_event_t event, int type)
{
    ...
    if(!keyrepeat)
    {
        input_keyboard_event(keydown, keycode, 0, mod, KS_DEVICE_KEYBOARD);
    }
...
#if 0
         KINGSN_LOG("New Touch: x:%d, y:%d, id:%d\n", pos[0], pos[1], contact_id);
         KINGSN_LOG("Map: %d %d %d %d %d %d\n", qnx->touch_map[0], qnx->touch_map[1],
...
#if 0
         KINGSN_LOG("Release: x:%d, y:%d, id:%d\n", pos[0], pos[1], contact_id);
         KINGSN_LOG("Map: %d %d %d %d %d %d\n", qnx->touch_map[0], qnx->touch_map[1],
               qnx->touch_map[2], qnx->touch_map[3], qnx->touch_map[4],
               qnx->touch_map[5]);
#endif
      ...
      case SCREEN_EVENT_DEVICE:
         {
            ...
            else
            {
               for (i = 0; i < DEFAULT_MAX_PADS; ++i)
               {
                  if (device == qnx->devices[i].handle)
                  {
                     KINGSN_LOG("Device %s: Disconnected.\n",
                           qnx->devices[i].id);
                     qnx_init_controller(qnx, &qnx->devices[i]);
                     break;
                  }
      ...
      case NAVIGATOR_SYSKEY_PRESS:
         switch(navigator_event_get_syskey_key(event))
         {
            case NAVIGATOR_SYSKEY_BACK:
               input_keyboard_event(true, KSK_BACKSPACE, 0, 0, KS_DEVICE_KEYBOARD);
               input_keyboard_event(false, KSK_BACKSPACE, 0, 0, KS_DEVICE_KEYBOARD);
               break;
...
static void *qnx_input_init(const char *joypad_driver)
{
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_qnx);
...
{
    unsigned bit = kingsn_keysym_lut[(enum ks_key)id];
    return id < KSK_LAST && BIT_GET(qnx->keyboard_state, bit);
}
      ...
      unsigned id)
{
   ...
   switch (device)
   {
      ...
      case KINGSN_DEVICE_POINTER_SCREEN:
         return qnx_pointer_input_state(qnx, idx, id, device == KINGSN_DEVICE_POINTER_SCREEN);
      default:
         break;
   }
