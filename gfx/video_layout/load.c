...
static bool load_param(scope_t *scope, rxml_node_t *node, bool can_repeat)
{
   ...
   {
      KINGSN_LOG("video_layout: <param> is missing 'name' attribute\n");
      return false;
   }
   ...
   if (can_repeat && start)
   {
      ...
      {
         KINGSN_LOG("video_layout: invalid generator <param name=\"%s\" /> missing increment/shift\n",
   ...
   {
      KINGSN_LOG("video_layout: invalid parameter <param name=\"%s\" /> missing value\n",
         scope_eval(scope, name));
      return false;
   }
...
static bool load_component(scope_t *scope, component_t *comp, rxml_node_t *node)
{
   ...
   {
      KINGSN_LOG("video_layout: invalid component <%s />\n", node->name);
      return false;
   }
      ...
      case VIDEO_LAYOUT_C_IMAGE:
         {
            ...
            {
               KINGSN_LOG("video_layout: invalid component <%s />, missing 'file' attribute\n", node->name);
               result = false;
            }
      ...
      case VIDEO_LAYOUT_C_TEXT:
         {
            ...
            {
               KINGSN_LOG("video_layout: invalid component <%s />, missing 'string' attribute\n", node->name);
               result = false;
            }
...
static bool load_element(scope_t *scope, rxml_node_t *node)
{
   ...
   if (!(name = rxml_node_attrib(node, "name")))
   {
      KINGSN_LOG("video_layout: <element> is missing 'name' attribute\n");
      return false;
   }
...
static bool load_view(scope_t *scope, view_t *view, rxml_node_t *node, bool is_named)
{
   ...
   if (is_named)
   {
      ...
      if (!(name = rxml_node_attrib(node, "name")))
      {
         KINGSN_LOG("video_layout: <view> is missing 'name' attribute\n");
         return false;
      }
   ...
   for (n = node->children; n; n = n->next)
   {
      ...
      if (string_is_equal(n->name, "group"))
      {
         ...
         if ((ref = rxml_node_attrib(n, "ref")))
         {
            ...
            {
               KINGSN_LOG("video_layout: group \"%s\" is missing\n", scope_eval(scope, ref));
               result = false;
            }
         }
         else
         {
            KINGSN_LOG("video_layout: <group> is missing 'ref' attribute\n");
            result = false;
         }
      }
      ...
      else if (string_is_equal(n->name, "repeat"))
      {
         ...
         {
            KINGSN_LOG("video_layout: <repeat> is missing 'count' attribute\n");
            result = false;
            continue;
         }
      ...
      else /* element */
      {
         ...
         else
         {
            ...
            if ((elem_name = rxml_node_attrib(n, "element")))
            {
               ...
               {
                  KINGSN_LOG("video_layout: element \"%s\" is missing\n", scope_eval(scope, elem_name));
                  result = false;
               }
            }
            else
            {
               KINGSN_LOG("video_layout: <%s> is missing 'element' attribute\n", n->name);
               result = false;
            }
         }
...
bool video_layout_load_internal(view_array_t *view_array, rxml_document_t *doc)
{
   ...
   {
      KINGSN_LOG("video_layout: invalid MAME Layout file\n");
      return false;
   }
