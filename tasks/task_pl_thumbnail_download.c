...
static void download_pl_thumbnail(pl_thumb_handle_t *pl_thumb)
{
   ...
   if (get_thumbnail_paths(pl_thumb, path, sizeof(path), url, sizeof(url)))
   {
      ...
      if (!path_is_valid(path) || pl_thumb->overwrite)
      {
         ...
         pl_thumb->http_task = (ks_task_t*)task_push_http_transfer_file(
...
static void task_pl_thumbnail_download_handler(ks_task_t *task)
{
   pl_thumb_handle_t *pl_thumb = NULL;
...
static bool task_pl_thumbnail_finder(ks_task_t *task, void *user_data)
{
   pl_thumb_handle_t *pl_thumb = NULL;
      ...
      void *user_data, const char *err)
{
   ...
   if (do_refresh)
   {
      ...
      menu_driver_ctl(KINGSN_MENU_CTL_REFRESH_THUMBNAIL_IMAGE, &i);
   }
   
#endif
}

static void task_pl_entry_thumbnail_free(ks_task_t *task)
{
   pl_thumb_handle_t *pl_thumb = NULL;
...
static void task_pl_entry_thumbnail_download_handler(ks_task_t *task)
{
   pl_thumb_handle_t *pl_thumb = NULL;
...
static bool task_pl_entry_thumbnail_finder(ks_task_t *task, void *user_data)
{
