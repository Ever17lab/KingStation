...
#define GL_CHECK_ERROR() do { \
   ...
   { \
      log_cb(KS_LOG_ERROR, "GL error at line: %d\n", __LINE__); \
      abort(); \
   } \
} while(0)
#else
...
static GLuint fft_compile_shader(fft_t *fft, GLenum type, const char *source)
{
   ...
   if (!status)
   {
      ...
      log_cb(KS_LOG_ERROR, "Failed to compile.\n");
      glGetShaderInfoLog(shader, sizeof(log_info), &log_len, log_info);
      log_cb(KS_LOG_ERROR, "ERROR: %s\n", log_info);
      return 0;
   }
      ...
      const char *vertex_source, const char *fragment_source)
{
   ...
   if (!status)
   {
      ...
      log_cb(KS_LOG_ERROR, "Failed to link.\n");
