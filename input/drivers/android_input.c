...
#define ANDROID_KEYBOARD_PORT_INPUT_PRESSED(binds, id) (BIT_GET(android_key_state[ANDROID_KEYBOARD_PORT], kingsn_keysym_lut[(binds)[(id)].key]))
...
static bool android_input_set_sensor_state(void *data, unsigned port,
      enum ks_sensor_action action, unsigned event_rate);
      ...
      case APP_CMD_GAINED_FOCUS:
         {
            ...
            bool enable_accelerometer = (android_app->sensor_state_mask &
                  (UINT64_C(1) << KS_SENSOR_ACCELEROMETER_ENABLE)) &&
                        !android_app->accelerometerSensor;
            bool enable_gyroscope     = (android_app->sensor_state_mask &
                  (UINT64_C(1) << KS_SENSOR_GYROSCOPE_ENABLE)) &&
                        !android_app->gyroscopeSensor;

            kingsn_ctl(KINGSN_CTL_SET_PAUSED, &boolean);
            kingsn_ctl(KINGSN_CTL_SET_IDLE,   &boolean);
            video_driver_unset_stub_frame();

            if (enable_accelerometer)
               input_sensor_set_state(0,
                     KS_SENSOR_ACCELEROMETER_ENABLE,
      ...
      case APP_CMD_VIBRATE_KEYPRESS:
      {
         ...
         if (env && g_android && g_android->doVibrate)
         {
            CALL_VOID_METHOD_PARAM(env, g_android->activity->clazz,
                  g_android->doVibrate, (jint)-1, (jint)KS_RUMBLE_STRONG, (jint)255, (jint)1);
         }
         break;
      }
   }
...
static void *android_input_init(const char *joypad_driver)
{
   ...
   input_keymaps_init_keyboard_lut(kingsn_key_map_android);
   ...
   {
      KINGSN_WARN("Unable to open libandroid.so\n");
   }
      ...
      AInputEvent *event,size_t motion_ptr)
{
   ...
   if (av_info)
   {
      ...
      const struct ks_game_geometry *geom = (const struct ks_game_geometry*)&av_info->geometry;
      ...
      AInputEvent *event, int keycode, int *handled)
{
   ...
   input_keyboard_event(keydown, keyboardcode,
         keyboardcode, mod, KS_DEVICE_KEYBOARD);
      ...
      int source)
{
      ...
      strstr(device_name, "NVIDIA Corporation NVIDIA Controller v01.0")))
   {
      ...
      KINGSN_LOG("Special Device Detected: %s\n", device_model);
      {
#if 0
         KINGSN_LOG("- Pads Mapped: %d\n- Device Name: %s\n- IDS: %d, %d, %d",
   ...
   {
      /* only use the hack if the device is one of the built-in devices */
      KINGSN_LOG("ARCHOS GAMEPAD Detected: %s\n", device_model);
      {
         if ( pad_id1 < 0 )
            pad_id1 = id;
         else
...
static void android_input_poll(void *data)
{
   ...
   while ((ident =
            ALooper_pollAll((input_config_binds[0][KINGSN_PAUSE_TOGGLE].valid 
               && input_key_pressed(KINGSN_PAUSE_TOGGLE,
               ...
               NULL, NULL, NULL)) >= 0)
   {
      ...
      {
         if (kingsn_ctl(KINGSN_CTL_IS_PAUSED, NULL))
      ...
      case KINGSN_DEVICE_POINTER_SCREEN:
         switch (id)
         {
            case KS_DEVICE_ID_POINTER_X:
               if (device == KINGSN_DEVICE_POINTER_SCREEN)
            ...
            case KINGSN_DEVICE_ID_POINTER_BACK:
            {
               const struct ks_keybind *keyptr = 
                  &input_autoconf_binds[0][KINGSN_MENU_TOGGLE];
               if (keyptr->joykey == 0)
      ...
      enum ks_sensor_action action, unsigned event_rate)
{
   ...
   switch (action)
   {
      case KS_SENSOR_ACCELEROMETER_ENABLE:
         if (!android_app->accelerometerSensor)
         ...
         BIT64_CLEAR(android_app->sensor_state_mask, KS_SENSOR_ACCELEROMETER_DISABLE);
         BIT64_SET(android_app->sensor_state_mask, KS_SENSOR_ACCELEROMETER_ENABLE);
         return true;

      case KS_SENSOR_ACCELEROMETER_DISABLE:
         if (android_app->sensorEventQueue &&
             android_app->accelerometerSensor)
         ...
         BIT64_CLEAR(android_app->sensor_state_mask, KS_SENSOR_ACCELEROMETER_ENABLE);
         BIT64_SET(android_app->sensor_state_mask, KS_SENSOR_ACCELEROMETER_DISABLE);
         return true;

      case KS_SENSOR_GYROSCOPE_ENABLE:
         if (!android_app->gyroscopeSensor)
            android_input_enable_sensor_manager(android_app);
         ...
         BIT64_CLEAR(android_app->sensor_state_mask, KS_SENSOR_GYROSCOPE_DISABLE);
         BIT64_SET(android_app->sensor_state_mask, KS_SENSOR_GYROSCOPE_ENABLE);
         return true;

      case KS_SENSOR_GYROSCOPE_DISABLE:
         if (android_app->sensorEventQueue &&
             android_app->gyroscopeSensor)
            ASensorEventQueue_disableSensor(android_app->sensorEventQueue,
                  android_app->gyroscopeSensor);
         ...
         BIT64_CLEAR(android_app->sensor_state_mask, KS_SENSOR_GYROSCOPE_ENABLE);
         BIT64_SET(android_app->sensor_state_mask, KS_SENSOR_GYROSCOPE_DISABLE);
         return true;
