...
static enum kingsn_shader_type supported_shader_type = KINGSN_SHADER_NONE;
            ...
            for (i = 0; renderchain_d3d_drivers[i]; i++)
            {
               ...
               *renderchain_handle = data;
               ...
               if (string_is_equal(renderchain_d3d_drivers[i]->ident, "cg_d3d9"))
                  supported_shader_type = KINGSN_SHADER_CG;
               else if (string_is_equal(renderchain_d3d_drivers[i]->ident, "hlsl_d3d9"))
                  supported_shader_type = KINGSN_SHADER_HLSL;
...
{
   KINGSN_LOG("[D3D9]: Render pass info:\n");
   KINGSN_LOG("\tTexture width: %u\n", info->tex_w);
   KINGSN_LOG("\tTexture height: %u\n", info->tex_h);

   KINGSN_LOG("\tScale type (X): ");

   switch (info->pass->fbo.type_x)
   {
      case KINGSN_SCALE_INPUT:
         KINGSN_LOG("Relative @ %fx\n", info->pass->fbo.scale_x);
         break;

      case KINGSN_SCALE_VIEWPORT:
         KINGSN_LOG("Viewport @ %fx\n", info->pass->fbo.scale_x);
         break;

      case KINGSN_SCALE_ABSOLUTE:
         KINGSN_LOG("Absolute @ %u px\n", info->pass->fbo.abs_x);
         break;
   }

   KINGSN_LOG("\tScale type (Y): ");
   ...
   KINGSN_LOG("\tBilinear filter: %s\n",
         info->pass->filter == KINGSN_FILTER_LINEAR ? "true" : "false");
}
      ...
      bool rgb32)
{
   ...
   link_info.pass  = NULL;
   link_info.tex_w = input_scale * KINGSN_SCALE_BASE;
   link_info.tex_h = input_scale * KINGSN_SCALE_BASE;
   ...
   {
      KINGSN_ERR("[D3D9]: Renderchain could not be initialized.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to init render chain.\n");
      return false;
   }

   KINGSN_LOG("[D3D9]: Renderchain driver: %s\n", d3d->renderchain_driver->ident);
   ...
   for (i = 1; i < d3d->shader.passes; i++)
   {
      ...
      {
         KINGSN_ERR("[D3D9]: Failed to add pass.\n");
         return false;
      }
      d3d9_log_info(&link_info);
   }
      ...
      if (d3d->renderchain_driver->add_lut)
      {
         ...
         for (i = 0; i < d3d->shader.luts; i++)
         {
            if (!d3d->renderchain_driver->add_lut(
                     ...
                     d3d->shader.lut[i].filter == KINGSN_FILTER_UNSPEC 
                     ? video_smooth 
                     : (d3d->shader.lut[i].filter == KINGSN_FILTER_LINEAR)))
            {
               KINGSN_ERR("[D3D9]: Failed to init LUTs.\n");
               return false;
            }
         }
      }
...
static bool d3d9_init_singlepass(d3d9_video_t *d3d)
{
   ...
   pass->fbo.type_y                      = KINGSN_SCALE_VIEWPORT;
...
static bool d3d9_init_multipass(d3d9_video_t *d3d, const char *shader_path)
{
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to parse shader preset.\n");
      return false;
   }

   KINGSN_LOG("[D3D9]: Found %u shaders.\n", d3d->shader.passes);
   ...
   for (i = 0; i < d3d->shader.passes; i++)
   {
      ...
      d3d->shader.pass[i].fbo.type_x  = KINGSN_SCALE_INPUT;
      d3d->shader.pass[i].fbo.type_y  = KINGSN_SCALE_INPUT;
   }
   ...
   if (use_extra_pass)
   {
      ...
      pass->fbo.type_x  = KINGSN_SCALE_VIEWPORT;
      pass->fbo.type_y  = KINGSN_SCALE_VIEWPORT;
...
static bool d3d9_init_base(void *data, const video_info_t *info)
{
   ...
   if (!g_pD3D9)
   {
      KINGSN_ERR("[D3D9]: Failed to create D3D interface.\n");
      return false;
   }
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to initialize device.\n");
      return false;
   }
...
static bool d3d9_initialize(d3d9_video_t *d3d, const video_info_t *info)
{
   ...
   else if (d3d->needs_restore)
   {
...
#ifdef HAVE_MENU
      menu_driver_ctl(KINGSN_MENU_CTL_DEINIT, NULL);
#endif
      ...
      if (!d3d9_reset(d3d->dev, &d3dpp))
      {
         ...
         ret = d3d9_init_base(d3d, info);
         if (ret)
            KINGSN_LOG("[D3D9]: Recovered from dead state.\n");
      }
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to initialize render chain.\n");
      return false;
   }
      ...
      void **input_data)
{
   ...
   KINGSN_LOG("[D3D9]: Monitor size: %dx%d.\n",
...
#if defined(HAVE_CG) || defined(HAVE_HLSL)
   {
      ...
      enum kingsn_shader_type type = video_shader_parse_type(shader_preset);
      ...
      input_driver_t **input, void **input_data)
{
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to init D3D.\n");
      free(d3d);
      return NULL;
   }
      ...
      const void *image_data, unsigned num_images)
{
   ...
   for (i = 0; i < num_images; i++)
   {
      ...
      if (!overlay->tex)
      {
         KINGSN_ERR("[D3D9]: Failed to create overlay texture\n");
         return false;
      }
      ...
      const char *msg, video_frame_info_t *video_info)
{
   ...
   if (d3d->needs_restore)
   {
      ...
      if (!d3d9_restore(d3d))
      {
         KINGSN_ERR("[D3D9]: Failed to restore.\n");
         return false;
      }
   }
   ...
   {
      KINGSN_ERR("[D3D9]: Failed to render scene.\n");
      return false;
   }
      ...
      enum kingsn_shader_type type, const char *path)
{
   ...
   switch (type)
   {
      ...
      case KINGSN_SHADER_HLSL:
         ...
         {
            KINGSN_WARN("[D3D9]: Shader preset %s is using unsupported shader type %s, falling back to stock %s.\n",
      ...
      case KINGSN_SHADER_NONE:
         break;
      default:
         KINGSN_WARN("[D3D9]: Only Cg shaders are supported. Falling back to stock.\n");
   }

   if (!d3d9_process_shader(d3d) || !d3d9_restore(d3d))
   {
      KINGSN_ERR("[D3D9]: Failed to set shader.\n");
      return false;
   }
      ...
      float alpha)
{
            ...
            d3d->menu->tex_h != height)
   {
      ...
      if (!d3d->menu->tex)
      {
         KINGSN_ERR("[D3D9]: Failed to create menu texture.\n");
         return;
      }
      ...
      uintptr_t *id)
{
   ...
   if (!tex)
   {
      KINGSN_ERR("[D3D9]: Failed to create texture\n");
      return;
   }
...
static uint32_t d3d9_get_flags(void *data)
{
   ...
   if (supported_shader_type == KINGSN_SHADER_CG)
      BIT32_SET(flags, GFX_CTX_FLAGS_SHADERS_CG);
   else if (supported_shader_type == KINGSN_SHADER_HLSL)
