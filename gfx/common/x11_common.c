...
static void x11_set_window_class(Display *dpy, Window win)
{
   ...
   hint.res_name   = (char*)"KingStation"; /* Broken header. */
   hint.res_class  = (char*)"KingStation";
...
static void x11_set_window_pid(Display *dpy, Window win)
{
    ...
    if (gethostname(hostname, scret + 1) == -1)
        KINGSN_WARN("Failed to get hostname.\n");
    else
    {
...
static void xdg_screensaver_inhibit(Window wnd)
{
   ...
   KINGSN_LOG("[X11]: Suspending screensaver (X11, xdg-screensaver).\n");
   ...
   if (ret == -1)
   {
      ...
      KINGSN_WARN("Failed to launch xdg-screensaver.\n");
   }
   else if (WEXITSTATUS(ret))
   {
      xdg_screensaver_available = false;
      KINGSN_WARN("Could not suspend screen saver.\n");
   }
}
...
bool x11_create_input_context(Display *dpy, Window win, XIM *xim, XIC *xic)
{
   ...
   *xim            = XOpenIM(dpy, NULL, NULL, NULL);
   ...
   if (!*xim)
   {
      KINGSN_ERR("[X11]: Failed to open input method.\n");
      return false;
   }
   ...
   *xic = XCreateIC(*xim, XNInputStyle,
   ...
   if (!*xic)
   {
      KINGSN_ERR("[X11]: Failed to create input context.\n");
      return false;
   }
...
static void x11_handle_key_event(unsigned keycode, XEvent *event, XIC ic, bool filter)
{
   ...
   input_keyboard_event(down, key, chars[0], mod, KS_DEVICE_KEYBOARD);

   for (i = 1; i < num; i++)
      input_keyboard_event(down, KSK_UNKNOWN,
            chars[i], mod, KS_DEVICE_KEYBOARD);
}
...
bool x11_input_ctx_new(bool true_full)
{
   ...
   video_driver_display_type_set(KINGSN_DISPLAY_X11);
