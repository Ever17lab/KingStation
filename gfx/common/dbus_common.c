...
void dbus_ensure_connection(void)
{
    ...
    {
        KINGSN_LOG("[DBus]: Failed to get DBus connection. Screensaver will not be suspended via DBus.\n");
        dbus_error_free(&err);
    }
...
bool dbus_screensaver_inhibit(void)
{
   ...
   if (dbus_screensaver_cookie == 0)
   {
      KINGSN_ERR("[DBus]: Failed to suspend screensaver via DBus.\n");
   }
   else
   {
      KINGSN_LOG("[DBus]: Suspended screensaver via DBus.\n");
   }
