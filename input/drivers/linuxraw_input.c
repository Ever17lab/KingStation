...
static void *linuxraw_input_init(const char *joypad_driver)
{
   ...
   {
      KINGSN_WARN("stdin is already used for content loading. Cannot use stdin for input.\n");
      return NULL;
   }
      ...
      case KS_DEVICE_JOYPAD:
         if (id == KS_DEVICE_ID_JOYPAD_MASK)
         {
            ...
            for (i = 0; i < KINGSN_FIRST_CUSTOM_BIND; i++)
            {
               ...
               {
                  if (
                        linuxraw->state[kingsn_keysym_lut[
                        (enum ks_key)binds[port][i].key]]
                        )
                     ret |= (1 << i);
               }
            }
         ...
         if (id < KINGSN_BIND_LIST_END)
         {
            ...
            {
               if ((linuxraw->state[kingsn_keysym_lut
                        [(enum ks_key)binds[port][id].key]]
                   ))
                  return 1;
            }
         }
...
{
   return (1 << KS_DEVICE_JOYPAD) | (1 << KS_DEVICE_ANALOG);
}
