...
#define FUNC_PUSH_TASK(funcname, handlerfunc, message) \
   bool funcname(ks_task_callback_t cb) \
   { \
...
static void task_wifi_scan_handler(ks_task_t *task)
{
   if (!task)
      return;
...
static void task_wifi_enable_handler(ks_task_t *task)
{
   if (!task)
      return;
...
static void task_wifi_disable_handler(ks_task_t *task)
{
   if (!task)
      return;
...
static void task_wifi_disconnect_handler(ks_task_t *task)
{
...
static void task_wifi_connect_handler(ks_task_t *task)
{
   if (!task)
      return;
...
bool task_push_wifi_connect(ks_task_callback_t cb, void *netptr) {
   char msg[128];
   ks_task_t           *task = task_init();
