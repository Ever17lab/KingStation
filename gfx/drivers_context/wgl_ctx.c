...
static void create_gl_context(HWND hwnd, bool *quit)
{
   ...
   if (win32_hrc)
   {
      KINGSN_LOG("[WGL]: Using cached GL context.\n");
   ...
   else
   {
      ...
      if (win32_hrc && !core_context && !debug)
      {
         ...
         if (win32_hw_hrc)
         {
            ...
            {
               KINGSN_LOG("[WGL]: Failed to share contexts.\n");
               *quit = true;
            }
         }
         else
   ...
   if (core_context || debug)
   {
      ...
      *aptr = 0;
      ...
      if (pcreate_context)
      {
         ...
         for (i = 0; i < version_rows; i++)
         {
            ...
            if (context)
            {
               ...
               if (win32_use_hw_ctx)
               {
                  ...
                  if (!win32_hw_hrc)
                  {
                     KINGSN_ERR("[WGL]: Failed to create shared context.\n");
                     *quit = true;
                     break;
                  }
               }
         ...
         if (!context)
         {
            KINGSN_ERR("[WGL]: Failed to create core context. Falling back to legacy context.\n");
            *quit = true;
         }
      }
      else
         KINGSN_ERR("[WGL]: wglCreateContextAttribsARB not supported.\n");
   }
...
static void gfx_ctx_wgl_swap_interval(void *data, int interval)
{
   ...
   switch (win32_api)
   {
      case GFX_CTX_OPENGL_API:
         ...
         KINGSN_LOG("[WGL]: wglSwapInterval(%i)\n", win32_interval);
         if (!p_swap_interval(win32_interval))
            KINGSN_WARN("[WGL]: wglSwapInterval() failed.\n");
#endif
         break;
      ...
      bool fullscreen)
{
   ...
   {
      KINGSN_ERR("[WGL]: win32_set_video_mode failed.\n");
      goto error;
   }
