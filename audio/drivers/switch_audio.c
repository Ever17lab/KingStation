...
static ssize_t switch_audio_write(void *data, const void *buf, size_t size)
{
	...
	if (!swa->current_buffer)
   {
      ...
      {
         KINGSN_LOG("Failed to get released buffer?\n");
         return -1;
      }
      ...
      unsigned *new_rate)
{
   ...
   if (num_names != 1)
   {
      KINGSN_ERR("got back more than one AudioOut\n");
      goto fail_audio_ipc;
   }
   ...
   {
      KINGSN_ERR("expected sample rate of %d, got sample rate of %d\n",
   ...
   *new_rate           = swa->output.sample_rate;
   ...
   KINGSN_LOG("[Audio]: Audio initialized\n");
