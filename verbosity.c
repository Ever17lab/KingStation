...
typedef struct verbosity_state
{
   ...
   Mutex mtx;
#endif
   /* If this is non-NULL. KINGSN_LOG and friends
...
void ks_main_log_file_init(const char *path, bool append)
{
   ...
   if (!tmp)
   {
      KINGSN_ERR("Failed to open system event log file: %s\n", path);
      return;
   }
...
void KINGSN_LOG_BUFFER(uint8_t *data, size_t size)
{
   unsigned i, offset;
   int padding     = size % 16;
   uint8_t buf[16] = {0};
   ...
   for (i = 0, offset = 0; i < size; i++)
   {
      ...
      {
         offset = 0;
         KINGSN_LOG("%02x%02x%02x%02x%02x%02x%02x%02x  %02x%02x%02x%02x%02x%02x%02x%02x\n",
...
void KINGSN_DBG(const char *fmt, ...)
{
   ...
   va_start(ap, fmt);
   KINGSN_LOG_V(FILE_PATH_LOG_INFO, fmt, ap);
   va_end(ap);
}
...
void KINGSN_LOG_OUTPUT(const char *msg, ...)
{
   va_list ap;
   va_start(ap, msg);
   KINGSN_LOG_OUTPUT_V(FILE_PATH_LOG_INFO, msg, ap);
   va_end(ap);
}
...
void KINGSN_WARN(const char *fmt, ...)
{
   ...
   va_start(ap, fmt);
   KINGSN_WARN_V(FILE_PATH_LOG_WARN, fmt, ap);
   va_end(ap);
}
...
void KINGSN_ERR(const char *fmt, ...)
{
   ...
   va_start(ap, fmt);
   KINGSN_ERR_V(FILE_PATH_LOG_ERROR, fmt, ap);
   va_end(ap);
}
#endif
      ...
      )
{
   ...
   if (!log_to_file && logging_to_file)
   {
      ks_main_log_file_deinit();
      /* ...and revert to console */
      ks_main_log_file_init(NULL, false);
      return;
   }
   ...
   if (!string_is_empty(log_file_path))
   {
      ...
      if (!string_is_empty(log_directory))
      {
         if (!path_is_directory(log_directory))
         {
            if (!path_mkdir(log_directory))
            {
               ...
               ks_main_log_file_init(NULL, false);
               KINGSN_ERR("Failed to create system event log directory: %s\n", log_directory);
               return;
            }
         }
      }
      ...
      ks_main_log_file_init(log_file_path, log_file_created);
      if (g_verbosity->initialized)
   ...
   ks_main_log_file_init(NULL, false);
   KINGSN_ERR("Failed to initialise system event file logging...\n");
}
