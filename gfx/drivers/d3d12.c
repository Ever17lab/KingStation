...
static void d3d12_set_filtering(void* data, unsigned index, bool smooth, bool ctx_scaling)
{
   ...
   {
      if (smooth)
         d3d12->samplers[KINGSN_FILTER_UNSPEC][i] = d3d12->samplers[KINGSN_FILTER_LINEAR][i];
      else
         d3d12->samplers[KINGSN_FILTER_UNSPEC][i] = d3d12->samplers[KINGSN_FILTER_NEAREST][i];
   }
}
...
static bool d3d12_gfx_set_shader(void* data, enum kingsn_shader_type type, const char* path)
{
   ...
   {
      KINGSN_WARN("[D3D12] Only Slang shaders are supported. Falling back to stock.\n");
      return false;
   }
   ...
   for (i = 0; i < d3d12->shader_preset->passes; source = &d3d12->pass[i++].rt)
   {
      ...
      if (!slang_process(
               d3d12->shader_preset, i, KINGSN_SHADER_HLSL, 50, &semantics_map,
      ...
      input_driver_t** input, void** input_data)
{
   ...
   {
      KINGSN_ERR("[D3D12]: win32_set_video_mode failed.\n");
      goto error;
   }
...
error:
   KINGSN_ERR("[D3D12]: failed to init video driver.\n");
...
static void d3d12_init_render_targets(d3d12_video_t* d3d12, unsigned width, unsigned height)
{
   ...
   for (i = 0; i < d3d12->shader_preset->passes; i++)
   {
      ...
      KINGSN_LOG("[d3d12]: Updating framebuffer size %u x %u.\n", width, height);
      ...
      video_frame_info_t* video_info)
{
...

         {
            ...
            while (texture_sem->stage_mask)
            {
...

               {
                  ...
                  if (texture_sem->filter == KINGSN_FILTER_NEAREST)
   ...
   if (texture)
   {
      ...
      d3d12_set_sampler(d3d12->queue.cmd, d3d12->samplers[KINGSN_FILTER_UNSPEC][KINGSN_WRAP_DEFAULT]);
      D3D12SetGraphicsRootConstantBufferView(
   ...
   if (d3d12->overlays.enabled)
   {
      ...
      D3D12SetGraphicsRootDescriptorTable(
            ...
            d3d12->samplers[KINGSN_FILTER_UNSPEC][KINGSN_WRAP_DEFAULT]);
      ...
      unsigned width, unsigned height, float alpha)
{
   ...
   d3d12->menu.texture.sampler = settings->bools.menu_linear_filter
      ? d3d12->samplers[KINGSN_FILTER_LINEAR][KINGSN_WRAP_DEFAULT]
      : d3d12->samplers[KINGSN_FILTER_NEAREST][KINGSN_WRAP_DEFAULT];
}
   ...
   switch (filter_type)
   {
      ...
      case TEXTURE_FILTER_LINEAR:
         texture->sampler = d3d12->samplers[
            KINGSN_FILTER_LINEAR][KINGSN_WRAP_EDGE];
         break;
      case TEXTURE_FILTER_MIPMAP_NEAREST:
         texture->desc.MipLevels = UINT16_MAX;
      case TEXTURE_FILTER_NEAREST:
         texture->sampler = d3d12->samplers[
            KINGSN_FILTER_NEAREST][KINGSN_WRAP_EDGE];
         break;
   }
