...
static bool connmanctl_tether_status(connman_t *connman)
{
   ...
   KINGSN_LOG("[CONNMANCTL] Tether Status: command: \"%s\", output: \"%s\"\n",
         connman->command, ln);
      ...
      connman_t *connman, bool switch_on, char* apname, char* passkey)
{
   ...
   KINGSN_LOG("[CONNMANCTL] Tether toggle: command: \"%s\"\n",
         connman->command);
   ...
   while (fgets(output, sizeof(output), command_file))
   {
      ...
      KINGSN_LOG("[CONNMANCTL] Tether toggle: output: \"%s\"\n",
            output);
   ...
   KINGSN_LOG("[CONNMANCTL] Tether toggle: command finished\n");
      ...
      connman_t *connman, char* ssid, size_t buffersize)
{
   ...
   KINGSN_LOG("[CONNMANCTL] Get Connected SSID: command: \"%s\", output: \"%s\"\n",
      ...
      connman_t *connman, char* servicename, size_t buffersize)
{
   ...
   KINGSN_LOG("[CONNMANCTL] Testing configured services for activity: command: \"%s\"\n",
         connman->command);
   ...
   while (fgets(temp, buffersize, command_file))
   {
      ...
      if ((temp_size + 1) == 0)
      {
         KINGSN_WARN("[CONNMANCTL] Service name empty.\n");
         continue;
      }
      ...
      KINGSN_LOG("[CONNMANCTL] Service: \"%s\", status: \"%s\"\n",
            temp, ln);
      ...
      if (ln[0] == '1')
      {
         ...
         KINGSN_LOG("[CONNMANCTL] Service \"%s\" considered as currently online\n",
               servicename);
...
static void connmanctl_tether_start_stop(void *data, bool start, char* configfile)
{
   ...
   KINGSN_LOG("[CONNMANCTL] Tether start stop: begin\n");

   if (start) /* we want to start tethering */
   {
      KINGSN_LOG("[CONNMANCTL] Tether start stop: request to start access point\n");

      if (connmanctl_tether_status(connman)) /* check if already tethering and bail out if so */
      {
         KINGSN_LOG("[CONNMANCTL] Tether start stop: AP already running\n");
      ...
      {
         KINGSN_WARN("[CONNMANCTL] Tether start stop: config \"%s\" does not exist\n",
               configfile);

         if (!(command_file = fopen(configfile, "w")))
         {
            KINGSN_ERR("[CONNMANCTL] Tether start stop: cannot create config file \"%s\"\n",
                  configfile);
         ...
         KINGSN_LOG("[CONNMANCTL] Tether start stop: creating new config \"%s\"\n",
               configfile);

         snprintf(apname, sizeof(apname), "LakkaAccessPoint");
         snprintf(passkey, sizeof(passkey), "KingStation");

         fprintf(command_file, "APNAME=%s\nPASSWORD=%s", apname, passkey);

         fclose(command_file);

         KINGSN_LOG("[CONNMANCTL] Tether start stop: new config \"%s\" created\n",
               configfile);
      }
      else
      {
         ...
         KINGSN_LOG("[CONNMANCTL] Tether start stop: config \"%s\" exists, reading it\n",
               configfile);
         ...
         KINGSN_LOG("[CONNMANCTL] Tether start stop: parsing command: \"%s\"\n",
               connman->command);
         ...
         while (fgets(ln, sizeof(ln), command_file))
         {
            ...
            if ((ln_size + 1) > 1)
            {
               ...
               if (i == 1)
               {
                  ...
                  KINGSN_LOG("[CONNMANCTL] Tether start stop: found APNAME: \"%s\"\n",
                        apname);
               ...
               if (i == 2)
               {
                  ...
                  KINGSN_LOG("[CONNMANCTL] Tether start stop: found PASSWORD: \"%s\"\n",
                        passkey);
               ...
               if (i > 2)
               {
                  KINGSN_WARN("[CONNMANCTL] Tether start stop: you should not get here...\n");
                  break;
               }
            }
         }
      ...
      if (!apname || !passkey)
      {
         KINGSN_ERR("[CONNMANCTL] Tether start stop: APNAME or PASSWORD missing\n");
      ...
      KINGSN_LOG("[CONNMANCTL] Tether start stop: checking if not connected to a wi-fi network...\n");
      ...
      if (strlen(ssid) != 0)
      {
         ...
         if (strlen(service) != 0)
         {
            ...
            KINGSN_LOG("[CONNMANCTL] Tether start stop: connected to SSID \"%s\", service \"%s\"\n",
                  ssid, service);
            ...
            KINGSN_LOG("[CONNMANCTL] Tether start stop: disconnecting from service \"%s\", command: \"%s\"\n",
            ...
            while (fgets(ln, sizeof(ln), command_file))
            {
               ...
               KINGSN_LOG("[CONNMANCTL] Tether start stop: output: \"%s\"\n",
                     ln);
            ...
            KINGSN_LOG("[CONNMANCTL] Tether start stop: disconnect end\n");
         }
      }
   ...
   {
      KINGSN_LOG("[CONNMANCTL] Tether start stop: request to stop access point\n");

      if (!connmanctl_tether_status(connman)) /* check if not tethering and when not, bail out */
      {
         KINGSN_LOG("[CONNMANCTL] Tether start stop: access point is not running\n");
   ...
   KINGSN_LOG("[CONNMANCTL] Tether start stop: calling tether_toggle()\n");

   /* call the tether toggle function */
   connmanctl_tether_toggle(connman, start, apname, passkey);

   KINGSN_LOG("[CONNMANCTL] Tether start stop: end\n");
}
