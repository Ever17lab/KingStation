   ...
   bool wraparound)
{
   ...
   if (settings->uints.input_remap_ids[user_idx][btn_idx] < KINGSN_CUSTOM_BIND_LIST_END - 1)
      settings->uints.input_remap_ids[user_idx][btn_idx]++;
   else if (settings->uints.input_remap_ids[user_idx][btn_idx] == KINGSN_CUSTOM_BIND_LIST_END - 1)
...
static int action_right_shader_filter_default(unsigned type, const char *label,
      bool wraparound)
{
   kingsn_setting_t *setting = menu_setting_find_enum(MENU_ENUM_LABEL_VIDEO_SMOOTH);
   if (!setting)
      return menu_cbs_exit();
      ...
      bool wraparound)
{
   ...
   kingsn_ctl(KINGSN_CTL_CORE_OPTION_NEXT, &idx);
