...
static void gfx_ctx_emscripten_get_canvas_size(int *width, int *height)
{
   ...
   if (!is_fullscreen)
   {
      ...
      if (r != EMSCRIPTEN_RESULT_SUCCESS)
      {
         KINGSN_ERR("[EMSCRIPTEN/EGL]: Could not get screen dimensions: %d\n",
            r);
         *width = 800;
         *height = 600;
      }
      ...
      bool *resize, unsigned *width, unsigned *height)
{
   ...
   *resize     = false;
      ...
      input_height != emscripten->fb_height)
   {
      ...
      if (r != EMSCRIPTEN_RESULT_SUCCESS)
         KINGSN_ERR("[EMSCRIPTEN/EGL]: error resizing canvas: %d\n", r);
      ...
      if (r != EMSCRIPTEN_RESULT_SUCCESS)
         KINGSN_ERR("[EMSCRIPTEN/EGL]: error resizing canvas css: %d\n", r);
...
static void *gfx_ctx_emscripten_init(void *video_driver)
{
   ...
   if (g_egl_inited)
   {
      KINGSN_LOG("[EMSCRIPTEN/EGL]: Attempted to re-initialize driver.\n");
   ...
   KINGSN_LOG("[EMSCRIPTEN/EGL]: Dimensions: %ux%u\n", width, height);
#endif
