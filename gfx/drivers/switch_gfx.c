      ...
      input_driver_t **input, void **input_data)
{
   ...
   KINGSN_LOG("loading switch gfx driver, width: %d, height: %d\n", video->width, video->height);
      ...
      const char *msg, video_frame_info_t *video_info)
{
   ...
   if(width > 0 && height > 0)
   {
            ...
            sw->last_height != height)
      {
         ...
         {
            KINGSN_ERR("failed to generate scaler for main image\n");
            return false;
         }
      ...
      unsigned width, unsigned height, float alpha)
{
         ...
         sw->menu_texture.height != height)
   {
      ...
      if (!sw->menu_texture.pixels)
      {
         KINGSN_ERR("failed to allocate buffer for menu texture\n");
         return;
      }
      ...
      {
         KINGSN_ERR("failed to generate scaler for menu texture\n");
         return;
      }
   }
