...
bool d3d12_init_base(d3d12_video_t* d3d12)
{
...

   {
      ...
      for (;;)
      {
         ...
         KINGSN_LOG("[D3D12]: Found GPU at index %d: %s\n", i, str);
      ...
      int width, int height, void* corewindow)
{
   ...
   if (FAILED(hr))
   {
      KINGSN_ERR("[D3D12]: Failed to create the swap chain (0x%08X)\n", hr);
      return false;
   }
...
bool d3d12_init_descriptors(d3d12_video_t* d3d12)
{
   ...
   d3d12->desc.sampler_heap.desc.NumDescriptors =
         SLANG_NUM_BINDINGS * GFX_MAX_SHADERS + 2 * KINGSN_WRAP_MAX;
...
void d3d12_init_samplers(d3d12_video_t* d3d12)
{
   ...
   for (i = 0; i < KINGSN_WRAP_MAX; i++)
   {
      ...
      d3d12->samplers[KINGSN_FILTER_LINEAR][i] =
            d3d12_create_sampler(d3d12->device, &desc, &d3d12->desc.sampler_heap);

      desc.Filter = D3D12_FILTER_MIN_MAG_MIP_POINT;
      d3d12->samplers[KINGSN_FILTER_NEAREST][i] =
            d3d12_create_sampler(d3d12->device, &desc, &d3d12->desc.sampler_heap);
   }
}
